...
type Ring struct {
	...
	hold time.Duration
	Out  func([]byte)
	// OnSkip reports the seq the ring GAVE UP on, at the instant it gave up.
	//
	// U13 round 3: this used to be `func()`. It carried no seq, so hold.go's
	// LatenessRatchet could not tell an OnOld for a seq the ring had actually
	// HELD FOR from an OnOld for a seq the ring never held for at all --
	// flushTo below advances `next` past missing seqs WITHOUT skipping them,
	// because a window overflow is a COUNT bound and no hold could have saved
	// them. Feeding those arrivals to the ratchet made it learn a hold nothing
	// observed supported: MEASURED on the rig's windowed model, 8,213 ms
	// against 525 ms for the filtered rule at N5-het@0.95, on 15 of 18 cells
	// (highn_battery.py B6c CTRL `subst B6c ratchet := UNFILTERED`; per-cell
	// table in b6c_constant_probe.txt). The seq is what closes that, and
	// nothing else in ring.go changes: the two call sites pass the seq they
	// were already incrementing past.
	OnSkip func(seq uint32)
...
// Mask returns the ring's own index mask (size-1). It is the ring's GEOMETRY,
// exported so a consumer that has to remember something per buffered seq can size
// its record from the ring instead of inventing a number. hold.go's ratchet is
// the only caller.
func (r *Ring) Mask() uint32 { return r.mask }

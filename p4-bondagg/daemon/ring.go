...
type Ring struct {
	...
	// oldRun/oldAt track a run of nothing-but-old arrivals in the in-order
	// (max) path, for peer-restart resync. U113: ported from
	// server/ring.go:71-72 (see resync() below) -- the daemon side never
	// cleared armed on a restarted peer, so a routine bond-agg restart
	// (act_agg_restart) made every post-restart seq classify as old forever.
	oldAt  time.Time
	oldRun bool
	...
	Dups uint64
	// Resyncs counts peer-restart resyncs (see resync() below). U113.
	Resyncs uint64
}
...
func (r *Ring) Push(seq uint32, data []byte, now time.Time) {
	...
	if int32(seq-r.next) < 0 {
		...
		// Only a seq OUTSIDE the ring window can be a restarted sequence
		// space. Anything within mask of next is an ordinary straggler or
		// a duplicate copy, and must never arm the resync -- a stalled
		// path dribbling late copies would otherwise re-anchor the ring
		// backwards and replay. U113, ported from server/ring.go:171-177.
		...
		// A whole hold horizon of nothing but old seqs means the peer's
		// sequence space restarted (e.g. act_agg_restart). Drop the stale
		// buffer and re-arm the warm-up on this frame.
		r.resync(seq, now)
		r.store(seq, data, now)
		return
	}
...
// Callers MUST hold r.mu. U113, ported from server/ring.go:200-217.
...
// reanchor handles a forward jump further than the ring can hold: deliver
// every buffered entry in seq order, then jump next to seq. Bounded by the
// ring size because Push never stores a seq more than mask ahead of next --
// the old flushTo walked next..seq one seq at a time, so a single frame
// carrying a far-future seq (garbage, or a spoof past the magic/ver check)
// made that walk up to 2^31 iterations long while holding the ring lock.
// U113, ported from server/ring.go:223-237. Callers MUST hold r.mu.

...
//        This substitution is what U9/EQ-1 has to PRICE: Arm A's trace harness
//        drives PullFIFO/Drive with the rig's own admission decisions rather
//        than a socket, which is what makes a frame-for-frame comparison
//        possible at all -- and is exactly why Arm A cannot judge the
//        substitution it was handed. See the register preamble below.
//
// =============================================================================
...
// written down so nothing downstream reads fidelity into a comment.
//
// EQ-1 DOES NOT ADJUDICATE S1 OR S2. IT PRICES THEM (U9b). An earlier revision
// of this line named EQ-1 the adjudicator of both, on the grounds that both are
// only visible frame-for-frame. That is backwards. EQ-1's Arm A
// (eq1_replay_test.go) SUPPLIES the oracle's draw order and its room() from the
// trace -- it has to, because the Go core has no observable counterpart of
// _local_ms to compare against -- so the one question Arm A can never answer is
// whether the order it was handed is the order this file would have chosen. What
// adjudicates a substitution nothing can compare is a measurement of its
// CONSEQUENCE, and that is ARM B (eq1_free_test.go, whose header carries the same
// correction): the real Drive goroutines, the real pool, the real send path,
// against a two-stage fake device clocked by the trace's own capacity samples,
// with nothing telling any link when to draw.
//
// WHAT ARM B MEASURES, on the five committed fixtures. The command is `cd
// p4-bondagg/daemon && go test . -run EQ1Free -v`, and it is the one that was
// actually run: there is no root go.mod and no go.work in this repo (only
// p4-bondagg/daemon/go.mod and p4-bondagg/server/go.mod), so the package-path
// form `go test ./p4-bondagg/daemon` that earlier drafts of this comment printed
// cannot resolve from the repo root. Four runs, 2026-09-05. The numbers are
// scheduling-dependent by construction, which is why Arm B REPORTS them and
// gates none of them, and they are SAMPLES, not bounds: three later independent
// runs landed outside the mid-rig ranges below (see the U9b/U9d rows):
//
//   * the four EDGE traces, where the bottleneck IS the local device the socket
//     sits on: max|d share| 0.00 (n1, one link), 0.013 (n2), 0.020-0.027 (n3),
//     0.009-0.015 (n5). Barging mutex order costs a few percent of share against
//     hungriest-first -- because a jammed link's write refuses and it stops
//     drawing, so share still falls out of real drain in aggregate. S1's
//     consequence is SMALL exactly where the substituted signal exists.
//
//   * n5-het-mid, where the rig puts the capacity on the DOWNSTREAM stage and
//     gives the local one base*20 (reserved_composite.py's build_rig,
//     @"loc = a['base'] * local_mult" and the down_cap_fn=trace beside it).
//     SAMPLES, n=4:
//     ASSIGNED max|d share| 0.226-0.234, and 0.156-0.172 measured on what
//     actually gets through both stages, with 9.4k-10.5k stage-2 taildrops
//     against the oracle's recorded 2533. Six later independent runs landed
//     OUTSIDE all three of those spans -- three verification runs (ASSIGNED
//     0.215 / 0.225 / 0.225, DELIVERED 0.153 / 0.154 / 0.162, taildrop 8838) and
//     three in this fix round (ASSIGNED 0.245 / 0.223 / 0.217, DELIVERED 0.180 /
//     0.164 / 0.153, taildrop 9994 / 9296 / 9631). That is what a sample span
//     does and a bound does not: over n=10 the observed spread is ASSIGNED
//     0.215-0.245, DELIVERED 0.153-0.180, taildrop 8.8k-10.5k, and the next run
//     may land outside THAT too. Nothing here is gated. What IS gated is the
//     two-sided trace-derived bar (rig_tdrop > 0 -> stage 2 must drop, == 0 ->
//     it must not) plus two-stage conservation. The finding those samples carry
//     is the SIGN and the ORDER: there the socket never refuses -- peak local
//     occupancy 1-5 ms against a 300 ms ceiling -- so room() has no signal at
//     all, and S1 is paid in downstream LOSS instead.
//
// That is the price of the S1/S2 pair, measured rather than argued. It is not a
// finding that they are acceptable: the mid number is what an E1/G1 measurement
// on the real path has to be argued against.
//
...
func (f *PullFIFO) Enqueue(payload []byte, now time.Time) uint32 {
	...
	s := f.seq
	f.seq++
	f.enqLocked(cp, s, now)
	f.mu.Unlock()
	f.content()
	return s
}

// enqueueSeq is Enqueue's body with the seq ASSIGNED BY THE CALLER and the
// payload NOT copied here. It is the fan-out entry point (U138): PullCore.Offer
// assigns ONE seq for the whole fan-out and hands the SAME read-only backing
// array to every link's FIFO, so a fan-out offer is one payload copy and N
// PullFrame headers -- not N payload copies. Nothing after this point writes
// through fr.payload: send() only reads it (Pack/PackRsvd copy into out[]), and
// Return/bound only move the header.
//
// It is a distinct method rather than an argument to Enqueue because Enqueue
// must keep assigning the seq and pushing the frame UNDER ONE ACQUISITION of
// mu -- the header's "seq is assigned HERE, in app order" claim is exactly that
// atomicity, and splitting it into two acquisitions would let two concurrent
// offers push in the opposite order to their seqs. Both paths therefore share
// enqLocked and differ only in who owns the counter.
//
// f.seq is advanced to seq+1 so a FIFO fed by this path still answers correctly
// if anything ever calls Enqueue on it. Under fan-out nothing does:
// PullCore.Offer is the only writer of a per-link FIFO.
func (f *PullFIFO) enqueueSeq(payload []byte, seq uint32, now time.Time) {
	f.mu.Lock()
	f.seq = seq + 1
	f.enqLocked(payload, seq, now)
	f.mu.Unlock()
	f.content()
}

// enqLocked is the shared body: build the header, push it, apply the bound, and
// keep the high-water marks. Caller holds mu and calls content() after
// unlocking. It does NOT copy payload -- both callers have already decided who
// owns that memory.
func (f *PullFIFO) enqLocked(payload []byte, seq uint32, now time.Time) {
	fr := &PullFrame{seq: seq, enq: now, payload: payload}
	f.pushBack(fr)
...
// PullSendBufLen is the size of a per-link TX buffer, and the MacLen term is
// not optional. Pack can fill exactly MaxPayload+HdrLen bytes, and authGate.Seal
// then appends MacLen more; a buffer sized MaxPayload+HdrLen therefore writes
// out of range on the first full-size frame the moment a key exists. Pinned by
// TestPullSendBuffersCarryTheTrailer. See daemon/auth.go BUFFER SIZES.
const PullSendBufLen = MaxPayload + HdrLen + MacLen

// authTX is the sender-side half of the transport MAC, shared by every link
// because the peer is ONE server across all N of them: a tag verified on link 2
// is proof about the peer, not about link 2, and SendAuth's silence fallback
// must not fire because one link happens to be quiet.
//
// A nil *authTX is the no-key posture and every method on it is a no-op, so the
// pull core behaves exactly as it did before U31 when no key file exists. That
// is what keeps this additive.
//
// FIELD ORDER IS LOAD-BEARING, same rule as PullLink: lastRx is accessed with
// sync/atomic and must be 8-byte aligned, and on the armv7 crossbuild target a
// pointer is 4 bytes, so an int64 placed after gate could land 4-byte aligned
// and atomic.LoadInt64 PANICS at run time. Keep it first.
type authTX struct {
	lastRx int64 // unix nanos, atomic: last frame received from the peer, any link
	gate   *authGate
}

// MarkRx records that the peer was heard from. Called on EVERY received frame,
// on every link, before anything else looks at it.
func (a *authTX) MarkRx(now time.Time) {
	if a == nil {
		return
	}
	atomic.StoreInt64(&a.lastRx, now.UnixNano())
}

// Seal is integration point 1: seal buf[:n] if this client should be sealing at
// all. Returns the new length, or -1 if buf has no room for the trailer (a
// caller bug -- see authGate.Seal).
func (a *authTX) Seal(buf []byte, n int, now time.Time) int {
	if a == nil {
		return n
	}
	if !a.gate.SendAuth(now, time.Unix(0, atomic.LoadInt64(&a.lastRx))) {
		return n
	}
	return a.gate.Seal(buf, n, now)
}
...
type PullLink struct {
	// lastRxMs: MONOTONIC milliseconds since paths.go's monoEpoch -- NOT a Unix
	// wall-clock stamp. See MarkRx.
	lastRxMs  int64
	blockedNs int64
	okWriteNs int64
	minOkNs   int64
	...
	// fanout is TRUE only under AGG_SCHED=lightning (U138): every source carries
	// every frame, so the SERVER would otherwise see N copies of one seq arriving
	// on N pathIDs and route the downlink onto whichever arrived last. It is the
	// send path's one branch -- it makes send() emit the downlink hint
	// d = (0 - idx) mod 256 instead of Pack's 0, pinning the ONE downlink copy to
	// AGG_PATHS[0]. FALSE in every other mode, which is Pack's rsvd = 0 and the
	// wire this daemon has always emitted.
	//
	// Written once by PullCore.SetFanout, before Start, for the same reason cap
	// and gate say so: the send path reads it without synchronisation and the
	// scheduler is a start-time fact.
	fanout bool
}
...
func newPullLinkSock(idx int, ifname string, sock linkSocket, dst *net.UDPAddr) *PullLink {
	...
	atomic.StoreInt64(&l.lastRxMs, monoAgeMS(time.Now()))
	return l
}
...
//
// U13 blocker B5: this was time.Now().UnixMilli(), a WALL-clock stamp, and so was
// RxAge's other operand. UnixMilli DISCARDS the monotonic reading time.Now()
// carries, so an NTP step moved the stored value and the query independently and
// every link's liveness moved with it -- RxAge feeds SetAlive, and SetAlive is
// what takes a link out of the pool. Both ends now go through monoAgeMS
// (paths.go), anchored on monoEpoch, which reads the monotonic clock.
//
// U13 derived this fix while the ratchet it originally protected (hold.go's
// LatenessRatchet, whose Reset rode SetAlive -> AliveSet -> SameAliveSet) was
// still the reason it mattered. U159 deleted the reorder ring and the ratchet
// with it; the defect below is independent of both and is kept on its own
// merits. Bar: TestRxAgeIsAnchoredOnTheMonotonicEpoch (mono_test.go).
func (l *PullLink) MarkRx() { atomic.StoreInt64(&l.lastRxMs, monoAgeMS(time.Now())) }

// RxAge is how long since anything arrived on this link. `now` must come from
// time.Now() (or time.Now().Add): monoAgeMS reads the monotonic reading embedded
// in it, which is what makes the age immune to a wall-clock step.
func (l *PullLink) RxAge(now time.Time) time.Duration {
	return time.Duration(monoAgeMS(now)-atomic.LoadInt64(&l.lastRxMs)) * time.Millisecond
}
...
func (l *PullLink) send(fr *PullFrame, out []byte) sendResult {
	...
	// THE DOWNLINK HINT (U138). Header byte [3] on a FlagData frame is the
	// downlink-hint delta d, and the server resolves it as
	// HintTarget(pathID, d) = pathID + d (server/frame.go:113).
	//
	//   - NOT fan-out (every mode but `lightning`): d = 0, so target = this
	//     frame's own pathID -- "prefer the link this frame is riding", which is
	//     exactly what Pack has always emitted (frame.go, Pack). Byte-identical
	//     wire, no branch taken.
	//   - fan-out (`lightning`): every link carries every seq, so N copies of one
	//     frame reach the server on N pathIDs and d = 0 would let `last` flip
	//     per frame between links of different OWD. d = (0 - idx) mod 256 makes
	//     target = idx + (0 - idx) = 0 for EVERY link, so the server pins its
	//     single downlink copy to AGG_PATHS[0] -- the primary -- and falls back
	//     to `last` only once that endpoint is older than EpMaxAge. The uplink
	//     is N copies; the downlink stays ONE, and the server stays mode-blind
	//     (ADR-003 amendment, U119).
	//
	// byte(-l.idx) IS (0 - idx) mod 256: a non-constant int->byte conversion in
	// Go truncates to the low 8 bits, and idx < MaxLinks = 256 is enforced by
	// disabled() above and by the resolver (discover.go wireCeiling).
	rsvd := byte(0)
	if l.fanout {
		rsvd = byte(-l.idx)
	}
	m := PackRsvd(out, FlagData, byte(l.idx), rsvd, fr.seq, nowMS(), l.fseq, fr.payload)
	// INTEGRATION POINT 1 (daemon/auth.go): seal after Pack, before the write.
	// A nil auth or a keyless gate returns m unchanged, which is the pre-U31
	// wire byte for byte. The sent-bytes meter below counts the WriteToUDP
	// return, which is already the SEALED length -- integration point 3, and it
	// has to be the sealed length because that is what the server counts on
	// arrival (server/rx.go, x.stats.OnData(f.pid, len(b))).
	sealNow := time.Now()
	if sm := l.auth.Seal(out, m, sealNow); sm < 0 {
		// The buffer has no room for the trailer. Not a path condition and not
		// retryable -- it is a caller allocating out[] too small -- so the frame
		// is consumed rather than returned, exactly like a path-down error. The
		// attributable counter is authGate.SealShort(), reported in STAT; errs
		// only says a frame died here.
		atomic.AddUint64(&l.errs, 1)
		return sendPathDown
	} else {
		m = sm
	}
	// t0 starts AFTER the seal, deliberately: d below is charged to blockedNs
	// and okWriteNs, and BlockedMs is E1's edge-vs-mid discriminator (pull.go
	// header). Folding a constant ~250 ns of HMAC into the socket's measured
	// time would put a CPU cost inside a measurement about a device queue.
	t0 := time.Now()
...
//
// FAN-OUT (U138). FIFOs is nil in every mode but `lightning`, and nil means
// "one shared pool" -- the pull core U7 wrote, byte for byte: every link draws
// from FIFO, every frame is drawn exactly once, and nothing below branches. When
// FIFOs is non-nil it has one PullFIFO per link and each link drives its OWN
// pool, so Offer's single seq lands once in each of them and every source
// carries every frame (ADR-003:38). Drive, Draw and Return are UNTOUCHED by
// this: a link cannot tell which pool it was handed.
type PullCore struct {
	...
	// seq is the fan-out sequence counter and it is used ONLY when FIFOs is
	// non-nil. With the shared pool the counter that matters is FIFO.seq (one
	// pool, one counter); under fan-out there are N pools and the seq has to be
	// assigned ABOVE them, once, or the copies of one datagram would carry N
	// different seqs and WireGuard's anti-replay window at the peer -- which is
	// what performs first-wins since U159 deleted both rings -- would see N
	// distinct counters and accept all N instead of one.
	seq uint32
}

// SetAuth installs the transport MAC on every link. Nil (the default) is the
// no-key posture: the wire is byte-identical to pre-U31. Called once, before
// Start, so no link is drawing while the field is written.
func (c *PullCore) SetAuth(a *authTX) {
	c.Auth = a
	for i := range c.Links {
		c.Links[i].auth = a
	}
...
// `speed`'s draw order can silently equal `max`'s.
//
// U159 MADE THAT WORSE AND IT IS RECORDED RATHER THAN GLOSSED. Delivery used to
// differ unconditionally, so a degraded `speed` was still not `max`. The reorder
// ring is deleted at both ends now and every mode delivers on arrival, so RANK
// is the ONLY axis left between the two: with a nil ranker they ARE wholly
// identical. The mode is still reported honestly (PSTAT sched=), and the fix if
// it matters is a ranker that cannot be nil, not a second delivery policy.
//
...
// Fanout reports whether this core is in per-link-FIFO mode.
func (c *PullCore) Fanout() bool { return c.FIFOs != nil }

// fifoFor is the ONE place the two pool shapes are resolved, and it is why
// Drive/Draw/Return needed no change at all: a link is handed a *PullFIFO and
// has never known whether anyone else draws from it.
func (c *PullCore) fifoFor(i int) *PullFIFO {
	if c.FIFOs == nil {
		return c.FIFO
	}
	return c.FIFOs[i]
...
// SetFanout installs (or removes) the per-link pools. It is the structural half
// of AGG_SCHED=lightning, and ApplySched is what calls it -- see sched.go for
// why the wiring is a named function rather than three lines in runPullClient.
//
// Call BEFORE Start and before the first Offer, for the same reason SetCap and
// SetSched say so: the draw and offer paths read these fields without
// synchronisation, and the scheduler is a start-time fact.
//
// Idempotent in both directions: applying the same policy twice rebuilds
// nothing, and turning fan-out off restores the shared pool exactly (FIFO itself
// is never replaced, so a core that was never started is byte-identical to a
// fresh one).
func (c *PullCore) SetFanout(on bool) {
	if !on {
		c.FIFOs = nil
		for i := range c.Links {
			c.Links[i].fanout = false
		}
		return
	}
	if len(c.FIFOs) != len(c.Links) {
		c.FIFOs = make([]*PullFIFO, len(c.Links))
		for i := range c.FIFOs {
			c.FIFOs[i] = NewPullFIFO()
		}
	}
	for i := range c.Links {
		c.Links[i].fanout = true
	}
}
...
func (c *PullCore) Start() {
	for i := range c.Links {
		go c.Links[i].Drive(c.fifoFor(i))
	}
}
...
//
// Under FAN-OUT it assigns ONE seq and enqueues the SAME read-only payload into
// every link's FIFO: one payload copy, N PullFrame headers. That is the whole of
// `lightning` on the send side -- there is no duplicator, no trigger and no
// threshold, because "every source, every packet" is not a decision anything
// makes per frame (ADR-003:38). First copy wins at the peer inside WireGuard,
// by its anti-replay window (U159 deleted both rings, which used to be the
// dedup memory; this layer now delivers every copy, and must, or WireGuard
// would never see the winner). And a
// dead or refusing source sheds only ITS OWN copies, by the age limb of its own
// pool -- nothing about the other links changes.
func (c *PullCore) Offer(payload []byte, now time.Time) uint32 {
	if c.FIFOs == nil {
		return c.FIFO.Enqueue(payload, now)
	}
	// Atomic, not a plain increment: PullFIFO.Enqueue assigns its seq under the
	// pool mutex, so the shared-pool path is safe for a concurrent caller and the
	// fan-out path must not be weaker. runPullClient offers from ONE goroutine
	// (the WG reader), so this is uncontended there; the ORDER of two concurrent
	// offers within a pool is not promised by either path.
	s := atomic.AddUint32(&c.seq, 1) - 1
	cp := make([]byte, len(payload))
	copy(cp, payload)
	for _, f := range c.FIFOs {
		f.enqueueSeq(cp, s, now)
	}
	return s
}

// TrimAll applies the pool bound's AGE limb to every pool this core owns and
// returns how many frames it shed in total. One pool under the shared default,
// N under fan-out -- the caller's cadence and the hold it passes are unchanged.
func (c *PullCore) TrimAll(now time.Time, maxAge time.Duration) int {
	if c.FIFOs == nil {
		return c.FIFO.Trim(now, maxAge)
	}
	aged := 0
	for _, f := range c.FIFOs {
		aged += f.Trim(now, maxAge)
	}
	return aged
}

// WakeAll is the control cadence over every pool this core owns.
func (c *PullCore) WakeAll() {
	if c.FIFOs == nil {
		c.FIFO.Wake()
		return
	}
...
// Stats and ByteStats are PullFIFO's, summed over the pools this core owns. With
// the shared pool (the default) the sum is over one element and the numbers are
// the FIFO's own, unchanged. Under fan-out they are TOTALS across the N per-link
// pools: enq counts N per offer, because N frames really were enqueued.
func (c *PullCore) Stats() (depth, peak int, enq, drawn, stale uint64) {
	if c.FIFOs == nil {
		return c.FIFO.Stats()
	}
	for _, f := range c.FIFOs {
	...
	return depth, peak, enq, drawn, stale
}

func (c *PullCore) ByteStats() (bytes, peakBytes, max int, qdrops, returns uint64) {
	if c.FIFOs == nil {
		return c.FIFO.ByteStats()
	}
	for _, f := range c.FIFOs {
		b, pb, mx, qd, rt := f.ByteStats()
		bytes += b
		peakBytes += pb
		max += mx
		qdrops += qd
	...
	return bytes, peakBytes, max, qdrops, returns
}

...
type PullLink struct {
	// lastRxMs: MONOTONIC milliseconds since hold.go's monoEpoch -- NOT a Unix
...
//
// U13 round 2 (blocker B5): this was time.Now().UnixMilli(), a WALL-clock stamp,
// and so was RxAge's other operand. RxAge feeds SetAlive, SetAlive feeds
// AliveSet -> SameAliveSet -> LatenessRatchet.Reset, and Reset is the only thing
// that ever lowers the derived hold -- so a wall-clock step could re-anchor, or
// fail to re-anchor, the hold this whole unit derives. Both ends now go through
// monoAgeMS (hold.go), which is anchored on monoEpoch and reads the monotonic
// clock. paths.go's own liveness is unchanged; paths.go is frozen.
...
func (l *PullLink) send(fr *PullFrame, out []byte) sendResult {
	...
	m := Pack(out, FlagData, byte(l.idx), fr.seq, stampMS(), l.fseq, fr.payload)
	t0 := time.Now()

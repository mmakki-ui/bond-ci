...
type PullCore struct {
	...
	// different seqs and the peer's dedup (server/ring.go:152) would deliver N
...
// makes per frame (ADR-003:38). First copy wins at the peer by the seq dedup
// that both rings already do (server/ring.go:152, daemon/ring.go:176-187), and a

...
//
// capGateAdmit -- the cap and rank-gate half of Drive's preamble, callable from the other
// draw loop. U250: DriveLit had neither, so arming spotty-class duplication silently dropped
// the delivered-rate cap on every link and speed's rank gate with it, on the loop every link
// runs once it is armed. Returns false when the caller must not draw this pass.
//
// It is a SECOND spelling of what Drive still does inline, not a shared call site: folding
// Drive onto it means moving code that contains three returns and three continues, which is a
// control-flow rewrite of the hot loop and belongs in its own unit. The bar below covers both
// loops, so the two cannot diverge silently again.
func (l *PullLink) capGateAdmit(f *PullFIFO) bool {

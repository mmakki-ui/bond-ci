...
// ---------------------------------------------------------------------------
// U138 -- FAN-OUT. `lightning` is ADR-003:38's "every source, every packet", and
// before this unit the pull core had no send path for it at all: Offer was
// FIFO.Enqueue into ONE pool and Draw popped each frame exactly once, so
// whatever the mode was called, exactly one link carried each frame.
//
// These bars execute the claim end to end -- through the real Drive loop and a
// real send() into each link's own socket -- rather than asserting that a slice
// of pools exists.
// ---------------------------------------------------------------------------

// fanoutCore builds a core of n links over fake sockets and puts it in fan-out
// mode. socks[i] is link i's socket; outcomes are per link (nil = always
// accept).
func fanoutCore(n int, outcomes func(i int) []error) (*PullCore, []*fakeSock) {
	socks := make([]*fakeSock, n)
	c := &PullCore{FIFO: NewPullFIFO()}
	for i := 0; i < n; i++ {
		socks[i] = newFakeSock(0, outcomes(i)...)
		c.Links = append(c.Links, newPullLinkSock(i, "if", socks[i], testDst()))
	}
	c.SetFanout(true)
	return c, socks
}

// closeFanout closes every pool so the draw goroutines exit.
func closeFanout(c *PullCore) {
	for _, f := range c.FIFOs {
		f.Close()
	}
}

// THE FAN-OUT BAR. Every link's socket must see EVERY seq, exactly once, with a
// byte-identical payload -- that is the whole of `lightning` on the send side,
// and none of it was true before this unit at any N >= 2.
//
// "exactly once" is as load-bearing as "every": one seq written twice on one
// link is not redundancy, it is a doubled uplink for nothing, and the peer's
// dedup would hide it completely. N is swept because the failure the deliverable
// seeds (Offer enqueuing into FIFOs[0] only) is GREEN at N=1 -- at one link,
// fan-out and no fan-out are the same wire.
func TestPullFanoutEveryLinkCarriesEverySeq(t *testing.T) {
	const frames = 6
	for _, n := range []int{1, 2, 3, 5} {
		c, socks := fanoutCore(n, func(int) []error { return nil })
		c.Start()

		want := make([][]byte, frames)
		for k := 0; k < frames; k++ {
			// Distinct payload per frame, so a link carrying the right COUNT of
			// the wrong frames still fails.
			want[k] = []byte{byte(k), byte(k * 7), 0xA5}
			if got := c.Offer(want[k], time.Now()); got != uint32(k) {
				closeFanout(c)
				t.Fatalf("N=%d: Offer returned seq %d for frame %d -- ONE seq is assigned "+
					"per offer, above the pools, or the copies of one datagram carry "+
					"different seqs and the peer's dedup delivers N of them", n, got, k)
			}
		}

		for i, s := range socks {
			if !waitFor(func() bool { return len(s.frames()) >= frames }, 5*time.Second) {
				closeFanout(c)
				t.Fatalf("N=%d: link %d sent %d of %d frames. Under fan-out every link "+
					"carries every frame; a link short of the full set means the offer "+
					"reached only some of the pools", n, i, len(s.frames()), frames)
			}
		}
		closeFanout(c)
		...
		for i, s := range socks {
			w := s.frames()
			if len(w) != frames {
				t.Fatalf("N=%d: link %d wrote %d frames, want exactly %d -- a seq written "+
					"twice on one link is a doubled uplink the peer's dedup hides",
					n, i, len(w), frames)
			}
			seen := map[uint32]bool{}
			for _, fw := range w {
				fl, pid, sq, _, _, pay, err := Unpack(fw.b)
				if err != nil {
					t.Fatalf("N=%d link %d: unpack: %v", n, i, err)
				}
				if fl != FlagData || int(pid) != i {
					t.Fatalf("N=%d link %d: flags=%d pathID=%d, want DATA on %d", n, i, fl, pid, i)
				}
				if sq >= frames {
					t.Fatalf("N=%d link %d: seq %d was never offered", n, i, sq)
				}
				if seen[sq] {
					t.Fatalf("N=%d link %d: seq %d written TWICE on the same link", n, i, sq)
				}
				seen[sq] = true
				if !bytes.Equal(pay, want[sq]) {
					t.Fatalf("N=%d link %d seq %d: payload %v, want %v -- the copies must be "+
						"byte-equal; one payload copy is shared by every pool", n, i, sq, pay, want[sq])
				}
			}
			if len(seen) != frames {
				t.Fatalf("N=%d link %d: carried %d distinct seqs, want %d", n, i, len(seen), frames)
			}
		}
	}
}

// A source that will not send must shed ITS OWN copies and nothing else. That is
// the property that makes fan-out safe to run on a box whose second uplink is
// dead: with one shared pool a link that never drains cannot hold anything back
// (it returns the frame and another link takes it), but with per-link pools it
// owns a queue, and if that queue's shedding were shared the dead link would
// throttle the live ones.
//
// The refusing link's pool must therefore fill and then be shed by the AGE limb
// of ITS OWN bound, while every healthy link delivers the full set.
func TestPullFanoutRefusingLinkShedsOnlyItsOwn(t *testing.T) {
	const frames = 5
	const bad = 1
	c, socks := fanoutCore(3, func(i int) []error {
		if i == bad {
			return []error{wrapErrno(syscall.ENOBUFS)}
		}
		return nil
	})
	c.Start()
	...
	for i, s := range socks {
		...
		if !waitFor(func() bool { return len(s.frames()) >= frames }, 5*time.Second) {
			t.Fatalf("healthy link %d sent %d of %d -- the refusing link starved it",
				i, len(s.frames()), frames)
		}
	}
	// The refuser holds its own copies: it drew one, was refused, returned it at
	// the head and parked. Nothing it did removed a frame from anyone else.
	if !waitFor(func() bool { d, _, _, _, _ := c.FIFOs[bad].Stats(); return d == frames }, 5*time.Second) {
		d, _, _, _, _ := c.FIFOs[bad].Stats()
		t.Fatalf("refusing link's pool holds %d frames, want %d", d, frames)
	}
	if c.Links[bad].Sent() != 0 {
		t.Fatalf("the refusing link sent %d frames", c.Links[bad].Sent())
	}
	for i := range c.FIFOs {
		if i == bad {
			continue
		}
		d, _, _, drawn, _ := c.FIFOs[i].Stats()
		if d != 0 || drawn != frames {
			t.Fatalf("healthy pool %d: depth=%d drawn=%d, want 0/%d", i, d, drawn, frames)
		}
	}

	// The shed is the AGE limb, on the refuser's pool only. Every frame is older
	// than 1ms by the time TrimAll is called with a clock a second ahead, so the
	// only pool with anything to shed is the one holding frames.
	c.TrimAll(time.Now().Add(time.Second), time.Millisecond)
	for i := range c.FIFOs {
		_, _, _, _, stale := c.FIFOs[i].Stats()
		want := uint64(0)
		if i == bad {
			want = frames
		}
		if stale != want {
			t.Fatalf("pool %d shed %d frames by the age limb, want %d -- a dead source "+
				"must shed only its own copies", i, stale, want)
		}
	}
}

// THE DOWNLINK BAR. The uplink is N copies; the downlink must stay ONE, and the
// server is mode-blind (it routes onto the endpoint the hint names, and onto
// `last` when the hint is 0). Under fan-out N copies of one seq arrive on N
// pathIDs, so with the pre-U138 rsvd=0 the server's `last` would flip per frame
// between links of different OWD -- a downlink that reorders itself for free.
//
// d = (0 - idx) mod 256 makes HintTarget(pathID, d) = idx + (0 - idx) = 0 on
// EVERY link, which is AGG_PATHS[0], the primary. The negative control is the
// same core without fan-out: rsvd must be 0, the byte every other mode has
// always emitted.
func TestSendHintPinsPrimaryUnderFanout(t *testing.T) {
	for _, n := range []int{1, 2, 3, 5} {
		c, socks := fanoutCore(n, func(int) []error { return nil })
		c.Start()
		c.Offer([]byte{1, 2, 3}, time.Now())
		for i, s := range socks {
			if !waitFor(func() bool { return len(s.frames()) >= 1 }, 5*time.Second) {
				closeFanout(c)
				t.Fatalf("N=%d: link %d never sent", n, i)
			}
		}
		closeFanout(c)
		for i, s := range socks {
			_, pid, rsvd, _, _, _, _, err := UnpackRsvd(s.frames()[0].b)
			if err != nil {
				t.Fatalf("N=%d link %d: %v", n, i, err)
			}
			if want := byte(-i); rsvd != want {
				t.Fatalf("N=%d link %d: hint d=%d, want %d = (0-%d) mod 256", n, i, rsvd, want, i)
			}
			// The property the byte exists for, computed the way the server
			// computes it (server/frame.go HintTarget: pathID + d).
			if target := pid + rsvd; target != 0 {
				t.Fatalf("N=%d link %d: the server would route the downlink to pathID %d, "+
					"not to AGG_PATHS[0]. Under fan-out every copy must name the SAME "+
					"downlink or the single reply copy chases whichever uplink arrived "+
					"last", n, i, target)
			}
		}
	}

	// NEGATIVE CONTROL: no fan-out, no hint. This is the wire every other mode
	// emits and it must not have moved.
	f := NewPullFIFO()
	fs := newFakeSock(0, nil)
	l := newPullLinkSock(2, "if2", fs, testDst())
	f.Enqueue([]byte{9}, time.Now())
	go l.Drive(f)
	if !waitFor(func() bool { return len(fs.frames()) >= 1 }, 5*time.Second) {
		f.Close()
		t.Fatal("negative control: nothing sent")
	}
	f.Close()
	_, pid, rsvd, _, _, _, _, err := UnpackRsvd(fs.frames()[0].b)
	if err != nil {
		t.Fatal(err)
	}
	if rsvd != 0 || pid != 2 {
		t.Fatalf("without fan-out the frame carried pathID=%d hint=%d, want 2/0 -- "+
			"rsvd=0 means `prefer the link this frame is riding`, the pre-U138 wire",
			pid, rsvd)
	}
}

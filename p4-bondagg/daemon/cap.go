...
//   (ROADMAP.md:149, p5-execution-handover.md:58). So E2b is BUILT and DISABLED:
//   p5-execution-handover.md:85 sequences E2b as built with its ENABLEMENT set
//   by E1, and ROADMAP.md:193 records U15a as "built flag-gated; E1 sets
//   enablement".
//
...
//   the file:
//     - The legend (masterpiece_dp.py:90-91) reads "the (*) rows are set for
...
//
// The failure it prevents (p5-execution-handover.md:38, echo.go:76-81): the
...
//
// =============================================================================
// THE TWO HALVES (masterpiece_dp.py:113-136 states the same split)
//
...
//   The exits that DO exist, and they are the whole list: local congestion
//   returning (the regime is edge again -- both the ratio arm and the idle arm
//   check it), a server restart (rebase clears it), the DeadIval staleness
//   fail-open in Admit, and the zero-rate deadlock break in Admit. Every one of
//   them is an event, not a gradual recovery.
//
...
//       BACKLOG.  reserved_composite.py:274 reads inflight_kb =
...
//       echo claiming an ancient txstamp asks for a deeper ring. Bounded, and the
//       bound is why it is acceptable here: growth is at most ONE marker per
//       echo (growRing), a marker is 12 bytes, the stamp must lie between this
//       link's first and most recent ping, and every deepening is LOGGED. So the
//       worst case is a slow, visible, per-link memory growth on a link an
//       attacker is already able to deny outright -- strictly smaller than the
//       throughput denial S11 already grants them. U31 closes it.
//
...
//   CapStats.Inert, a once-per-link WARNING naming the ring depth and the last
//   span, and mk= / rtt= / inert on the PSTAT line (pullrun.go). RTT on this
...
type CapConfig struct {
	...
	// constructor, reserved_composite.py Sim.__init__ target_ms (:153), and of
	// 03-reserved-composite/ackclock_sim.py Sim.__init__ (:80). It carries NO
...
type capLink struct {
	...
	// inertLogged makes the inertness WARNING once per link rather than once per
	// echo. The condition itself stays readable in CapStats and on PSTAT.
	inertLogged bool
}
...
func (c *Cap) foldRecord(id int, ts uint32, rx uint64, srvMS uint32, bp uint64, now time.Time) bool {
	...
	if !ok {
		...
		l.nUnaligned++
		if l.nFold == 0 && !l.inertLogged {
			// REPORT INERTNESS, do not be silently inert. Echoes are arriving and
			// this link has never produced a reading, so the cap is enabled,
			// consuming them, and protecting nothing.
			l.inertLogged = true
			log.Printf("pull-cap: link %d INERT -- an echo could not be aligned to any ping "+
				"marker and this link has never folded a reading, so the cap is ON and "+
				"measuring nothing for it. ring=%d markers span=%dms cadence=%v. If the ring "+
				"is still sizing itself this clears within a few echoes and PSTAT fold= starts "+
				"counting; if fold= stays 0 the stamps are not ours (stale peer, forgery, or a "+
				"server that is not echoing) and the cap is inert for this link.",
				id, l.mkCap, l.rttMS, PingIval)
		}
		return false
	}
	...
	// ring.go uses for seq (ring.go:129,155,166,193). A naive int64 subtraction
	...
	if dms <= 0 || sentAt < l.lastSent {
		...
		//
		// Either way there is no reading. Re-baseline on the NEW values rather
		// than holding the old ones: holding them would freeze the meter for the
		// whole size of a backward clock step, with the latch stuck at whatever
		// it was, and freezing a latch is the failure mode this file exists to
		// avoid. The cost is losing one interval's bytes from the detector
		// window, which is bounded and self-heals on the next echo.
		l.nBadIval++
		c.rebase(l, rx, sentAt, srvMS, bp, now)
		return false
	}
...
// oracle consults room() (reserved_composite.py:341).
//
...
type CapStats struct {
	...
	Inert     bool   // echoes arrived, none ever folded: measuring nothing
}
...
func (c *Cap) Stats(link int) CapStats {
	...
	return CapStats{
		...
		Grows:     l.mkGrow,
		SpanMS:    l.rttMS,
		Inert:     l.nFold == 0 && l.nUnaligned > 0,
	}
}

// Inert reports that echoes for this link have arrived and NOT ONE of them has
// ever produced a reading: the cap is enabled, consuming echoes, and measuring
// nothing. It exists because the failure this unit shipped in round 1 was SILENT
// inertness, and a safety mechanism that is quietly doing nothing is worse than
// one that is off -- the operator believes the path is protected.
//
// It is deliberately a plain predicate over the counters rather than a separate
// flag, so it cannot drift away from what the counters say.
// Test: TestCapInertIsReportedNotSilent.
	...
	if len(missing) > 0 {
		return false, cfg, fmt.Errorf(
			"%w: AGG_PULL_CAP=on but %v are unset. Every one of them is a number NOBODY HAS "+
				...
				"(ROADMAP.md:149). This build ships no default for any of them ON PURPOSE, so "+
...
func LogCapPosture(on bool, cfg CapConfig, n int) {
	if !on {
		log.Print("pull-cap: OFF (default). E2b is BUILT and DISABLED: the delivered-rate cap " +
			...
			"one is what G1/E1 measures (ROADMAP.md:149). With the flag off nothing in this " +

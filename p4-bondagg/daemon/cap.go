...
//
// This file was written as ADDITIVE: it did not touch the EIF push stack
// (eif.go, estr.go, fec.go) and left the push entry points alone. U128 has
// since DELETED that stack (ADR-002 / U127), so the sentence now records why
// this file needed no change when it went: nothing here ever called into it.
//
...
//      int32 -- the same wrap-safe idiom lossmeter.go uses for fseq
//      (lossmeter.go:64,67,92,104) -- and a
...
//       EQ-1 ADJUDICATES NONE OF (a)-(c), AND IT DOES NOT ADJUDICATE S1/S2
//       EITHER (U9b). This line used to claim both. The correction is pull.go's
//       register preamble: EQ-1's Arm A supplies the oracle's draw order and
//       room() FROM the trace, so it cannot judge a substitution it was handed;
//       Arm B (eq1_free_test.go) prices the CONSEQUENCE of S1/S2 by running the
//       real core free against a two-stage fake device -- max|d share| 0.00-0.03
//       on the four edge traces, and on n5-het-mid 0.215-0.245 assigned and
//       0.153-0.180 delivered over n=10 SAMPLES (spans, not bounds; nothing here
//       is gated -- see pull.go's Arm B block). Measured 2026-09-05 by `cd
//       p4-bondagg/daemon && go test . -run EQ1Free -v`; there is no root go.mod
//       or go.work, so the `./p4-bondagg/daemon` package-path form this comment
//       used to print does not resolve from the repo root.
//       Neither arm reaches the three above, and the reason is stronger than
//       scheduling noise: no trace record carries inflight_kb at all, so there
//       is nothing on the oracle side to compare an accumulated deficit to.
//       Pricing (a)-(c) needs a MEASUREMENT on the path -- E1 -- not a replay.
//
...
func (c *Cap) foldRecord(id int, ts uint32, rx uint64, srvMS uint32, bp uint64, now time.Time) bool {
	...
	// lossmeter.go uses for fseq (lossmeter.go @"int32(fseq-m.next) < 0"). A
	// naive int64 subtraction produces a ~4.29e9 ms denominator once per wrap
	// and reports a delivered rate of about zero (echo.go:95-99).

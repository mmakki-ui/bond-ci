...
// nsched_model.py's _fec_report, wSkip/(wDel+wSkip), i.e. the skip accounting
// THIS meter now does itself (drain below), running unconditionally. It used to
// be the ring's; U159 deleted both rings and the accounting stayed here. The
// push tree that carried the other branch is preserved at tag
// `eif-push-reference`.
//
...
// owd-adaptive horizon paths.go owd.Hold still computes (U159 deleted the ring
// that also consumed it; this meter is now its only consumer). This is
// TIME-based -- the O(1) gap timer is blockN/blockAt at :49-50 below, in this
// file, not a mirror of anything -- and is the fix for the
...
// overdue run up to the next buffered arrival (one overdue epoch, one batch).

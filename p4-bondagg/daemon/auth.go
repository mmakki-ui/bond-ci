...
//
// U128 RE-POINTED THIS PARAGRAPH. It used to say that the PUSH client and push
// server entry points in main.go, plus eif.go / estr.go / fec.go, were DEPLOYED
// and left byte-identical by U31 because sealing frozen reference code would
// buy nothing. That is no longer the tree: ADR-002 as amended by U127 makes the
// reference a git tag, and U128 deleted those entry points and those files. So
// there is no unsealed push peer left in this module to carve an exception for
// -- every frame this binary sends is a pull-client frame and goes through the
// gate below. The push tree, unsealed as it was, is at tag
// `eif-push-reference`.
//

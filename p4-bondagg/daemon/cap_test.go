...
// (cap.go LAG ALIGNMENT, p5-execution-handover.md
// @"difference **lag-aligned** counters", server/echo.go:76-81).
...
func TestCapInertIsReportedNotSilent(t *testing.T) {
	...
	foreign := func(k int) {
		...
		c.FoldEcho(uint32(9000+k), capEchoPayload(uint32(700+k), capRec{id: 0, bytes: 1}),
			[]uint64{0}, now.Add(time.Duration(k)*PingIval))
	}
	// INERTNESS IS NOT INSTANT, and that is deliberate: a link one echo old has
	// not yet had time to align anything. Reporting it inert is what produced
	// the spurious startup WARNING, because pullrun stamps each link's ping
	// inside the per-link loop and a loop that crosses a millisecond boundary
	// hands link 1 a stamp link 0's echo cannot match.
	foreign(0)
	if c.Inert(0) {
		t.Fatalf("one unalignable echo on a link zero cadences old reported inert: %+v",
			c.Stats(0))
	}
	for k := 1; k < 10; k++ {
		foreign(k)
	...
	st := c.Stats(0)
	if st.Folds != 0 || st.Unaligned != 10 {
		t.Fatalf("setup: %+v", st)
	}
	if !st.Inert || !c.Inert(0) {
		t.Fatalf("a link that folded nothing for %v does not report inert: %+v", DeadIval, st)
	}
...
// ---------------------------------------------------------------------------
// 9. ROUND-3 FIXES. Each of these runs the attack that was MEASURED against
//    round 2 and shows it no longer works, and where the old behaviour can be
//    executed it is executed rather than described.
// ---------------------------------------------------------------------------

// inertLifetime is ROUND 2's INERTNESS PREDICATE, verbatim, kept as a scratch
// copy in the test file in the same idiom as unalignedInflight and
// admitWithoutZeroRateGuard. Nothing in the datapath calls it. It exists so the
// defect -- a LIFETIME predicate reporting a currently-inert link as healthy --
// is executed on the same trace the real predicate is executed on.
func inertLifetime(c *Cap, link int) bool {
	l := &c.link[link]
	l.mu.Lock()
	defer l.mu.Unlock()
	return l.nFold == 0 && l.nUnaligned > 0
}

// B1. A link that folds ONCE and then never again is inert, and round 2 said it
// was fine. Measured on the runner against round 2: one fold followed by 199
// unalignable echoes read folds=1 unaligned=199 INERT=false, for ever -- the cap
// on, consuming echoes, measuring nothing, reporting nothing, with Admit's
// DeadIval limb returning true so there was no other symptom either. That is the
// round-1 silent-inertness class moved, not closed.
//
// Both predicates are run on ONE trace: the old one still says false, the new
// one says true.
func TestCapInertIsCurrentStateNotLifetime(t *testing.T) {
	c := newTestCap(t, 1)
	base := time.Now()

	// One good cadence: a ping, and the echo that answers it. This is the fold
	// that made the lifetime predicate blind for the rest of the process.
	capFold(c, 0, 1000, 0, 0, 500_000, 0, base)
	if c.Stats(0).Folds != 1 {
		t.Fatalf("setup: the first echo did not fold: %+v", c.Stats(0))
	}

	// Then the far end stops echoing stamps we recognise -- a stale peer, a
	// forged source, or a round trip that has outgrown a ring that can no longer
	// measure its own span. 199 echoes, one per cadence, none alignable.
	for k := 1; k <= 199; k++ {
		c.FoldEcho(uint32(90_000+k),
			capEchoPayload(uint32(500_000+k), capRec{id: 0, bytes: 1}),
			[]uint64{0}, base.Add(time.Duration(k)*PingIval))
	}

	st := c.Stats(0)
	if st.Folds != 1 || st.Unaligned != 199 {
		t.Fatalf("trace did not reproduce the measured shape: %+v", st)
	}
	if inertLifetime(c, 0) {
		t.Fatal("the round-2 predicate fired here; then this trace is not the one that " +
			"defeated it and the test proves nothing")
	}
	if !st.Inert || !c.Inert(0) {
		t.Fatalf("a link with %d unalignable echoes and no alignment for %v does not "+
			"report inert: %+v", st.Unaligned, DeadIval, st)
	}
}

// The same predicate must not fire on a healthy link, or it is noise. A link
// folding every cadence is never inert, at any N.
func TestCapInertDoesNotFireOnAFoldingLink(t *testing.T) {
	for _, n := range []int{1, 2, 3, 5, 8} {
		c := newTestCap(t, n)
		base := time.Now()
		for cad := 0; cad < 20; cad++ {
			ts := uint32(1000 + cad*int(PingIval/time.Millisecond))
			now := base.Add(time.Duration(cad) * PingIval)
			recs := make([]capRec, n)
			bps := make([]uint64, n)
			for i := 0; i < n; i++ {
				c.MarkPing(i, ts, uint64(cad)*120_000)
				recs[i] = capRec{id: byte(i), bytes: uint64(cad) * 120_000}
			}
			c.FoldEcho(ts, capEchoPayload(uint32(500_000+cad*100), recs...), bps, now)
		}
		for i := 0; i < n; i++ {
			if c.Inert(i) {
				t.Fatalf("N=%d link %d: a link folding every cadence reports inert: %+v",
					n, i, c.Stats(i))
			}
		}
	}
...
// THE DIAGNOSTIC INFLATION, pinned as a property instead of left as a surprise.
// pullrun sends one ping per link per cadence and the server answers EACH ping
// with an echo carrying EVERY link's records, so on a healthy client each link
// folds on the first echo of the cadence -- which consumes its only marker --
// and the other N-1 echoes count unaligned. unal therefore reads (N-1)x fold on
// a PERFECTLY HEALTHY path. The PSTAT comment used to tell the operator unal was
// "the alignment failing and the number to read first"; it is now INERT that is
// the number to read first, and this test is why.
func TestCapUnalignedIsNMinusOnePerFold(t *testing.T) {
	const cadences = 12
	for _, n := range []int{1, 2, 3, 5} {
		c := newTestCap(t, n)
		base := time.Now()
		for cad := 0; cad < cadences; cad++ {
			ts := uint32(1000 + cad*int(PingIval/time.Millisecond))
			now := base.Add(time.Duration(cad) * PingIval)
			recs := make([]capRec, n)
			bps := make([]uint64, n)
			for i := 0; i < n; i++ {
				c.MarkPing(i, ts, uint64(cad)*120_000)
				recs[i] = capRec{id: byte(i), bytes: uint64(cad) * 100_000}
			}
			pay := capEchoPayload(uint32(500_000+cad*100), recs...)
			// N pings out, N echoes back, each carrying every link's record.
			for e := 0; e < n; e++ {
				c.FoldEcho(ts, pay, bps, now)
			}
		}
		for i := 0; i < n; i++ {
			st := c.Stats(i)
			if st.Folds != cadences {
				t.Fatalf("N=%d link %d: folded %d of %d cadences: %+v",
					n, i, st.Folds, cadences, st)
			}
			if want := uint64(cadences * (n - 1)); st.Unaligned != want {
				t.Fatalf("N=%d link %d: unaligned %d, the wiring says exactly (N-1)x folds "+
					"= %d. If this moved, the PSTAT guidance moved with it: %+v",
					n, i, st.Unaligned, want, st)
			}
			if c.Inert(i) {
				t.Fatalf("N=%d link %d: (N-1)x unaligned on a healthy path read as inert: %+v",
					n, i, st)
			}
		}
	}
...
// B2. A NON-POSITIVE srvMS INTERVAL IS NOT A LATCH EXIT. echo.go states srvMS is
// WALL clock and that an NTP step corrupts it, so this is a live path. Round 2
// routed it into rebase(), which clears the latch and zeroes the queue
// accumulator: measured, "before: latched=true clears=0 bad=0 | after a dms<=0
// reading: latched=false clears=1 bad=1". That was a fourth, unlisted exit
// inside the block that declares the list of exits complete.
//
// The old path is EXECUTED here, not described: rebase is called directly on an
// identically-latched link, which is exactly what the old line did.
func TestCapBadIntervalIsNotALatchExit(t *testing.T) {
	latch := func() (*Cap, time.Time) {
		c := newTestCap(t, 1)
		base := time.Now()
		capFold(c, 0, 1, 0, 0, 1000, 0, base)
		capFold(c, 0, 2, 1_000_000, 500_000, 1400, 0, base.Add(400*time.Millisecond))
		if !c.Latched(0) || c.Stats(0).Clears != 0 {
			t.Fatalf("setup: not latched clean: %+v", c.Stats(0))
		}
		return c, base
	}

	// THE ATTACK, on the old code path.
	old, oldBase := latch()
	l := &old.link[0]
	l.mu.Lock()
	old.rebase(l, 1_000_000, 2_000_000, 900, 0, oldBase.Add(800*time.Millisecond))
	l.mu.Unlock()
	if old.Latched(0) || old.Stats(0).Clears != 1 {
		t.Fatalf("the old path did not release the latch here, so this test is not "+
			"exercising the defect: %+v", old.Stats(0))
	}

	// THE SAME EVENT, through the real fold. The server's wall clock steps back
	// 500 ms mid-latch.
	c, base := latch()
	capFold(c, 0, 3, 2_000_000, 1_000_000, 900, 0, base.Add(800*time.Millisecond))
	st := c.Stats(0)
	if st.BadIvals != 1 {
		t.Fatalf("the backward step was not counted as a bad interval: %+v", st)
	}
	if !st.Latched || st.Clears != 0 {
		t.Fatalf("a broken server clock released a mid-deficit latch: %+v", st)
	}
	// The rate IS dropped, and that is what keeps this from becoming a new
	// permanent latch: with no denominator Admit takes the deadlock break and
	// this link degrades to plain pull with its latch bit intact.
	if _, ok := c.DelivBytesPerMs(0); ok {
		t.Fatalf("a skipped interval left a rate behind: %+v", st)
	}
	if !c.Admit(0, base.Add(800*time.Millisecond)) {
		t.Fatal("a latched link with no denominator refused a draw: that is the permanent " +
			"latch, arriving through the clock")
	}

	// And the meter resumes on the stepped clock: the latch was preserved, not
	// frozen.
	capFold(c, 0, 4, 3_000_000, 1_500_000, 1000, 0, base.Add(1200*time.Millisecond))
	d, ok := c.DelivBytesPerMs(0)
	if !ok || math.Abs(d-5000) > 1 {
		t.Fatalf("the meter did not resume after the skipped interval: %v ok=%v", d, ok)
	}
	if st := c.Stats(0); !st.Latched || st.Clears != 0 {
		t.Fatalf("the latch did not survive the step: %+v", st)
	}
}

// The list of latch exits in cap.go's header is a CLAIM ABOUT THE CODE, and the
// project's most-repeated defect is a comment asserting a property the code
// lacks. So the claim is checked mechanically: `l.latched = false` appears in
// exactly the four places the list names, rebase is reached from exactly the two
// documented events, and the skipped-interval path calls skipInterval instead.
// A fifth exit added without updating the list fails here.
func TestCapLatchExitsAreTheDocumentedSet(t *testing.T) {
	b, err := os.ReadFile("cap.go")
	if err != nil {
		t.Fatalf("cap.go: %v", err)
	}
	src := string(b)
	if n := strings.Count(src, "l.latched = false"); n != 4 {
		t.Fatalf("cap.go clears the latch in %d places; the header lists 4 (rebase, "+
			"evaluate's too-little-traffic arm, evaluate's clear/congested arm, Admit's "+
			"DeadIval fail-open). Add the new one to that list or remove it", n)
	}
	if n := strings.Count(src, "c.rebase(l,"); n != 2 {
		t.Fatalf("rebase is called from %d sites; the documented events are a counter "+
			"regression and the first reading, i.e. 2. A third caller is a fourth latch "+
			"exit -- which is exactly the defect round 3 fixed", n)
	}
	if n := strings.Count(src, "c.skipInterval(l,"); n != 1 {
		t.Fatalf("skipInterval is called from %d sites, want 1 (the non-positive srvMS "+
			"interval)", n)
	}
}

// THE SELF-SIZING MECHANISM USED TO DIE AT ~24.9 DAYS OF UPTIME, SILENTLY.
// measureSpan requires int32(ts - mkFirst) >= 0 and mkFirst was set once, at the
// first ping ever. Once uptime passes 2^31 ms every current stamp reads as older
// than it, so no span is measurable for any echo, the ring can never deepen
// again and SpanMS freezes. Measured against round 2: at 24.9 days an ordinary
// echo lagged 100 ms gave span=0ms grows=0 markers=1 folds=0 unaligned=1.
// Routers run for months.
func TestCapMarkerFloorSurvivesTheStampEpochRoll(t *testing.T) {
	cad := uint32(PingIval / time.Millisecond)
	const roll = uint32(1) << 31 // ~24.86 days of uptime in milliseconds

	// The control first: the defect, computed the way measureSpan computes it,
	// against a floor anchored at the first ping ever.
	if int32(roll+10*cad-0) >= 0 {
		t.Fatal("the test's own stamps do not cross the epoch, so it demonstrates nothing")
	}

	c := newTestCap(t, 1)
	c.MarkPing(0, 0, 0) // the first ping ever, at stamp 0
	for k := uint32(0); k < 12; k++ {
		c.MarkPing(0, roll+k*cad, uint64(k)*1000)
	}
	// An ordinary echo answering the ping sent one cadence ago.
	c.FoldEcho(roll+10*cad, capEchoPayload(500_000, capRec{id: 0, bytes: 1}),
		[]uint64{0}, time.Now())
	st := c.Stats(0)
	if st.SpanMS != int32(cad) {
		t.Fatalf("after the epoch roll the span reads %dms, want %dms: no span means the "+
			"ring can never deepen again and the meter is dead for the life of the "+
			"process: %+v", st.SpanMS, cad, st)
	}
	if st.Grows == 0 || st.Markers <= capMarkersInitial {
		t.Fatalf("the ring did not deepen from the measured span after the roll: %+v", st)
	}
}

...
//
// THE DEFECT THIS REPLACES. runClient and runServer resolved
...
// serverPathSpace decides how wide the server's per-pathID arrays must be, and it
// is a DIFFERENT question from the client's.
//
// The server cannot discover this. The quantity is the number of sources the
// CLIENT has, and the server's own route table describes the server's own WANs --
// a different box. Probing locally would answer a question nobody asked. So the
// old default was doubly wrong here: it named the client's interfaces in the
// server's code, where a name means nothing at all and only the COUNT is used.
//
// Precedence.
//
// FIRST, AGG_PATHS set: its length. That is the client's source list as supplied
// by whatever wires the pair up, and it wins.
//
// SECOND, AGG_N set to a positive integer: that. A server operator has a count,
// not interface names, and until now there was no way to say so.
//
// THIRD, neither: the wire's own pathID space, MaxLinks. This is the same answer the
// E3 server already reached and states at server/owd.go:18 -- "Sized by MaxLinks,
// not by a configured N" -- so the two servers agree. It assumes no count, admits
// every pathID the wire can express, and costs a bounded static footprint that
// TestServerPathSpaceFootprintIsBounded measures rather than asserts.
func serverPathSpace(aggPaths, aggN string) (int, string, error) {
	if strings.TrimSpace(aggPaths) != "" {
		names, _, err := splitPaths(aggPaths)
		if err != nil {
			return 0, "", err
		}
		return len(names), "AGG_PATHS (the client's source list, supplied)", wireCeiling(len(names), "AGG_PATHS")
	}
	if v := strings.TrimSpace(aggN); v != "" {
		n, err := strconv.Atoi(v)
		if err != nil || n <= 0 {
			return 0, "", fmt.Errorf("AGG_N=%q is not a positive integer", aggN)
		}
		return n, "AGG_N (an explicit peer source count)", wireCeiling(n, "AGG_N")
	}
	return MaxLinks, "the wire's own one-byte pathID space (frame.go:9); no count was supplied and the " +
		"peer's source count is not observable from this box, so every addressable pathID is admitted " +
		"(same rule as server/owd.go:18)", nil
}

// pushFlatPrior is the AGG_W-unset per-path capacity prior for the EIF push entry
// points: parseW's own per-path fallback, applied identically to EVERY path.
//
// This is U36's answer ("AGG_W unset = no prior", its pullNoPrior on the unmerged
// branch u36-aggw-default) carried onto the push path, not a second answer
// invented here. The pull core needs no prior at
// all because it has neither CapEst nor an AIMD rate controller; the push path
// does have both -- w seeds NewCapEst(w[i]) and NewSched(w) -- so it cannot take
// an empty vector, and the equivalent statement is a FLAT one. Flat expresses no
// preference between paths; the deleted "20000,15000" expressed a preference
// between two specific ones, and left every path above the second on parseW's
// fallback anyway, so it was never a coherent vector for any N but 2.
//
// N-generic and symmetric under permutation of the source list, for every N, by
// construction: nothing here reads an index.
//
// NOT A DERIVATION. The magnitude is parseW's pre-existing fallback and no
// measurement stands behind it; it is inherited, not chosen here. What changes is
// that it is now the SAME on every path. What the per-source prior should
// actually be is a measurement nobody has taken on this hardware -- E1/G1 is the
// experiment -- and it stays an open question, as it does for the pull path.
func pushFlatPrior(n int) []float64 { return parseW("", n) }

// pushPrior resolves AGG_W for the push entry points. Set: parsed positionally,
// exactly as before, so an operator vector still binds. Unset or blank: flat.
func pushPrior(s string, n int) []float64 {
	if strings.TrimSpace(s) == "" {
		return pushFlatPrior(n)
	}
	return parseW(s, n)
}

...
// below for the current count). Nothing in pull.go, frame.go,
...
// -----------------------------------------------------------------------------
// U159 DELETED BOTH RESEQUENCER RINGS, and they were the dedup memory this
// section used to name. First-wins now lives OUTSIDE both Go modules, in
// WireGuard's anti-replay window: every copy of a datagram carries the same
// WireGuard counter, the first copy to arrive is accepted, and every later
// copy of that counter is rejected by the replay window. This layer delivers
// on arrival and does not dedup at all -- the server delivers every copy it
// receives (server TestServerDeliversEveryCopyOfADuplicatedSeq), which is a
// REQUIREMENT here, not a leak: WireGuard cannot reject a copy it never sees.
// The bound that makes this work is STATED IN CODE at both ends and is finite:
// WGReplayWindow=2048 counters over MaxReorderSpreadMS=350 ms of cross-path
// spread (pullrun.go:866,872 and server/main.go:103,109), i.e. about 5851
// frames/s before the envelope closes -- ReorderDepthFrames/ReorderEnvelopeOK,
// logged at start-up by both binaries.
...
// UNTESTED here, and now untestable in EITHER module: the peer half is
// WireGuard's, not this repo's. The window size above is the wireguard-go
// value; which implementation the client actually runs has never been read off
// a box (U159 owes Mo that one-line check).
// TRADE, not a free win: a duplicate is now rejected AFTER WireGuard decrypts
// it, so every `lightning` copy costs the receiver a wasted decrypt.
//
// -----------------------------------------------------------------------------
// TTL
// -----------------------------------------------------------------------------
// dupTTL is the reorder hold the control loop already computes and already
// passes to PullFIFO.Trim -- owd.Hold(HoldMin, HoldMax),
...
func (l *PullLink) DriveLit(f *PullFIFO, lit *Lightning) {
	...
	for {
		...
		// U250: the cap and the rank gate, which this loop did not have. Consulted
		// here for the reason Drive consults them here -- a link that must not carry
		// this frame takes none, rather than taking one and returning it.
		if !l.capGateAdmit(f) {
			if f.Closed() {
				return
			}

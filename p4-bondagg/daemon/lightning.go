...
//
//   1. THE FACT IS NOT PLUMBED YET. build_agg_env (bond-xctl:634-655) emits
//      AGG_LISTEN / AGG_SERVER / AGG_PATHS / AGG_W and nothing else; the metered
//      column stops at gl_sources and never reaches this daemon. Until
//      deploy/p5 emits AGG_SPOTTY, this daemon sees an EMPTY spotty set and
//      lightning nominates nothing -- inert, by the same degenerate path as an
//      all-steady box. That is the correct fail-safe and it is LOGGED at start,
//      not silent. deploy/p5/** is not U15b's to edit; the plumbing is reported
//      as an open item.
//      UNTESTED here: that bond-xctl would emit the right set. No hardware, and
//      no artifact under deploy/p5 is touched or executed by this unit.
//
...
func NewLightning(c *PullCore, devs []string) *Lightning {
	...
	if nsp == 0 {
		log.Printf("lightning: ENABLED but the spotty-class set is EMPTY (AGG_SPOTTY=%q "+
			...
			"protos) and build_agg_env does not yet emit it to this daemon.",
			spottyStr, devs)

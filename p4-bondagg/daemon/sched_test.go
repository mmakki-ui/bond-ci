...
func TestApplySchedWiresFanout(t *testing.T) {
	for _, tc := range []struct {
		...
		r, _ := schedRing(false, 30*time.Millisecond)
		if err := ApplySched(pol, c, r, NewRankerFor(pol, c.N())); err != nil {
	...
	if err := ApplySched(lit, c, r, nil); err != nil {
	...
	if err := ApplySched(schedPolicies["max"], c, r, nil); err != nil {
		t.Fatalf("max: %v", err)
	}
	...
	if err := ApplySched(schedPolicies["eco"], nil, nil, nil); err != nil {
...
func TestSchedEcoRefusesN2(t *testing.T) {
	...
	if p.Name != "eco" || p.Delivery != DeliverOnArrival || p.Hold != HoldNone || p.Fanout {
		t.Fatalf("eco resolved to %+v -- want deliver-on-arrival, no hold, no fan-out", p)
	}
	...
	c := &PullCore{FIFO: NewPullFIFO(), Links: schedLinks(2)}
	if err := ApplySched(schedPolicies["eco"], c, nil, nil); err == nil {

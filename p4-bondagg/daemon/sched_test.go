...
// U159 DELETED schedRing and bars 6-10 with it. They were the DELIVERY A/B --
// the same frames through both policies, asserting the delivery ORDER differed
// -- and the thing that made them differ was Ring.arrival. There is no ring at
// either end any more and every mode delivers on arrival, so the axis those
// bars measured no longer exists; keeping them by re-pointing them at something
// else would be a bar that measures nothing. What replaced them is
// deliver_test.go: the shipped path delivers on arrival, and no reorder hold is
// in the shipped tree at all.
//
// THE CONSEQUENCE, stated here because this file is where the claim lived:
// `max` and `speed` now differ in RANK ALONE. Bar 1 below is what still holds
// them apart, and pull.go's SetSched records that a nil ranker collapses the
// difference entirely.
...
func TestSchedMaxAndSpeedDifferInDeliveryAndRank(t *testing.T) {
	// NAME KEPT DELIBERATELY. docs/FEATURES.md F6 anchors on it, and after U159
	// the delivery half of the claim is answered elsewhere (deliver_test.go):
	// renaming it would silently break the feature tracker's evidence link for
	// no gain. What it asserts here is the half that is still a POLICY choice.
	...
	if mx.Rank == sp.Rank {
		t.Fatalf("rank is identical (%v): after U159 the draw order is the ONLY "+
			"axis between max and speed, so an identical rank makes them the same "+
			"datapath -- the state U17a exists to close", mx.Rank)
	}
	if sp.Rank != RankDeadlineHit {
		t.Fatalf("speed is not the settled policy: %+v", sp)
	}
	if mx.Rank != RankDrainOrder {
...
func TestSchedUnknownValueIsRefusedNotSilentlyMax(t *testing.T) {
	// The value has to be one NO table emits and this build does not implement.
	// It was "lightning" until U138 IMPLEMENTED lightning; a refusal test whose
	// refused value becomes implemented stops testing the refusal and starts
	// testing nothing, silently, which is why it is named here as a deliberately
	// unimplemented string rather than a mode.
...
func TestSchedUnsetInheritsMaxAndIsReportedAsInherited(t *testing.T) {
	p, explicit, err := SchedFromEnv(func(string) string { return "" }, 2)
	if err != nil {
	...
	if p.Name != SchedInherited || p.Rank != RankDrainOrder {
		t.Fatalf("unset must inherit the pre-AGG_SCHED datapath, got %+v", p)
	}
}

// 4
func TestSchedImplementedNamesAreTheFourSchedulers(t *testing.T) {
	got := SchedNames()
	// U138 added eco and lightning. Sorted, so the error text is stable.
	want := []string{"eco", "lightning", "max", "speed"}
	if len(got) != len(want) {
		// U144: this used to say bond-xctl's AGG_SCHED_TABLE and this build "must
		// agree on the set". That was already false when U138 landed and is a
		// DECISION now, not drift: ADR-003 ("Names") keeps AGG_SCHED_TABLE as the
		// AGGREGATE mode set -- what arity >= 2, is_agg and ecod's stand-down are
		// keyed on -- and says the daemon's scheduler map "is not that table". The
		// table names max and speed; this map implements four. So the two sets are
		// deliberately different and a mismatch here says nothing about the box.
		// What this bar pins is only that the map's OWN set is exactly these four,
		// so a scheduler cannot be added or dropped without a bar moving.
		t.Fatalf("implemented schedulers = %v, want %v. Adding or removing a scheduler "+
			"is a deliberate change and must move this list in the same commit", got, want)
	}
	for i := range want {
		if got[i] != want[i] {
...
func TestSchedDescribeNamesTheActualPolicyNotJustTheMode(t *testing.T) {
	...
	// U159 INVERTED the second half of this bar, and the inversion is the
	// finding. It used to read "max must NOT claim ON ARRIVAL", because
	// delivery was the axis that separated the modes. The reorder ring is
	// deleted at both ends now: EVERY mode delivers on arrival, so a Describe
	// line that still said "in-order ring" for `max` would be the false number
	// in a log this bar exists to prevent. What must differ is RANK.
	if !strings.Contains(sp, "ON ARRIVAL") || !strings.Contains(mx, "ON ARRIVAL") {
		t.Fatalf("a mode does not state deliver-on-arrival: max=%q speed=%q", mx, sp)
	}
	// "in-order ring", not "in-order": `drain-order` contains the shorter
	// string, which made the first cut of this check red against a correct tree.
	if strings.Contains(mx, "in-order ring") || strings.Contains(sp, "in-order ring") {
		t.Fatalf("a mode still advertises in-order delivery: max=%q speed=%q", mx, sp)
	}
	if !strings.Contains(sp, "K4") || !strings.Contains(mx, "drain-order") {
		t.Fatalf("the rank half -- the only axis left between the two -- is not in "+
			"the line: max=%q speed=%q", mx, sp)
	}
}
...
func TestApplySchedWiresBothHalves(t *testing.T) {
	...
	ApplySched(schedPolicies["speed"], nil, nil)
}
...
func TestApplySchedIsIdempotentAndReversible(t *testing.T) {
	...
	ApplySched(sp, c, NewRankerFor(sp, 2))
	ApplySched(sp, c, NewRankerFor(sp, 2))
	if c.Links[0].gate == nil {
		t.Fatal("applying the same policy twice changed the answer")
	}
	mx := schedPolicies["max"]
	ApplySched(mx, c, NewRankerFor(mx, 2))
...
// ---------------------------------------------------------------------------
// U138 -- the two schedulers the ladder was missing a datapath for, and the
// wiring that makes each of them a thing this binary DOES rather than a name.
// ---------------------------------------------------------------------------

// THE WIRING BAR FOR THE SEND HALF, and it is the same claim
// TestApplySchedWiresBothHalves makes about the other two: a policy that is
// correct and connected to nothing is the redline-6 defect verbatim. Before
// U138 the only halves ApplySched could get wrong were delivery and rank;
// `lightning` adds a THIRD, and it is the one that changes how many times a
// frame leaves the box, so it needs the same executed seam rather than an
// inspection of three lines in runPullClient.
//
// Each policy is applied at an arity it is legal at: eco only at N=1 (SchedArity
// refuses every other N -- TestSchedEcoRefusesN2), the rest at N=3.
func TestApplySchedWiresFanout(t *testing.T) {
	for _, tc := range []struct {
		name string
		n    int
	}{{"max", 3}, {"speed", 3}, {"eco", 1}, {"lightning", 3}} {
		pol := schedPolicies[tc.name]
		c := &PullCore{FIFO: NewPullFIFO(), Links: schedLinks(tc.n)}
		if err := ApplySched(pol, c, NewRankerFor(pol, c.N())); err != nil {
			t.Fatalf("%s at N=%d: ApplySched refused: %v", tc.name, tc.n, err)
		}
		if c.Fanout() != pol.Fanout {
			t.Fatalf("%s: core fanout=%v, want %v -- the SEND half of the policy never "+
				"reached the pool shape, so `lightning` would run U7's one-copy datapath "+
				"under lightning's name", tc.name, c.Fanout(), pol.Fanout)
		}
		if pol.Fanout {
			if len(c.FIFOs) != tc.n {
				t.Fatalf("%s: %d pools for %d links -- fan-out is one pool PER LINK",
					tc.name, len(c.FIFOs), tc.n)
			}
			seen := map[*PullFIFO]bool{}
			for i, f := range c.FIFOs {
				if f == nil {
					t.Fatalf("%s: pool %d is nil", tc.name, i)
				}
				if seen[f] {
					t.Fatalf("%s: pool %d is the SAME PullFIFO as an earlier link's -- "+
						"two links sharing a pool would each draw half the frames, which "+
						"is the shared pool wearing fan-out's name", tc.name, i)
				}
				seen[f] = true
			}
		} else if c.FIFOs != nil {
			t.Fatalf("%s: FIFOs installed with Fanout=false -- nil is the shared pool "+
				"and is what keeps every other mode byte-identical to U7", tc.name)
		}
		for i, l := range c.Links {
			if l.fanout != pol.Fanout {
				t.Fatalf("%s: link %d fanout=%v, want %v -- the send path decides the "+
					"downlink hint off this flag", tc.name, i, l.fanout, pol.Fanout)
			}
		}
	}

	// Reversible in both directions, and the shared pool must come back intact:
	// a core flipped lightning -> max is the pull core U7 wrote.
	c := &PullCore{FIFO: NewPullFIFO(), Links: schedLinks(3)}
	lit := schedPolicies["lightning"]
	if err := ApplySched(lit, c, nil); err != nil {
		t.Fatalf("lightning: %v", err)
	}
	pools := c.FIFOs
	if err := ApplySched(lit, c, nil); err != nil {
		t.Fatalf("lightning twice: %v", err)
	}
	if &pools[0] != &c.FIFOs[0] {
		t.Fatal("applying the same fan-out policy twice rebuilt the pools -- not idempotent")
	}
	if err := ApplySched(schedPolicies["max"], c, nil); err != nil {
		t.Fatalf("max: %v", err)
	}
	if c.FIFOs != nil || c.Links[0].fanout {
		t.Fatal("`max` left fan-out installed")
	}

	// Total: a nil core must not panic and must not be an arity error.
	if err := ApplySched(schedPolicies["eco"], nil, nil); err != nil {
		t.Fatalf("nil core: %v -- N is unknown, which is not an arity failure", err)
	}
}

// `eco` IS the single-source datapath, so it is REFUSED at any other arity --
// the U17a refusal doctrine applied to N instead of to a name. Both sides say
// it: SchedFromEnv answers about the environment before a socket is bound, and
// ApplySched answers about the core it is about to wire.
//
// The other three schedulers are N-generic and this must not touch them.
func TestSchedEcoRefusesN2(t *testing.T) {
	eco := func(string) string { return "eco" }
	for _, n := range []int{2, 3, 5} {
		p, explicit, err := SchedFromEnv(eco, n)
		if err == nil {
			t.Fatalf("N=%d: AGG_SCHED=eco was ACCEPTED and resolved to %+v. eco has no "+
				"rule for choosing among %d sources; serving it would report the box in "+
				"a mode the wire was not in", n, p, n)
		}
		if !explicit {
			t.Fatal("a refused eco must still report AGG_SCHED as explicitly set")
		}
		if !strings.Contains(err.Error(), "eco") || !strings.Contains(err.Error(), "lightning") {
			t.Fatalf("N=%d: the refusal must name what it refused and what to use "+
				"instead: %v", n, err)
		}
	}
	p, _, err := SchedFromEnv(eco, 1)
	if err != nil {
		t.Fatalf("N=1 is eco's ONLY legal arity and it was refused: %v", err)
	}
	if p.Name != "eco" || p.Rank != RankDrainOrder || p.Fanout {
		t.Fatalf("eco resolved to %+v -- want drain-order, no fan-out", p)
	}

	// The same rule from the other side, and it is the one a running daemon hits
	// if the environment and the source set are resolved at different moments.
	c := &PullCore{FIFO: NewPullFIFO(), Links: schedLinks(2)}
	if err := ApplySched(schedPolicies["eco"], c, nil); err == nil {
		t.Fatal("ApplySched wired eco onto a 2-link core")
	}
	if c.Fanout() || c.Links[0].gate != nil {
		t.Fatal("a REFUSED policy still mutated the core -- the refusal must happen " +
			"before anything is installed")
	}

	// N-genericity of the rest, asserted rather than assumed.
	for _, name := range []string{"max", "speed", "lightning"} {
		for _, n := range []int{1, 2, 3, 5} {
			if _, _, err := SchedFromEnv(func(string) string { return name }, n); err != nil {
				t.Fatalf("%s at N=%d was refused: %v -- only eco is arity-bound", name, n, err)
			}
		}
	}
}

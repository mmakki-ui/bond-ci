...
// forever, on a box with no console. Executed rather than argued.
	...
	for _, n := range []int{1, 2, 3, 5, 8} {
		for iter := 0; iter < 400; iter++ {
			...
			for i := 0; i < n; i++ {
				...
				di := int(next()) % len(ds)
				hd := int(next())%2 == 0
				setStats(r, i, hs[hi], hh, ds[di], hd)
			}
			// antisymmetry
			for a := 0; a < n; a++ {
				for b := 0; b < n; b++ {
					if a != b && r.Better(a, b) && r.Better(b, a) {
						t.Fatalf("n=%d iter=%d: %d and %d are each strictly better than "+
							"the other", n, iter, a, b)
					}
				}
			}
			// a source node always exists
			free := 0
			for a := 0; a < n; a++ {
				blocked := false
				for b := 0; b < n; b++ {
			...
			if free == 0 {
				t.Fatalf("n=%d iter=%d: EVERY link is blocked by another. The pool would "+
					"stall until the control tick and then stall again", n, iter)
			}
		}
	}
}

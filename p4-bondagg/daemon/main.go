...
// ---------------- client ----------------
func runClient() {
	// ---- U17a: this entry point implements ONE scheduler, and says so --------
	//
	// The shipped procd stanza launches AGG_MODE=client -- the EIF PUSH client,
	// which ADR-002 superseded -- and it now also passes AGG_SCHED (bond-xctl,
	// deploy/p5/init.d/bond-agg). That combination is the sharpest form of the
	// defect U17a exists to close: the orchestration would report the box in
	// `speed` while this binary ran the push datapath, which has no
	// deliver-on-arrival, no rank, and no way to acquire either -- `speed`'s
	// mechanisms live in the PULL core (pull.go / sched.go), and this stack is
	// deliberately frozen as the validated reference and the mid-bufferbloat
	// fallback.
	//
	// So it REFUSES rather than serving `speed` as `max`. That is a behaviour
	// change and it is the intended one: a mode this entry point cannot serve
	// must fail where somebody can see it. The CLIENT box is
	// reachable and recoverable, bond-xctl's engage verifies and reverts, and
	// nothing is deployed. The alternative -- start anyway -- is the silent
	// substitution itself.
	//
	// WHICH STANZA SHOULD RUN WHICH ENTRY POINT is NOT decided here: that is the
	// AGG_MODE=client-vs-pull-client question the U17a ROADMAP row names, it is
	// E1-gated, and flipping it is a deploy/p5 edit with its own bars.
	if pol, set, err := SchedFromEnv(func(k string) string { return env(k, "") }); err != nil {
		log.Fatalf("client: %v", err)
	} else if set && pol.Rank != RankDrainOrder {
		log.Fatalf("client: AGG_SCHED=%s, but AGG_MODE=client is the EIF PUSH reference "+
			"and implements %q only (%s). REFUSING to start rather than running %q while "+
			"the orchestration believes the box is in %q. The scheduler split lives in the "+
			"PULL core: run AGG_MODE=pull-client.",
			pol.Name, SchedInherited, schedPolicies[SchedInherited].Describe(),
			SchedInherited, pol.Name)
	}
	...
	// U36a: the source list is resolved, not assumed. AGG_PATHS when set wins
	// verbatim (the reconciler is the discoverer); unset, the sources are the
	// devices that currently carry an up default route, read from the kernel; and
	// only a box with no default route at all is refused. Contract, mechanism and
	// the reason for the mechanism: discover.go. This replaces a literal
	// two-interface default that SET N, and so described a two-source box on a
	// client that now has three.
	devs := mustSources("client")
	N := len(devs)
	...
	// U36a: AGG_W unset = a FLAT prior over the discovered N, which expresses no
	// preference between paths. The deleted default expressed a preference between
	// the first two and left every path above them on parseW's fallback, so it was
	// never a coherent vector for any N but 2. Same statement U36 made on the pull
	// path (its pullNoPrior, on the unmerged branch u36-aggw-default), carried here
	// rather than answered a second way.
	w := pushPrior(env("AGG_W", ""), N)
...
// ---------------- server ----------------
func runServer() {
	// U17a: the server is MODE-BLIND by design (C4), and that is a decision, not
	// an oversight -- so it is stated once here rather than left as the absence
	// that made `grep -rn AGG_SCHED p4-bondagg/server/` read as a gap. Both
	// schedulers differ only in what the CLIENT does: which link it draws on,
	// and how the client's own receive ring releases. The server's job -- unpack,
	// ring by seq, forward, echo -- is identical under both, so it consumes no
	// AGG_SCHED and must not grow a mode. If it is ever set here it is a stanza
	// mistake, and saying so beats ignoring it.
	if v := env("AGG_SCHED", ""); v != "" {
		log.Printf("server: AGG_SCHED=%q IGNORED. The server is mode-blind by design "+
			"(C4): max and speed differ only in the client's draw order and the client's "+
			"own delivery policy. Nothing on this side changes with the scheduler, and "+
			"nothing here should. Setting it is harmless and means nothing.", v)
	}
	listen := env("AGG_LISTEN", ":59402")
	wgStr := env("AGG_WG", "127.0.0.1:51820")
	// U36a: the server's N is the CLIENT's source count, which is not observable
	// from this box -- so the old default did not merely name the wrong interfaces,
	// it named interfaces at all in a place where only the count is ever used.
	// AGG_PATHS wins, then AGG_N, then the wire's own pathID space. See
	// serverPathSpace in discover.go, and server/owd.go:18 for the same conclusion
	// reached independently by the E3 server.
	N, nOrigin, nErr := serverPathSpace(env("AGG_PATHS", ""), env("AGG_N", ""))
	if nErr != nil {
		log.Fatal("server: ", nErr)
	}
	la, _ := net.ResolveUDPAddr("udp4", listen)
	...
	w := pushPrior(env("AGG_W", ""), N)
	log.Printf("server: %s -> %s (N=%d, pathID space from %s)", listen, wgStr, N, nOrigin)
	...
	var epMu sync.Mutex
	// U36a: lastRx starts at ZERO here, so a pathID that has never been heard from
	// is DEAD, not alive. Required by the default N above: with the pathID space
	// admitted whole, seeding every entry to now would make all 256 look alive for
	// one DeadIval and let eif.Pick choose a pathID with no endpoint.
	//
	// Behaviour-neutral at any N, not merely safe at a large one: the downlink can
	// only be sent to eps[p], which is nil until a frame arrives on p, so a
	// never-heard-from path could never carry a packet under the old seeding
	// either -- sendData returned false and the frame went to Backup or was
	// dropped. What changes is that the aliveness flag now says so.
	//
	// Deliberately NOT done in runClient, where the same seeding IS load-bearing:
	// the client picks a path before any pong has arrived, so zeroing it there
	// would txdrop every frame until the first receive. Asymmetric, for that
	// reason.
	lastRx := make([]int64, N)

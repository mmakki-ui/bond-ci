...
//
// U144: THIS COMMENT CARRIES NO `file.go:LINE` CITES, DELIBERATELY. It shipped
// with about forty of them and U138 alone invalidated most: `sched.go:173`,
// `pullrun.go:374/463/673` and a dozen others named unrelated lines within one
// unit of landing, while the prose around them stayed correct. Every reference
// below names a FILE and a SYMBOL or a verbatim code fragment, which `grep`
// resolves and a line-moving edit cannot rot. Keep it that way.
//
...
//
//   - PullLink (pull.go, `type PullLink`; allocated by NewPullCore, `c.Links =
//     append(...)`) -- one per path, unconditional.
//
//   - OWD (paths.go, `type OWD`; allocated by NewOWD) -- 3 parallel N-slices
//     (rel, jit, init); per-path share is one float64 + one float64 + one
//     bool.
//
//   - rxEst (qtrack2.go, `type rxEst`; allocated by NewRxEstSet, `est:
//     make([]rxEst, n)`) -- one per path, unconditional.
//
//   - LossMeter (lossmeter.go, `type LossMeter`; allocated by runPullClient,
//     the `lossM []*LossMeter` array plus its `lossM[i] = &LossMeter{}` loop).
//
//   - sLossE, delivBytes, lossByte (pullrun.go, the three `make([]T, N)` lines
//     runPullClient writes beside lossM) -- one
//     float64/uint64/uint32 each, per path.
//
//   - buf (pullrun.go, the per-path RX goroutine's `make([]byte, MaxAuthFrame)`) -- one per-path RX
//     goroutine allocates its own buffer, unconditionally, sized MaxAuthFrame
//     (HdrLen+6+MaxPayload+MacLen, frame.go/auth.go) = 1530 B.
//
//   - bpress (pullrun.go, `bpress := make([]uint64, N)` -- the bare
//     `make([]uint64, N)` matches twice in that file, delivBytes being the
//     other, so the anchor carries the assignment) -- same goroutine, also
//     unconditional (runPullClient's capOn gate only decides
...
//
//   - out (pull.go, PullLink.Drive's `make([]byte, PullSendBufLen)`) -- the per-path TX
...
//     `l.disabled()` early-return at the top of Drive, so every admitted link's
//     Drive goroutine (one per link, PullCore.Start)
//     allocates it. PullSendBufLen = MaxPayload+HdrLen+MacLen (pull.go, the const)
//     = 1500+16+8 = 1524 B.
//
...
//
//   - pc (pullrun.go, runPullClient's `make([]*net.UDPConn, N)`) -- pointee (the net.UDPConn/fd)
//     is OS-owned and NOT summed (see NOT counted, below); only the slot is
//     a daemon-side allocation, so only the slot counts.
//
//   - lossM (pullrun.go, runPullClient's `make([]*LossMeter, N)`) -- pointee IS summed by value
...
//     points to (the `lossM[i] = &LossMeter{}` loop beside it), so it counts
//     in addition to the struct sum, not instead of it.
//
//   - c.Links (pull.go, `[]*PullLink`, via NewPullCore's `append`) --
...
//   - cap.go's capLink (NewCap, `c := &Cap{cfg: cfg, link:
...
//   - lightning.go's spotty/perLink (the two `make(...)` lines in the
//     `&Lightning{}` literal): NewLightning returns nil unless
//     AGG_LIGHTNING=1 ("nil when lightning is OFF -- which is the DEFAULT",
//     its own header) AND, since U138, returns nil unconditionally under
//     AGG_SCHED fan-out (TestLightningIsOffUnderFanoutWhateverTheFlagSays).
//   - lightning.go's DriveLit `out` buffer (`out :=
//     make([]byte, MaxPayload+HdrLen)`): DriveLit only ever runs as a
//     goroutine launched by Lightning.Start, and only on the non-nil
//     receiver branch (Lightning.Start: `if lit == nil { c.Start();
//     return }`, else `go c.Links[i].DriveLit(...)`) -- i.e. only when
//     AGG_LIGHTNING=1. The default (lit==nil) branch launches Drive
//     instead, whose own `out` (PullLink.Drive) IS counted above. Same gate,
//     same exclusion, as spotty/perLink.
//   - sched.go's Ranker (pend/hit/miss/..., `type Ranker`): NewRankerFor
//     returns nil unless the policy is RankDeadlineHit (AGG_SCHED=speed);
//     `max` needs none (NewRankerFor returns nil on `p.Rank != RankDeadlineHit`). PullLink.gate stays a nil 16-byte
//     interface value.
//   - the reorder ring (DELETED by U159 -- it was the one per-daemon
//     allocation on this list, and it is gone from both ends).
//   - lossmeter.go's `m.seen` map (`m.seen =
//     make(map[uint32]struct{})`): lazily allocated on the first in-window
//     DATA (LossMeter.Data) -- so unlike the cap/lightning/
...
//     to fill; drain() bounds it IN TIME -- any
...
//     (drain's `now.Sub(m.blockAt) <= hold`) -- but the code states
...
//   - pullNoPrior's return slice (pullrun.go, `func pullNoPrior(n int)
//     []float64 { return make([]float64, n) }`), fed straight into
//     NewRxEstSet (`rxEst := NewRxEstSet(pullNoPrior(N))`): NewRxEstSet copies each element into
//     rxEst.floor/relQF and keeps no field pointing at
//     the slice itself (`type RxEstSet` has none) -- so it is
...
//
//   - PullFIFO (pull.go, `type PullFIFO`; allocated by NewPullFIFO) plus
//     the THREE sync.Cond values NewPullFIFO builds on it (cv/dcv/rcv). Under AGG_SCHED=lightning PullCore.SetFanout (pull.go)
...
//
//   - pull.go, PullFIFO.grow's ring copy and enqLocked's payload copy. These
//     are per-EVENT, not per-path. The reason USED to be "ONE FIFO per core"
//     and that reason is dead: since U138 fan-out allocates one pool PER LINK
//     (which is why the pool STRUCTURE is counted per path above). They stay
//     off the sum because they are ring growth and pool CONTENT, excluded for
//     the data-dependent reason already stated -- not because there is one pool.
//   - pullrun.go, the pong builder's `pr := make([]byte, HdrLen+pongLen+MacLen)`
//     -- per-pong temporary, discarded after the echo is built.
//   - pullrun.go, the control goroutine's `pb := make([]byte, HdrLen+MacLen)`
//     -- one buffer, one goroutine.
//   - pullrun.go, the WG reader's `buf := make([]byte, MaxPayload)` -- one
//     buffer, one goroutine.
//   - cap.go CapConfigFromEnv's `vals` map and sched.go SchedNames's `out`
//     slice -- startup/name-list allocations, once per process, not per path.
//   - lightning.go, the copy queue's `nq := make([]litCopy, cap0)` and Stats's
//     `per := make([]uint64, len(lit.perLink))` -- Lightning's own queue and
//     stat snapshot; nil unless AGG_LIGHTNING=1.

...
//
// They said nothing about the FROZEN push reference, whose two entry points
// carried parseW(env("AGG_W", "20000,15000"), N) and which U36 deliberately did
// not edit (ROADMAP U36). U128 has since DELETED those entry points (ADR-002 as
// amended by U127; the reference is the tag `eif-push-reference`), so the
// asymmetric literal is no longer reachable from anything on this tree.
// TestPushDefaultIsStillTwoShaped below is now purely the POSITIVE CONTROL that
// keeps the pull bars non-vacuous: it calls parseW directly to show the literal
// WOULD produce an asymmetric vector if the pull path ever read it. That is why
// parseW survives in main.go -- see its comment there.
...
// TestPushDefaultIsStillTwoShaped: POSITIVE CONTROL, kept for one job only.
// U128 deleted both push entry points from this tree (main.go header,
// eif-push-reference tag); no push behaviour exists here to assert or to
// break. What remains is the literal itself, frozen in ROADMAP U36's own
// wording and never edited (U36 deliberately left the frozen P4 push
// reference alone). This bar runs that literal through parseW -- the same
// parser the deleted push client used -- and shows it WOULD produce a
// privileged-path vector at the client's declared N=4 (docs/INTENT.md:193):
// [20000, 15000, 10000, 10000]. That is the positive control
// TestPullSeedIsIndependentOfAggW needs: without it, a green pull-side bar
// could not be told apart from a bar that cannot fail, because aggWCases'
// literal would be unproven capable of asymmetry. It is not a push-behaviour
// assertion and nothing here fails if push is ever un-deleted or re-edited
// elsewhere -- parseW is not called from any push path any more, only from
// this test.
...
// the direct reintroduction -- copying the deleted push client's
// `parseW(env("AGG_W", ...))` line into the pull path -- which is the one a
// future edit would actually make.

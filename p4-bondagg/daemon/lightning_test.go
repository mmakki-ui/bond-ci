...
//   * the PEER half of first-wins (since U159 it is WireGuard's anti-replay
//     window, outside both Go modules entirely -- nothing here compiles it),
//     and
...
// U138 made NewLightning return nil UNCONDITIONALLY under AGG_SCHED fan-out,
// whatever AGG_LIGHTNING says: under fan-out every source already carries every
// frame, so a copy here is a copy of a copy -- a third transmission of a seq the
// peer has already deduped, charged to the host link's BlockedMs (E1's
// discriminator). It is also what keeps Lightning.Start's `c.FIFO` line (the
// SHARED pool, which is empty under fan-out) unreachable rather than a branch.
//
// THAT GUARD SHIPPED WITH NO BAR. U138's own adversarial lens seeded `if false {`
// on it and the whole daemon suite stayed GREEN -- safety code that could not be
// shown able to fail. This is the bar that reddens on exactly that seed (U144).
//
// The NEGATIVE CONTROL is the point of the test: the same environment with the
// shared pool MUST build a Lightning. Without it a nil could come from the flag,
// the class fact, or anything else that returns nil, and the test would pass
// while measuring nothing.
func TestLightningIsOffUnderFanoutWhateverTheFlagSays(t *testing.T) {
	all := []string{"wan0", "wan1", "wan2", "wan3", "wan4"}
	for _, n := range []int{1, 2, 3, 5} {
		devs := all[:n]
		// Flag ON and every source named SPOTTY: every OTHER reason
		// NewLightning has to return nil is removed, so a nil below can only
		// be the fan-out guard.
		t.Setenv("AGG_LIGHTNING", "1")
		t.Setenv("AGG_SPOTTY", strings.Join(devs, ","))

		ctl := litLinks(devs, litSocks(n))
		if ctl.Fanout() {
			t.Fatalf("N=%d: control core must be the SHARED pool", n)
		}
		if lit := NewLightning(ctl, devs); lit == nil {
			t.Fatalf("N=%d: CONTROL FAILED -- AGG_LIGHTNING=1 with no fan-out must "+
				"build a duplicator; this test would otherwise prove nothing", n)
		}

		c := litLinks(devs, litSocks(n))
		c.SetFanout(true)
		if !c.Fanout() {
			t.Fatalf("N=%d: SetFanout(true) did not take", n)
		}
		if lit := NewLightning(c, devs); lit != nil {
			t.Fatalf("N=%d: AGG_SCHED fan-out must force the spotty-class duplicator "+
				"OFF whatever AGG_LIGHTNING says -- every source already carries every "+
				"frame, so this would be a copy of a copy on a deduped seq, and "+
				"Lightning.Start would hand every link the empty SHARED pool. got %+v",
				n, lit)
		}
	}
}
...
// ---------------------------------------------------------------------------
// U250 -- THE CAP AND THE RANK GATE ON THE LIGHTNING LOOP
//
// DriveLit is the loop EVERY link runs once spotty-class duplication is armed,
// and it consulted neither l.cap.Admit nor l.gate.MayDraw. So arming the
// duplicator silently dropped the delivered-rate cap on every link and speed's
// rank gate with it. `capGateAdmit` (pull.go) is the fix and BOTH of its limbs
// get their own bar here: one bar covering "the call is present" would pass on
// a seed that omitted either half, because each half alone still blocks the
// other's fixture. Omitting the cap reddens only the first; omitting the gate
// reddens only the second. A/B'd in both directions, U250.
// ---------------------------------------------------------------------------

// The CAP limb, and it is asserted on the COPY queue as well as the pool: a link
// past its delivered-rate cap must send neither a native frame nor a duplicate,
// and the copy limb sits BELOW the cap check in the loop, so a cap consulted in
// the wrong place would still leak copies. This is the DriveLit counterpart of
// TestPullDriveConsultsTheCapBeforeDrawing (cap_test.go) and it keeps that bar's
// live-meter refresher for the same reason: Admit reads time.Now(), so without
// it "the gate stayed shut" would rest on the observation window beating
// Admit's DeadIval fail-open, which is a wall-clock race and not the property.
func TestDriveLitConsultsTheCapBeforeDrawing(t *testing.T) {
	cfg := capTestCfg()
	...
	// Times must be near time.Now(): Admit's staleness horizon is the wall clock
	// the datapath itself uses.
	base := time.Now()
	capFold(cp, 1, 1, 0, 0, 1000, 0, base)
	capFold(cp, 1, 2, 100_000, 50_000, 1100, 0, base.Add(100*time.Millisecond))
	capFold(cp, 1, 3, 200_000, 100_000, 1200, 0, base.Add(200*time.Millisecond))
	if cp.Admit(1, time.Now()) {
		t.Fatal("fixture: link 1's cap is open, so this bar cannot test a closed one")
	}
	...
	c.SetCap(cp)
	now := time.Now()
	// Native work for link 1 to draw...
	for i := 0; i < 20; i++ {
		c.FIFO.Enqueue(make([]byte, 100), now)
	}
	// ...and copies from the spotty link 0 for it to host, so BOTH limbs of the
	// loop have something to send if the cap is not consulted.
	for i := 0; i < 5; i++ {
		lit.Nominate(c.Links[0], litFrame(uint32(1000+i), 100), now)
	}
	if depth, _, _, _, _, _, _, _, _ := lit.Stats(); depth != 5 {
		t.Fatalf("fixture: the copy queue holds %d copies, want 5 -- the copy limb of "+
			"this bar would measure nothing", depth)
	}
	go c.Links[1].DriveLit(c.FIFO, lit)

	// Keep the deficit LIVE for the whole observation (see the doc comment).
	stop := make(chan struct{})
	done := make(chan struct{})
	go func() {
		defer close(done)
		ts, sent, rx, srv := uint32(3), uint64(200_000), uint64(100_000), uint32(1200)
		for {
			select {
			case <-stop:
				return
			...
			rx += 50_000
			srv += 100
			capFold(cp, 1, ts, sent, rx, srv, 0, time.Now())
		}
	}()
	...
	close(stop)
	<-done
	if got := socks[1].tries(); got != 0 {
		t.Fatalf("DriveLit wrote %d frame(s) with its delivered-rate cap latched shut: "+
			"the lightning loop is ignoring the cap Drive consults", got)
	}
	if depth, _, _, drawn, _ := c.FIFO.Stats(); depth != 20 || drawn != 0 {
		t.Fatalf("a closed cap disturbed the pool: depth=%d drawn=%d, want 20 and 0", depth, drawn)
	}
	if depth, _, _, _, _, _, _, _, _ := lit.Stats(); depth != 5 {
		t.Fatalf("a closed cap leaked %d copy/copies to the wire limb: the cap is consulted "+
			"below the copy draw, not above it", 5-depth)
	}
	if st := cp.Stats(1); st.Clears != 0 {
		t.Fatalf("the cap was held shut by staleness or a clear, not by the bound: %+v", st)
	}
}

// The RANK-GATE limb. It is the loop EVERY link runs once spotty-class
// duplication is armed, and it consulted neither the cap nor the gate, so arming
// the duplicator silently turned speed's gate off on every link.
func TestDriveLitHonoursTheRankGate(t *testing.T) {
	socks := litSocks(2)
	c := litLinks([]string{"a", "b"}, socks)
	lit := litFor(c, []bool{true, false})
	defer c.FIFO.Close()
	...
	if g.MayDraw(1) {
		t.Fatal("fixture: link 1 must be gated shut by a better-ranked link, or this bar " +
			"measures nothing")
	}
	c.Links[1].gate = g
	now := time.Now()
	...
	go c.Links[1].DriveLit(c.FIFO, lit)
	if waitFor(func() bool { return socks[1].tries() > 0 }, 700*time.Millisecond) {
		t.Fatalf("DriveLit drew %d frame(s) while its rank gate was closed: the lightning "+
			"loop is ignoring the gate the `speed` scheduler installs", socks[1].tries())
	}
}

	...
	go func() {
		...
		for {
			...
			if now.Sub(lastStat) > time.Second {
				...
				fmt.Fprintf(&sb, "PSTAT n=%d depth=%d peak=%d qb=%d/%d peakb=%d enq=%d drawn=%d stale=%d qdrop=%d retq=%d hold=%dms del=%d skip=%d rxshed=%d authok=%d authbad=%d authshed=%d sealshort=%d gate=%d",

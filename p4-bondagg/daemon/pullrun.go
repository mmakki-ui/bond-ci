...
//
// The receive side is now four lines too: read a downlink frame, hand the
// payload to the WG socket. U159 DELETED the reorder ring at both ends -- see
// the DELIVER ON ARRIVAL block below. The control cadence is unchanged reuse of
// already-validated pieces: OWD (paths.go), RxEstSet (qtrack2.go) and LossMeter
// (fec.go).
//
...
// the E3 receive job (unpack -> forward to WG at arrival). The
...
func runPullClient() {
	...
	// U225/M2: the read-only stats file, read HERE with the other AGG_ env so the
	// daemon's whole environment surface is in one place. UNSET IS THE DEFAULT AND
	// THE DEFAULT IS NIL: with AGG_STATS absent this daemon writes no file, takes
	// no new lock and logs exactly the bytes it logged before this unit existed
	// (stats.go, the OFF-path rule). The shipped init script sets the literal
	// /var/run/p5/datapath.stats (deploy/p5/init.d/bond-agg).
	statsSink := NewStatsSink(env("AGG_STATS", ""))
	// The daemon's own start instant, and the only thing it is used for is the
	// `up=` field of that file: a reader that sees up= go backwards knows the
	// daemon respawned, which no other field in the snapshot tells it.
	...
	// ---- receive side: downlink -> WG, AT ARRIVAL ----
	owd := NewOWD(N)
	...
	var rxDeliver, rxShed uint64
	var wgAddr *net.UDPAddr
	var wgMu sync.Mutex
	// DELIVER ON ARRIVAL -- THE WHOLE RECEIVE PATH (U159, Mo 2026-09-02).
	//
	// The reorder ring is DELETED at both ends. It could never RECOVER a frame
	// -- this layer retransmits nothing -- so a hold only ever re-sequenced, at
	// a head-of-line cost of the full hold on every downlink loss. It was ALSO
	// the DEDUP memory that first-copy-wins needs for `lightning`, which puts
	// one copy of every frame on every source, and that is the part that has to
	// be replaced rather than dropped:
	//
	// WireGuard does the dedup now. Its anti-replay window rejects a repeated
	// counter, and wgSock is the last hop before it. TWO consequences, stated
	// here rather than discovered on the box:
	//  1. a duplicate is rejected AFTER WireGuard decrypts it, so `lightning`
	//     costs the receiver a wasted decrypt per extra copy -- a trade, not a
	//     free win;
	//  2. the anti-replay WINDOW is now load-bearing for reorder tolerance --
	//     see WGReplayWindow / ReorderEnvelopeOK below for the bound and for
	//     the one-line read of which implementation the client runs.
	//
	// The payload is handed to WriteToUDP WITHOUT a copy: nothing retains it
	// across the call, which is the whole reason there is no copy to make.
	deliver := func(b []byte) {
	...
	// U17a/U138: the policy becomes datapath state here, in one call, before any
	// RX goroutine exists and before core.Start (ApplySched says why each of
	// those matters). U159 removed its DELIVERY half -- every mode now delivers
	// on arrival, so there is no receive-side knob left for a policy to set.
	if err := ApplySched(sched, core, ranker); err != nil {
	...
	// CALLS ReorderDepthFrames/ReorderEnvelopeOK rather than restating their
	// arithmetic: the earlier form recomputed the bound inline, which left both
	// functions with no caller outside the bar that checks them.
	fpsCeil := WGReplayWindow * 1000 / MaxReorderSpreadMS
	log.Printf("pull-rx: DELIVER ON ARRIVAL (every mode, AGG_SCHED=%s). No hold, no "+
		"in-order release, no straggler skip, no dedup here. Reorder tolerance and "+
		"duplicate rejection are WireGuard's anti-replay window: %d frames against a "+
		"design spread of %d ms -- %d frames deep at the ceiling of %d frames/s "+
		"(~%d Mbit/s at %d-byte frames), inside the window: %v",
		sched.Name, WGReplayWindow, MaxReorderSpreadMS,
	...
	for i := 0; i < N; i++ {
		go func(p int) {
			...
			for {
				...
				atx.MarkRx(now)
				core.Links[p].MarkRx()
				// f.seq is NOT bound. The frame format still carries seq and Unpack
				// still returns it (TestClientWireSeqFieldSurvivesTheRingDeletion
				// pins that), but nothing between here and WireGuard reorders or
				// dedups by it (U159) -- and a binding kept alive by `_ = sq` is a
				// consumer that does not exist.
				ts, fseq, pay := f.ts, f.fseq, f.pay
				switch f.base {
				case FlagPing:
	...
	// ---- control cadence: pings, liveness, pool trim, STAT ----
	go func() {
		...
		for {
			...
			//
			// hd was the SAME hold the reorder ring used, which is how it
			// entered without a derivation of its own; U159 deleted that ring,
			// so this SENDER residence budget is now hd's only consumer and it
			// owns the debt outright. hd = clamp(spread+3*jit+250,
			...
			if now.Sub(lastStat) > statIval {
				...
				// hold= is the SENDER pool/copy residence budget, not a receiver
				// reorder horizon: U159 deleted the ring that used to report it,
				// and a field that kept the old meaning would be exactly the
				// false number in a log that the ring's own HoldDur avoided.
				// skip= is gone with it -- nothing on the receive path can skip.
				fmt.Fprintf(&sb, "PSTAT n=%d sched=%s depth=%d peak=%d qb=%d/%d peakb=%d enq=%d drawn=%d stale=%d qdrop=%d retq=%d hold=%dms del=%d rxshed=%d authok=%d authbad=%d authshed=%d sealshort=%d gate=%d",
					core.N(), sched.Name, depth, peak, qb, qbMax, qbPeak, enq, drawn, stale, qdrops, retq,
					owd.Hold(HoldMin, HoldMax).Milliseconds(),
					atomic.LoadUint64(&rxDeliver),
				...
				// U225/M2: ONE formatter. The string built above is the log line
				// AND the stats file's first line -- statsEmit logs it and hands
				// the same string to the sink, which embeds it verbatim. There is
				// no second fmt for the file, so the portal can never show a
				// number that no log line printed.
				//
				// The per-link loss axis is sLossE, computed by the LossIval limb
				// of THIS goroutine a few lines above and never printed until
				// now: same goroutine, so no new lock and no new read of core.
				// The ifname slice is built only when the sink is on, so the OFF
				// path adds one nil comparison per second and nothing else.
				var ifn []string
				if statsSink != nil {
					ifn = make([]string, 0, len(core.Links))
					for i := range core.Links {
						ifn = append(ifn, core.Links[i].Ifname())
					}
				}
				statsEmit(statsSink, sb.String(), now, now.Sub(daemonStart), ifn, sLossE)
			}
			time.Sleep(PingIval)
		}
	}()
...
// THE BOUND DELIVER-ON-ARRIVAL RELIES ON (U159). Mirrored VERBATIM in
// p4-bondagg/server/main.go: the two ends must not drift on it, and there is no
// package they can share.
//
// With no reorder ring, the wire's own cross-path reorder reaches WireGuard
// unsmoothed and WireGuard's anti-replay window is the only thing that tolerates
// it. That window counts FRAMES, so the bound has to be a frame count too --
// which is precisely why the deleted hold, stated in milliseconds, could never
// be compared against it. This is the "port-time check" of
// docs/knowledge/design/modes-max-speed-design.md:248 and its open item 7
// (:404), turned into a number.
const (
	...
	// Linux kernel module's window is larger (COUNTER_BITS_TOTAL 8192 less one
	// redundant word). 2048 is the SMALLER of the two, so designing against it
	// is safe whichever the client runs.
	//
	// WHICH ONE THIS CLIENT RUNS IS NOT KNOWN HERE, and nothing in this repo
	// may run on a box. Mo's one-line read, on the client (${CLIENT_LAN_IP}):
	//
	//	ssh root@${CLIENT_LAN_IP} 'lsmod | grep -c "^wireguard " ; command -v wireguard-go || echo no-userspace'
	//
	// 1 on the first field is the kernel module (the larger window);
	// wireguard-go present with the module absent is 2048.
	WGReplayWindow = 2048
	// MaxReorderSpreadMS is the cross-path arrival spread this datapath is
	// designed to present. NOT a new constant: it is HoldMax (main.go:44), the
	// reorder horizon the deleted ring was already willing to spend, reused as
	// what it always physically was -- the worst cross-link skew the bond
	// expects.
	MaxReorderSpreadMS = 350
)

// ReorderDepthFrames is the deepest reorder, IN FRAMES, that a spread of
// spreadMS presents at framesPerSec.
...
// framesPerSec. The envelope is FINITE and that is the honest half of the
// claim: WGReplayWindow*1000/MaxReorderSpreadMS is about 5851 frames/s, roughly
// 71 Mbit/s at MaxFrame. Above that a full spread of reorder is deeper than the
// window and WireGuard drops the late frames.

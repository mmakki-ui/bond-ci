...
==========================================
Round 2 compared the old and new limbs **by BOOLEAN OUTCOME on the 14 recorded
...
pass count ("9/14" where the record's own runs say 14/14) was therefore invisible
...
==========
  ...
  1. OUTCOME: the same limbs evaluated over the 14 recorded CI runs.
  ...
  3. CONTROLS: both `verdict()` bodies driven through limbs the 14-run record
     never fired (a duplicate, an out-of-order delivery, loss over bar).
  4. RECORD: `ladder_record.txt` == the recomputed threshold/count/envelope.
  5. BAND: leave-one-out over the record -- build each bar's DRIFT band from 13
     runs and require the held-out 14th to fall inside, for all 14 hold-outs.
     This is the measurement that licenses making DRIFT fatal on a job that must
     not flake; if it ever stops holding, this file goes red rather than the band
     quietly flaking the ladder.
...
(2) at 0 mismatches, because S2.tail is False on all 14 runs. (1) catches it.
...
if EMIT:
    ...
    print("%-*s | %-*s | pass/den | envelope" % (w0, "bar", w1, "threshold"))
    print("%s-|-%s-|----------|-------------------------" % ("-" * w0, "-" * w1))
    for b, d in rows:
        print("%-*s | %-*s | %8s | %s"
...
for b, d in rows:
    ...
    for field, want, got in (("threshold", d["threshold"], row["threshold"]),
        ...
        if want != got:
            bad_rec += 1
            print("DIVERGENT record %s %s: file %r, recomputed from the 14 runs %r"
                  % (b, field, got, want))

# ---------------------------------------------------------------------------
# (5) BAND -- leave-one-out false-positive rate of the fatal DRIFT band
# ---------------------------------------------------------------------------
n_loo = bad_loo = n_banded = 0
for b, d in rows:
    if not d["vals"]:
        continue
    vals = d["vals"]
    ...
    for i in range(len(vals)):
        ...
        bd = LR.band(min(keep), max(keep))
        if bd is None:
            continue          # the hold-out was the only outlier; no band, no claim
        n_loo += 1
        if not (bd[0] - 1e-9 <= vals[i] <= bd[1] + 1e-9):
            bad_loo += 1
            print("LOO-EXCLUDED %s: held-out value %s outside band [%s, %s] built "
                  "from the other %d runs -- the DRIFT band would flake here"
                  % (b, LR.fmt(vals[i]), LR.fmt(bd[0]), LR.fmt(bd[1]), len(keep)))
...
print("(1) OUTCOME    the same limbs over 14 recorded runs                     "
      ": %d compared, %d divergent" % (n_limb, bad_limb))
...
print("(5) BAND       leave-one-out DRIFT band over %2d banded bars            "
      ": %d compared, %d excluded" % (n_banded, n_loo, bad_loo))

print("\n(3) positive control -- limbs the 14-run record never fired:")
...
sys.exit(1 if (bad_struct or bad_limb or bad_stage or bad_rec or bad_loo or ctl) else 0)

...
def reorder_release(items, hold):
    ...
    while ptr < n or next_seq <= max_seq:
        ...
        if t_arr == INF and t_hold == INF:
            break
        # Termination guard for hold == 0 (design 4.6).  With `present` empty and
        # the next pending arrival's seq <= next_seq, a zero hold lets the flush
        # branch run without advancing anything, so the clock never reaches that
        # arrival -> livelock.  The added clauses short-circuit on hold > 0.0, so
        # they are consulted ONLY when hold <= 0: every positive hold is
        # byte-identical BY CONSTRUCTION.  Reference implementation:
        # modes-r2-study/expH_frontier.py :: reorder_release_z.

...
- the Go side is **measured and reported, not asserted**: the control loop in `daemon/pullrun.go`
  sleeps `PingIval = 100 ms` (`daemon/main.go:45`, slept at `daemon/pullrun.go:792`). So r1's
  "== ticker period in Go" **does not hold**: 10 ms in the model against 100 ms in the shipped
  loop. Every HOLD bar here is scored at a granularity 10× finer than the daemon can resolve. That is
  a new honest fail, printed in the banner on every run. The guard used to assert `ring.Tick(now)` in
  `main.go`; U159 deleted the reorder ring at both ends and U260 moved the assertion to what makes
  the cadence live — `PingIval` defined in `main.go` **and** referenced by the loop in `pullrun.go`.
...
6. **The model tick is 10 ms; the shipped control loop sleeps 100 ms** (`daemon/main.go:45`
   `PingIval`, slept at `daemon/pullrun.go:792`). Every HOLD bar is scored 10× finer than the daemon
   resolves. Tracked as U14d.

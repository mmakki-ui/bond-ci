...
def _gran_guard():
    ...
    # MEASURED, and it does NOT: the control loop sleeps PingIval, which is 10x the
    # model's DT.
    #
    # U260: this used to require `ring.Tick(now)` in main.go. U159 deleted the reorder ring
    # at both ends and the guard was not updated with it, so latency-bars died before it
    # measured anything -- on every branch. The guard still has to prove the cadence it
    # prints is REAL rather than a number scraped out of a comment, so the assertion moves
    # to what makes PingIval live: it must be defined in main.go and used by the control
    # loop in pullrun.go. Dropping the check instead would turn a guard into a comment.
    go = open(os.path.join(SIM, '..', 'daemon', 'main.go'), encoding='utf-8').read()
    # SEEDED DEFECT (U260 A/B, seed branch only): the pre-U260 assertion, restored.
    # `ring.Tick(now)` was deleted from the daemon by U159, so this trips on every
    # tree and the battery refuses to measure. latency-bars must go RED here.
    ...
    if not mg:
        raise SystemExit('GRAN GUARD FAILED: cannot read PingIval from daemon/main.go; '
                         'the Go control cadence is unmeasurable from here.')
    run = open(os.path.join(SIM, '..', 'daemon', 'pullrun.go'), encoding='utf-8').read()
    if 'PingIval' not in run:
        raise SystemExit('GRAN GUARD FAILED: pullrun.go does not mention PingIval, so the '
                         'constant read from main.go is no longer the control loop cadence '
                         'this guard reports.')
...
def main():
    ...
    say('  go tick : %.0f ms -- daemon/main.go PingIval, the control loop cadence in '
        'pullrun.go.' % GO_TICK_MS)
    say('            r1 sec 8 also asks for gran == the Go ticker period. MEASURED, '
        'and it')
    say('            does NOT hold: %.0f ms model vs %.0f ms shipped loop. Recorded as an'
        % (GRAN, GO_TICK_MS))

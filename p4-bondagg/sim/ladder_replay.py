...
it is: the record fixes a RANGE, not a distribution (the standing caveat this
unit already carries), so the band is a GROSS-CHANGE tripwire and nothing finer.
It was not picked to look safe -- it is the coarsest widening expressible as a
pure function of the record with no new number in it.

ITS FALSE-POSITIVE RATE IS MEASURED, AND THE MEASUREMENT IS NOT A CLEAN ZERO
===========================================================================
`loo()` below is that measurement: for each recorded value of a banded bar,
build the band from the OTHER values and ask whether the held-out one falls
inside. Round 3 shipped the claim "16 of 16 bars, 14/14 inside, ZERO exclusions"
in this docstring and in `ladder_gate.py`. That was an overstatement of what the
code does, in two ways, and both are now reported as numbers instead of prose:

  * SKIPPED hold-outs. When the other values have zero spread there is no band,
    so the hold-out is not a test of anything and `loo()` counts it `skipped`.
    On the 14-run record that was 3 of 224 hold-outs -- S5.loss (hold-out 0.1,
    the other 13 all 0.0), S6.k (12.0, other 13 all 20.0) and S7.k (12.0, other
    13 all 8.0) -- so the run printed `221 compared`, never 224.
  * TRIVIAL hold-outs. Holding out a value that is neither the unique min nor
    the unique max leaves the band unchanged, so it is inside by construction
    and tests nothing. `loo()` counts the rest `informative`. On the 14-run
    record only 25 of 221 hold-outs were informative, and S5.loss, S6.k and S7.k
    had ZERO informative hold-outs each: 13 trivial passes and 1 skip apiece.
    S5.loss is the bar whose whole band rested on a single 0.1 observation, and
    it is the bar that then reddened the fatal job on the 15th sample (1.70%,
    mirror run 33336749301). The evidence for its band was never there; the
    aggregate "0 exclusions" hid that a bar can contribute 14 hold-outs and 0
    tests.

On the 19-run record shipped here the exclusion count is NOT zero, and the
number is published per bar in `ladder_record.txt` rather than asserted here.
`ladder_equiv_check.py` recomputes it every CI run and fails if it changes, on
the same discipline as every other column: the record is a rendering of a
measurement and CI compares the two exactly.

Non-numeric bars (`*.order`, whose value is `True`/`False`) band by observed
TOKEN SET instead; there is no arithmetic to do on them, so they have no
hold-out row at all.
"""
import ast
import contextlib
...
def loo(vals):
    """Leave-one-out audit of the DRIFT band over one bar's recorded values.

    Returns (compared, informative, excluded, skipped, [excluded values]).

      skipped     -- the other values have zero spread, so no band is derivable
                     and the hold-out tests nothing. NOT a pass.
      informative -- removing the hold-out actually MOVED the band, i.e. it was
                     the unique min or the unique max. Any other hold-out is
                     inside by construction and tests nothing either; round 3
                     counted those as evidence and they are not.
      excluded    -- a hold-out the band built from the others rejects. This is
                     the false-positive the band would produce on that sample.

    `compared` counts hold-outs that had a band (skipped ones excluded), which
    is the number `ladder_equiv_check.py` has always printed. `informative` is
    the number that could ever have failed.
    """
    n_cmp = n_inf = n_skip = 0
    out = []
    lo_all, hi_all = min(vals), max(vals)
    for i in range(len(vals)):
        keep = vals[:i] + vals[i + 1:]
        b = band(min(keep), max(keep))
        if b is None:
            n_skip += 1
            continue
        n_cmp += 1
        if (min(keep), max(keep)) != (lo_all, hi_all):
            n_inf += 1
        if not (b[0] - 1e-9 <= vals[i] <= b[1] + 1e-9):
            out.append((vals[i], b))
    return n_cmp, n_inf, len(out), n_skip, out


def loo_text(d):
    """The record's hold-out column, in the grammar load_record() parses.

    `holdouts none` for a bar with no derivable band at all -- token bars and
    zero-spread bars. There is nothing to leave out.
    """
    vals = d.get("vals") or []
    if not vals or max(vals) - min(vals) == 0:
        return "holdouts none"
    n_cmp, n_inf, n_exc, n_skip, _ = loo(vals)
    return "holdouts %d informative %d excluded %d skipped %d" % (
        n_cmp, n_inf, n_exc, n_skip)
...
# ladder_record.txt
# --------------------------------------------------------------------------
ROW = re.compile(r"^\s*(\*|S\d\w*)\.(\w+)\s*\|\s*(.+?)\s*\|\s*(\d+)\s*/\s*(\d+)\s*\|"
                 r"\s*(.+?)\s*\|\s*(.+?)\s*$")
LOO_RE = re.compile(r"^holdouts (?:none|(\d+) informative (\d+) excluded (\d+) skipped (\d+))$")
...
def load_record(path=RECORD):
    ...
    for ln in open(path, encoding="utf-8"):
        ...
        d = {"threshold": m.group(3), "passes": int(m.group(4)),
             ...
             "loo": m.group(7)}
        if not LOO_RE.match(d["loo"]):
            raise AssertionError("ladder_record.txt: hold-out column %r does not "
                                 "parse. Grammar is `holdouts none` or `holdouts <n> "
                                 "informative <n> excluded <n> skipped <n>`." % d["loo"])

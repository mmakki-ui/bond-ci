...
# =============================================================================
# CANONICAL FOR: the 02-ackclock study line ONLY -- `pred_c/pred_iii/pred_iv/
#   pred_v/pred_vi`, `probe_*`, `sweep*`, `audit_*`, `verify_passes.py`,
#   `verify_repro.py` and their committed `*_out.txt`.  That line is ADR-002's
#   evidence for REJECTING ack-clocking as a replacement for the statistical cap.
#
# THIS IS NOT THE GATED ORACLE.  The ADR-004 rig loads
#   `../03-reserved-composite/ackclock_sim.py`, a DIFFERENT and EARLIER revision.
#   ADR-004 named this file for a day; that was wrong (amended `2c052b4`).
#
# This copy is the later revision of the `sched='C'` line and alone carries
#   `c_mode`, `lam_used`/`lam_samp`/`LAM_MAX_WIN`, `probe_frames` default 4 (see
#   `pred_iii.py:15`), RTT subsampling, a dict-valued `_trace` row, and different
#   `_c_budget` / `_pace_rate` derivations.  MEASURED (U35, 2026-08-30):
#   `pred_iii_out.txt`'s "C floor ON (probe=4)" @ ack_loss=15%, SEEDS=24, T=10,
#   rig=mid reproduces digit-for-digit HERE (gp=75341 loss=17.1% ... c_lost=3342)
#   and NOT under the 03 copy (gp=69449 loss=23.6% ... c_lost=5950).  So this file
#   is kept, not deleted: without it this line's published outputs cannot be
#   re-run.  Full record in `../rig_pin.py`.
#
...
STUDY_LINE = rig_pin.claim(__file__, '02-ackclock')
...
class Sim:
    def __init__(s, path_defs, offer_fn, T, seed, sched='ack',
                 ...
                 c_repair='age', c_derive_hold=False, c_mode='delay',
                 cwnd_gain=1.25, startup_gain=2.0, cruise_gain=1.0, pace_burst=4.0,
                 probe_frames=4, probe_int_ms=25.0,
        ...
        s.c_mode = c_mode                # 'delay' (bound in-flight TIME) | 'rate' (Little dual)
        ...
        s.lam_used = [0.0] * s.N         # windowed-MAX of lam (BBR btlbw) -> no-latch fill rate
        s.lam_samp = [deque() for _ in range(s.N)]
        s.LAM_MAX_WIN = 0.40             # window over which lambda_max remembers the peak (s)
        ...
        s.rtt_samp = [deque() for _ in range(s.N)]       # (t, rtt) round-trip samples (subsampled)
        s.last_rtt_t = [-1e9] * s.N
    ...
    def _c_budget(s, i):
        # CWND count-cap (Little): cwnd_gain * lambda_FASTDOWN * (RTTmin+tau). This
        # is the predicted queue-time of a NEW frame: inflight / delivered-rate.
        # Using the FAST-DOWN lambda makes the cap shrink the instant delivery
        # drops (a dip/dropout) -> C stops over-admitting -> queue bounded ~tau.
        rttm = s.rttmin[i] if s.rttmin[i] is not None else 0.0
        return s.cwnd_gain * s.lam[i] * (rttm + s.tau)

    def _pace_rate(s, i):
        # PACING rate (frames/sec) = gain * lambda_max.  lambda_max (windowed peak)
        # does not collapse on a dip, so the pacer never starves a rising cap; the
        # DELAY bound independently backs off when the queue actually builds.
        gain = s.startup_gain if s.c_startup[i] else s.cruise_gain
        return gain * s.lam_used[i]
    ...
    def _process_echoes(s, now):
        ...
        for (at, i, rc, sts, enq) in batch:
            rtt = at - enq                     # round-trip sample (path-send -> echo-back)
            # subsample RTT to ~5ms granularity: min/jitter need coverage, not one
            # sample per delivered frame (which is O(rate*window) -> quadratic cost).
            if rtt > 1e-6 and now - s.last_rtt_t[i] >= 0.005:
                s.rtt_samp[i].append((now, rtt)); s.last_rtt_t[i] = now
    ...
    def _c_update(s, now):
        ...
        for i in range(s.N):
            ...
            # windowed-MAX (BBR btlbw): the fill-rate estimate that does not latch
            s.lam_samp[i].append((now, s.lam[i]))
            while s.lam_samp[i] and s.lam_samp[i][0][0] < now - s.LAM_MAX_WIN:
                s.lam_samp[i].popleft()
            s.lam_used[i] = max((v for _, v in s.lam_samp[i]), default=0.0)
    ...
    def run(s):
        ...
        for tk in range(nticks):
            ...
            def room(i):
                ...
                if s.sched == 'C':
                    ...
                    if not s.has_reading[i]:
                        return True
                    # ESCAPE HATCH -- continuous MIN-WINDOW floor. A pure fast-down
                    # lambda would LATCH: once it drops to ~0 (a cap dip), the path
                    # stops sending, so nothing is delivered, so lambda stays 0
                    # forever. Keeping a few frames always in flight keeps the meter
                    # alive so lambda re-measures the moment capacity returns. Into a
                    # genuinely dead path this floor costs only probe_frames/RTT.
                    if inflight < s.probe_frames:
                        return True
                    if s.c_mode == 'delay':
                        # DELAY form of the law: bound the IN-FLIGHT TIME of the
                        # oldest un-reconciled frame to RTTmin+tau. This is the
                        # Little dual of the rate bound, but self-CLOCKS on the
                        # measured RTT so it cannot latch: a rising queue lifts the
                        # age -> back off; a draining queue drops it -> push. A dead
                        # path's oldest frame ages past the budget -> stop (fast-down,
                        # no lambda needed). Paced to avoid a single-tick clump.
                        rttm = s.rttmin[i] if s.rttmin[i] is not None else 0.0
                        if s.flight_ts[i] and (now - s.flight_ts[i][0]) >= rttm + s.tau:
                            return False                       # queue too deep -> back off
                        if inflight >= max(1.0, s._c_budget(i)):
                            return False                       # count-cap -> anti single-tick clump
                        return s.pace_tokens[i] >= 1.0         # paced release
                    # RATE form (Little dual): inflight <= lambda*(RTTmin+tau), paced.
            ...
            if getattr(s, '_do_trace', False) and tk % 5 == 0:
                def fage(i):
                    return (now - s.flight_ts[i][0]) * 1000.0 if s.flight_ts[i] else 0.0
                s._trace.append(dict(t=now, dc0=dcaps[0], lam0=s.lam[0], lmax0=s.lam_used[0],
                    cwnd0=s._c_budget(0), infl0=s._c_inflight(0), fage0=fage(0),
                    budg0=(s.rttmin[0] or 0)*1000+s.tau*1000, ptok0=s.pace_tokens[0],
                    db0=s.down[0].backlog_kb / PKT_KB,
                    infl1=s._c_inflight(1), cwnd1=s._c_budget(1), fage1=fage(1),
                    db1=s.down[1].backlog_kb / PKT_KB, pool=len(s.fifo)))

...
Every study runs on the **unmodified** `p4-bondagg/sim/nsched_model.py`
(sha256 `e8a7c67d2105…`, identical on dev) — the same validated physics as the EIF/nsched CI gate.
The one exception is `variants/nsched_model.py` (a forked copy used only by `hedge_measure.py`).
...
- `reserved_composite.py` (**`sched='Dc'` = the settled composite**) + `ackclock_sim.py`
  (pull/cap physics — NOTE: a different revision from `02-ackclock/ackclock_sim.py`)

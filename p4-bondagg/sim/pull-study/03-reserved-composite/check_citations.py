...
#: first cell names U10 or one of its lettered children (U10a, U10b, ...).
#: U10-FOLLOWED-BY-A-DIGIT is a DIFFERENT unit (U100, U108, U101a, ...) and must
#: not match -- found U10d, 2026-09-05, by check_citations.py itself: U108's
#: `main.go:210` citation (docs/ROADMAP.md, the "lossPeer race" row) was flagged
#: FATAL U10-owned because `\|\s*\**U10` is a prefix match with no boundary, so
#: "U108" satisfies it just as "U10" or "U10b" would. A bug in the checker's own
#: ownership test, not a real U10 defect.
_U10_OWNED = re.compile(r'\|\s*\**U10(?!\d)')
_U10_HEADING = re.compile(r'^U10(?!\d)')


def owned(src, line, heading):
    if src != 'docs/ROADMAP.md':
        return True
    if _U10_HEADING.match(heading):
        return True
    return bool(_U10_OWNED.match(line))

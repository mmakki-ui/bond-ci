#!/usr/bin/env python3
# =============================================================================
# reserved_composite.py -- SCHEDULER D (sched='D'), D'' (sched='Dpp') AND the
#   COMPOSITE Dc (sched='Dc').
#   A COPY of reserved_meter.py with pull / D / Dpp / redundant kept BYTE-FOR-BYTE
#   intact (so SimD('pull') still byte-matches ackclock_sim.Sim('pull'), and
#   SimD('Dpp') is unchanged), plus ONE addition: sched='Dc', the COMPOSITE.
#
# =========================== COMPOSITE Dc (sched='Dc') =======================
# Dpp shipped the meter-gated DUPLICATE but left NATIVE admission UNCAPPED
# (nat_cap=HUGE => native == pure PULL).  In the MID rig at high load that native
# flood drove every high-load row to PULL-level loss (~13%) instead of the shipped
# one-sided cap's ~6% -- the eviction spiral (spotty-link native share 23->58%).
# THE BUG: native was never subjected to the delivered-rate cap that ackclock's
# 'ewma' scheduler ships; only the duplicate was.
#
# Dc RESTORES the native cap.  It is the strict composition of the two shipped
# pieces, gated by the SAME delivered-rate meter (the ackclock 'ewma' one-sided
# cap), with NO new numeric knob:
#   NATIVE    : plain PULL admitted ONLY WHILE room() is open, where room() is now
#               the FULL ackclock 'ewma' cap = _meter_ok(i): LOCAL ms-gate open
#               (local-inflight-time < target_ms) AND far-inflight-time < target_ms
#               (lagged delivered-rate meter unlatched).  This is the ONLY change
#               vs Dpp -- native == the shipped one-sided cap, NOT nat_cap=HUGE.
#   DUPLICATE : STANDING spotty-CLASS nomination (every native frame landing on a
#               spotty-class link enqueues ONE copy, by class identity -- unchanged
#               from Dpp), admitted STRICTLY AFTER native through the SAME room()
#               (_meter_ok) gate onto a STEADY-class host, first-copy-wins at the
#               reorder ring, TTL == the ring hold (dup_ttl, owd/jit geometry).
# Everything else -- nomination, host set, PIECE-2 self-shedding meter re-check,
# TTL, degenerate == pull behaviour -- is IDENTICAL to Dpp.  ZERO new knobs
# (reuse target_ms + the meter's cap/clear at target_ms).  Instrumented: res_tx
# (admitted-dup) + tshare (spotty-CLASS native share -- the eviction-spiral watch).
#
...
# =============================================================================
# =============================================================================
# WHAT THIS RIG RUNS, ASSERTED RATHER THAN ASSUMED (ROADMAP U35)
# -----------------------------------------------------------------------------
# This module IS the ADR-004 gated datapath oracle. Its two dependencies each
# exist in more than one copy in this tree, and until U35 the copy that ran was
# chosen by `sys.path` -- i.e. by the caller's cwd and PYTHONPATH -- with nothing
# asserting the outcome.  ADR-004 recorded the WRONG file for a day.
# Both are now loaded BY PATH off this file's own `__file__` and their identity
# is asserted at import, so a wrong copy raises instead of quietly producing
# numbers attributed to the other file:
#   oracle  -> ./ackclock_sim.py                    (NOT ../02-ackclock/)
#   physics -> ../../nsched_model.py                (NOT ../variants/)
# `.github/scripts/rig_paired_gate.py::preflight()` checks the same thing from
# outside; this is the rig knowing it for itself.  Canonical decision, the
# measurements behind it, and what deleting either copy would cost:
# `../rig_pin.py`.
#   python -c "import reserved_composite as RC; print(RC.identity_banner())"
# =============================================================================
import math, random, sys as _sys, os as _os, importlib.util as _ilu
...
def _rig_pin():
    ...
    _sys.modules['rig_pin'] = m
    spec.loader.exec_module(m)
    return m


rig_pin = _rig_pin()
#: the oracle: THIS line's ackclock_sim.py, not 02-ackclock's different revision
A = rig_pin.pin_oracle(__file__, '03-reserved-composite')
#: the physics: the shipped nsched_model.py, not pull-study/variants/'s fork
M = rig_pin.pin_physics(__file__)
Stage = A.Stage; HUGE = A.HUGE; tether_cap = A.tether_cap
eth_cap = A.eth_cap; med = A.med; agg = A.agg

ORACLE_FILE = _os.path.abspath(A.__file__)
PHYSICS_FILE = _os.path.abspath(M.__file__)
RIG_FILE = _os.path.abspath(__file__)


def identity_banner(prefix='  '):
    """Exactly which files this rig is running. Print it before trusting a number."""
    return ('%srig     %s\n%soracle  %s\n%sphysics %s'
            % (prefix, RIG_FILE, prefix, ORACLE_FILE, prefix, PHYSICS_FILE))
...
class SimD:
    ...
    (sched='Dpp'), the COMPOSITE Dc (sched='Dc'), full 'redundant', or a self-check
    'pull'.
    ...
    D'' : NO r, NO budget, NO ttl knob (dup_ttl == the reorder-ring hold), NO local
          ...
          reserve_frac/ttl_ms are ignored for Dpp.  BUG: native is left UNCAPPED
          (nat_cap=HUGE == pure pull) -- only the duplicate sees the meter.
    Dc  : the COMPOSITE.  IDENTICAL to Dpp in every respect EXCEPT native is now
          cap-gated by the SAME meter: native room() == _meter_ok (the ackclock
          'ewma' one-sided cap), so BOTH native and duplicate obey the shipped
          delivered-rate cap.  This restores the native cap Dpp dropped.  NO new
          knobs (reuse target_ms).  reserve_frac/ttl_ms are ignored for Dc."""
    ...
    def __init__(s, path_defs, offer_fn, T, seed, sched='D', reserve_frac=0.25,
        ...
        s.spotty = [bool(d['spotty']) for d in path_defs]   # U10a: a missing class is LOUD
    ...
    def run(s):
        ...
        for tk in range(nt):
            ...
            if s.sched in ('Dpp', 'Dc'):
                # ---- D''/Dc host = alive, STEADY-class, delivered-rate METER UNLATCHED.
                ...
                # healthy.  A spotty path is never a host (not at_risk).  Dc uses the
                # SAME host set as Dpp (composition changes only native's gate below).
            ...
            def room(i):
                if not alive[i]:
                    return False
                if s.sched == 'Dc':
                    # ***THE ONLY CHANGE vs Dpp*** -- native is now CAP-GATED by the
                    # SAME meter the duplicate uses: the ackclock 'ewma' one-sided
                    # delivered-rate cap.  _meter_ok(i) == LOCAL ms-gate open
                    # (local-inflight-time < target) AND far-inflight-time < target
                    # (lagged delivered-rate meter unlatched).  Dpp left this UNCAPPED
                    # (nat_cap=HUGE == pure pull) -> native flooded at high load ->
                    # PULL-level loss + spotty-link eviction spiral.  No new knob.
                    return s._meter_ok(i)
                if s._local_ms(i) >= s.target_ms:
                    return False
            ...
            ttl = s.dup_ttl if s.sched in ('Dpp', 'Dc') else s.ttl
            if armed and s.mirror_q:
                ...
                while s.mirror_q and mguard < 200000:
                    ...
                    if s.sched in ('Dpp', 'Dc'):
                        # copies admitted through the SAME room()/_meter_ok gate as
                        # native, onto a STEADY-class host, first-copy-wins; piling
                        # copies climbs host inflight -> meter latches -> self-sheds.
            ...
            for i in range(s.N):
                ...
                if s.sched in ('Dpp', 'Dc'):
                    ...
                    # Dc feeds it too -- native's room() gate reads push_est every tick.
    ...
    def finalize(s):
        ...
        return {'gp': gp, 'loss': max(0.0, loss), 'p50': pct(.5), 'p95': pct(.95),
                ...
                'tshare': sum(a for a, sp in zip(s.assigned, s.spotty) if sp) / (sum(s.assigned) or 1),  # U10a: spotty CLASS, not index 0

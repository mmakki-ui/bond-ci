...
class SimD:
    """PULL + N-generic reserved-mirror (sched='D'), or full 'redundant', or a
    self-check 'pull'.  reserve_frac r in [0,1).  ttl_ms bounds how long a spotty
    frame stays mirror-eligible: SHORT ttl == mirror only the in-flight exposure
    window (cheap insurance); LONG ttl + big r == mirror the full stream (-> the
    'redundant' collapse).  Sweeping (r, ttl) is the honest reserve-sizing sweep."""

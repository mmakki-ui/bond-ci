...
def main():
    ...
    print('    median over %d seeds, and the seeds are COMMON across the grid, so the' % SEEDS)
    print('    comparison between two grid points is PAIRED per seed.  C4b reads each')
    print('    argmin against that pairing before any verdict is drawn from it.')
    print('=' * 110)
    ...
    if n_res_move:
        ...
        tm_why = ('C4b: the loss optimum MOVES with N. %d of %d off-N=2 rows beat the N=2 '
                  ...
                  "N-invariance is contradicted by C4's own table at this resolution, and "
                  'the %.0f ms grid step is coarser than the effect such a verdict would '
                  'have to rule out. Reported OPEN; no value changed.'
                  % (n_res_move, len(nmoved), SEEDS,
                     ', '.join('%s@%.2f %.0f vs %.0f ms' % (r[0], r[1], r[2], r[3])
                               for r in nmoved if r[4]), step))
    elif nmoved:
        tm_verdict = 'OPEN'
        tm_why = ('C4b: the argmin moves across N on the printed table, but no off-N=2 row '
                  ...
                  'invariance, and the %.0f ms grid step is coarser than the effect such a '
                  'verdict would have to rule out. Reported OPEN; no value changed.'
                  % (SEEDS, step))
    else:
        tm_verdict = 'OPEN'
    ...
    print('    highn_battery.py already computes for itself, over spotty_idx:')
    ...
    print("    grep -n 'tshare stable' docs/knowledge/design/p5-execution-handover.md")
    print('      -- names it as')
    print('      an E2c equivalence criterion for the SHIPPED standing-lightning datapath')
    print("    grep -n 'tshare 0.181' docs/HANDOFF.md   and")
    print("    grep -n 'tshare 0.181' docs/knowledge/decisions/ADR-003-mode-set.md")
    print('      -- both cite it as')
    ...
    ROWS = [
        ...
        ('target_ms = 40.0',
         ...
         '%s Separate, and NOT an N defect: 40 is not the argmin at 0.65. Left exactly '
         'as it is; an OPEN row is reported, never replaced.'
         % ('/'.join('%g' % best_t[(l_, 0.65)] for (l_, _) in CHAIN),
            '/'.join('%g' % best_t[(l_, 0.95)] for (l_, _) in CHAIN), tm_why)),
        ...
        ('tshare = assigned[0] / sum',
         ...
         '%.3f-%.3f from N=3. The N-generic form already exists -- '
         "grep -n 'sshare.. = sum' highn_battery.py computes the share over "
         'spotty_idx and reduces to tshare at N=2. The battery is '

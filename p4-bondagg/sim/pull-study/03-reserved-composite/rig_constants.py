...
#                         invented one.
#
# NO CONSTANT IS CHANGED BY THIS FILE.  It edits no other file, tunes no
# scheduler, and replaces no constant.  Every sweep below is a MEASUREMENT
# GRID, not a model knob: the grids are stated in the code and none of their
# values is carried into anything.
#   ONE THING DID CHANGE (U10a): it now ASSERTS.  C5 carries two bars and this
#   file EXITS NON-ZERO when either fails -- a report that cannot fail is not a
#   check, and C5's whole subject is a metric that was wrong for months while
#   every run of this file printed it in green.
#
# ---------------------------------------------------------------------------
# THE PROBES
...
#   C4  target_ms, MEASURED, ACROSS N AND LOAD.  The one per-link constant used
#       by every gated scheduler.  Swept live (it changes admission, so it cannot
#       be swept post hoc) at every N in the nested chain.
#   C4b the N axis, read against the seed pairing.  RESOLVED only when every one
#       of the TSEEDS paired seeds agrees on the sign of the difference.
#   C4c the LOAD axis, read the same way -- U10c's question, below.
#   C5  `tshare`, THE METRIC THAT WAS KEYED TO INDEX 0.  Until U10a it was
#       `assigned[0] / sum(assigned)` in both the rig and the oracle.  At N=2,
#       with the tether written first, that IS "the spotty link's share"; at
#       N>=3 it named one arbitrary link.  It is now the spotty-CLASS share.
#       C5 holds the live metric to an independently recomputed class share
#       (bar C5-1) and keeps the retired form measured beside it (bar C5-2).
#
# ---------------------------------------------------------------------------
# U10c DECISION -- target_ms IS LOAD-TRACKING, NOT PER-LINK INVARIANT.  MEASURED.
#   The question U10c owed: is `target_ms = 40.0` a per-link constant (one value,
#   correct on a link whatever the traffic) or does the right value TRACK the
#   offered load?  C4c answers it on the same paired-seed sign test C4b uses.
#   Within one mix, the argmin found at the OTHER load is scored against this
#   row's own argmin AT THIS LOAD, so only target_ms differs between the two
#   numbers being compared.
#     RESULT (rig_constants.txt beside this file -- SEEDS=24 TSEEDS=24 T=9.0
#     RIG=mid, exit 0, "BARS: 2 asserted, 0 failed"): 8 of 8 rows RESOLVED 24/24.
#     The paired median penalty for using the other load's pick runs -0.129 pt
#     (N2-het@0.65) to -3.882 pt (N4-het@0.95).  At 0.65 the argmin sits at
#     22-30 ms and at 0.95 it sits at 34-38 ms, on EVERY mix.
#     A per-link constant cannot depend on the offered load.  This one does.
#   THE DECISION DOES NOT CHANGE 40.0.  no-arbitrary-constants forbids swapping
#   one unmeasured number for another, and ADR-004 condition 2 (nothing in this
#   rig has ever been compared to a real router) is open.  What it changes is
#   WHAT WOULD CLOSE the row: not a better constant, but a target expressed as a
#   function of the offered load, derived and then measured against hardware.
#   THE SAME RUN, ON THE OTHER AXIS (C4b, so the two are not confused): the
#   argmin also MOVES with N -- 3 of 6 off-N=2 rows resolved 24/24 -- and that
#   now stands at a 2 ms grid step, so it is no longer an artefact of a grid
#   coarser than the effect a verdict has to rule out.  target_ms stays OPEN.
#
# Env: SEEDS(8) TSEEDS(2*SEEDS) WORKERS(14) T(9.0) RIG(mid)
# Run: python rig_constants.py > rig_constants.txt 2> rig_constants.err
#      The decision above was measured with SEEDS=24 TSEEDS=24.
#
# CITATIONS IN THIS FILE ARE CHECKED BY A PROGRAM, NOT BY READING.
#   python check_citations.py --ref=dev
# U10 shipped three `file:line` citations that pointed at the wrong line on the
# branch that merges -- one of them broken by the same commit that wrote it, and
# re-emitted into the artifact by a print() on every future run.  The rule the
# checker enforces: a bare `path:line` is valid only if that line is non-blank and
# IDENTICAL on this tree and on the merge target; a citation into a file that moves
# carries no line number at all and is written anchored, as
# `grep -n 'RE' path`, which the checker resolves on both trees.  Run it before
# committing anything that cites a line.
#
# =============================================================================
...
#: C4/C4b/C4c run at their OWN seed count (U10c).  C4b's verdict is a sign test over
#: the paired seeds, so its resolution is set by this number and by nothing else; C3
#: and C5 are unaffected and stay at SEEDS.
TSEEDS  = int(os.environ.get('TSEEDS', str(2 * SEEDS)))
...
#: U10c: the 20-40 ms bracket at 2 ms.  The old grid stepped 10 ms there, which is
#: coarser than the effect a target_ms verdict has to rule out, so every C4b verdict
#: it produced was floored by the grid rather than by the seeds.  The outer brackets
#: (10 / 60 / 80 / 120) are unchanged so the earlier tables stay comparable.
TARGET_FINE = [20.0 + 2.0 * i for i in range(11)]             # 20,22,...,40 ms
TARGET_GRID = [10.0] + TARGET_FINE + [60.0, 80.0, 120.0]      # ms, brackets 40.0
LOADS = [0.65, 0.95]
...
def w_share(task):
    ...
    #: three numbers, deliberately: the LIVE metric as the sim now reports it, the
    #: spotty-CLASS share recomputed here from o.assigned (independent of the sim's
    #: own arithmetic), and the RETIRED index-0 form kept as a post-hoc expression
    #: so C5 can still show what it would have said.  U10a made the first two equal
    #: by construction; C5 asserts that, and measures the third against them.
    return (mi, sched, seed, m['tshare'], sum(o.assigned[i] for i in sp) / tot,
            o.assigned[0] / tot, [x / tot for x in o.assigned])
...
def main():
    try:
        ...
        pass
    t0 = time.time()
    #: hard bars this file asserts (U10a added the first two).  Non-empty at the end
    #: means a NON-ZERO exit: this file used to report only, and a report cannot fail.
    fails = []
    H = '#' * 110
    print(H)
    print('# rig_constants.py -- U10 ITEM 4: per-link constants settled at N=2')
    print('# seeds=%d  tseeds=%d  T=%.1fs  rig=%s' % (SEEDS, TSEEDS, T, RIG))
    print(H)
    ...
    LIVE = [
        ...
        ('tshare = spotty-class share', 'reserved_composite.py, ackclock_sim.py (both '
         "grep -n \"'tshare'\")",
         'share of native traffic on the spotty CLASS', 'LIVE, see C5 (U10a)'),
    ]
    ...
    if not all(gates):
        print('  ABORTING.  A self-gate that only prints is not a gate: every number below')
        print('  would have been emitted into the artifact anyway.  Exit 2.')
        sys.stdout.flush()
        sys.exit(2)
    print()
    ...
    censored = [l_ for (l_, _) in CHAIN
                if best_by_mix[l_] in (HOLD_GRID[0], HOLD_GRID[-1])]
    print('  The optimum ordering across the chain is %s.'
          % ' '.join('N=%d:%.0f' % (len(nm_), best_by_mix[l_] * 1000.0)
                     for (l_, nm_) in CHAIN))
    if censored:
        print('  READ THAT ORDERING AS CENSORED, NOT AS A SHAPE.  %s sits ON the grid'
              % ', '.join(censored))
        print('  boundary (%.0f-%.0f ms), so its argmin is where the sweep STOPPED, not'
              % (HOLD_GRID[0] * 1000.0, HOLD_GRID[-1] * 1000.0))
        print('  where the curve turned, and the curve is flat near the optimum. The')
        print('  non-monotonicity of this ordering is therefore NOT established here and no')
        print('  conclusion is drawn from it.')
    print('  WHAT IS ESTABLISHED WITHOUT THE ORDERING: the formula misses the optimum by')
    print('  %.0f-%.0f ms at every N and the gap does not close as N grows, so the defect'
          % (min(gaps), max(gaps)))
    print('  is a LEVEL error, not a failure to scale with N -- the opposite of what item 4')
    print('  was looking for, and it survives even if every argmin above moved a grid step.')
    ...
    print()
    print('=' * 110)
    print('C4  target_ms ACROSS N AND LOAD -- swept LIVE (it changes admission, so it cannot')
    print('    be swept post hoc).  The one per-link constant every gated scheduler reads.')
    print('    grid: %s ms   (the shipped value is 40.0)'
          % ', '.join('%g' % x for x in TARGET_GRID))
    print('    U10c: %.0f ms steps across %.0f-%.0f ms, %d seeds (was %.0f ms steps, 8 seeds).'
          % (TARGET_FINE[1] - TARGET_FINE[0], TARGET_FINE[0], TARGET_FINE[-1], TSEEDS, 10.0))
    print('=' * 110)
    print('    AN ARGMIN IS NOT A RESULT WITHOUT AN ERROR BAR.  Every cell below is a')
    print('    median over %d seeds, and the seeds are COMMON across the grid, so the' % TSEEDS)
    print('    comparison between two grid points is PAIRED per seed.  C4b reads each')
    print('    argmin against that pairing before any verdict is drawn from it, and C4c')
    print('    reads the LOAD axis the same way.')
    print('=' * 110)
    ttasks = [(mi, names, L, sch, sd, tm)
              for mi, (lab, names) in enumerate(CHAIN)
              for L in LOADS for sch in ('Dc',) for sd in range(TSEEDS)
              for tm in TARGET_GRID]
    tres = {}
    with ProcessPoolExecutor(max_workers=WORKERS) as ex:
        for (mi, sch, sd, L, tm, loss, gp, p95) in ex.map(w_target, ttasks, chunksize=4):
            tres.setdefault((mi, L, tm), {})[sd] = (loss, gp, p95)

    def tcell(mi, L, tm, k=0):
        """per-seed values at one grid point, ORDERED BY SEED -- the pairing key."""
        d = tres[(mi, L, tm)]
        return [d[sd][k] for sd in sorted(d)]

    best_j = {}
    for L in LOADS:
        print('  load=%.2f   Dc loss%% by target_ms' % L)
        print('    %-9s %3s %s' % ('mix', 'N', ' '.join('%7.0f' % x for x in TARGET_GRID)))
        for mi, (lab, names) in enumerate(CHAIN):
            row = [med(tcell(mi, L, tm)) for tm in TARGET_GRID]
            jb = min(range(len(row)), key=lambda j: row[j])
            best_j[(mi, L)] = jb
            print('    %-9s %3d %s   best=%gms'
                  % (lab, len(names), ' '.join('%7.3f' % v for v in row), TARGET_GRID[jb]))
        print('    %-9s %3s %s' % ('', '', ' '.join('%7s' % '' for _ in TARGET_GRID)))
        print('  load=%.2f   Dc gp by target_ms' % L)
        for mi, (lab, names) in enumerate(CHAIN):
            row = [med(tcell(mi, L, tm, 1)) for tm in TARGET_GRID]
            jb = max(range(len(row)), key=lambda j: row[j])
            print('    %-9s %3d %s   best=%gms'
                  % (lab, len(names), ' '.join('%7.0f' % v for v in row), TARGET_GRID[jb]))
    sys.stdout.flush()

    # ---- C4b  read every argmin against the seed pairing --------------------
    print()
    print('-' * 110)
    print('C4b  IS THE ARGMIN SEED-RESOLVED?  For each row: the argmin, the runner-up, and')
    print('     the PAIRED per-seed difference loss(argmin) - loss(runner-up).  The seeds')
    print('     are common to both grid points, so this is the difference of two runs of')
    print('     the SAME world.  RESOLVED means all %d seeds agree on the sign (a sign'
          % TSEEDS)
    print('     test at p = 2^-%d); anything less is inside seed noise and CANNOT carry a'
          % TSEEDS)
    print('     verdict.  This section changes no value and no bar.')
    print('-' * 110)
    print('  %-9s %5s %8s %10s %11s %11s %8s  %s'
          % ('mix', 'load', 'argmin', 'runner-up', 'paired med', 'worst seed',
             'seeds', 'verdict'))
    for L in LOADS:
        for mi, (lab, names) in enumerate(CHAIN):
            jb = best_j[(mi, L)]
            row = [med(tcell(mi, L, tm)) for tm in TARGET_GRID]
            j2 = min((j for j in range(len(row)) if j != jb), key=lambda j: row[j])
            d = [x - y for x, y in zip(tcell(mi, L, TARGET_GRID[jb]),
                                       tcell(mi, L, TARGET_GRID[j2]))]
            nneg = sum(1 for x in d if x < 0)
            print('  %-9s %5.2f %8.0f %10.0f %11.3f %11.3f %4d/%-3d  %s'
                  % (lab, L, TARGET_GRID[jb], TARGET_GRID[j2], med(d), max(d),
                     nneg, len(d),
                     'RESOLVED' if nneg == len(d) else 'inside seed noise'))
    print()
    print("  THE N AXIS, PAIRED THE SAME WAY.  Take the N=2 row's own argmin and score it")
    print('  on every other mix at the same load.  If the constant were N-invariant the')
    print('  N=2 choice would be as good anywhere; a RESOLVED negative difference means')
    print('  the optimum genuinely MOVED with link count.')
    nmoved = []
    print('  %-9s %5s %10s %9s %11s %11s %8s  %s'
          % ('mix', 'load', 'own argmin', 'N=2 pick', 'paired med', 'worst seed',
             'seeds', 'verdict'))
    for L in LOADS:
        t2 = TARGET_GRID[best_j[(0, L)]]
        for mi, (lab, names) in enumerate(CHAIN):
            if mi == 0:
                continue
            tb = TARGET_GRID[best_j[(mi, L)]]
            if tb == t2:
                print('  %-9s %5.2f %10.0f %9.0f %11s %11s %8s  same grid point'
                      % (lab, L, tb, t2, '--', '--', '--'))
                continue
            d = [x - y for x, y in zip(tcell(mi, L, tb), tcell(mi, L, t2))]
            nneg = sum(1 for x in d if x < 0)
            ok = (nneg == len(d))
            nmoved.append((lab, L, tb, t2, ok))
            print('  %-9s %5.2f %10.0f %9.0f %11.3f %11.3f %4d/%-3d  %s'
                  % (lab, L, tb, t2, med(d), max(d), nneg, len(d),
                     'MOVED (resolved)' if ok else 'inside seed noise'))
    n_res_move = sum(1 for r in nmoved if r[4])
    step = min(TARGET_GRID[j + 1] - TARGET_GRID[j] for j in range(len(TARGET_GRID) - 1))
    print()
    print('  GRID RESOLUTION.  The finest spacing on this grid is %.0f ms, so nothing here'
          % step)
    print('  can rule an effect in or out below one grid step -- the shipped 40 ms and its')
    print('  neighbours included.')
    sys.stdout.flush()

    # ---- C4c  the LOAD axis, paired the same way ---------------------------
    print()
    print('-' * 110)
    print('C4c  PER-LINK INVARIANT, OR LOAD-TRACKING?  (U10c owes this decision.)  Same')
    print('     pairing, the OTHER axis.  Within one mix, take the argmin found at the')
    print('     OTHER load and score it against this row\'s own argmin AT THIS LOAD, on the')
    print('     common seeds.  Both numbers are measured at the SAME load, so only')
    print('     target_ms differs between them.  A RESOLVED negative difference means the')
    print('     value that is best at one offered load is MEASURABLY worse at the other,')
    print('     which is what "tracks load" means and what "per-link invariant" forbids.')
    print('-' * 110)
    print('  %-9s %5s %10s %11s %11s %11s %8s  %s'
          % ('mix', 'load', 'own argmin', 'other-load', 'paired med', 'worst seed',
             'seeds', 'verdict'))
    lmoved = []
    for mi, (lab, names) in enumerate(CHAIN):
        for L in LOADS:
            Lo = [x for x in LOADS if x != L][0]
            tb = TARGET_GRID[best_j[(mi, L)]]
            to = TARGET_GRID[best_j[(mi, Lo)]]
            if tb == to:
                print('  %-9s %5.2f %10.0f %11.0f %11s %11s %8s  same grid point'
                      % (lab, L, tb, to, '--', '--', '--'))
                continue
            d = [x - y for x, y in zip(tcell(mi, L, tb), tcell(mi, L, to))]
            nneg = sum(1 for x in d if x < 0)
            ok = (nneg == len(d))
            lmoved.append((lab, L, tb, to, ok))
            print('  %-9s %5.2f %10.0f %11.0f %11.3f %11.3f %4d/%-3d  %s'
                  % (lab, L, tb, to, med(d), max(d), nneg, len(d),
                     'TRACKS LOAD (resolved)' if ok else 'inside seed noise'))
    l_res = sum(1 for r in lmoved if r[4])
    print()
    if l_res:
        tm_axis = ('LOAD-TRACKING, MEASURED, NOT PER-LINK INVARIANT. %d of %d rows where the '
                   'two loads pick different grid points are RESOLVED: the other load\'s '
                   'pick is worse here by a margin every one of the %d paired seeds agrees '
                   'on (%s). A per-link constant cannot depend on the offered load, and '
                   'this one does.'
                   % (l_res, len(lmoved), TSEEDS,
                      ', '.join('%s@%.2f %.0f vs %.0f ms' % (r[0], r[1], r[2], r[3])
                                for r in lmoved if r[4])))
    elif lmoved:
        tm_axis = ('NOT ESTABLISHED EITHER WAY on the load axis: the two loads pick '
                   'different grid points on %d rows, but not one separates from seed noise '
                   'over %d paired seeds. A null result is not an equivalence result, so '
                   'per-link invariance is NOT established by it.' % (len(lmoved), TSEEDS))
    else:
        tm_axis = ('CONSISTENT WITH per-link invariance on the load axis -- every mix picks '
                   'the same grid point at both loads -- but consistency is not a '
                   'measurement: with a %.0f ms step the grid cannot see a smaller move.'
                   % step)
    print('  -> LOAD-AXIS DECISION (U10c): %s' % tm_axis.split('.')[0] + '.')
    for ln in _wrap(tm_axis, 100):
        print('     %s' % ln)
    print()
    print('  THE DECISION IS RECORDED, NOT ACTED ON.  40.0 is left exactly where it is:')
    print('  no-arbitrary-constants forbids swapping one unmeasured number for another,')
    print('  and this rig has never been compared to a real router (ADR-004 condition 2).')
    print('  What the decision changes is WHAT WOULD CLOSE the row: not a better constant,')
    print('  but a target that is a function of the offered load, derived and then measured')
    print('  against hardware.')
    sys.stdout.flush()

    if n_res_move:
        tm_verdict = 'OPEN'
        tm_why = ('C4b: the loss optimum MOVES with N. %d of %d off-N=2 rows beat the N=2 '
                  'pick by a margin every one of the %d seeds agrees on (%s). '
                  "N-invariance is contradicted by C4's own table, now at a %.0f ms step "
                  'across %.0f-%.0f ms (U10c), so the move is no longer an artefact of a '
                  'grid coarser than the effect. Reported OPEN; no value changed.'
                  % (n_res_move, len(nmoved), TSEEDS,
                     ', '.join('%s@%.2f %.0f vs %.0f ms' % (r[0], r[1], r[2], r[3])
                               for r in nmoved if r[4]),
                     step, TARGET_FINE[0], TARGET_FINE[-1]))
    elif nmoved:
        tm_verdict = 'OPEN'
        tm_why = ('C4b: the argmin moves across N on the printed table, but no off-N=2 row '
                  'separates from the N=2 pick by more than seed noise over %d seeds. That '
                  'is a NULL result, not an equivalence result: it cannot establish '
                  'invariance, and with a %.0f ms step the grid still cannot see a move '
                  'smaller than that. Reported OPEN; no value changed.'
                  % (TSEEDS, step))
    else:
        tm_verdict = 'OPEN'
        tm_why = ('C4b: every row picks the same grid point, which is CONSISTENT with '
                  'N-invariance but does not measure it -- with a %.0f ms step the grid '
                  'cannot see a smaller move, and a null result is not an equivalence '
                  'result. Reported OPEN; no value changed.' % step)
    print('  -> target_ms classification carried into C6: %s' % tm_verdict)
    for ln in _wrap(tm_why, 100):
        print('     %s' % ln)
    sys.stdout.flush()

    # ================= C5  the retired index-0 metric ========================
    print()
    print('=' * 110)
    print('C5  `tshare` IS NOW THE SPOTTY-CLASS SHARE (U10a).  reserved_composite.py and')
    print('    ackclock_sim.py both report sum(assigned[i] for i spotty) / sum(assigned),')
    print('    built on the per-def `spotty` identity -- the same quantity highn_battery.py')
    print('    has always computed for itself:')
    print("      grep -n 'sshare.. = sum' highn_battery.py   and")
    print("      grep -n '^def spotty_idx' highn_battery.py")
    print('    Three columns below: the LIVE metric as the sim reports it, the class share')
    print('    RECOMPUTED here from o.assigned (so the sim does not grade its own work),')
    print('    and the RETIRED `assigned[0]/sum` form kept as a post-hoc expression so the')
    print('    size of the defect it carried stays on the record.  THIS SECTION IS A BAR:')
    print('    live != recomputed on any row is a hard failure.')
    print('=' * 110)
    ...
    sres = {}
    with ProcessPoolExecutor(max_workers=WORKERS) as ex:
        for (mi, sch, sd, ts, ss, i0, shares) in ex.map(w_share, stasks, chunksize=4):
            sres.setdefault((mi, sch), []).append((ts, ss, i0, shares))
    print('  %-9s %3s %-7s %10s %12s %11s %10s   %s'
          % ('mix', 'N', 'sched', 'live', 'recomputed', 'retired i0',
             'i0 error', 'per-link shares'))
    live_max_dev = 0.0
    pull_class = {}
    for mi, (lab, names) in enumerate(MIXES):
        for sch in ('pull', 'Dc'):
            ...
            i0 = med([r[2] for r in rs])
            sh = rs[0][3]
            live_max_dev = max(live_max_dev,
                               max(abs(r[0] - r[1]) for r in rs))
            if sch == 'pull':
                pull_class[lab] = ss
            print('  %-9s %3d %-7s %10.3f %12.3f %11.3f %+10.3f   %s'
                  % (lab, len(names), sch, ts, ss, i0, i0 - ss,
                     ' '.join('%.3f' % x for x in sh)))
    print()
    print('  BAR C5-1  live == recomputed on every one of the %d runs: max |dev| = %.3e -> %s'
          % (len(stasks), live_max_dev, 'PASS' if live_max_dev == 0.0 else 'FAIL'))
    if live_max_dev != 0.0:
        fails.append('C5-1: live tshare != recomputed spotty-class share, max dev %.3e'
                     % live_max_dev)
    print('  BAR C5-2  the retired form is exact at N=2 and wrong from N=3, by construction:')
    n2err = max(abs(med([r[2] for r in sres[(0, s)]]) - med([r[1] for r in sres[(0, s)]]))
                for s in ('pull', 'Dc'))
    hi_err = max(abs(med([r[2] for r in sres[(mi, s)]]) - med([r[1] for r in sres[(mi, s)]]))
                 for mi in range(1, len(MIXES)) for s in ('pull', 'Dc'))
    lo_err = min(abs(med([r[2] for r in sres[(mi, s)]]) - med([r[1] for r in sres[(mi, s)]]))
                 for mi in range(1, len(MIXES)) for s in ('pull', 'Dc'))
    print('            N=2 |error| = %.3f ; N>=3 |error| spans %.3f-%.3f -> %s'
          % (n2err, lo_err, hi_err,
             'PASS' if (n2err == 0.0 and lo_err > 0.0) else 'FAIL'))
    if not (n2err == 0.0 and lo_err > 0.0):
        fails.append('C5-2: retired index-0 form is not exact at N=2 (%.3f) or not wrong '
                     'from N=3 (min %.3f)' % (n2err, lo_err))
    print()
    print('  CITATION LINE (U10a).  What `pull` actually puts on the SPOTTY CLASS at load')
    print('  0.65, %d seeds, RIG=%r, T=%.1fs -- the number the mode split is argued from:'
          % (SEEDS, RIG, T))
    print('    %s' % '   '.join('%s %.3f' % (lab, pull_class[lab])
                                for lab, _ in MIXES))
    print('  spanning %.3f-%.3f. The retired index-0 form read %.3f-%.3f on the same runs,'
          % (min(pull_class.values()), max(pull_class.values()),
             min(med([r[2] for r in sres[(mi, 'pull')]]) for mi in range(len(MIXES))),
             max(med([r[2] for r in sres[(mi, 'pull')]]) for mi in range(len(MIXES)))))
    print('  i.e. it UNDERSTATED the effect the mode split exists to fix, at every N>=3.')
    print('  The three downstream citations quoted the N=2 edge-rig reading of that retired')
    print('  form; U10a re-derives them from THIS line (docs/HANDOFF.md,')
    print('  docs/knowledge/decisions/ADR-003-mode-set.md,')
    print('  docs/knowledge/design/p5-execution-handover.md).')
    ...
    for L in LOADS:
        for mi, (lab, names) in enumerate(CHAIN):
            best_t[(lab, L)] = TARGET_GRID[best_j[(mi, L)]]
    gaps = [best_by_mix[l_] * 1000.0 - hf_by_mix[l_] * 1000.0 for (l_, _) in MIXES]
    ...
    for mi, (lab, names) in enumerate(MIXES):
        if len(names) > 2:
            rs = sres[(mi, 'Dc')]
            # U10a: r[0] (live) IS r[1] (class share) now, so the defect size has to be
            # read off r[2], the RETIRED index-0 form, which C5 keeps post hoc for this.
            ts_err.append(abs(med([r[2] for r in rs]) - med([r[1] for r in rs])))
    ROWS = [
        ...
        ('target_ms = 40.0',
         ...
         'N=2,3,4,5 respectively -- the argmin MOVES on both axes on the printed table. '
         '%s Separate, and NOT an N defect: 40 is not the argmin at 0.65. C4c, THE '
         'U10c DECISION ON THE OTHER AXIS: %s Left exactly as it is; an OPEN row is '
         'reported, never replaced.'
         % ('/'.join('%g' % best_t[(l_, 0.65)] for (l_, _) in CHAIN),
            '/'.join('%g' % best_t[(l_, 0.95)] for (l_, _) in CHAIN), tm_why, tm_axis)),
        ...
        ('tshare = spotty-class share',
         'share of native traffic on the spotty CLASS, over every link whose def carries '
         'spotty=True, in the rig and in the oracle alike',
         'N-INVARIANT',
         'CLOSED by U10a. It was `assigned[0]/sum`, a per-link metric keyed to INDEX 0: '
         'exact at N=2, because index 0 IS the only spotty link there, and wrong by '
         '%.3f-%.3f from N=3. It now uses the same construction highn_battery.py has '
         "always used for itself (grep -n 'sshare.. = sum' highn_battery.py, "
         "grep -n '^def spotty_idx' highn_battery.py) and reduces to the old reading at "
         'N=2 exactly, so the N=2 record is unbroken. C5 bar C5-1 holds the live metric '
         'to the independently recomputed class share; C5-2 keeps the retired form '
         'measured so the size of the defect stays on the record. The three downstream '
         'citations were re-derived from C5 CITATION LINE.'
    ...
    nform = sum(1 for r in ROWS if 'in form' in r[2].lower())
    print('  READ THE FIRST THREE COUNTS WITH THIS: %d of the %d rows carry an explicit'
          % (nform, len(ROWS)))
    print('  "OPEN in value" and are counted under their FORM label, so the settled count')
    print('  is an upper bound.  Rows whose VALUE has never been measured against anything')
    print('  outside the rig: %d of %d.  ADR-004 condition 2 is what closes them.'
          % (sum(1 for r in ROWS if r[2] == 'OPEN' or 'in form' in r[2].lower()),
             len(ROWS)))
    print('  Nothing above was changed by this file.')

    print()
    print('=' * 110)
    print('elapsed %.1fs' % (time.time() - t0))
    print('BARS: %d asserted, %d failed' % (2, len(fails)))
    for f in fails:
        print('  FAIL %s' % f)

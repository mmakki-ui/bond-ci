...
class Sim:
    def __init__(s, path_defs, offer_fn, T, seed, sched='ack',
        ...
        s.mirror = mirror; s.spotty = [bool(d['spotty']) for d in path_defs]  # U10a: spotty CLASS, loud if absent
    ...
    def finalize(s):
        ...
        gp = deliv_data * PKT_KB / Teff; atot = sum(s.assigned) or 1
        ...
        return {'gp': gp, 'loss': max(0.0, loss), 'p50': pct(.5), 'p95': pct(.95),
                ...
                'tshare': sum(a for a, sp in zip(s.assigned, s.spotty) if sp) / atot,  # U10a: spotty CLASS, not index 0

...
#
# ---------------------------------------------------------------------------
# DECISION (U10b, 2026-09-05): CANONICAL-GEOMETRY-ONLY, NOT RE-DERIVED ON AN
# ENSEMBLE BASIS -- for every bar, not just the two named below.
# ---------------------------------------------------------------------------
# The criterion (need not be re-derived): a bar is GEOMETRY-ESTABLISHED iff its
# whole ensemble range lies on one side of its threshold. Measured here: B3(loss)
# is not established on N3-het (31/32) or N4-teth (25/32), and B2 is not
# established on any of its 5 cells (statistic spread 1.26-2.60 pt against a
# 0.5 pt tolerance) -- U10's row has the full numbers, independently reproduced
# by a second sampler family. The N3-het flip re-scored at 12 seeds (up from 3)
# is still 31/32, range [+0.316, +0.465], never crossing zero; the two seeds
# that do cross sit at -0.092 / -0.109, inside U33's ~1 pt geometry noise floor.
# So the count is real, not a low-seed artefact, and the CI bars stay scored on
# the canonical geometry alone, unchanged: the ensemble does not fit a CI job
# (415-560 s at SEEDS=6; x32 does not), and widening either bar's tolerance to
# paper over a geometry sensitivity it was never designed to absorb is worse
# than naming the sensitivity here. Any future bar added to this file inherits
# the same rule unless its own review says otherwise: canonical geometry is the
# CI bar, the ensemble is a REVIEW tool that lives in this file, not in CI.
#
# ---------------------------------------------------------------------------
# THE PROBES
# ---------------------------------------------------------------------------
...
#     (The gate's classes, cited by SYMBOL rather than by a line number that the
#     next edit above them invalidates:
#      grep -n '^PAIRED' .github/scripts/rig_paired_gate.py
#      grep -n '^RELATIVE' .github/scripts/rig_paired_gate.py)
#
...
def main():
    ...
    if not (g0_ok and det and nc_ok and corr_ok):
        print()
        print('  SAMPLER INVALID -- ABORTING BEFORE ANY NUMBER IS SCORED.  The header of')
        print('  this file promises exactly that, and a self-gate that only prints does')
        print('  not deliver it: every number would already be in the artifact by the')
        print('  time a reader reached the verdict.  properties=%s determinism=%s '
              'negative-controls=%s correlation=%s.  Exit 2.'
              % (g0_ok, det, nc_ok, corr_ok))
        sys.stdout.flush()
        sys.exit(2)
    sys.stdout.flush()
    ...
    print("            (grep -n '^PAIRED' .github/scripts/rig_paired_gate.py).")
    print('  B2        `Dc loss <= ewma loss + 0.5`.  The comparison is paired; the 0.5 pt')
    print('            TOLERANCE is absolute.  Gated as PAIRED, same tuple.')
    ...
    print("            hand-placed schedule.  Gated as RELATIVE")
    print("            (grep -n '^RELATIVE' .github/scripts/rig_paired_gate.py).")
    print()
    ...
    print('VERDICT')
    print('=' * 112)
    # G0 aborts on failure before scoring, so reaching here means it passed.
    assert g0_ok and det and nc_ok and corr_ok

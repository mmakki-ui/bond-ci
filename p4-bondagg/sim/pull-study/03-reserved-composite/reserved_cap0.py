#!/usr/bin/env python3
# =============================================================================
# reserved_cap0.py -- SCHEDULER D (RESERVED-MIRROR, N-GENERIC) + candidate D'.
#
# This is a COPY of reserved_dp.py with one addition: sched='Dp' (= D', the
# cap0-health candidate).  D / pull / redundant / oracle refs are UNCHANGED and
# byte-for-byte identical to reserved_dp.py, so every D' number is paired against
# the exact same validated physics + reference schedulers.
#
# =========================== CANDIDATE D' (sched='Dp') =======================
# ONE work-conserving admission loop, TWO frame classes:
#   NATIVE    : plain PULL -- head frame to the hungriest path with local_ms <
#               target (the one-sided delivered-rate/local-ms cap of pull layered
#               on native admission, as shipped).  Byte-for-byte pull.
#   DUPLICATE : when a native frame lands on an AT-RISK source (spotty class,
#               carrying traffic), place ONE copy onto the hungriest gated HOST.
#               HOST GATE (Fable's shipped spec):
#                   local_ms(host) < target/2   AND
#                   drain_ewma[host] >= 0.75 * cap0[host]
#               where cap0 is the NOMINAL config capacity (health yardstick,
#               meter-free).  NO reserve fraction r, NO mir_budget / tot_budget,
#               NO TTL knob -- the duplicate's TTL is just the reorder-ring hold
#               (first-copy-wins).  N-generic: host/at-risk over range(N), no
#               privileged path; hosts are STEADY-class (not at_risk), so all-
#               steady and all-spotty both leave the duplicate un-armed (== pull).
#
# =============================================================================
...
class SimD:
    """PULL + N-generic reserved-mirror (sched='D'), candidate D' (sched='Dp'),
    full 'redundant', or a self-check 'pull'.  reserve_frac r in [0,1) and ttl_ms
    apply to 'D' only (D': NO r, NO TTL -- the duplicate rides the reorder-ring
    hold and arms purely on the local_ms<target/2 & drain>=0.75*cap0 host gate).
    ttl_ms bounds how long a spotty frame stays mirror-eligible under 'D': SHORT
    ttl == mirror only the in-flight exposure window (cheap insurance); LONG ttl +
    big r == mirror the full stream (-> the 'redundant' collapse)."""
    ...
    def run(s):
        ...
        for tk in range(nt):
            ...
            #   D  : local-ms < target                 (the pull admission gate)
            #   D' : local-ms < target/2  (STRICTER)   (this candidate's duplicate gate,
            #        Fable's shipped spec) AND drain_ewma >= 0.75*cap0.  cap0 is the
            #        NOMINAL config capacity used as the health yardstick (meter-free).
            health_ms = (0.5 * s.target_ms) if s.sched == 'Dp' else s.target_ms
            healthy = [alive[i] and s._local_ms(i) < health_ms
            ...
            # D' (sched='Dp'): NO reserve fraction -- the duplicate arms purely on the
            # gate (an at-risk source carrying traffic AND at least one gated host).
            if s.sched == 'Dp':
            ...
            nat_cap = [HUGE] * s.N
            # D' (sched='Dp'): NO reserve budget carve-out at all.  Native stays PURE
            # pull (nat_cap=HUGE, the one-sided delivered-rate/local-ms cap of plain
            # pull layered on native admission, as shipped); the duplicate is admitted
            # work-conservingly onto gated host slack, no mir_budget / tot_budget knob.
                ...
                for i in cand:
                    if s.local[i].offer(seq, s.enq[seq], lcaps[i]):
                        ...
                        if armed and at_risk[i]:
                            s.mir_offered += 1
                            if s.sched == 'Dp':
                                # ---- D' DUPLICATE class (same work-conserving loop) ----
                                # A native frame just landed on an at-risk source; place
                                # ONE duplicate onto the hungriest gated host (host[]
                                # already encodes local-ms<target/2 & drain>=0.75*cap0 &
                                # steady-class).  Re-check local-ms dynamically so the
                                # copy self-limits as we fill the host this tick.  No
                                # budget, no TTL -- first-copy-wins is the reorder ring.
                                hc = [h for h in range(s.N)
                                      if host[h] and h not in s.sent_on[seq]
                                      and s._local_ms(h) < 0.5 * s.target_ms]
                                if s.lat_bias:
                                    hc.sort(key=lambda h: (s.defs[h]['down_owd']
                                                           + s.defs[h]['loc_owd'],
                                                           s._local_ms(h)))
                                else:
                                    hc.sort(key=s._local_ms)
                                for h in hc:
                                    if s.local[h].offer(seq, s.enq[seq], lcaps[h]):
            ...
            if armed and s.mirror_q:
                ...
                while s.mirror_q and mguard < 200000:
                    ...
                    if now - qt > s.ttl:                 # aged out: exposure window closed
                ...
                while s.mirror_q and now - s.mirror_q[0][2] > s.ttl:

...
// either box: U40 HAS read them, and the server inventory records `(no
// /etc/p5)` (docs/knowledge/inventory/2026-08-30-server-brume2.txt:117).
// E0/U25 owns /etc/p5 and E8/U28 owns getting the secret there; neither is
// written yet. The CLIENT half IS wired since round 3 -- the pull client calls
// the gate at four points (daemon/pull.go send, daemon/pullrun.go RX loop and
// its ping/pong paths) -- so the gate can close the moment a key exists on
// both ends. Until then this unit changes the exposure of a running install
// only by removing the ping-source aiming primitive. Read every "closed"
// below as "closed once a key is on both ends".
//
...
//	8. The boxes HAVE been read (U40) but the inventories record no MTU for
//	any interface (grep -in mtu docs/knowledge/inventory/*.txt -> no hits),
...
//
// UNMEASURED, and named so: the ARM cost. CI is x86-64 only. U40 has read the
// boxes and the server is aarch64_cortex-a53
// (docs/knowledge/inventory/2026-08-30-server-brume2.txt:8,10) -- so the 64-bit
// crossbuild target is right -- but the inventory captured no /proc/cpuinfo
// Features line, so whether those cores carry the ARMv8 SHA-2 extension, a
// ratio of roughly 8x between the two answers, is NOT known here. What IS known is the

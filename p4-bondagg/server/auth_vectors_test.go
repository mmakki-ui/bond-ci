...
// newTestRx builds the real receive path over the real deliver-on-arrival
// writer, with no sockets. keys == nil is the pre-U31 server: authentication
// off. `got` is what reached the tunnel, in delivery order (U159: that is
// ARRIVAL order, because nothing between Handle and WireGuard reorders).
func newTestRx(keys [][]byte) (*rxPath, *peers, *echoBudget, *[]uint32) {
	pr := &peers{}
	st := &LinkStats{}
	bud := &echoBudget{}
	got := new([]uint32)
	g := newAuthGate(keys, 10*time.Second, roleServer)
	x := newRxPath(g, pr, st, bud, func(b []byte) {
		if len(b) >= 4 {
...
// seqPay makes a payload whose first 4 bytes are the seq, so the delivered
// bytes identify which frame came out.
//
...
func TestOpenGateReflectsViaAForgedDataFrame(t *testing.T) {
	...
	x, pr, _, _ := newTestRx(nil) // no key: the posture of every install today
	x.Handle(plain(FlagData, 1, 1, 1, seqPay(1)), cli, now)
	...
	y, _, _, _ := newTestRx([][]byte{testKey(1)})
...
// VECTOR 4 -- RESTATED BY U159, and the restatement is the point.
//
// It used to read "a forged far-future seq costs a full hold of uplink": Push
// re-anchored `next` onto the forged seq and every legitimate frame after it
// was beyond-window-old and dropped. THAT VECTOR NO LONGER EXISTS, because the
// state it attacked -- `next` -- is gone with the ring. Nothing on the receive
// path holds seq state for a forgery to move, so a forged seq cannot cost a
// single legitimate frame, with a key or without one.
//
// What survives is the plainer vector the ring used to sit on top of: a forged
// DATA frame INJECTS its payload into the tunnel. The gate closes that; the
// deletion does not. WireGuard drops the injected bytes at decrypt (they carry
// no valid session), so the cost is CPU, not a breach -- which is exactly the
// same trade the DELIVER ON ARRIVAL block records for duplicate copies.
func TestForgedFarSeqCannotInjectIntoTheTunnel(t *testing.T) {
	key := testKey(1)
	cli := mustAddr(t, "203.0.113.9:59402")
	atk := mustAddr(t, "198.51.100.7:1234")
	t0 := time.Now()

	x, _, _, got := newTestRx([][]byte{key})
	...
	// Both legitimate frames reach the tunnel, in arrival order, and the
	// forgery reaches nothing.
	eq(t, *got, 1, 2)

	// Positive control: with no key the forged payload is injected.
	...
	if !inj {
		t.Fatal("the control did not reproduce the injection: this test proves nothing")
	}
}

...
// echo budget or get into the tunnel -- and a decision buried in a `for` loop
// around a UDP socket cannot be tested.
//
// U159: `out` replaced the reorder ring. A DATA payload is handed to it AT
// ARRIVAL, in the order the wire delivered it, and nothing between here and
// WireGuard buffers, re-sequences or dedups. See main.go's DELIVER ON ARRIVAL
// block for the substitution that pays for it.
type rxPath struct {
	gate    *authGate
	peers   *peers
	stats   *LinkStats
...
func newRxPath(g *authGate, pr *peers, st *LinkStats, bud *echoBudget, out func([]byte)) *rxPath {
	return &rxPath{
		gate:    g,
		peers:   pr,
		stats:   st,
...
// endpoint table, the meter, the echo budget or the tunnel. The four
...
// (TestForgedPingCannotStarveTheEchoBudget) and injection into the tunnel
// (TestForgedFarSeqCannotInjectIntoTheTunnel).
//
...
func (x *rxPath) Handle(b []byte, ra *net.UDPAddr, now time.Time) ([]byte, *net.UDPAddr) {
	...
	case FlagData:
		...
		// METER FIRST, at ARRIVAL, ahead of the wg write -- the oracle's semantics
		...
		// DELIVER ON ARRIVAL (U159). f.seq is READ off the wire -- the frame
		// format is unchanged -- and it is deliberately NOT consulted here:
		// nothing on this path reorders and nothing dedups. A duplicate copy
		// (`lightning` sends one per source) reaches WireGuard, whose
		// anti-replay window rejects the repeat AFTER decrypting it. See
		// main.go's DELIVER ON ARRIVAL block for that trade and its bound.
		x.out(f.pay)
		return nil, nil
	case FlagPing:

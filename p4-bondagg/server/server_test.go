...
// eq compares what reached the tunnel with what was expected, in order. It is
// shared with auth_vectors_test.go, which is now the only place that asserts on
// delivered frames: U159 deleted the resequencer this section used to cover,
// and deliver-on-arrival has no state of its own to test here -- the bars that
// replaced it are in deliver_test.go.

...
#           file        rm -f
#           dir         the ONLY recursive removals, and only these three: each
#                       is a directory the old package itself deletes or owns whole
...
p1|svc|/etc/init.d/cake-autorate|p1-autorate/autoratectl:47-48 (`off` disables then stops the controller); deploy/p5/shape-install:147 names it E7's to remove
...
p1|uci|sqm.wgclient1|p1-autorate/bootstrap-autorate.sh:90-98 created it; autoratectl:51-52 only sets enabled=0, so the OBJECT outlives `off` and E7 owns deleting it (p5-execution-handover.md:80)
P5OLDROWS
}
...
p5_old_svc_flags() {
    p5_glob_hits "/etc/rc.d/[SK][0-9][0-9]$1"
}
...
p5_old_flag_removable() {
    case "$1" in /etc/rc.d/[SK][0-9][0-9]?*) : ;; *) return 1 ;; esac
...
# exists to SAVE would be moved INSIDE the tree the same plan later `rm -rf`s,
...
p5_old_quar_dest_ok() {
    ...
    # in one of them is refused by the same data that schedules the rm -rf.

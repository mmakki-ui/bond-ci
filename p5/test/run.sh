...
# EVERY `dm arm` BELOW CARRIES --no-boot-limb (U290), and that is not a way round
# the new refusal -- it is what these bars have always been. `arm` now refuses
# unless crond is RUNNING, the crontab directory is writable and the tool is
# running from its installed path; crond is not running under WSL or on a CI
# runner, and these bars run the package copy against a fake root. Not one of them
# owns the boot limb: they own the RECORD, the deadline, the firing path and the
# uninstall refusal. The boot limb's own bars are DM-10, DM-45, DM-56..DM-67 in
# deploy/server/test-p5-fw-deadman.sh, which builds a fake box that SATISFIES the
# three conditions instead of opting out of them. Arming here without the flag
# would be asserting a limb nothing in this file ever checks.
...
rm -f "$MARK"
dm arm --after 60 --restore "touch $MARK" --label t1 --no-timer --no-boot-limb; rc=$?
a=0
...
dm arm --restore "touch $MARK" --label t2 --no-timer --no-boot-limb; rc=$?
grep -q 'REQUIRED and has no default' "$TMPBASE/dm.err" && a=0 || a=1
dm arm --after 10 --label t2 --no-timer --no-boot-limb; rc2=$?
grep -q 'restore is REQUIRED' "$TMPBASE/dm.err" && b=0 || b=1
dm arm --after later --restore "true" --label t2 --no-timer --no-boot-limb; rc3=$?
...
dm arm --after 0 --restore "touch $MARK" --label t1 --no-timer --no-boot-limb
dm check; rc=$?
a=0
[ -f "$MARK" ] || a=1
[ -f "$RD/etc/p5/deadman/t1" ] && a=1
...
rm -f "$MARK"
dm arm --after 0 --restore "touch $MARK" --label t3 --no-timer --no-boot-limb
...
dm arm --after 0 --restore "exit 7" --label t4 --no-timer --no-boot-limb
dm check; rc=$?
a=0
...
rm -f "$MARK"
dm arm --after 2 --restore "touch $MARK" --label t62 --no-boot-limb
...
rm -f "$MARK"
dm arm --after 60 --restore "touch $MARK" --label t6 --no-timer --no-boot-limb

...
# kind. p4-bondagg/server/auth.go:221 sets ReopenDefault = 30s, and the authGate
...
#   KEY-17..KEY-23  the PREFLIGHTS refuse. After engage the last SSTAT/PSTAT
#                   line must say gate=1 (CLOSED) and authbad=0. This is the
#                   half that catches an operator who declared nothing at all,
#                   which is the case no installer flag can reach.
...
# log to opposite verdicts, so a run that passes has exercised the refusing
# path rather than merely failing to reach it.
...
pgbar KEY-18 "$PRE_S" "$AGR" "$AGL/s-open"    0 1 'the auth gate is OPEN' \
    "the SERVER preflight REFUSES when the gate has REOPENED (gate=0) -- the state a wrong transport key degrades to after 30s instead of announcing itself"
pgbar KEY-19 "$PRE_S" "$AGR" "$AGL/s-authbad" 0 1 'failed their MAC' \
    "the SERVER preflight REFUSES on authbad>0 even while the gate is still shut: frames are arriving that this key cannot verify, so the two boxes do not carry the same secret"
...
pgbar KEY-23 "$PRE_C" "$AGR" "$AGL/c-open"    0 1 'the auth gate is OPEN' \
    "the CLIENT preflight REFUSES on gate=0: both ends count the same thing and the client is the box an operator can still reach, so it must not report a green preflight over an unauthenticated wire"

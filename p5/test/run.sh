...
# VS-SRC-1: the pair is in the package and reaches the stamp verbatim. $TMPBASE/
# stamp.out is the `p5-version` print from the VS-1 block above, unmodified since.
...
a=0
[ -n "$sv_p_sha" ] || { a=1; echo "  the package PROVENANCE carries no P5_SRC_SHA -- nothing names the sha a gate run is keyed to"; }
[ -n "$sv_p_ref" ] || { a=1; echo "  the package PROVENANCE carries no P5_SRC_REF"; }
...
chk "$(yn "$([ "$a" = 0 ]; echo $?)")" \
    "VS-SRC-1" "the package names an integration sha (${sv_p_sha#*=}, ref ${sv_p_ref#*=}) and the stamp carries BOTH fields through verbatim, so the box can name the gate run that covers it"
...
chk "$(yn "$([ "$a" = 0 ]; echo $?)")" \
    "VS-SRC-2" "--verify PRINTS the recorded source sha and reports $sv_want, the verdict the package's own fields imply (rc=$rc). Drift detection now answers two questions, not one"
...
# red for the right reason. What is asserted is the thing that matters: the
# block names one of the two integration refs and resolves it to the sha this
# checkout actually holds, independently re-read with git here.
...
if command -v git >/dev/null 2>&1 && git -C "$_pk_repo" rev-parse --git-dir >/dev/null 2>&1; then
    pk5_out=$(sh "$_src_blk" "$_pk_repo" 2>"$TMPBASE/pk5.err"); pk5_rc=$?
    pk5_ref=${pk5_out%% *}; pk5_sha=${pk5_out##* }
    pk5_want=$(git -C "$_pk_repo" rev-parse "${pk5_ref:-refs/heads/dev}^{commit}" 2>/dev/null)
    a=0
    ...
    [ "$pk5_rc" = 0 ] || { a=1; echo "  the block exited $pk5_rc in this checkout:"; sed 's/^/    /' "$TMPBASE/pk5.err"; }
    case "$pk5_ref" in
        refs/remotes/origin/dev|refs/heads/dev) : ;;
        *) a=1; echo "  it named ref '$pk5_ref', which is neither refs/remotes/origin/dev nor refs/heads/dev" ;;
    esac
    echo "$pk5_sha" | grep -qE '^[0-9a-f]{40}$' || { a=1; echo "  it produced '$pk5_sha', which is not a 40-character object id"; }
    { [ -n "$pk5_want" ] && [ "$pk5_sha" = "$pk5_want" ]; } || { a=1; echo "  it produced '$pk5_sha' but git reads $pk5_ref as '$pk5_want' in this checkout"; }
    chk "$(yn "$([ "$a" = 0 ]; echo $?)")" \
        "PK-5" "the builder's OWN source-sha block, executed out of scripts/build-p5-package.sh, resolves $pk5_ref to $pk5_sha -- the sha git independently reads for that ref here. That is the value PROVENANCE carries and the box compares against"

    # MU-PK5 -- prove PK-5's subject can fail, and in the one direction that
    # would be silently corrosive. A checkout with no dev ref must ABORT the
    # build naming the refs it tried. The alternative -- defaulting to HEAD --
    # writes a field that equals P5_GIT_COMMIT by construction, which makes
    # every box-side comparison pass for ever. The repo here is an EMPTY
    # `git init`: no commit, so no branch ref exists whatever the local
    # init.defaultBranch is set to.
    _mu_repo="$TMPBASE/nodevref"
    rm -rf "$_mu_repo"; mkdir -p "$_mu_repo"
    git init -q "$_mu_repo" >/dev/null 2>&1
    mu5_out=$(sh "$_src_blk" "$_mu_repo" 2>&1); mu5_rc=$?
    a=0
    [ "$mu5_rc" = 0 ] && { a=1; echo "  the block exited 0 with no dev ref reachable -- the build would emit a package naming nothing, or naming HEAD to itself"; }
    echo "$mu5_out" | grep -q 'refs/heads/dev' || { a=1; echo "  the abort did not NAME the refs it tried: $mu5_out"; }
    chk "$(yn "$([ "$a" = 0 ]; echo $?)")" \
        "MU-PK5" "MUTATION: the same block against a checkout with NO dev ref exits $mu5_rc and names the refs it looked for. A missing integration ref is a fatal, named build input here -- never a quiet fallback to HEAD, which is the one value that makes the box-side check unfailable"
    rm -rf "$_mu_repo"
else
    bad "PK-5" "cannot reach a git checkout at $_pk_repo, so the builder's source-sha resolution cannot be executed. This bar FAILS rather than skipping, for the same reason P-2 does: the resolution is the half of U292 that runs on the build host"
    bad "MU-PK5" "not run -- PK-5 could not reach a git checkout (see above)"
fi

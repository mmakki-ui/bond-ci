...
P5_SRC_REF=refs/heads/dev
P5_SRC_SHA=0000000000000000000000000000000000000000
EOF
...
# src_want PKGDIR -> MATCH, MISMATCH or ABSENT (U292). The harness's OWN answer
# to the question src_report answers on the box, computed here from the
# package's PROVENANCE rather than read off p5-version -- so every bar below
# fails when the two disagree, in either direction.
src_want() {
    _sw_s=$(grep '^P5_SRC_SHA=' "$1/PROVENANCE" 2>/dev/null | head -1); _sw_s=${_sw_s#*=}
    _sw_c=$(grep '^P5_GIT_COMMIT=' "$1/PROVENANCE" 2>/dev/null | head -1); _sw_c=${_sw_c#*=}
    _sw_d=$(grep '^P5_GIT_DIRTY=' "$1/PROVENANCE" 2>/dev/null | head -1); _sw_d=${_sw_d#*=}
    if [ -z "$_sw_s" ]; then
        echo ABSENT
    elif [ "$_sw_s" = "$_sw_c" ] && [ "$_sw_d" = no ]; then
        echo MATCH
    else
        echo MISMATCH
    fi
...
#
# U292 MOVED THE EXPECTED STATUS FROM A LITERAL 0 TO A DERIVED ONE, and that is
# a strengthening rather than a relaxation. --verify now also reports whether
# the installed bytes are the tree at the source sha the package names. In
# SYNTHETIC mode the fixture is consistent, so the expectation is 0 and this bar
# is the run it has always been. In REAL mode a package built off a unit branch
# or a dirty tree genuinely is NOT the integration sha it names, and --verify
# saying so with a non-zero status is the whole of U292; a bar hard-coded to 0
# would have gone red on the correct answer. The expectation is computed from
# the PACKAGE, so a --verify that returns the wrong one still fails here.
vs4_want=0
[ "$(src_want "$P9")" = MISMATCH ] && vs4_want=1
...
chk "$(yn "$([ "$rc" = "$vs4_want" ] && [ "$a$b" = 00 ]; echo $?)")" "VS-4" "--verify right after install exits $rc (the package's own source record says $(src_want "$P9"), so $vs4_want) and names the one self-referential row rather than skipping it"

# ===========================================================================
# SOURCE-SHA bars (U292) -- can a gate run be NAMED for the bytes on the box?
# ===========================================================================
# The package already pinned every FILE and recorded the commit it was COMPILED
# from. Neither answers the question a deploy actually asks: bar A runs at the
# integration sha, and on a unit branch that is the one sha P5_GIT_COMMIT never
# holds. These four bars cover the fact (recorded, carried), the verdict
# (derived on the box), the seeded failure (a sha that is not the built tree)
# and the neutrality claim (a package without the field still installs).
#
# WHAT THEY MEASURE IN SYNTHETIC MODE, said plainly: mkpkg's fixture supplies
# the pair, so VS-SRC-1 measures p5-install's carry-through and p5-version's
# reading of it, NOT the builder. The builder's own half is PK-5 below, which
# runs the shipped resolution out of scripts/build-p5-package.sh in both modes.

# VS-SRC-1: the pair reaches the stamp verbatim -- INCLUDING when there is no
# pair, in which case nothing may be invented. $TMPBASE/stamp.out is the
# `p5-version` print from the VS-1 block above, unmodified since.
#
# WHICH SIDE THIS BAR TAKES IS READ OFF THE PACKAGE, not hard-coded, for the
# same reason VS-4's expected status is (:1119-1127). Synthetic mode's fixture
# always supplies the pair, so on the e0 gate this is the full carry-through
# assertion it has always been. A REAL package is built wherever the builder
# ran, and a checkout with no integration ref -- which is exactly what
# actions/checkout@v4 leaves on the runner -- correctly produces a package with
# no pair. Demanding one there is demanding the builder invent a sha, which is
# the one thing U292 forbids. The absent side is not a free pass: it asserts
# p5-install carried the absence through rather than filling it in, and the
# BUILDER's own half is barred hermetically and unconditionally by PK-5/PK-5O
# below, which do not depend on the mode or on the host's refs.
sv_p_ref=$(grep '^P5_SRC_REF=' "$P9/PROVENANCE" | head -1)
sv_p_sha=$(grep '^P5_SRC_SHA=' "$P9/PROVENANCE" | head -1)
sv_s_ref=$(grep '^P5_SRC_REF=' "$TMPBASE/stamp.out" | head -1)
sv_s_sha=$(grep '^P5_SRC_SHA=' "$TMPBASE/stamp.out" | head -1)
a=0
if [ -n "$sv_p_sha" ] || [ -n "$sv_p_ref" ]; then
    sv1_side="names an integration sha (${sv_p_sha#*=}, ref ${sv_p_ref#*=}) and the stamp carries BOTH fields through verbatim, so the box can name the gate run that covers it"
    [ -n "$sv_p_sha" ] || { a=1; echo "  the package PROVENANCE carries P5_SRC_REF but no P5_SRC_SHA -- half a record is not one"; }
    [ -n "$sv_p_ref" ] || { a=1; echo "  the package PROVENANCE carries P5_SRC_SHA but no P5_SRC_REF"; }
    echo "${sv_p_sha#*=}" | grep -qE '^[0-9a-f]{40}$' || { a=1; echo "  P5_SRC_SHA is not a 40-character object id: '${sv_p_sha#*=}'"; }
    [ "$sv_p_ref" = "$sv_s_ref" ] || { a=1; echo "  stamp says '$sv_s_ref', package says '$sv_p_ref'"; }
    [ "$sv_p_sha" = "$sv_s_sha" ] || { a=1; echo "  stamp says '$sv_s_sha', package says '$sv_p_sha'"; }
else
    sv1_side="names NO integration sha (built where neither refs/remotes/origin/dev nor refs/heads/dev resolves) and the stamp carries that absence through -- p5-install invented neither field, which is what makes the box-side verdict able to fail"
    [ -z "$sv_s_sha" ] || { a=1; echo "  the package names no source sha but the stamp grew one: '$sv_s_sha'"; }
    [ -z "$sv_s_ref" ] || { a=1; echo "  the package names no source ref but the stamp grew one: '$sv_s_ref'"; }
fi
chk "$(yn "$([ "$a" = 0 ]; echo $?)")" \
    "VS-SRC-1" "the package $sv1_side"

# VS-SRC-2: --verify SURFACES the record and its verdict agrees with the package.
# The verdict is computed by src_want from PROVENANCE, so ABSENT is a verdict
# like the other two and this bar fails if p5-version reports a different one.
vrun --verify >"$TMPBASE/src2.out" 2>&1; rc=$?
sv_want=$(src_want "$P9")
a=0
if [ "$sv_want" = ABSENT ]; then
    grep -q '^SRCSHA   RECORD' "$TMPBASE/src2.out" && { a=1; echo "  the package records no source sha but --verify printed a RECORD line:"; grep '^SRCSHA' "$TMPBASE/src2.out" | sed 's/^/    /'; }
else
    grep -q "^SRCSHA   RECORD  P5_SRC_SHA=${sv_p_sha#*=} " "$TMPBASE/src2.out" || { a=1; echo "  --verify did not print the source record; it printed:"; grep '^SRCSHA' "$TMPBASE/src2.out" | sed 's/^/    /'; }
fi
grep -q "^SRCSHA   $sv_want" "$TMPBASE/src2.out" || { a=1; echo "  --verify did not report the verdict $sv_want the package implies"; }
chk "$(yn "$([ "$a" = 0 ]; echo $?)")" \
    "VS-SRC-2" "--verify reports $sv_want, the verdict the package's own fields imply, and prints the recorded source sha exactly when there is one (rc=$rc). Drift detection now answers two questions, not one"

# VS-SRC-3: MUTATION -- seed a source sha that is not the tree the package was
# built from. This is the bar the whole unit turns on: the verdict is derived,
# so a seeded sha has to move it, and a recorded verdict never would have.
PS3=$TMPBASE/pkgsrc3; rm -rf "$PS3"; mkpkg "$PS3" </dev/null
SEEDSHA=deadbeefdeadbeefdeadbeefdeadbeefdeadbeef
grep -v '^P5_SRC_SHA=' "$PS3/PROVENANCE" > "$TMPBASE/src3.prov"
echo "P5_SRC_SHA=$SEEDSHA" >> "$TMPBASE/src3.prov"
cp "$TMPBASE/src3.prov" "$PS3/PROVENANCE"; remanifest "$PS3"
RS3=$TMPBASE/rsrc3; rm -rf "$RS3"
inst "$RS3" "$PS3" client; irc=$?
P5_ROOT="$RS3" sh "$BIN/p5-version" --verify >"$TMPBASE/src3.out" 2>&1; rc=$?
a=0
[ "$irc" = 0 ] || { a=1; echo "  the seeded package did not install (rc=$irc) -- this bar needs one that installs and THEN flags"; }
[ "$rc" = 1 ] || { a=1; echo "  --verify exited $rc on a seeded source sha, want 1"; }
grep -q '^SRCSHA   MISMATCH' "$TMPBASE/src3.out" || { a=1; echo "  --verify did not flag it:"; grep '^SRCSHA' "$TMPBASE/src3.out" | sed 's/^/    /'; }
grep -q "$SEEDSHA" "$TMPBASE/src3.out" || { a=1; echo "  --verify flagged without NAMING the seeded sha"; }
chk "$(yn "$([ "$a" = 0 ]; echo $?)")" \
    "VS-SRC-3" "MUTATION: a package whose P5_SRC_SHA is not the commit it was built from still INSTALLS (rc=$irc) and --verify exits $rc naming both shas. The verdict is derived from two recorded facts, so seeding one moves it"
rm -rf "$RS3" "$PS3"

# VS-SRC-4: the neutrality claim, made mechanical. U292 adds a recorded fact; a
# package that predates it must install exactly as before and must not be
# reported as drift. "Behaviour-neutral" is a claim, and a claim with no bar is
# the thing this battery exists to stop.
PS4=$TMPBASE/pkgsrc4; rm -rf "$PS4"; mkpkg "$PS4" </dev/null
grep -v '^P5_SRC_SHA=' "$PS4/PROVENANCE" | grep -v '^P5_SRC_REF=' > "$TMPBASE/src4.prov"
cp "$TMPBASE/src4.prov" "$PS4/PROVENANCE"; remanifest "$PS4"
RS4=$TMPBASE/rsrc4; rm -rf "$RS4"
inst "$RS4" "$PS4" client; irc=$?
P5_ROOT="$RS4" sh "$BIN/p5-version" --verify >"$TMPBASE/src4.out" 2>&1; rc=$?
a=0
[ "$irc" = 0 ] || { a=1; echo "  a package with no source-sha pair FAILED to install (rc=$irc): the change is not behaviour-neutral"; }
[ "$rc" = 0 ] || { a=1; echo "  --verify exited $rc on an install with no source record, want 0"; }
grep -q '^SRCSHA   ABSENT' "$TMPBASE/src4.out" || { a=1; echo "  --verify did not SAY the record is absent; silence is the wrong answer:"; grep '^SRCSHA' "$TMPBASE/src4.out" | sed 's/^/    /'; }
chk "$(yn "$([ "$a" = 0 ]; echo $?)")" \
    "VS-SRC-4" "a package carrying NO P5_SRC_SHA still installs (rc=$irc) and --verify still passes (rc=$rc) while SAYING no gate run can be named for it -- absence reported as absence, never as drift and never as silence"
rm -rf "$RS4" "$PS4"
...
for d in $(pkg_dests "$P9" client | sed 's:/[^/]*$::' | sort -u); do
    # U262: OWNED means "inside P5's namespace", not "one of three exact names". The
    # package writes /usr/lib/p5/portal/{bin,catalogue,lib,www,www/cgi-bin}, every one of
    # them matched by contract/namespace `both|/usr/lib/p5/*`, so removal SHOULD take them
    # -- and the old test counted each correct removal as a shared directory wrongly
    # deleted. What stays shared is what the namespace does not claim: /etc/init.d and
    # /usr/sbin.
    case "$d" in /usr/lib/p5|/usr/lib/p5/*|/etc/p5|/etc/p5/*) continue ;; esac
...
# GM-1 (U246): a DISCOVERED fact whose NAME carries a glob metacharacter must not wedge the
# box. MEASURED before the fix, on a root holding only /etc/p5/plain and /etc/p5/we[ird:
# --check named both, and --remove --recover answered "REFUSING THE ENTIRE REMOVAL ...
# Offending entries: file|/etc/p5/we[ird" and unlinked NOTHING -- not even /etc/p5/plain.
# Nothing removable means nothing installable: one filename bricks the recovery verb, and
# /etc/p5/* is where E6 writes facts whose names are E6's.
RGM="$TMPBASE/gm"; rm -rf "$RGM"; mkdir -p "$RGM/etc/p5" "$RGM/usr/lib/p5"
printf 'y\n' > "$RGM/etc/p5/plain"
printf 'x\n' > "$RGM/etc/p5/we[ird"
P5_ROOT="$RGM" sh "$BIN/p5-uninstall" --check --role client > "$TMPBASE/gm.check" 2>&1
gm_named=0; grep -q 'we\[ird' "$TMPBASE/gm.check" && gm_named=1
unin "$RGM" --remove --recover --role client >/dev/null 2>&1; gm_rc=$?
gm_left=$(find "$RGM" -type f 2>/dev/null | wc -l)
P5_ROOT="$RGM" TMPDIR="$CRTMP" sh "$BIN/p5-install" --package "$P9" --role client >/dev/null 2>&1; gm_irc=$?
rm -rf "$RGM"
chk "$(yn "$([ "$gm_named" = 1 ] && [ "$gm_rc" = 0 ] && [ "$gm_left" = 0 ] && [ "$gm_irc" = 0 ]; echo $?)")" \
    "GM-1" "a fact named with a glob metacharacter is named by --check (named=$gm_named), removed by --remove --recover (rc=$gm_rc, $gm_left files left) and does not block the next install (rc=$gm_irc) -- one filename used to refuse the ENTIRE removal and brick the box"

# GM-2 (U246): the install gate and the removal gate must agree. p5_phys_ok was called from
# p5_removable alone, so a box whose ancestor directory is a SYMLINK took the install and
# then refused every removal -- and told the operator to run the verb that was refusing.
# The install must refuse instead, while the box is still recoverable.
RSL="$TMPBASE/sym"; rm -rf "$RSL"; mkdir -p "$RSL/usr" "$RSL/elsewhere/p5"
ln -s ../elsewhere/p5 "$RSL/usr/lib" 2>/dev/null || ln -s "$RSL/elsewhere/p5" "$RSL/usr/lib"
P5_ROOT="$RSL" TMPDIR="$CRTMP" sh "$BIN/p5-install" --package "$P9" --role client \
    > "$TMPBASE/sym.out" 2>&1; sl_rc=$?
sl_files=$(find "$RSL/elsewhere" -type f 2>/dev/null | wc -l)
rm -rf "$RSL"
chk "$(yn "$([ "$sl_rc" != 0 ] && [ "$sl_files" = 0 ]; echo $?)")" \
    "GM-2" "an install whose ancestor directory is a SYMLINK out of the namespace is REFUSED (rc=$sl_rc) and writes nothing ($sl_files files) -- it used to succeed and then be unremovable, with the refusal naming --remove as the remedy"
# CR-6 (U242): the printed plan and the run must agree about the staging twins, and the
# sweep must be BOUNDED to twins of PLANNED paths. README.md measured the gap the other way
# round -- `.p5-incoming` appeared ZERO times in the printed plan while the run unlinked
# twins in three directories the plan says it cannot touch. The sweep itself is NOT the
# defect and is not re-tested here: HP-4 already requires it. So this bar fixes the CLAIM,
# then fences the behaviour.
#
# The plan is read from the harness's own capture files: unin() already redirects stdout to
# $TMPBASE/uout and stderr to $TMPBASE/uerr, so a redirection on the CALL captures nothing --
# the first build of this bar did exactly that and reported disclosed=0 on a correct fix.
RC6="$TMPBASE/cr6"; inst "$RC6" "$P9" client
unin "$RC6" --remove --dry-run --role client
cr6_disc=0
grep -q 'p5-incoming' "$TMPBASE/uout" "$TMPBASE/uerr" 2>/dev/null && cr6_disc=1
# A .p5-incoming name that is NOT the twin of any planned path, planted INSIDE P5's own
# directory: that is the strongest form of the bound -- not "outside the namespace", but
# "inside it and still not ours". P5 must never derive, name or touch it.
cr6_other="/usr/lib/p5/.p5-incoming.not-a-p5-path"
printf 'operator file\n' > "$RC6$cr6_other"
unin "$RC6" --remove --role client; cr6_rc=$?
cr6_kept=0; [ -e "$RC6$cr6_other" ] && cr6_kept=1
rm -rf "$RC6"
chk "$(yn "$([ "$cr6_disc" = 1 ] && [ "$cr6_kept" = 1 ]; echo $?)")" \
    "CR-6" "the printed plan NAMES the staging-twin sweep it performs (disclosed=$cr6_disc) and a .p5-incoming name that is not a twin of a planned path is left alone even inside P5's own directory (kept=$cr6_kept, rc=$cr6_rc) -- the plan promised less than the run did"
...
# EN-1/EN-2 (U243): removal REFUSES while P5 is ENGAGED, and the refusal is a gate,
# not a wedge -- the same shape as DM-8/DM-9 for the deadman, because it is the same
# hazard: --remove stops and disables the feeder while the tunnel endpoint still points
# at its loopback listener. The fact is the one `p5 on` writes, not a flag invented here.
REN="$TMPBASE/eng"; inst "$REN" "$P9" client
mkdir -p "$REN/etc/p5"
printf 'engaged\n' > "$REN/etc/p5/rc"
before=$(find "$REN" | sort | sha256sum)
unin "$REN" --remove --role client; rc=$?
after=$(find "$REN" | sort | sha256sum)
grep -q 'P5 IS ENGAGED' "$TMPBASE/uerr" && a=0 || a=1
chk "$(yn "$([ "$rc" = 5 ] && [ "$before" = "$after" ] && [ "$a" = 0 ]; echo $?)")" \
    "EN-1" "--remove refuses while P5 is engaged (exit 5) and the tree is byte-identical (rc=$rc)"
rm -f "$REN/etc/p5/rc"
unin "$REN" --remove --role client; rc=$?
chk "$(yn "$([ "$rc" = 0 ]; echo $?)")" \
    "EN-2" "with the engagement fact gone the same --remove succeeds (rc=$rc): a gate, not a wedge"
rm -rf "$REN"
...
# --- RB-16 (U245): the SERVER runbook names no program the contract does not declare ---
# O17 of that document enumerated the problem and asked for exactly this bar. Only COMMAND
# TEXT is scanned (a backticked span, or an sh-fenced line with its [SERVER] label stripped)
# and only sections 0-8 -- sections 9 and 11 are prose ABOUT the p5-fw-deadman -> p5-deadman
# rename, where naming the dead tool is the point. Lines carrying an md5 are the historical
# measurement rows, which name the repo FILE deploy/server/p5-fw-deadman, not a command.
# The declared set is derived from p5/contract/paths (what the box carries) plus p5/bin (what
# the operator runs FROM the package, p5-install among them), so a rename in the tree reddens
# this bar instead of quietly making the document wrong. A line carrying `rb16-exempt` is
# prose that names a dead tool on purpose -- that is the record, and O17 said renaming it
# would erase the problem.
RB16_DOC="$_rb_repo/docs/deploy-p5-server.md"
if [ -f "$RB16_DOC" ] && [ -f "$_rb_repo/p5/contract/paths" ]; then
    rb16_known=$({ awk -F'|' '/^file\|/ { gsub(/ /,"",$3); n=split($3,a,"/"); print a[n] }' \
                        "$_rb_repo/p5/contract/paths"
                   ls "$_rb_repo/p5/bin" 2>/dev/null
                 } | grep '^p5' | LC_ALL=C sort -u)
    rb16_cut=$(awk '/^## 9/ { print NR; exit }' "$RB16_DOC")
    [ -n "$rb16_cut" ] || rb16_cut=999999
    rb16_out=$(awk -v cut="$rb16_cut" '
        /^```/ { if (inf) { inf=0; nsh=0; next } inf=1; nsh=(substr($0,4) ~ /^sh[ \t\r]*$/); next }
        NR >= cut { exit }
        /md5|rb16-exempt/ { next }
        {
            if (inf) { if (nsh) { s=$0; sub(/^\[[A-Z]*\][ \t]*/,"",s); print NR "|" s } next }
            s=$0; out=""
            while (match(s, "`[^`]*`")) { out = out " " substr(s, RSTART+1, RLENGTH-2); s = substr(s, RSTART+RLENGTH) }
            if (out != "") print NR "|" out
        }' "$RB16_DOC" | awk -F'|' '
        { l=$1; t=" " substr($0, index($0,"|")+1)
          while (match(t, /[ ;(]p5-[a-z0-9-]+/)) {
              tok=substr(t,RSTART,RLENGTH); t=substr(t,RSTART+RLENGTH)
              sub(/^[ ;(]/,"",tok)
              # a token followed by "." is a FILENAME (p5-execution-handover.md), not a
              # command the operator types -- the runbook cites documents by name too
              if (substr(t,1,1) != ".") print l "|" tok
          } }' | LC_ALL=C sort -u | while IFS='|' read -r rb16_l rb16_t; do
            printf '%s\n' "$rb16_known" | grep -qxF -- "$rb16_t" \
                || echo "  deploy-p5-server.md:$rb16_l tells the operator to type '$rb16_t', and no contract/paths row declares that program"
        done)
    rb16_n=$(printf '%s' "$rb16_out" | grep -c . )
    [ -n "$rb16_out" ] && printf '%s\n' "$rb16_out"
    chk "$(yn "$([ "$rb16_n" = 0 ]; echo $?)")" \
        "RB-16" "every p5-* program the SERVER runbook tells the operator to type in sections 0-8 is declared in p5/contract/paths ($rb16_n findings). U38a deleted p5-fw-deadman and the runbook kept arming the rollback with it -- on a console-less box that is the step whose failure has no recovery"
else
    skipbar "RB-16" "docs/deploy-p5-server.md or p5/contract/paths is not in this tree"
fi
...
# PK-5 -- THE BUILDER'S SOURCE-SHA RESOLUTION, RUN OUT OF THE SHIPPED FILE.
#
# The other half of U292 lives in scripts/build-p5-package.sh, and neither mode
# of this battery builds a package: real mode is handed one, synthetic mode
# fabricates a fixture, and building one here would need the Go toolchain and a
# cross-compile to assert one field. A grep over the builder would be the usual
# substitute and it would prove nothing about what the code DOES.
#
# So the resolution block is DELIMITED in the builder and EXECUTED from there --
# the shipped bytes, not a copy and not a description of them. Reverting the
# builder deletes the block, the extraction comes back empty, and this bar goes
# red for the right reason.
#
# THE FIXTURE REPOS ARE BUILT HERE, and that is the U292 fix round's correction.
# The first shape of this bar ran the block against the AMBIENT checkout and
# asserted it resolved, which turned a property of the ENVIRONMENT into a bar:
# on the mirror runner actions/checkout@v4 fetches one branch at depth 1, so
# neither integration ref exists, and PK-5 went red at 5339aca for the one
# reason that is not a defect in the product. Three throwaway repos made with
# git here make all three cases reachable in EVERY checkout, and they pin the
# PRECEDENCE claim, which the ambient version could not test at all: it only
# ever exercised whichever of the two refs the host happened to have.
_src_blk="$TMPBASE/srcblk.sh"
_src_body=$(sed -n '/^# --- BEGIN P5_SRC (/,/^# --- END P5_SRC /p' "$_pk_repo/scripts/build-p5-package.sh")
{
    echo 'REPO=$1'
    echo 'die() { echo "build-p5-package: FATAL: $*" >&2; exit 1; }'
    echo 'log() { printf "build-p5-package: %s\n" "$*" >&2; }'
    printf '%s\n' "$_src_body"
    echo 'printf "%s %s\n" "${SRC_REF:-}" "${SRC_SHA:-}"'
} > "$_src_blk"
# HEAD is pointed at a named branch that is NOT dev before the commit, so the
# fixtures do not depend on the host's init.defaultBranch: a machine configured
# with `init.defaultBranch = dev` would otherwise hand MU-PK5 the very ref it
# exists to prove is absent.
_pk5_mkrepo() {     # _pk5_mkrepo DIR BRANCH MSG -> prints the commit sha
    rm -rf "$1"; mkdir -p "$1"
    git init -q "$1" >/dev/null 2>&1 || return 1
    git -C "$1" symbolic-ref HEAD "refs/heads/$2" >/dev/null 2>&1 || return 1
    git -C "$1" -c user.email=e0@invalid -c user.name=e0 -c commit.gpgsign=false \
        commit -q --allow-empty -m "$3" >/dev/null 2>&1 || return 1
    git -C "$1" rev-parse HEAD
}
if command -v git >/dev/null 2>&1; then
    # ARM 1 -- refs/heads/dev only, which is the shape ci-wsl.sh's bare base has.
    _h_repo="$TMPBASE/pk5-heads"
    _h_sha=$(_pk5_mkrepo "$_h_repo" pk5-base "heads arm")
    git -C "$_h_repo" branch -f dev HEAD >/dev/null 2>&1
    h_out=$(sh "$_src_blk" "$_h_repo" 2>"$TMPBASE/pk5h.err"); h_rc=$?
    h_ref=${h_out%% *}; h_sha=${h_out##* }
    h_want=$(git -C "$_h_repo" rev-parse 'refs/heads/dev^{commit}' 2>/dev/null)
    # ARM 2 -- BOTH refs present and DISAGREEING, which is the only fixture that
    # can tell "origin/dev first" from "whichever one it found". The clone gives
    # refs/remotes/origin/dev at $_h_sha; refs/heads/dev is then forced to a
    # SECOND, different commit. Naming the local ref is now a visible wrong
    # answer instead of an indistinguishable right one.
    _r_repo="$TMPBASE/pk5-remote"
    rm -rf "$_r_repo"
    git clone -q "$_h_repo" "$_r_repo" >/dev/null 2>&1
    git -C "$_r_repo" -c user.email=e0@invalid -c user.name=e0 -c commit.gpgsign=false \
        commit -q --allow-empty -m "local dev moved on" >/dev/null 2>&1
    git -C "$_r_repo" branch -f dev HEAD >/dev/null 2>&1
    r_out=$(sh "$_src_blk" "$_r_repo" 2>"$TMPBASE/pk5r.err"); r_rc=$?
    r_ref=${r_out%% *}; r_sha=${r_out##* }
    r_want=$(git -C "$_r_repo" rev-parse 'refs/remotes/origin/dev^{commit}' 2>/dev/null)
    r_local=$(git -C "$_r_repo" rev-parse 'refs/heads/dev^{commit}' 2>/dev/null)
    a=0
    [ -n "$_src_body" ] || { a=1; echo "  scripts/build-p5-package.sh carries no '# --- BEGIN P5_SRC (' block, so nothing in the builder resolves an integration sha and PROVENANCE can name none"; }
    [ "$h_rc" = 0 ] || { a=1; echo "  heads arm: the block exited $h_rc:"; sed 's/^/    /' "$TMPBASE/pk5h.err"; }
    [ "$h_ref" = refs/heads/dev ] || { a=1; echo "  heads arm: it named ref '$h_ref', not refs/heads/dev, in a repo that has only that one"; }
    { [ -n "$h_want" ] && [ "$h_sha" = "$h_want" ]; } || { a=1; echo "  heads arm: it produced '$h_sha' but git reads refs/heads/dev as '$h_want' there"; }
    [ "$r_rc" = 0 ] || { a=1; echo "  remote arm: the block exited $r_rc:"; sed 's/^/    /' "$TMPBASE/pk5r.err"; }
    [ "$r_ref" = refs/remotes/origin/dev ] || { a=1; echo "  remote arm: with both refs present and disagreeing it named '$r_ref', so the documented precedence does not hold"; }
    { [ -n "$r_want" ] && [ "$r_sha" = "$r_want" ]; } || { a=1; echo "  remote arm: it produced '$r_sha' but git reads refs/remotes/origin/dev as '$r_want' there"; }
    { [ -n "$r_local" ] && [ "$r_sha" != "$r_local" ]; } || { a=1; echo "  remote arm: the fixture did not actually disagree (origin/dev '$r_want' vs refs/heads/dev '$r_local'), so precedence was not tested"; }
    echo "$h_sha$r_sha" | grep -qE '^[0-9a-f]{80}$' || { a=1; echo "  one of the two arms did not produce a 40-character object id: '$h_sha' '$r_sha'"; }
    chk "$(yn "$([ "$a" = 0 ]; echo $?)")" \
        "PK-5" "the builder's OWN source-sha block, executed out of scripts/build-p5-package.sh, resolves refs/heads/dev to $h_sha where that is the only integration ref, and prefers refs/remotes/origin/dev ($r_sha) over a refs/heads/dev that disagrees ($r_local) -- each sha re-read independently with git in its own fixture. That is the value PROVENANCE carries and the box compares against"

    # MU-PK5 -- the one direction that would be silently corrosive: a checkout
    # with no integration ref must record NOTHING. Defaulting to HEAD writes a
    # field that equals P5_GIT_COMMIT by construction and makes every box-side
    # comparison pass for ever.
    #
    # THE FIXTURE HAS A COMMIT, and that is the point. The first shape of this
    # bar used an EMPTY `git init`, where HEAD does not resolve either -- so a
    # seeded HEAD fallback would have produced an empty sha there too and the
    # mutation this bar names could not have been detected by it. With a commit
    # present, HEAD has a value, and asserting the block did not emit it is a
    # real discrimination.
    #
    # The block must NOT abort: the runner's own checkout is exactly this shape
    # (one branch, depth 1, no dev ref), and a `die` here took e0,
    # p5-package-battery and p5-skeleton down together at 5339aca. Absent is a
    # state the product has -- p5-install takes the pair as optional and
    # p5-version prints SRCSHA ABSENT (VS-SRC-4) -- so the builder must be able
    # to produce it.
    _mu_repo="$TMPBASE/pk5-nodev"
    mu_head=$(_pk5_mkrepo "$_mu_repo" nodevref "no integration ref here")
    mu5_out=$(sh "$_src_blk" "$_mu_repo" 2>"$TMPBASE/pk5m.err"); mu5_rc=$?
    mu5_err=$(cat "$TMPBASE/pk5m.err")
    mu5_ref=${mu5_out%% *}; mu5_sha=${mu5_out##* }
    a=0
    [ "$mu5_rc" = 0 ] || { a=1; echo "  the block exited $mu5_rc instead of continuing; the runner's checkout has no dev ref and a build that aborts there takes the gate with it:"; sed 's/^/    /' "$TMPBASE/pk5m.err"; }
    [ -z "$mu5_ref" ] || { a=1; echo "  it named ref '$mu5_ref' in a repo that has no integration ref at all"; }
    [ -z "$mu5_sha" ] || { a=1; echo "  it produced sha '$mu5_sha' with no integration ref reachable"; }
    { [ -n "$mu_head" ] && [ "$mu5_sha" != "$mu_head" ]; } || { a=1; echo "  it fell back to HEAD ($mu_head) -- that sha equals P5_GIT_COMMIT by construction and makes the box-side comparison unfailable"; }
    echo "$mu5_err" | grep -q 'refs/remotes/origin/dev' || { a=1; echo "  the report did not NAME refs/remotes/origin/dev: $mu5_err"; }
    echo "$mu5_err" | grep -q 'refs/heads/dev' || { a=1; echo "  the report did not NAME refs/heads/dev: $mu5_err"; }
    chk "$(yn "$([ "$a" = 0 ]; echo $?)")" \
        "MU-PK5" "MUTATION: the same block against a checkout whose HEAD resolves ($mu_head) but which has NO dev ref records NOTHING (ref='$mu5_ref' sha='$mu5_sha'), exits $mu5_rc so the build continues, and names both refs it tried. The seeded defect is the HEAD fallback, and it would be caught: $mu_head is what a fallback would have written"

    # PK-5O -- THE OMISSION, run out of the shipped file the same way. Whether
    # the two PROVENANCE lines are WRITTEN is a separate decision from whether
    # the sha resolved, and getting it wrong is not cosmetic: `P5_SRC_SHA=` with
    # no value is PRESENT-BUT-MALFORMED to p5-install's prov_opt and refuses the
    # install as an integrity failure. An unguarded emitter would therefore
    # brick every package built where MU-PK5's fixture shape holds -- which is
    # every gate run. Both directions, because a guard that never emits is as
    # wrong as one that always does.
    _emit_blk="$TMPBASE/emitblk.sh"
    _emit_body=$(sed -n '/^ *# --- BEGIN P5_SRC_EMIT/,/^ *# --- END P5_SRC_EMIT/p' "$_pk_repo/scripts/build-p5-package.sh")
    printf '%s\n' "$_emit_body" > "$_emit_blk"
    o_absent=$(SRC_REF= SRC_SHA= sh "$_emit_blk" 2>&1)
    o_fake=0123456789abcdef0123456789abcdef01234567
    o_present=$(SRC_REF=refs/heads/dev SRC_SHA=$o_fake sh "$_emit_blk" 2>&1)
    a=0
    [ -n "$_emit_body" ] || { a=1; echo "  scripts/build-p5-package.sh carries no '# --- BEGIN P5_SRC_EMIT' block, so the guard this bar measures is not in the shipped file"; }
    [ -z "$o_absent" ] || { a=1; echo "  with SRC_REF/SRC_SHA empty it still wrote PROVENANCE lines, which p5-install reads as malformed and refuses: $o_absent"; }
    [ "$o_present" = "P5_SRC_REF=refs/heads/dev
P5_SRC_SHA=$o_fake" ] || { a=1; echo "  with the pair resolved it wrote '$o_present', not the two expected key=value lines"; }
    chk "$(yn "$([ "$a" = 0 ]; echo $?)")" \
        "PK-5O" "the builder's OWN PROVENANCE emitter, executed out of scripts/build-p5-package.sh, writes both P5_SRC_* lines when the pair resolved and writes NOTHING when it did not -- not an empty value, which prov_opt reads as malformed and which would refuse every install built from a checkout with no dev ref"
    rm -rf "$_h_repo" "$_r_repo" "$_mu_repo"
else
    bad "PK-5" "git is not on PATH, so the builder's source-sha resolution cannot be executed against a fixture checkout. This bar FAILS rather than skipping, for the same reason P-2 does: the resolution is the half of U292 that runs on the build host"
    bad "MU-PK5" "not run -- PK-5 could not reach git (see above)"
    bad "PK-5O" "not run -- PK-5 could not reach git (see above)"
fi

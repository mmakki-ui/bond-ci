...
# line removed. UNTIL U291 that mutant ran to COMPLETION -- exit 0, a full
# stamp, a complete-looking record -- on a box carrying no secret at all,
# because the contract set-equality (IN-14/IN-15) compares DECLARED
# destinations, not what landed, so nothing else in this battery noticed.
#
# U291 INVERTED THIS BAR, and the inversion is the measurement. p5-install now
# RE-READS the placed key before it stamps anything and refuses when it is
# absent or malformed, so the mutant must DIE there: non-zero, no stamp, and a
# message that names the secret. A mutant that still reaches the stamp means
# that refusal has been removed. The old expectation is not weakened by this,
# it is subsumed -- "the install finishes with no key" was the defect, and the
# bar that used to record it now forbids it.
...
[ "$murc" = 0 ] && { mu=1; echo "  the mutant exited 0: the post-placement re-read of the secret is not refusing"; }
[ -f "$RKM/usr/lib/p5/stamp" ] && { mu=1; echo "  the mutant wrote a stamp: it got past the re-read that must refuse first"; }
grep -q 'transport secret' "$TMPBASE/mkerr" 2>/dev/null \
    || { mu=1; echo "  the mutant's refusal never named the transport secret, so an operator could not act on it"; }
chk "$(yn "$([ "$mu" = 0 ]; echo $?)")" \
    "MU-KEY-1" "MUTATION: with the placement step deleted the installer REFUSES (rc=$murc, non-zero), writes NO stamp, places NO key and names the transport secret -- an install can no longer finish with authentication off, and the step KEY-1 measures is one whose removal is loud"
...
# ===========================================================================
# THE KEY ID IS A CHECK, AND THE GATE IS ASSERTED AFTER ENGAGE (U291)
# ===========================================================================
# WHAT WAS WRONG. p5-install placed a secret, exited 0, and then PRINTED
# "compare P5_TRANSPORT_KEY_ID in the two stamps ... two different secrets is
# not an open wire -- it is a dead one". Right diagnosis, nobody's job: the
# rung exited 0 and the operator advanced. And the failure is not the loud
# kind. p4-bondagg/server/auth.go:220 sets ReopenDefault = 30s, and the authGate
# rule at :307-330 REOPENS the gate after that long with no valid tag -- on
# purpose, because the server has no console and a rule that can lock the
# legitimate client out loses the box. So a wrong key does not drop the tunnel;
# it serves it UNAUTHENTICATED. Fail-open at the wire, fail-open at the
# installer, one paragraph in between.
#
# TWO MECHANISMS, because they answer different halves:
#   KEY-12..KEY-16  the INSTALLER refuses. A declared --peer-key-id that does
#                   not match the key it is about to place is a non-zero exit
#                   before the box is touched; so is a declaration with nothing
#                   to adopt; and the placed key is re-read after placement.
#   KEY-17..KEY-27  the PREFLIGHTS refuse, but ONLY UNDER --post-engage. After
#                   engage the last SSTAT/PSTAT line must say gate=1 (CLOSED).
#                   This is the half that catches an operator who declared
#                   nothing at all, which is the case no installer flag reaches.
# The 30s reopen is NOT changed and no bar here asserts anything about it.
#
# WHY THE MODE IS THE DIVIDER (U291 fix round). gate=1 means the gate is CLOSED,
# and it closes only on a VALID tag inside the 30s horizon -- so a box that has
# not yet been handed a frame reads gate=0, correctly. p4-bondagg/server/main.go:57-63
# sets StatHeartbeat = 60s, so an IDLE server emits a fresh gate=0 SSTAT line
# every minute: the first cut of this block, which refused on a present gate!=1
# line in BOTH modes, therefore refused every server rung run before the client
# engages and every client rung run while the client is off -- and diagnosed it
# as "two different transport secrets", which is false. Pre-engage the verdict
# is now a REPORT and rc=0; --post-engage it REFUSES, which is the rung the
# detector was built for. KEY-24/KEY-25 hold that pre-engage floor.
#
# WHY authbad>0 IS NEVER A REFUSAL. `g.authBad++` (p4-bondagg/server/auth.go:517,
# p4-bondagg/daemon/auth.go:370) is cumulative for the life of the process and
# never reset, so one noise frame on a public port sets it for good; and while
# gate=1 a valid tag arrived inside 30s, which is evidence the two boxes DO
# share a secret. KEY-19/KEY-26/KEY-27 assert it reports at rc=0 in both modes.
#
# THE FIXTURES ARE ASSERTED, not assumed: KEY-13's wrong id is proven to differ
# from the real one before it is used, and KEY-20/KEY-21 drive the SAME empty
# log to opposite verdicts, while KEY-18/KEY-24 and KEY-23/KEY-25 drive the SAME
# gate=0 log to opposite verdicts on the MODE alone -- so a run that passes has
# exercised the refusing path rather than merely failing to reach it.

# Box A generates; box B adopts A's key. One package serves both -- the roots
# differ, which is what makes them two boxes.
PKA=$TMPBASE/pkgkidA; mkpkg "$PKA" </dev/null
RKA=$TMPBASE/rkidA
inst "$RKA" "$PKA" client; kid_arc=$?
KIDA=$(sed -n 's/^P5_TRANSPORT_KEY_ID=//p' "$RKA/usr/lib/p5/stamp" 2>/dev/null | head -1)
KEYA="$RKA/etc/p5/transport.key"

# KEY-12: the MATCHING declaration installs, and the two boxes' ids agree.
RKB=$TMPBASE/rkidB
inst "$RKB" "$PKA" client --transport-key "$KEYA" --peer-key-id "$KIDA"; k12rc=$?
k12=0
[ "$kid_arc" = 0 ] || { echo "  box A's install did not succeed (rc=$kid_arc), so there is nothing to adopt"; k12=1; }
[ -n "$KIDA" ] || { echo "  box A's stamp carries no P5_TRANSPORT_KEY_ID to declare"; k12=1; }
[ "$k12rc" = 0 ] || { echo "  a MATCHING --peer-key-id was REFUSED (rc=$k12rc):"; tail -5 "$TMPBASE/err" | sed 's/^/    /'; k12=1; }
KIDB=$(sed -n 's/^P5_TRANSPORT_KEY_ID=//p' "$RKB/usr/lib/p5/stamp" 2>/dev/null | head -1)
{ [ -n "$KIDB" ] && [ "$KIDB" = "$KIDA" ]; } || { echo "  the two boxes' key ids differ after an adopt: A='$KIDA' B='$KIDB'"; k12=1; }
grep -qx "P5_TRANSPORT_KEY_PEER=$KIDA" "$RKB/usr/lib/p5/stamp" 2>/dev/null \
    || { echo "  box B's stamp does not record the peer id the install was checked against"; k12=1; }
chk "$(yn "$([ "$k12" = 0 ]; echo $?)")" \
    "KEY-12" "a second box adopting the first one's secret with a MATCHING --peer-key-id installs (rc=$k12rc), both stamps carry the same P5_TRANSPORT_KEY_ID ($KIDA), and the second stamp records which peer id it was checked against"

# KEY-13: THE MISMATCH. This is the defect, and the refusal has to land BEFORE
# the box is touched -- an install that refuses after placing files leaves a
# half-built box on the rung that gates the point of no return.
RKC=$TMPBASE/rkidC
BADID=0123456789ab
[ "$BADID" = "$KIDA" ] && BADID=ba9876543210
inst "$RKC" "$PKA" client --transport-key "$KEYA" --peer-key-id "$BADID"; k13rc=$?
k13=0
[ -n "$KIDA" ] && [ "$BADID" != "$KIDA" ] || { echo "  the fixture is not a mismatch: real='$KIDA' seeded='$BADID'"; k13=1; }
[ "$k13rc" = 0 ] && { echo "  a MISMATCHED --peer-key-id was ACCEPTED -- the dead wire installs green"; k13=1; }
[ -e "$RKC/usr/lib/p5/stamp" ] && { echo "  the refusal came too late: a stamp was written"; k13=1; }
[ -e "$RKC/etc/p5/transport.key" ] && { echo "  the refusal came too late: a secret was placed"; k13=1; }
grep -q "$BADID" "$TMPBASE/err" || { echo "  the refusal never named the id it was given"; k13=1; }
grep -q "$KIDA" "$TMPBASE/err" || { echo "  the refusal never named the id the key actually has"; k13=1; }
chk "$(yn "$([ "$k13" = 0 ]; echo $?)")" \
    "KEY-13" "a MISMATCHED --peer-key-id FAILS the install rung (rc=$k13rc, non-zero), names both ids, and leaves NO stamp and NO secret on the box -- the comparison p5-install used to print as prose now refuses before the box is touched"

# KEY-14: declared, but nothing to adopt. A generated secret cannot equal the
# peer's, so accepting this would be a check that can never fail.
RKD=$TMPBASE/rkidD
inst "$RKD" "$PKA" client --peer-key-id "$KIDA"; k14rc=$?
k14=0
[ "$k14rc" = 0 ] && { echo "  --peer-key-id with no --transport-key was accepted: the install would GENERATE a fresh secret and call it checked"; k14=1; }
[ -e "$RKD/usr/lib/p5/stamp" ] && { echo "  a stamp was written despite the refusal"; k14=1; }
chk "$(yn "$([ "$k14" = 0 ]; echo $?)")" \
    "KEY-14" "--peer-key-id with no --transport-key is REFUSED (rc=$k14rc): a secret generated on this box cannot equal the peer's, so admitting it would make the comparison unfailable"

# KEY-15: a malformed id is a usage error, not a mismatch. The distinction
# matters on the rung: 5 tells the operator the boxes disagree, 2 tells them
# they mistyped, and conflating the two sends them to the wrong box.
RKE=$TMPBASE/rkidE
inst "$RKE" "$PKA" client --transport-key "$KEYA" --peer-key-id "not-hex-at-all"; k15rc=$?
k15=0
[ "$k15rc" = 2 ] || { echo "  a malformed --peer-key-id gave rc=$k15rc, want 2 (usage)"; k15=1; }
[ -e "$RKE/usr/lib/p5/stamp" ] && { echo "  a stamp was written despite the refusal"; k15=1; }
chk "$(yn "$([ "$k15" = 0 ]; echo $?)")" \
    "KEY-15" "a malformed --peer-key-id exits 2 (usage), not 5 (rc=$k15rc), and writes nothing: 'you mistyped' and 'the two boxes disagree' are different instructions to the operator"

# KEY-16: THE HONEST DEFAULT. No declaration is still a legal install -- the
# first box has no peer to declare -- but the box must RECORD that nothing was
# compared, and the run must say so on the way out. An install that looks
# identical whether or not the check was made is the state this unit closes.
RKF=$TMPBASE/rkidF
inst "$RKF" "$PKA" client; k16rc=$?
k16=0
[ "$k16rc" = 0 ] || { echo "  an undeclared install was refused (rc=$k16rc) -- the first box has no peer to declare"; k16=1; }
grep -qx 'P5_TRANSPORT_KEY_PEER=undeclared' "$RKF/usr/lib/p5/stamp" 2>/dev/null \
    || { echo "  the stamp does not record that no peer comparison was made"; k16=1; }
grep -q 'NOT CHECKED' "$TMPBASE/err" || { echo "  the installer did not say, on the way out, that nothing compared the two secrets"; k16=1; }
grep -q -- '--post-engage' "$TMPBASE/err" || { echo "  the installer did not name the check that closes it after engage"; k16=1; }
chk "$(yn "$([ "$k16" = 0 ]; echo $?)")" \
    "KEY-16" "an install with NO --peer-key-id still succeeds (rc=$k16rc) -- the first box has no peer -- but stamps P5_TRANSPORT_KEY_PEER=undeclared and says on stderr that nothing compared the two secrets, naming --post-engage as the check that does"

# --- the post-engage gate bars ---------------------------------------------
# pgbar NAME SCRIPT ROOT STATLOG POSTENGAGE WANTRC MUSTSAY DESC
# The same shape as pfbar above, with the two knobs this block reads. The
# preflight is driven as a PROGRAM against a fixture root, never sourced: a bar
# that re-implements the predicate stops testing the shipped one.
pgbar() {
    _pg_n="$1"; _pg_s="$2"; _pg_r="$3"; _pg_l="$4"; _pg_e="$5"; _pg_w="$6"; _pg_m="$7"; _pg_d="$8"
    if [ ! -f "$_pg_s" ]; then
        bad "$_pg_n" "the preflight script is not in this tree: $_pg_s (this bar FAILS rather than skipping)"
        return 0
    fi
    P5_ROOT="$_pg_r" P5_STAT_LOG="$_pg_l" P5_POST_ENGAGE="$_pg_e" sh "$_pg_s" >"$TMPBASE/pg.out" 2>&1; _pg_rc=$?
    _pg_bad=0
    if [ "$_pg_w" = 0 ]; then
        [ "$_pg_rc" = 0 ] || { echo "  expected exit 0, got $_pg_rc:"; grep -i 'REFUSE' "$TMPBASE/pg.out" | sed 's/^/    /'; _pg_bad=1; }
    else
        [ "$_pg_rc" = 0 ] && { echo "  expected a NON-ZERO exit, got 0 -- the preflight passed a box whose bonded wire is unauthenticated"; _pg_bad=1; }
    fi
    grep -q "$_pg_m" "$TMPBASE/pg.out" || { echo "  the output never said '$_pg_m':"; tail -20 "$TMPBASE/pg.out" | sed 's/^/    /'; _pg_bad=1; }
    chk "$(yn "$([ "$_pg_bad" = 0 ]; echo $?)")" "$_pg_n" "$_pg_d (rc=$_pg_rc)"
}

# The fixture root carries a WELL-FORMED, owner-only secret, so the block above
# these passes and the only thing under test is the gate verdict. The log lines
# are the real format, copied from the two Fprintf calls
# (p4-bondagg/server/main.go:540, p4-bondagg/daemon/pullrun.go:676) with the
# tail that matters left exact -- authbad= and gate= are the two fields read.
AGR=$TMPBASE/agroot; mkkey "$AGR" 600
AGL=$TMPBASE/aglog; mkdir -p "$AGL"
_ag_head='daemon.notice p5-server[1]: SSTAT del=41 towg=41 dn=0 dnnoroute=0 echo=7 echoshed=0 echonoep=0 echotrunc=0 bad=0 ign=0 wgerr=0'
printf '%s authok=418 authbad=0 authshed=2 sealshort=0 gate=1\n' "$_ag_head" > "$AGL/s-closed"
printf '%s authok=418 authbad=0 authshed=2 sealshort=0 gate=0\n' "$_ag_head" > "$AGL/s-open"
printf '%s authok=418 authbad=7 authshed=2 sealshort=0 gate=1\n' "$_ag_head" > "$AGL/s-authbad"
: > "$AGL/s-empty"
_ag_phead='daemon.notice p5-datapath[1]: PSTAT n=2 sched=eco depth=0 peak=3 qb=0/64 peakb=9 enq=41 drawn=41 stale=0 qdrop=0 retq=0 hold=0ms del=41 rxshed=0'
printf '%s authok=418 authbad=0 authshed=2 sealshort=0 gate=1\n' "$_ag_phead" > "$AGL/c-closed"
printf '%s authok=418 authbad=0 authshed=2 sealshort=0 gate=0\n' "$_ag_phead" > "$AGL/c-open"
printf '%s authok=418 authbad=7 authshed=2 sealshort=0 gate=1\n' "$_ag_phead" > "$AGL/c-authbad"

pgbar KEY-17 "$PRE_S" "$AGR" "$AGL/s-closed"  0 0 'transport-auth-gate: ok' \
    "the SERVER preflight passes when the last SSTAT says gate=1 (CLOSED) and authbad=0"
pgbar KEY-18 "$PRE_S" "$AGR" "$AGL/s-open"    1 1 'REFUSE: the auth gate is OPEN' \
    "UNDER --post-engage the SERVER preflight REFUSES when the gate has REOPENED (gate=0) -- the state a wrong transport key degrades to after 30s instead of announcing itself"
pgbar KEY-19 "$PRE_S" "$AGR" "$AGL/s-authbad" 0 0 'REPORT: authbad=' \
    "the SERVER preflight REPORTS authbad>0 and exits 0: the counter is cumulative for the life of the process (p4-bondagg/server/auth.go:517) so one noise frame on a public port sets it for good, and gate=1 says a VALID tag arrived inside 30s -- so it cannot carry the 'two different secrets' diagnosis the first cut printed"
pgbar KEY-20 "$PRE_S" "$AGR" "$AGL/s-empty"   0 0 'NOT EVALUATED' \
    "with NO SSTAT line and no --post-engage the SERVER preflight reports rather than refusing -- before engage there is nothing to judge, and a bar that refused here would be unrunnable at S2"
pgbar KEY-21 "$PRE_S" "$AGR" "$AGL/s-empty"   1 1 'REFUSE' \
    "the SAME empty log under --post-engage REFUSES: after the switch a running daemon has reported an SSTAT line, so silence is an answer and not an excuse. KEY-20 and this bar are one fixture driven to opposite verdicts, which is what makes either of them a measurement"
pgbar KEY-22 "$PRE_C" "$AGR" "$AGL/c-closed"  0 0 'transport-auth-gate: ok' \
    "the CLIENT preflight passes when the last PSTAT says gate=1 (CLOSED) and authbad=0"
pgbar KEY-23 "$PRE_C" "$AGR" "$AGL/c-open"    1 1 'REFUSE: the auth gate is OPEN' \
    "UNDER --post-engage the CLIENT preflight REFUSES on gate=0: both ends count the same thing and the client is the box an operator can still reach, so it must not report a green preflight over an unauthenticated wire"

# The pre-engage floor (U291 fix round). These four are the bars that would have
# caught the first cut: it refused on a present gate!=1 line in BOTH modes, and
# an idle server heartbeats exactly that line once a minute (StatHeartbeat = 60s,
# p4-bondagg/server/main.go:57-63), so every rung before engage refused.
# KEY-24/KEY-25 drive the SAME fixtures as KEY-18/KEY-23 with the mode as the
# only difference, so the divider itself is what is measured.
pgbar KEY-24 "$PRE_S" "$AGR" "$AGL/s-open"    0 0 'REPORT: the auth gate is OPEN' \
    "WITHOUT --post-engage the SAME gate=0 SSTAT that KEY-18 refuses is a REPORT at rc=0: the gate closes only on a valid tag inside 30s, so a server that has not yet been handed a frame has never closed it, and refusing here would refuse every server rung at S0..S2"
pgbar KEY-25 "$PRE_C" "$AGR" "$AGL/c-open"    0 0 'REPORT: the auth gate is OPEN' \
    "WITHOUT --post-engage the SAME gate=0 PSTAT that KEY-23 refuses is a REPORT at rc=0: the client reads gate=0 whenever it is off or has not yet engaged, which is every rung the runbook runs this preflight at before the switch"
pgbar KEY-26 "$PRE_S" "$AGR" "$AGL/s-authbad" 1 0 'REPORT: authbad=' \
    "authbad>0 with gate=1 is a REPORT at rc=0 EVEN UNDER --post-engage on the server: the gate being shut is itself evidence a valid tag arrived inside 30s, so the cumulative bad count cannot mean the two boxes carry different secrets"
pgbar KEY-27 "$PRE_C" "$AGR" "$AGL/c-authbad" 1 0 'REPORT: authbad=' \
    "the same on the CLIENT, whose counter is the same cumulative field (p4-bondagg/daemon/auth.go:370): authbad>0 under --post-engage reports and exits 0"

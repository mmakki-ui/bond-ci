...
# CT-1/CT-2/CT-3 (U249/U231): the boot limb lives in a file P5 does not own, so no declared
# path can see it. Before this, an uninstalled box reported CLEAN while cron ran `check` on a
# deleted binary every minute.
RCT="$TMPBASE/cron"; inst "$RCT" "$P9" client
mkdir -p "$RCT/etc/crontabs"
printf '*/5 * * * * /usr/bin/somebody-elses-job\n' > "$RCT/etc/crontabs/root"
# CT-4's line: it carries the MARK and nothing else. p5-deadman:179-186 says in so many
# words that CRONMARK must never be matched as a bare unanchored substring, because an
# operator line that merely mentions it is not P5's -- so removal has to require BOTH the
# mark and the installed binary path, exactly as p5_cron_hits reports.
printf '*/7 * * * * /root/operator-note.sh # p5-deadman reminder\n' >> "$RCT/etc/crontabs/root"
printf '* * * * * "/usr/sbin/p5-deadman" check >/dev/null 2>&1 # p5-deadman\n' >> "$RCT/etc/crontabs/root"
P5_ROOT="$RCT" sh "$BIN/p5-uninstall" --check --role client > "$TMPBASE/ct.check" 2>&1
ct_seen=0; grep -q 'deadman boot limb' "$TMPBASE/ct.check" && ct_seen=1
chk "$(yn "$([ "$ct_seen" = 1 ]; echo $?)")" \
    "CT-1" "--check reports P5's deadman crontab line, which is not a declared path and which no other probe can see (seen=$ct_seen)"
unin "$RCT" --remove --role client >/dev/null 2>&1; ct_rc=$?
# `grep -c ... || echo 0` is WRONG here and CT-2 was red on the mirror because of it, on a
# tree where the removal had WORKED: a file that EXISTS with no match makes grep print "0"
# AND exit 1, so the `|| echo 0` fired too and the count read "0\n0", which is not "0". The
# fallback is only for the file being ABSENT (exit 2, no output at all), so take grep's own
# count whenever it produced one and default only on empty.
ct_left=$(grep -c '/usr/sbin/p5-deadman' "$RCT/etc/crontabs/root" 2>/dev/null)
[ -n "$ct_left" ] || ct_left=0
ct_theirs=$(grep -c 'somebody-elses-job' "$RCT/etc/crontabs/root" 2>/dev/null)
[ -n "$ct_theirs" ] || ct_theirs=0
ct_marked=$(grep -c 'operator-note' "$RCT/etc/crontabs/root" 2>/dev/null)
[ -n "$ct_marked" ] || ct_marked=0
rm -rf "$RCT"
chk "$(yn "$([ "$ct_rc" = 0 ] && [ "$ct_left" = 0 ]; echo $?)")" \
    "CT-2" "--remove strips the deadman line before unlinking the binary it names (rc=$ct_rc, lines left=$ct_left)"
chk "$(yn "$([ "$ct_theirs" = 1 ]; echo $?)")" \
    "CT-3" "the operator's own crontab line survives the removal ($ct_theirs kept) -- P5 edits one marked line in a file it does not own"
chk "$(yn "$([ "$ct_marked" = 1 ]; echo $?)")" \
    "CT-4" "an operator line that merely MENTIONS the mark survives too ($ct_marked kept): removal requires the mark AND the installed binary path, the same conjunction p5_cron_hits reports on, so it can never take a line --check never claimed"

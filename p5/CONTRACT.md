> **⚠ AMENDED (2026-09-06, U240): §6 'deadman boot limb OPEN/un-fired' is stale (U38a shipped+tested the crontab limb); §7 'management-report unbuilt (U26)' is stale (shipped+tested; E7 is client-only).**
...
2. **The standalone-product requirement forces the `p5` infix.** The deploy
   shape was stated here as "cleanly uninstall the old package, then install P5 fresh"
   (`p5-execution-handover.md:8`) -- SUPERSEDED 2026-09-05 (G2): the client order is switch-off -> install alongside -> quiescence-gated switch-on -> surgical removal (`docs/knowledge/design/p5-deploy-sequence.md` sections 1-3). For E7 to remove precisely the old stack and
   ...
   **ADR-005 (U51) extends the same consequence to the reconciler, `ecod`, the
   watchdog and the DAG table.** `deploy/p5` shipped those under `bond-xctl`,
   `bond-ecod`, `bond-watchdog` and `bond.dag` — also on the old stack's
   removal list (`contract/foreign`) — so E0's own contract used to mark
   `deploy/p5`'s destinations FOREIGN to P5. `contract/paths` now carries the
   seven rows the rename needs (`/usr/sbin/p5-reconciler`, `/usr/sbin/p5-ecod`,
   `/usr/sbin/p5-watchdog`, `/etc/init.d/p5-{ecod,watchdog}`,
   `/usr/lib/p5/dag`, `/var/run/p5/*`); the `/usr/sbin/p5-*`, `/etc/init.d/p5-*`
   and `/usr/lib/p5/*` patterns in the table above already admit them, so the
   namespace rule needed no change. Bar `PATH-ADR5` (landed in `p5/test/run.sh:328-370`
   with its `MU-PATH-ADR5` mutation pair; E0 battery 142/0 at U51's merge) asserts every destination ADR-005 §4
   names resolves to a row here. **The rename in the shipped files themselves
   is U51a — this contract only reserves the destinations.**
...
**`p5-uninstall --check | --list | --quiescent | --switch-off | --remove-old |
--remove | --purge  [--role client|server] [--dry-run]`** — the only thing that
removes P5, and the only thing that removes the stack it replaces. The **P5 half
is E0's and is implemented**: `--check` (derived from `contract/paths`),
`--list`, and `--remove` (with `--dry-run` and `--recover`).

**The old-stack half is TWO verbs, and the split is the deploy order.** Mo's
order is: switch the old stack **off**, install P5 **alongside** it, switch P5
**on**, and only then remove the old stack **surgically**. P5's whole install,
its switch-on and its soak happen *between* the two old-stack steps, and the box
holds the state after the first one for as long as the operator wants — so they
cannot be one command, and each is defined by what it does *not* do:

| verb | what it does | what it must never do |
|---|---|---|
| `--quiescent` | read-only. Is nothing of the old stack running, nothing of it flagged for the next boot, and the tunnel endpoint off its local socket? The **same predicate the reconciler's `engage` guard runs**, so the operator and the machine read the same lines | write anything; execute an old init script to ask whether it is enabled (it reads the rc.d flag — U153) |
| `--switch-off` | runs the old package's **own** restore verbs (`<tool> off`), stops and disables its services, takes their rc.d boot flags, then prints a **rollback block**: one `<tool> on` per restore verb *this run actually executed*, in reverse order, read off the run's own record | **unlink or move any file**. The only paths it removes are rc.d boot flags, and the block it prints puts those back. Every byte of the old stack is still on disk afterwards — that is what P5 is installed alongside |
| `--remove-old` | quarantines what cannot be recreated, unlinks what the derived list names, empties each declared directory **member by member**, reports everything it did not take | **execute anything of the old stack** — no init script, no restore verb — and it refuses (5) unless `--quiescent` already passes, and refuses while a P5 install or removal is unfinished |
| `--purge` | the three of them in order, in one command, for a box being abandoned | be used by the runbook. The split exists *because* of the days between the two edges |

`--switch-off` and `--purge` **refuse (5) against an engaged P5**, before any
action and with zero invocations: `<tool> off` ends in `apply_endpoint
live_direct` (`p2-engarde/bondctl:263-269`), so on a switched-on box it would
re-point a **live P5 tunnel** behind the reconciler's back. Two terms — the
`/etc/p5/rc` intent fact `p5 on` writes, and, as a backstop for a box whose fact
was removed by hand, a peer endpoint on the same loopback host as the old
daemon's declared socket but not that socket. `--remove-old` does **not** carry
that gate and must not: at that point P5 *is* engaged, by design, and the verb
runs nothing of the old stack. Bars `SO-1`..`SO-3`, `RO-1`..`RO-5`, `PIN-ROWS`.
...
The old-stack half is U26/E7's and is implemented too: `--quiescent`,
`--switch-off`, `--remove-old`, `--purge` and `--check --scope old`.
...
| a removal takes a tree it never enumerated | there is no recursive removal anywhere in this product (Mo's standing requirement, in his words: *"uninstalling shouldn't rm so that is not an excuse ... it needs to be selective switch off first install and then remove surgically"*). A directory is emptied **member by member**, each member through the same point-of-use gate the single-file arms use, and then `rmdir`-ed — and `rmdir` on a non-empty directory fails and is reported rather than forced. **What that trade can leave behind, stated rather than discovered:** a member the path check rejects; a symlink, fifo, socket or device; a subdirectory under `/etc/bond`, `/root/cake-autorate` or `/var/run/bond`; a file that appeared after plan time; any unknown file under `/root/cake-autorate`, because a directory inside the operator's own home is not swept blind; the directory that therefore stays standing; the two quarantined files; and the UCI object. **Each one prints `LEFT: <path> (<reason>)`, the verb exits 1, and `--check --scope old` says NOT CLEAN naming it.** A removal that names what survived beats a tree taken without a manifest | `RMRF-0` zero recursive removals over all four files with **no exclusion list** · `NR-2` the argv shim · `RM-9`, `RM-12`..`RM-16` · `MU-RMRF` |
...
working stack removed and its replacement not yet installed. **The old stack must
be QUIESCENT before the switch** (`p5-uninstall --quiescent`, U208, enforced by
the engage row's `old_quiescent` guard) **and is removed after it, surgically**
(`--remove-old`, U209). Cleanliness — `--check --scope both` — is the wrong ask.

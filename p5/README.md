...
- the payload files themselves, copied from the repo, plus the two Go binaries
  ...
  (both boxes are aarch64), which is the packaging decision p5/CONTRACT.md section 7 ("What this contract does not settle", the architecture bullet -- cited by anchor, not line, per the U200 rule)
  assigns to U28.
...
- **U28 / E8** is the builder above. What it still cannot ship, because no
  ...
  (`:160`, conditional on E5), and `portal/*`, which has no
  `contract/paths` row. `deploy/p5/shape-install` and `shape/*` LEFT that list
  with U210 and did not gain a row: they are PACKAGE TOOLS, copied to
  `<pkg>/bin/p5-shape-install` and `<pkg>/shape/` and RUN from the unpacked
  package, exactly as `p5-install` itself is. The controller they check for did
  gain one -- `/etc/init.d/p5-shape`, renamed out of P1's live
  `/etc/init.d/cake-autorate`, which `contract/foreign` declares P1's. `/etc/init.d/p5-server` is NOT in that list any more:
...
- **RESOLVED (U242) — as a DISCLOSURE defect, not a gate defect.** UNLINK and SELFDROP
...
- **CLOSED (U246). Install and removal use DIFFERENT gates, and they disagree in the direction
  `p5_incoming_of` line runs after it on a path the plan never named.
  **U242 tried the obvious fix first and the seeded A/B refused it:** gating the
  sweep on `p5_ns_ok` of the twin turns `HP-4` RED, because that bar REQUIRES the
  sweep of P5’s own staged netifd hook in `/etc/hotplug.d/iface` — a directory P5
  does not exclusively own — since a leftover stage there is not litter, it is a
  live hook nobody enabled. Nothing on disk separates P5’s own killed stage from a
  decoy at that exact name, and `HP-4` settles which way that ambiguity resolves.
  **So the sweep stays and the CLAIM was the defect.** The printed plan now names
  the twin rule it performs, says it applies in those three directories too, and
  says the name is not decoy-distinguishable. `CR-6` holds both halves: the plan
  discloses the sweep, and a `.p5-incoming` name that is not a twin of a planned
  path is left alone even inside `/usr/lib/p5`.
...
- **One ordinary filename in E6's fact directory wedges the box — PARTLY CLOSED (U246), and
  the three names no longer behave the same.** Re-measured on `f9c475f`:
  `wan[0].cfg` is **CLOSED** — `--remove --recover` 0, 0 files left, reinstall 0.
  `wan cfg` and `wan..cfg` are **still open** — 4, 2 files left, unchanged from
  `origin/dev`. Control `wan.cfg` gives 0 and a clean box.
  U246 closed the GLOB-METACHARACTER case by threading provenance
  (`declared` vs `found`) through `p5_path_sane_found` and `p5_phys_ok`, so a
  name that is legal on disk but not legal to declare is removable. It did NOT
  take the other remedy its own ROADMAP row names: `p5_glob_hits` still expands
  `${P5_ROOT}$1` UNQUOTED (`p5/bin/p5-uninstall:659`, `:732`,
  `p5/lib/p5-common.sh:1082`), so a hit containing a SPACE still word-splits and
  a `..` component still re-resolves, and one bad expansion refuses the WHOLE
  removal. That is why exactly two of the three names still wedge.
  ...
  **A fourth call site still applies the strict rule to a DISCOVERED path** —
  `p5_dir_under` at `p5/lib/p5-common.sh:1704` (`p5_path_sane "$_p5du_path"`) and
  `:1707` (`p5_phys_ok` with no provenance argument, so it defaults to
  `declared`). Measured on a `/var/run/p5/v-1/su[b` directory: 1, REFUSED RMDIR,
  directory left, next install 5. Pre-existing, strictly improved by U246 and not
  a regression — U246 threaded provenance through three of the four sites, not
  four. Listed here so the count is not read as complete.

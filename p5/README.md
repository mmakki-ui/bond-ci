...
The **driver is complete and it really installs**; the **payload it drives does
not exist yet**. On this branch the battery builds synthetic packages and drives
the real scripts end to end — install, verify, interrogate, plan a removal,
remove, recover from a crash at every step — against a temp root. No bar uses a
stub.
...
- **U28 / E8** supplies the package: `MANIFEST.sha256`, `PROVENANCE`, and
  `payload/filemap` (rows of `mode|role|src|dest`). Every dest is judged
  against all three contract files at install time.
...
- **A contract glob row can make `--remove` EXECUTE an undeclared
  `/etc/init.d/p5-*` as root.** `emit_plan` step a gates the `/etc/rc.d` FLAG
  (`plan_gate file "$hit"`) and then execs `/etc/init.d/<svc> stop|disable`; the
  file that is EXECUTED is never gated — neither planner nor executor asks
  `p5_declared` about it, both only anchor the NAME to `p5|p5-*`. Reproduced on
  the RECOVERY path with no rogue name and no contract edit: a record-less
  (`damaged`) box — the exact state `--recover` exists for — carrying
  `/etc/init.d/p5-datapath` and its `/etc/rc.d/S94p5-datapath` flag ran that
  script twice as root and `--remove` exited **0**. In the reproduction the
  script deleted the operator's SSH key. E0 ships no init script, so E0 alone
  cannot produce the file; **E5 ships `/etc/init.d/p5-server`**, and after that
  the recovery verb on the box with no console executes it before establishing
  that P5 placed it. The asymmetry that makes it reachable is in the contract
  itself: `/etc/rc.d/[SK][0-9][0-9]p5-*` is `role=both` while the init scripts it
  points at are `role=client` / `role=server`, so the flag survives every role
  filter its own target does not.
- ~~**SELFDROP has no point-of-use gate.**~~ **CLOSED.** B1 added a
  ...
  it. **The mid-run lever is itself the open executor defect above, used only
  as a clock; nothing here fixes that.**

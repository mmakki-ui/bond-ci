...
SCOPE = [
    ...
    # U186. These two meet the criterion above and were outside SCOPE anyway.
    # The portal harness is PRINTED by the `portal` CI job exactly as
    # ecosim/p5/run.sh is by `recon-ecosim`, and the catalogue is READ at
    # runtime by the CGI, the injection whitelist and the page. MEASURED cost of
    # the gap: U120 found a cite written into BOTH files that named the COMMENT
    # above a dispatch arm rather than an arm, plus two more in the catalogue
    # that named comments -- caught by a reviewer reading them, by no gate.
    # Widening SCOPE surfaced 13 further bare line numbers in the same two
    # files, every one of them pre-existing. They are re-anchored to symbols in
    # the same commit, not exempted: narrowing SCOPE to duck the backlog would
    # rebuild the hole this row exists to close.
    "orchestration/ecosim/p5/portal/run.sh",
    "deploy/p5/portal/catalogue/modes",
    "deploy/p5/bond-xctl",

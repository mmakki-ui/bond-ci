...
#   installed    BOND_DIR present                  (universal guard)
...
# bond-xctl guard_installed(). `eng_installed` is a SEPARATE world fact
...
_EG_STATES = [S0(), S0(rc=True, eng=True, ep='local'),
              S0(rc=True, mode='speed', agg=True, ep='speed'),
...
_eg_speed_from_engaged_ok = _eg_edge_reachable_without_engarde(
    ...
    lambda c: c['mode'] == 'speed' and c['agg'] and not c['eng'] and c['ep'] == 'speed')
...
check("EG-M mutant `guard_installed = BOND_DIR AND engarde-client` (the PRE-U50a guard, "
      "bond-xctl:736 before this unit) is CAUGHT by EG-2 AND by EG-1: the guard's answer "

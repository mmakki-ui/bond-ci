...
#   2. It is what the shipped defect is about. `bond-xctl build_agg_env`
#      builds `AGG_PATHS=$P,$O` from primary + `head -1` of the rest: at N=3
#      a source is SILENTLY DISCARDED. Truncation is invisible to a count
#      (the count of live sources is unchanged) but visible in the enrolled
#      SET. The proof can only catch that class with a set.
#   3. There is a real fact on the box to mirror: bond-xctl writes
#      `$BOND_DIR/applied_wans` (:207-209) and `agg_env AGG_PATHS` (:293-301).
...
# deploy/p5/97-bond gates on `[ "$INTERFACE" = "$(cat $BOND_DIR/wg-logical)" ]`,
...
def a_genconf(s,w,a):
    # regenerate engarde.yml + applied_wans (bond-xctl genconf, :193-210).
...
def a_env_gen(s,w,a):
    # regenerate agg_env (bond-xctl act_env_gen / build_agg_env, :289-311).
    # AGG_PATHS must carry EVERY live source -- the shipped builder carries
    # primary + `head -1` and silently discards the rest; NG-2 is the bar
    # that catches exactly that, and NG-M1 proves the bar has teeth.
...
GUARDS = dict(installed=g_installed, manual=g_manual,
              ...
              # `two_wans` is the spelling the SHIPPED bond.dag:57 still uses
              # (deploy/p5 is owned elsewhere; renaming it there is a one-line
              # DAG diff). Both spellings resolve to the SAME N-generic
              # predicate, asserted by NG-0, so the rename needs no model change.
...
def reconcile(s, w, refresh=False):
    ...
    if d == 'engaged_speed':
        ...
        # ENTRY guard on the `speed` edge (bond.dag:`two_wans`), and converge() gives
        ...
        # in the shipped artifact too (deploy/p5/bond-xctl:841 `engaged_speed)
        # converge speed ;;` has the same hole, one level up from bond-xctl:553
        # build_agg_env, whose `fail` exits the process mid-edge).
        #
...
check("NG-1 speed engages iff N >= %d, ref==cand at every N in %s" % (MIN_AGG_SOURCES, str(NG_LADDER)),
      _ng_arity())
check("NG-2 [MODEL BAR, artifact NOT yet fixed] no truncation: the MODEL enrolls ALL N "
      "live sources. The SHIPPED bond-xctl:293-301 build_agg_env still emits "
      "AGG_PATHS=$P,$O (primary + head -1) and discards sources 3..N, so this bar "
      "constrains the model, NOT the box", _ng_no_truncation())
check("NG-3 no privileged N: lifecycle outcome identical for every N >= %d" % MIN_AGG_SOURCES,
      _ng_no_privileged_n())
check("NG-4 mode selection is N-generic: eco = primary only, lightning = all N, at every N",
      _ng_mode_selection())
...
def _mut_env_gen(s,w,a):    s['agg_srcs'] = mode_sources(s,w)[:2]   # bond-xctl:293 defect
...
check("NG-M1 mutant `AGG_PATHS = first 2 sources` (the shipped build_agg_env defect) is CAUGHT "
      "by NG-2 AND by ref==cand equivalence", caught_trunc)

# BOTH registered spellings must be mutated, not just one. The DAG interpreter
# resolves the guard by the name the SHIPPED bond.dag uses, and U6 renamed that
# row's guard `two_wans` -> `enough_sources`. Patching only the legacy alias would
# leave the mutant UNREACHED and this bar vacuously true -- the precise failure
# mode the NG-M self-tests exist to prevent. Mutating both keeps NG-M2 correct
...
check("CH-3 arity loss MID-FLIGHT resolves to a DEFINED outcome at every N in %s: aggregate "
      "torn down, single feeder, `mode speed` PIN retained, config re-enrolled over the live "
      "sources. NO LONGER A MODEL-ONLY BAR (U32): the SHIPPED reconciler had the same hole -- "
      "`engaged_speed) converge speed ;;` left bond-agg up on a stale AGG_PATHS whenever the "
      "`enough_sources` guard refused, forever -- and the sustainment this bar named is now IN "
      "the artifact as `bond-xctl sustain` (deploy/p5/bond-xctl sustain_speed), an UNCAPPED "
      "safety pass bond-watchdog runs every cycle: with bond-agg RUNNING under a speed pin and "
      "the live count below MIN_AGG_SOURCES on 2 consecutive TRUSTED observations, walk "
      "`switch`. AND IT IS NOW GATED IN LAYER-2: the SUS-1..SUS-7 bars in "
      "orchestration/ecosim/p5/run.sh drive the real artifacts and FAIL on dev (7 failures, "
      "exit 1) while passing here -- the artifact twin is no longer ungated, so this bar is no "
      "longer the only thing holding the property. THE MODEL DOES NOT COVER three artifact-side "
      "properties the Layer-2 bars do: probe-failure-is-not-source-loss (SUS-4), the cap "
      "exemption (SUS-5), and the confirmation window itself (SUS-1) -- this model has no "
      "failing probe, no actions/hour ring on the sustainment path, and no notion of a sample "
      "count, so it constrains the OUTCOME of arity loss and not the EVIDENCE required to "
      "declare it"
...
check("CH-6 total source loss (N -> 0 mid-flight) converges, keeps INV1, never reaches the "
      ...
      "enrolls the empty set, while bond-xctl:436 build_engarde_conf calls `fail` -> the "
...
def _mut_genconf_cached(s,w,a):
    ...
    # defect bond-xctl:119 forbids ("a cached source table would be STATE the
    # level-triggered reconciler must not keep"). Note it still branches on MODE
...
check("CH-M1 mutant `candidate CACHES the source list` (ignores every appear/disappear -- "
      "the cached-source-table defect bond-xctl:119 forbids) is CAUGHT by CH-EQ (%d seqs, "

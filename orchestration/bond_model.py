...
# AGG_SCHED (U17) == bond-xctl `agg_sched_of()` -- the ONE place a MODE maps to
...
# (mode -> sched), never a branch: a third aggregate scheduler is ONE entry here
# plus ONE in bond-xctl, and ZERO new dag rows/nodes/guards/actions.
AGG_SCHED = {'max': 'max', 'speed': 'speed'}
...
def reconcile(s, w, refresh=False):
    ...
    if d == 'engaged_agg':
        ...
        # in the shipped artifact too (deploy/p5/bond-xctl `engaged_agg)
        # converge agg ;;` has the same hole, one level up from
...
check("CH-3 [MODEL BAR, artifact NOT yet fixed] arity loss MID-FLIGHT resolves to a DEFINED "
      ...
      "same hole the model just closed -- bond-xctl:841 `engaged_agg) converge speed ;;` "

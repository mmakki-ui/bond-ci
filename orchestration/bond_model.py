...
def W(capable=True, agg_ok=True, sources=DEFAULT_SOURCES, agg_installed=True,
      installed=True, races=0, peer_absent=False, pool=None, shape_ok=True,
      native_sqm=False):
...
check("CH-3 [MODEL BAR, artifact NOT yet fixed] arity loss MID-FLIGHT resolves to a DEFINED "
      ...
      "same hole the model just closed -- bond-xctl:841 `engaged_speed) converge speed ;;` "
      "leaves bond-agg up on a stale AGG_PATHS when guard_two_wans (bond-xctl:645) refuses, "
      "forever; the fix is one arm in bond-xctl reconcile(), owned by deploy/p5"
...
# not on the model.
LIFECYCLE_EDGES = ('engage','disengage','switch','speed','speed_revert')
_sh0 = []
...
check("SH-0 the SHIPPED bond.dag carries shaping on every lifecycle edge: "
      "`shape_apply` is the LAST action of engage/disengage/switch/speed/speed_revert, "
...
reconcile(_t, _w)
_sh3 = (node(_t) == 'engaged' and _t['susp'] == 'none' and _t['eng'] and _t['ep'] == 'local'
        and not _t['shaped'])
# and the same for the speed edge
_t2 = S0(rc=True, mode='speed'); _w2 = W(shape_ok=False)
reconcile(_t2, _w2)
_sh3 = _sh3 and _t2['agg'] and _t2['susp'] == 'none' and _t2['ep'] == 'speed' and not _t2['shaped']
check("SH-3 INV8 NON-ESCALATION: with shaping failing hard, `engage` still reaches "
      "`engaged` (eng up, ep local) and `speed` still reaches the aggregate, NO suspend "
...
_t['mode'] = 'lightning'
_t['shaped'] = False; _t['shape_mtu'] = None
reconcile(_t, _w)                                # back down off speed
_sh4_back = (_t['mtu'] == 1420 and _t['shaped'] and _t['shape_mtu'] == 1420)
check("SH-4 MTU ordering (EFFECT, not action order): a shaping attach on the `speed` "
      "edge happens AFTER the MTU settles (applied against 1408, not 1420), and the "
      "same coming back off speed (1420)", _sh4_speed and _sh4_back)
...
_t = S0(rc=True); _w = W()
reconcile(_t, _w)                                # converged at 1420
_pre = _t['shape_ops']
_t['mode'] = 'speed'; reconcile(_t, _w)          # speed edge moves MTU to 1408
check("SH-4b NAMED LIMIT: an already-converged box crossing an MTU change does NOT "
      ...
      _t['shape_ops'] == _pre and _t['shape_mtu'] == 1420 and _t['mtu'] == 1408)
...
def _mut_shape_escalates(s,w,a):
    ...
    a_shape_apply(s,w,a)
    if not s['shaped']:
        s['eng']=False; s['ep']='direct'; s['susp']='suspended'
def _m1():
    t = S0(rc=True); ww = W(shape_ok=False)
    reconcile(t, ww)
    return node(t) == 'engaged' and t['susp'] == 'none' and t['eng']
...
reconcile(_t, _w)
_restarts = 0
_prev = (_t['eng'], _t['agg'], _t['ep'], _t['susp'])
for _ in range(20):
    reconcile(_t, _w)
    _now = (_t['eng'], _t['agg'], _t['ep'], _t['susp'])
...
reconcile(_t, _w)
_sh_n1 = (node(_t) == 'engaged' and _t['susp'] == 'none' and _t['eng']

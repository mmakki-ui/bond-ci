...
# only — no packets, no timing. Facts: rc (the engagement fact /etc/p5/rc),
...
CITED = {
    ...
    # U47a: the canonical procd unit. It was absent here, which is why Layer-1
    # literally could not open the stanza that CONSUMES agg_env -- and why the
    # emission<->stanza key set was guarded by nothing at either layer.
    'init.d/bond-agg': os.path.join(P5_DIR, 'init.d', 'bond-agg'),
...
#   rc    engagement INTENT = /etc/p5/rc            (bondctl on/off; U114)
#   aggen THE feeder's rc.d enable flag = MECHANISM  (agg_enable/agg_disable)
...
# =====================================================================
#  MF-ENV-* (U47a) -- THE EMISSION <-> PROCD KEY SET, COMPARED MECHANICALLY
# =====================================================================
# `build_agg_env` writes /etc/p5/agg_env; the procd unit SOURCES that file and
# re-exports each key on one `procd_set_param env` line. Those are two
# hand-maintained lists of the same names in two files, and until this block
# NOTHING compared them at either layer. Deleting an `echo` from the emitter, or
# a KEY="$KEY" from a stanza, left the model green while the daemon fell back to
# its own env() default -- that IS the U47 defect, and AGG_SPOTTY is only the key
# it happened to. Layer-2 cannot host this: the ecosim shims AGG_SVC to a present
# bin/svc-agg, so `act_agg_install`'s `[ -x "$AGG_SVC" ] && return 0` fires and
# NEITHER stanza is ever executed by the harness.
#
# THREE PURE FUNCTIONS OVER TEXT, never over paths. That is load-bearing rather
# than stylistic: MF-ENV-M1/M2 feed MUTATED source through the same functions and
# assert the comparison goes UNEQUAL, which is impossible if an extractor can only
# read the shipped tree.
#
# KEYED ON SHAPE, never on values or line numbers. U141 renamed ordered_wans ->
# mode_wans and changed the AGG_SCHED value expression inside this very block
# without touching the `echo "KEY=..."` shape; an extractor written against
# values or positions would have broken on that merge.
#
# The emitter parser ENUMERATES the block, it does not grep it: any line inside
# `{ ... } > "$1"` that is not blank, not a comment and not `echo "KEY=..."` is a
# HARD ERROR. A grep would silently drop a key someone rewrote as a printf and
# still print PASS -- which is the exact shape of the bug being guarded.
CANON_STANZA = 'init.d/bond-agg'
# The keys a stanza may give a literal value. Their VALUES stay Layer-2's
# (run.sh AGG-L15 for AGG_MODE; the U225 ST/stats bars for AGG_STATS); Layer-1
# asserts the literal SET is exactly this, so a new unmeasured literal cannot
# appear unnoticed.
# 2026-09-05 (U225 follow-up at merge): AGG_STATS joined the set. U225 put the
# stats-file path as a LITERAL in init.d/bond-agg on purpose -- its stanza
# comment says why: agg_env is compared against agg_env.applied to decide a
# feeder bounce, so a new key there would restart every installed datapath on
# upgrade for a read-only file. This bar did its job: it went red on dev from
# a8951fe (U225) until this line, because U225's gate set did not include the
# model arm. The value is asserted at Layer-2, not here.
AGG_ENV_LITERALS = frozenset(('AGG_MODE', 'AGG_STATS'))

_AGG_FN_RX   = re.compile(r'^\s*build_agg_env\(\)\s*\{')
_AGG_GRP_RX  = re.compile(r'^\s*\{\s*$')
_AGG_END_RX  = re.compile(r'^\s*\}\s*>\s*"\$1"\s*$')
_AGG_ECHO_RX = re.compile(r'^\s*echo "([A-Z_][A-Za-z0-9_]*)=')
_AGG_SKIP_RX = re.compile(r'^\s*(#.*)?$')
# A DELEGATED EMITTER: a line inside the redirect group that is nothing but the name of a
# function defined in this same file. U253/U252 introduced two (cap_env, tx_backoff_env)
# because their keys are CONDITIONAL -- emitted only when the box carries the operator fact --
# and a conditional cannot live inside the group without turning every `if` line into an
# unrecognised emission. Before this, such a line was a HARD ERROR, which is how U253 shipped
# with seven keys the procd stanza never passed through: the comparison MF-ENV-1 exists to make
# could not run at all, so nothing said the enable path stopped one hop short of the daemon.
_AGG_CALL_RX = re.compile(r'^\s*([a-z_][a-z0-9_]*)\s*$')
_AGG_DEF_RX  = r'^%s\(\)\s*\{'
_AGG_CLOSE_RX = re.compile(r'^\}\s*$')
# Anything in a delegated body that WRITES TO STDOUT must be an `echo "KEY="` line. The body
# is real shell -- guards, assignments, `case` -- so unlike the group it cannot enumerate every
# line; what it CAN do is refuse every other way of producing output. That keeps MF-ENV-2's
# teeth where they matter: rewriting one of cap_env's echoes as a printf, or hiding a key in a
# `for` loop over a variable, is a hard error and not a silently dropped key. (`printf` in a
# HELPER the body calls through `$( )` is not stdout of the body; the helper is not delegated.)
_AGG_OUT_RX  = re.compile(r'(^|[\s;&|(])(echo|printf|cat)\b')

def _agg_delegated_keys(actions_text, name):
    """The keys a DELEGATED emitter `name()` echoes, in emission order. One level only:
    a delegated body that itself delegates is refused rather than followed, because a
    parser that quietly stops descending drops keys."""
    lines = actions_text.splitlines()
    defs  = [i for i, ln in enumerate(lines) if re.match(_AGG_DEF_RX % re.escape(name), ln)]
    if len(defs) != 1:
        raise SystemExit("emitted_agg_keys: the emitter group calls %s, and this file defines "
                         "it %d times -- need exactly 1. A call the parser cannot resolve is a "
                         "key set it cannot read." % (name, len(defs)))
    end = [i for i in range(defs[0] + 1, len(lines)) if _AGG_CLOSE_RX.match(lines[i])]
    if not end:
        raise SystemExit("emitted_agg_keys: %s() is never closed by a `}` at column 0" % name)
    keys = []
    for ln in lines[defs[0] + 1:end[0]]:
        if _AGG_SKIP_RX.match(ln):
            continue
        m = _AGG_ECHO_RX.match(ln)
        if m:
            keys.append(m.group(1)); continue
        if _AGG_CALL_RX.match(ln) and re.search(_AGG_DEF_RX % re.escape(ln.strip()),
                                                actions_text, re.M):
            raise SystemExit("emitted_agg_keys: %s() delegates again to %s(). This parser "
                             "reads ONE level; a second would have to stop somewhere, and a "
                             "parser that stops descending drops keys silently."
                             % (name, ln.strip()))
        if _AGG_OUT_RX.search(ln):
            raise SystemExit("emitted_agg_keys: delegated emitter %s() writes to stdout on a "
                             "line this parser cannot read as `echo \"KEY=\"`: %r. An emission "
                             "form this parser does not know is a HARD ERROR, never a silently "
                             "dropped key." % (name, ln.strip()[:80]))
    # NO zero-key hard error here, deliberately, and the reason is a measurement: MF-ENV-M1
    # mutates the tree by deleting the `echo "<KEY>="` line for EVERY emitted key in turn, and
    # tx_backoff_env emits exactly one, so its mutant leaves a body with no echo at all. A hard
    # error there would kill the whole model run instead of printing a bar. An emitter that
    # emits nothing is still caught, one bar over: its keys become passed-but-not-emitted in
    # MF-ENV-1, which is the comparison, not a parser opinion.
    return keys

def emitted_agg_keys(actions_text):
    """The AGG_* keys `build_agg_env` EMITS, in emission order, out of the TEXT
    of xctl-actions.sh. Every arm below is a hard error rather than a dropped
    key, because a parser that quietly matches nothing produces the empty set,
    and the empty set trivially `agrees` with anything."""
    lines = actions_text.splitlines()
    fn = [i for i, ln in enumerate(lines) if _AGG_FN_RX.match(ln)]
    if len(fn) != 1:
        raise SystemExit("emitted_agg_keys: %d `build_agg_env() {` lines, need exactly 1 -- "
                         "the emitter moved or forked" % len(fn))
    end = [i for i, ln in enumerate(lines) if i > fn[0] and _AGG_END_RX.match(ln)]
    if len(end) != 1:
        raise SystemExit("emitted_agg_keys: %d redirect-group closers after the emitter, need "
                         "exactly 1 -- the `{ ... } > $1` group IS the shape being parsed"
                         % len(end))
    grp = [i for i in range(fn[0] + 1, end[0]) if _AGG_GRP_RX.match(lines[i])]
    if len(grp) != 1:
        raise SystemExit("emitted_agg_keys: %d bare `{` group openers in the emitter body, "
                         "need exactly 1" % len(grp))
    keys = []
    for i in range(grp[0] + 1, end[0]):
        m = _AGG_ECHO_RX.match(lines[i])
        if m:
            keys.append(m.group(1)); continue
        if _AGG_SKIP_RX.match(lines[i]):
            continue
        m = _AGG_CALL_RX.match(lines[i])
        if m and re.search(_AGG_DEF_RX % re.escape(m.group(1)), actions_text, re.M):
            keys.extend(_agg_delegated_keys(actions_text, m.group(1))); continue
        raise SystemExit("emitted_agg_keys: unrecognised line in the emitter group: %r. An "
                         "emission form this parser does not know is a HARD ERROR, never a "
                         "silently dropped key." % lines[i].strip()[:80])
    if not keys:
        raise SystemExit("emitted_agg_keys: zero keys -- that is exactly the shape a broken "
                         "parser produces, so it must not read as `the sets agree`")
    dup = sorted(set(k for k in keys if keys.count(k) > 1))
    if dup:
        raise SystemExit("emitted_agg_keys: duplicate key(s) %s -- agg_env is SOURCED, so a "
                         "duplicate key is last-writer-wins" % dup)
    return tuple(keys)

_SVC_OPEN_RX  = re.compile(r"<<'SVCEOF'")
_SVC_CLOSE_RX = re.compile(r'^SVCEOF$')

def agg_stanzas(initd_text, actions_text):
    """[(name, stanza_text)] -- every procd stanza that consumes agg_env,
    DISCOVERED rather than assumed. The canonical unit is always a member. The
    xctl-actions.sh fallback heredoc is a member WHEN IT EXISTS, and its absence
    is LEGAL -- U126 deletes it, so this block needs no edit in either merge
    order. A forked or unterminated fallback is the drift this refuses."""
    if not initd_text.strip():
        raise SystemExit("agg_stanzas: the canonical procd unit is empty or unreadable. "
                         "p5/payload/filemap installs it as the client's datapath init "
                         "script, so a tree without it cannot install")
    out = [(CANON_STANZA, initd_text)]
    lines  = actions_text.splitlines()
    opens  = [i for i, ln in enumerate(lines) if _SVC_OPEN_RX.search(ln)]
    closes = [i for i, ln in enumerate(lines) if _SVC_CLOSE_RX.match(ln)]
    if len(opens) > 1 or len(closes) > 1 or len(opens) != len(closes):
        raise SystemExit("agg_stanzas: %d fallback heredoc openers / %d closers in "
                         "xctl-actions.sh -- need 0 or 1 of each, matched"
                         % (len(opens), len(closes)))
    if opens:
        if closes[0] <= opens[0]:
            raise SystemExit("agg_stanzas: the fallback heredoc closer precedes its opener")
        out.append(('xctl-actions.sh fallback', '\n'.join(lines[opens[0] + 1:closes[0]])))
    return out

_ENV_LINE_RX = re.compile(r'procd_set_param\s+env\s+(.*)$')
# The BACK-REFERENCE is deliberate: AGG_SPOTTY="$AGG_PATHS" must NOT count as
# consuming AGG_SPOTTY. A key is consumed only when it is passed through as
# itself.
_ENV_REF_RX  = re.compile(r'^([A-Z_][A-Za-z0-9_]*)="\$\1"$')
_ENV_LIT_RX  = re.compile(r'^([A-Z_][A-Za-z0-9_]*)=("?)([^"$\s]*)\2$')
_SRC_AGG_ENV = '. /etc/p5/agg_env'

def stanza_env_keys(stanza_text):
    """(consumed, literal, problems) for ONE stanza.

    consumed -- keys passed through as KEY="$KEY"
    literal  -- keys given a literal (unexpanded) value in the stanza
    problems -- this stanza's RED reasons, in words

    The design's two-tuple gains `problems` because a stanza that stopped
    sourcing agg_env, grew a second env line, or grew an unparsed token is a
    verdict a BAR must print by name -- not a hard error that kills the run
    before any bar prints. An unparsed token is not an absent token."""
    problems = []
    if _SRC_AGG_ENV not in stanza_text:
        problems.append("does not source agg_env, so every AGG_* would expand EMPTY while "
                        "the key sets still matched -- the comparison would be theatre")
    envl = [m.group(1) for m in
            (_ENV_LINE_RX.search(ln) for ln in stanza_text.splitlines()) if m]
    if len(envl) != 1:
        problems.append("has %d `procd_set_param env` lines, need exactly 1" % len(envl))
        return frozenset(), frozenset(), tuple(problems)
    consumed, literal = set(), set()
    for tok in envl[0].split():
        m = _ENV_REF_RX.match(tok)
        if m:
            consumed.add(m.group(1)); continue
        m = _ENV_LIT_RX.match(tok)
        if m:
            literal.add(m.group(1)); continue
        problems.append("unparsed token %r on the env line" % tok)
    return frozenset(consumed), frozenset(literal), tuple(problems)

def _agg_keyset_delta(emitted, consumed):
    """(emitted-but-not-passed, passed-but-not-emitted). Both empty == agree."""
    return (tuple(sorted(set(emitted) - set(consumed))),
            tuple(sorted(set(consumed) - set(emitted))))

def _agg_read(path):
    try:
        with open(path) as f:
            return f.read()
    except OSError as e:                                        # wrong worktree, bad mode
        raise SystemExit("U47a: cannot read %s (%s)" % (path, e))

_AGG_ACTIONS_TEXT = _agg_read(CITED['xctl-actions.sh'])
_AGG_INITD_TEXT   = _agg_read(CITED[CANON_STANZA])
_AGG_EMITTED      = emitted_agg_keys(_AGG_ACTIONS_TEXT)
_AGG_STANZAS      = agg_stanzas(_AGG_INITD_TEXT, _AGG_ACTIONS_TEXT)
_AGG_PARSED       = [(n, stanza_env_keys(t)) for n, t in _AGG_STANZAS]
_AGG_NAMES        = [n for n, _ in _AGG_STANZAS]
# Cited only when it EXISTS -- citing a deleted fallback would make this block
# fail on the very merge it was designed to survive.
_AGG_FB_CITE = (cite('xctl-actions.sh', "<<'SVCEOF'")
                if len(_AGG_STANZAS) > 1 else 'absent (deleted by U126) -- legal')

check("MF-ENV-3 stanza discovery is NON-VACUOUS: %d stanza(s) consuming agg_env were "
      "discovered %s, and the canonical unit (%s) is always among them; the fallback heredoc "
      "(%s) is DISCOVERED, so its deletion is legal and its return is compared automatically. "
      "`nothing was compared` can never print as PASS"
      % (len(_AGG_NAMES), _AGG_NAMES,
         cite(CANON_STANZA, 'procd_set_param command /usr/sbin/p5-datapath'), _AGG_FB_CITE),
      len(_AGG_NAMES) >= 1 and CANON_STANZA in _AGG_NAMES)

_agg_deltas = [(n, _agg_keyset_delta(_AGG_EMITTED, c)) for n, (c, _l, _p) in _AGG_PARSED]
_agg_bad    = [(n, d[0], d[1]) for n, d in _agg_deltas if d[0] or d[1]]
check("MF-ENV-1 emission <-> procd: for EVERY discovered stanza, the key set the stanza "
      "passes through as KEY=\"$KEY\" EQUALS the set build_agg_env emits (%s -> %d keys %s), "
      "compared in BOTH directions and per stanza. Dropping an echo from the emitter, or a "
      "KEY=\"$KEY\" from one stanza, is now RED by name instead of an empty string the daemon "
      "papers over with its own default. Offenders (stanza, emitted-not-passed, "
      "passed-not-emitted): %s"
      % (cite('xctl-actions.sh', 'build_agg_env() {'), len(_AGG_EMITTED), list(_AGG_EMITTED),
         _agg_bad if _agg_bad else 'none'),
      not _agg_bad)

def _agg_emit_raises(text):
    try:
        emitted_agg_keys(text); return False
    except SystemExit:
        return True
# An emission REWRITTEN into a form the parser does not know -- the way a key
# gets lost without anyone deleting it. Built from whatever key the emitter
# actually emits FIRST, never a hardcoded name: a probe keyed on one key name
# goes vacuous the moment that key is deleted, which is precisely when this bar
# must still be able to speak.
_agg_printf_mut = _AGG_ACTIONS_TEXT.replace('echo "%s=' % _AGG_EMITTED[0],
                                            'printf "%%s\\n" "%s=' % _AGG_EMITTED[0])
check("MF-ENV-2 the emitter parser ENUMERATES the redirect group, it does not grep it: %d "
      "keys, zero duplicates, and EVERY non-blank non-comment line inside build_agg_env's "
      "group (closer %s) was RECOGNISED as `echo \"KEY=\"`. Non-vacuous in both directions -- "
      "rewriting one echo as a printf is a HARD ERROR rather than a silently dropped key, and "
      "the shipped text still parses"
      % (len(_AGG_EMITTED), cite('xctl-actions.sh', '} > "$1"')),
      len(_AGG_EMITTED) > 0
      and len(set(_AGG_EMITTED)) == len(_AGG_EMITTED)
      and _agg_printf_mut != _AGG_ACTIONS_TEXT
      and _agg_emit_raises(_agg_printf_mut)
      and not _agg_emit_raises(_AGG_ACTIONS_TEXT))

_agg_shape_bad = [(n, p, sorted(l)) for n, (_c, l, p) in _AGG_PARSED
                  if p or l != AGG_ENV_LITERALS]
check("MF-ENV-4 every stanza SOURCES agg_env (%s), carries EXACTLY ONE `procd_set_param env` "
      "line (%s), leaves no token on it unparsed, and its literal-valued key set is exactly "
      "%s -- the VALUE of that literal stays Layer-2's, this bar owns only the SET, so a new "
      "unmeasured literal cannot appear unnoticed. Offenders (stanza, problems, literals): %s"
      % (cite(CANON_STANZA, '. /etc/p5/agg_env'),
         cite(CANON_STANZA, 'procd_set_param env AGG_MODE='),
         sorted(AGG_ENV_LITERALS), _agg_shape_bad if _agg_shape_bad else 'none'),
      not _agg_shape_bad)

# ---- MF-ENV-M1/M2: the bars have teeth, proven on MUTATED TEXT ---------
# Both mutants also assert the UNMUTATED comparison is EQUAL, so a comparator
# that always returns unequal fails too. Same pattern as NG-M1 / SH-M1 / CITE-M1.
_agg_clean = not any(_agg_keyset_delta(_AGG_EMITTED, c)[0]
                     or _agg_keyset_delta(_AGG_EMITTED, c)[1]
                     for _n, (c, _l, _p) in _AGG_PARSED)
# The mutants probe EVERY emitted key, each one DERIVED from _AGG_EMITTED --
# never a hardcoded name (the principle stated above MF-ENV-2). A literal
# `AGG_SPOTTY` here would turn a LEGAL SYMMETRIC RENAME red for the wrong
# reason: rename the key in the emitter AND in every stanza and the two still
# agree, MF-ENV-1 stays green, but a mutant that edits a name no longer in the
# text stops applying and reports failure about itself, not about the tree.
# ADR-003 sec 2 renames exactly this key (AGG_SPOTTY -> AGG_SPOTTY_DUP, U133's
# to land), so that is a scheduled event, not a hypothetical. Probing the whole
# emitted set is also strictly stronger than probing one name.
_agg_m1 = []
for _k in _AGG_EMITTED:
    _mt = '\n'.join(ln for ln in _AGG_ACTIONS_TEXT.splitlines()
                    if not ln.strip().startswith('echo "%s=' % _k))
    _mk = emitted_agg_keys(_mt)
    _agg_m1.append((_k, _mt != _AGG_ACTIONS_TEXT
                    and _k not in _mk
                    and len(_mk) == len(_AGG_EMITTED) - 1
                    and all(_k in _agg_keyset_delta(_mk, c)[1]
                            for _n, (c, _l, _p) in _AGG_PARSED)))
_agg_m1_bad = [k for k, ok in _agg_m1 if not ok]
check("MF-ENV-M1 mutant `build_agg_env stops emitting <KEY>` (the U47 defect exactly: procd "
      "hands the daemon an empty string and lightning.go's own env default silently takes "
      "over) is CAUGHT for EVERY emitted key on EVERY stanza -- the comparison reports it "
      "passed-but-not-emitted. Probed keys are the emitted set %s, DERIVED, so a symmetric "
      "rename of any key stays green and only a real asymmetry is red. Non-vacuous: each "
      "mutation really removed exactly that one key from the emitter, and the same comparator "
      "on the UNMUTATED text reports every stanza EQUAL. Keys the mutant failed to catch: %s"
      % (list(_AGG_EMITTED), _agg_m1_bad if _agg_m1_bad else 'none'),
      len(_agg_m1) == len(_AGG_EMITTED) and bool(_AGG_EMITTED)
      and not _agg_m1_bad and _agg_clean)

_agg_m2 = []
for _n, _t in _AGG_STANZAS:
    for _k in _AGG_EMITTED:
        _mut = _t.replace(' %s="$%s"' % (_k, _k), '')
        _mc, _ml, _mp = stanza_env_keys(_mut)
        _agg_m2.append(((_n, _k), _mut != _t
                        and _k in _agg_keyset_delta(_AGG_EMITTED, _mc)[0]))
_agg_m2_bad = [p for p, ok in _agg_m2 if not ok]
check("MF-ENV-M2 mutant `one stanza stops passing <KEY>=\"$<KEY>\"` is CAUGHT for EVERY "
      "discovered stanza %s crossed with EVERY emitted key -- %d (stanza, key) mutations, and "
      "the comparison reports each one emitted-but-not-passed. The key is DERIVED from the "
      "emitted set, never a literal, so a symmetric rename is legal here too. Non-vacuous: "
      "each mutation really changed its stanza's text, and the same comparator on the "
      "UNMUTATED stanzas reports EQUAL. Pairs the mutant failed to catch: %s"
      % (_AGG_NAMES, len(_agg_m2), _agg_m2_bad if _agg_m2_bad else 'none'),
      len(_agg_m2) == len(_AGG_STANZAS) * len(_AGG_EMITTED) and bool(_agg_m2)
      and not _agg_m2_bad and _agg_clean)
...
def S0(**kw):
    ...
    #   rc     the STORED engagement fact /etc/p5/rc          (bondctl on/off)
    #   aggen  the feeder's rc.d ENABLE flag, the MECHANISM     (agg_enable/disable)
    # U114 split these. They were ONE key while desired() executed `$AGG_SVC
    # enabled`, and a scenario that says `rc=True` means "an engaged box", so
    # aggen DEFAULTS to rc and every pre-U114 scenario keeps its exact meaning.
    # Naming it explicitly is how the state that matters is written:
    # S0(rc=True, aggen=False) = intent recorded, feeder not enabled -- the fresh
    # or damaged box on which `bondctl on` used to be a permanent silent no-op.
    s = dict(rc=False, aggen=None, mode='lightning', auto=False, agg=False,
    ...
    s.update(kw)
    if s['aggen'] is None: s['aggen'] = s['rc']
    return s
...
def W(capable=True, agg_ok=True, sources=DEFAULT_SOURCES, agg_installed=True,
      ...
      eng_installed=True, shape_ok=True, native_sqm=False, foreign_shaper=False,
      old_quiescent=True):
    ...
    # foreign_shaper (U210): the file at $SHAPE_SVC is absent, or is there and
    # carries no P5 ownership marker -- a controller P5 did not write. == the
    # artifact's shape_svc_owned() returning false. It is a WORLD fact and not a
    # state fact because nothing P5 does changes it: the controller is placed by
    # p5-install from a filemap row, and an override pointing elsewhere is the
    # operator's. Both shape leaves must then do NOTHING (INV8).
    # old_quiescent (U208, G5): the OLD P1/P2/P3/P4 stack is stopped, not
    # enabled for the next boot, and off the tunnel endpoint. It is a WORLD
    # fact, not a state key: P5 never sets it -- the operator does, with
    # `p5-uninstall --switch-off` -- and P5 only refuses to come up while it is
    # False. It defaults True so every scenario written before this unit keeps
    # its exact meaning: they all model a box whose switch has already happened.
    return dict(capable=capable, agg_ok=agg_ok, sources=tuple(sources),
                ...
                native_sqm=native_sqm, foreign_shaper=foreign_shaper,
                old_quiescent=old_quiescent,
...
def node(s):
    ...
    # every mode is the same feeder. `susp` overrides. desired() is the STORED-fact
    # TARGET twin (below) and reads a DIFFERENT key (U114): if both halves read one
    # key they are equal by construction and reconcile can never see a delta.
    if s['susp']=='suspended':            return 'suspended'
    if s['susp']=='suspended_degraded':   return 'suspended_degraded'
    if s['aggen']:                        return 'engaged'
    return 'off'

def desired(s):
    # DESIRED lifecycle target = pure function of the STORED facts (rc, mode) only,
    # where rc is the FILE /etc/p5/rc (U114), never the feeder rc.d flag -- that is
    # node()'s business. == xctl-probe.sh `[ -f "$BOND_DIR/rc" ]`.
...
# U216 -- WHY THE ep_is_direct FIX HAS NO TWIN HERE, stated rather than left as a
# silent hole. `s['ep']` above is a two-valued ENUM ('direct' | 'agg'). This layer
# has no address, no port and NO HOSTNAME DIMENSION at all, so the defect U216
# closes -- uci's `end_point` being a DDNS NAME while `wg show endpoints` prints
# the RESOLVED IP, which made the literal compare in converged(off) false forever
# and re-walked disengage on every reconcile -- is not expressible in this state
# space and cannot be modelled here without inventing one. Adding an address
# dimension would be a second model of the shell's string handling, which is the
# thing Layer-1 exists NOT to be: it models the STATE MACHINE, and 'direct' is
# already the correct abstraction of "pointed at the server, whatever it is
# spelled as". The compare is therefore measured where the strings are real:
# Layer-2, ecosim/p5/run.sh, bars EP-1/EP-2, on a fixture whose `direct` and `ep`
# deliberately differ. Layer-1's contribution is unchanged and still load-bearing:
# it fixes WHICH edge disengage is and what its leaves do, so a re-walk here is
# still the same six actions.
...
# agg_enable / agg_disable are the MECHANISM (U114), not the fact: they move the
# feeder's rc.d flag, which node() observes. U141 had them write the engagement
# fact itself, which is what made `bondctl on` unable to repair a box whose init
# script was missing -- the enable failed, desired() read off, and the engage row
# that installs the file was never selected. They stay REQUIRED: rc.d is what
# starts the feeder at boot, so a fact without the flag comes up direct.
def a_agg_enable(s,w,a): s['aggen']=True
def a_agg_disable(s,w,a): s['aggen']=False
def a_agg_restart(s,w,a): s['agg']=True
def a_agg_stop(s,w,a):  s['agg']=False
def a_env_gen_if_enabled(s,w,a):
    if s['aggen']: a_env_gen(s,w,a)    # == `svc_enabled "$AGG_SVC" && act_env_gen`
def a_agg_restart_if_enabled(s,w,a):
    if s['aggen']: s['agg']=True       # T3 live switch: restart, endpoint untouched
...
def a_shape_apply(s,w,a):
    ...
    if w.get('native_sqm'):
        return
    # OWNERSHIP (U210) == the artifact's shape_svc_owned(), and placed exactly
    # where the artifact places it: AFTER the desired-off branch (so `off` still
    # tears down a controller P5 owns) and after the native-SQM guard, BEFORE any
    # service verb. Zero effective ops, no attach, no enable, no restart: the box
    # stays un-converged and retries, the same contained R3 cost as native_sqm.
    # Moving this test above the desired-off branch would stop act_shape_clear
    # ever running on `off` -- must-not 15 in the deploy-sequence design.
    if w.get('foreign_shaper'):
        return
    s['shape_ops'] += 1
    if not w['shape_ok']:
...
def a_shape_clear(s,w,a):
    # OWNERSHIP FIRST in this leaf (U210). A teardown that disables and stops a
    # controller P5 did not write is the same defect as an apply that enables
    # one, pointed the other way -- on the client it would stop P1's live shaper
    # from inside P5. The qdisc removal is skipped with it, because the qdisc on
    # that device may be the foreign controller's: P5 removes only what P5
    # attached. The observed state is deliberately NOT rewritten to False here;
    # the box is left exactly as it was found.
    if w.get('foreign_shaper'):
        return
...
def g_old_quiescent(s,w,a):
    # == xctl-dag.sh `guard_old_quiescent`: the shell runs
    # `p5-uninstall --quiescent` and this reads the same answer as a world fact.
    # The Layer-1 half of the gate is not the PREDICATE (that is measured in E0,
    # against real rc.d flags and injected wg/ubus/pgrep) -- it is the EDGE
    # ALGEBRA: with the fact False, engage and switch must leave the node exactly
    # where it was and run ZERO actions, from every node they are reachable from.
    return w['old_quiescent']
GUARDS = dict(installed=g_installed, manual=g_manual,
              agg_installed=g_agg_installed,
              sources_for_mode=g_sources_for_mode,
              # U208: registered HERE as well as in bond.dag, and deliberately
              # without a .get() anywhere near GUARDS[g] -- a token added to the
              # table with no Python twin must KeyError and redden this gate,
              # because a guard that silently resolves to nothing is a gate that
              # has stopped measuring.
              old_quiescent=g_old_quiescent,
...
def reconcile(s, w, refresh=False):
    ...
    if refresh and s['aggen']:         # == xctl-dag.sh `svc_enabled "$AGG_SVC"`
...
def cand_event(s, w, ev):
    ...
    if ev=='on':
        s['rc']=True                           # bondctl writes the rc FILE fact (U114)
        reconcile(s,w)
    elif ev=='off':
...
def cand_reboot(s,w):
    ...
    if s['aggen']: s['agg']=True               # rc.d starts what is ENABLED (U114)
    if s['rc']:
        reconcile(s,w)                         # 97-bond hook re-engages (T9)

def cand_crash(s,w):
    # procd respawn: the feeder dies and is immediately restarted. procd supervises
    # what rc.d STARTED, so this keys on the enable flag, not on the intent.
    if s['aggen']: s['agg']=True
...
def _engage_ref(s,w):
    ...
    a_env_gen(s,w,None); a_agg_enable(s,w,None)
...
def _refresh_ref(s,w):
    # A CONFIG-ONLY trigger (a mode write, ecod's flip) on the reference machine.
    # TWO arms since U114, and the second one is the specification statement, not
    # a reference bent to fit the candidate: the box realises its RECORDED INTENT.
    # While `rc` and the feeder's rc.d flag were one key, `bondctl on` set that
    # flag even when the engage edge was refused (guards, no live source), so a
    # later mode write always took the `switch` arm and the box came up by
    # accident. With the intent stored separately, "intent set, feeder not
    # enabled" is a real state (the fresh/damaged box), and a trigger that found
    # the box there and only ran `switch` would leave it off forever -- exactly
    # the silent no-op this unit exists to remove. == xctl-dag.sh reconcile:
    # `[ "$1" = refresh ] && svc_enabled "$AGG_SVC"` picks `switch`, everything
    # else falls through to `converge engage`.
    if s['aggen']:
...
def ref_event(s, w, ev):
    if ev=='on':
        # bondctl writes the rc FACT first (`: > $BOND_DIR/rc`, U114) and only then
        # runs the executor, so the intent is recorded even when the edge is
        # guard-refused -- which is what lets a later tick converge a box the first
        # attempt could not. Modelled on both machines, as it is on the box.
        s['rc']=True
        _engage_ref(s,w)
    elif ev=='off':                            # T2
...
# asserted.
KEYS=('rc','aggen','mode','auto','agg','ep','susp','agg_srcs','agg_sched')
LIFECYCLE_KEYS=('rc','aggen','mode','auto','agg','ep','susp','agg_sched')  # N-independent projection
...
# =====================================================================
#  QG -- U208 / G5: THE QUIESCENCE GUARD, LAYER 1.
#
#  What this layer proves is the EDGE ALGEBRA, not the predicate: with
#  old_quiescent False the two edges that bring P5 UP must be refused BEFORE any
#  action runs, from every node they are reachable from, and the refusal must
#  cost nothing -- no endpoint move, no feeder enable, no MTU change, no shaping
#  bounce. `converge` runs the guards before the actions, so "zero ops" is
#  observable as "the state dict is byte-identical afterwards"; every action in
#  this model writes state, including a_shape_apply (shape_ops), so an action
#  that ran cannot hide.
#
#  The complementary halves are elsewhere and are named here so nobody reads
#  this block as the whole gate: Layer-2 (ecosim QG-1..3, MU-QG) proves the
#  WIRING -- that bond-xctl actually calls the checker and that bondctl's exit
#  propagates -- and E0 (QG-E1..E7, QS-LIT) measures the PREDICATE against real
#  rc.d flags, injected wg/ubus/pgrep and a real sqm fixture.
_QG_FROM = ['off', 'engaged', 'suspended', 'suspended_degraded']
def _qg_state(node):
    """A state sitting at `node`, otherwise ordinary."""
    if node == 'off':      return S0(mode='eco')
    if node == 'engaged':  return S0(rc=True, aggen=True, agg=True, ep='agg', mode='eco')
    return S0(rc=True, aggen=True, agg=True, ep='agg', mode='eco', susp=node)

def _qg_refused(intent):
    """True iff `intent` is refused from EVERY node with old_quiescent False,
    leaving the state completely unchanged."""
    w = W(old_quiescent=False)
    for n in _QG_FROM:
        s0 = _qg_state(n)
        before = copy.deepcopy(s0)
        if converge(s0, w, intent, None):
            return False                    # the edge was taken
        if s0 != before:
            return False                    # an action ran, or the node moved
        if node(s0) != n:
            return False
    return True

def _qg_allowed():
    """CONTROL: the same intents from the same nodes with the fact True."""
    w = W(old_quiescent=True)
    for n in _QG_FROM:
        s0 = _qg_state(n)
        if not converge(s0, w, 'engage', None):  return False
        if node(s0) != 'engaged':                return False
    s1 = _qg_state('engaged')
    if not converge(s1, W(old_quiescent=True), 'switch', None): return False
    return True

check("QG-L1 with old_quiescent FALSE the `engage` and `switch` edges are REFUSED from "
      "all four nodes (off, engaged, suspended, suspended_degraded): the node is unchanged "
      "and the state dict is byte-identical, so ZERO actions ran -- no endpoint move, no "
      "feeder enable, no MTU change and no shaping bounce. This is the mechanical "
      "pre-switch gate: P5 does not come up over a live old stack",
      _qg_refused('engage') and _qg_refused('switch'))
check("QG-L2 CONTROL with old_quiescent TRUE the same edges from the same nodes are "
      "WALKED: engage reaches `engaged` from each of the four, and switch is taken. "
      "QG-L1 is a refusal and not a broken fixture",
      _qg_allowed())
check("QG-L3 the escape and the way back carry NO quiescence guard: `disengage` and "
      "`suspend` are still walked with old_quiescent FALSE, from every node. A guard on "
      "either would trap a box whose old stack came half-alive -- the failure this gate "
      "exists to prevent, not to cause",
      all(converge(_qg_state(n), W(old_quiescent=False), 'disengage', None)
          for n in _QG_FROM)
      and all(converge(_qg_state(n), W(old_quiescent=False), 'suspend', None)
              for n in _QG_FROM))

# QG-M: the mutation self-test. Drop the term from the guard and require QG-L1
# to go false -- without this, QG-L1 is green on a model that never reads the
# fact. Same shape as EG-M.
_qg_caught = _with(GUARDS, 'old_quiescent', lambda s,w,a: True,
                   lambda: not (_qg_refused('engage') and _qg_refused('switch')))
check("QG-M mutant `g_old_quiescent = always True` (the guard registered but blind) is "
      "CAUGHT by QG-L1: the refusal disappears and the edges are walked over a live old "
      "stack. Non-vacuous proof that QG-L1 measures the guard rather than assuming it",
      _qg_caught)

# =====================================================================
#  RC-M -- U114's node/desired SPLIT: the LAYER-1 half of the engagement fact.
#  desired() is a pure function of the STORED fact `rc` (== the shell's
#  `[ -f "$BOND_DIR/rc" ]`); node() reads OBSERVED reality, the feeder's rc.d
#  enable flag (`aggen`). The two halves MUST read different keys: if they read
#  one key they are equal by construction and reconcile can never see a delta.
#
#  The state that makes the split observable is S2 = (rc present, feeder NOT
#  enabled) -- the fresh or damaged box. Pre-U114 that state was unrepresentable
#  and its symptom was a silent no-op: `bondctl on` enabled the service BEFORE
#  reconcile, so with /etc/init.d/p5-datapath missing the enable failed
#  silently, desired() read `off`, the engage row was never selected, and
#  act_agg_install -- the fallback that writes that very file, ON that row --
#  never ran. `bondctl on` was a permanent no-op on exactly the box that
#  needed repairing.
#
#  WHY THIS BAR HAS TO EXIST SEPARATELY. No other scenario in this file
#  constructs S2: S0() defaults `aggen` to `rc`, so in EVERY pre-U114 scenario
#  the two halves agree, and a node() collapsed back onto `rc` stays green
#  across the whole file. Layer-1 carried 97 changed lines of this unit with
#  no bar that could tell the split from the collapse. RC-M1 is that bar and
#  RC-M2 proves it is not vacuous.
# =====================================================================

def _rc_m1():
    """S2 (rc=True, aggen=False): the halves DISAGREE, and ONE reconcile picks
    the `engage` row and converges the box to engaged."""
    walked = []
    real_converge = converge
    def spy(s, w, intent, arg=None, depth=0):
        if depth == 0: walked.append(intent)
        return real_converge(s, w, intent, arg, depth)
    s, w = S0(rc=True, aggen=False), W()
    obs, tgt = node(s), desired(s)
    ok = _with_global('converge', spy, lambda: reconcile(s, w))
    return (obs == 'off' and tgt == 'engaged'      # observed != stored: a delta
            and walked == ['engage']               # and reconcile derives THAT edge
            and ok and s['aggen'] and s['agg'] and s['ep'] == 'agg')

check("RC-M1 the split state S2 (rc fact PRESENT, feeder rc.d flag NOT set -- the fresh or "
      "damaged box) is REPRESENTABLE and CONVERGES: node()=off reads OBSERVED reality while "
      "desired()=engaged reads the STORED fact, so reconcile sees a delta and walks exactly "
      "the `engage` row, leaving the feeder enabled, running, and the endpoint on the "
      "aggregate. This is the state `bondctl on` used to be a permanent silent no-op on, and "
      "it is unreachable the moment the two halves read one key",
      _rc_m1())

def _mut_node_reads_rc(s):
    # THE COLLAPSE: node() reading the STORED fact instead of observed reality --
    # i.e. `if s['rc']: return 'engaged'` where the shipped model reads s['aggen'].
    if s['susp'] == 'suspended':          return 'suspended'
    if s['susp'] == 'suspended_degraded': return 'suspended_degraded'
    if s['rc']:                           return 'engaged'
    return 'off'

check("RC-M2 mutant `node() reads s['rc']` (the two halves collapsed onto ONE key) is CAUGHT "
      "by RC-M1: with node() reporting stored intent, S2 reads `engaged`, the delta the "
      "reconciler exists to see disappears, and the damaged box looks converged. Non-vacuous "
      "proof that RC-M1 measures the split rather than assuming it",
      not _with_global('node', _mut_node_reads_rc, _rc_m1))
...
# ---- SH-20 / SH-21: the reconciler may not drive a controller it did ---
# ---- not write (U210) --------------------------------------------------
# THE DEFECT THIS CLOSES was live: `shape_apply` rides the DISENGAGE edge as
# well as engage/switch, and `direct` is DEFINED as bond-off PLUS shaping, so
# ANY reconcile on an unconverged box runs this leaf -- including at deploy
# state S2, before P5 is ever engaged. The leaf ran `$SHAPE_SVC enable;
# restart` against a default of /etc/init.d/cake-autorate, which on the client
# is P1's LIVE controller. P5 would have re-enabled, from inside the
# reconciler, the service the operator had just switched off.
# Both leaves must therefore do NOTHING when the controller is not P5's, and
# both directions are checked because forgetting the teardown leaf is the same
# defect pointed the other way.
_t = S0(rc=True); _w = W(foreign_shaper=True)
reconcile(_t, _w)
_sh17 = (_t['shape_ops'] == 0 and not _t['shaped'] and _t['shape_mtu'] is None
         and node(_t) == 'engaged' and _t['susp'] == 'none' and _t['agg'] and _t['ep'] == 'agg')
# CONTROL: the SAME world with the marker present drives it, so the zeros above
# are about ownership and not about a world in which nothing happens.
_c = S0(rc=True); reconcile(_c, W())
_sh17 = _sh17 and _c['shape_ops'] == 1 and _c['shaped']
check("SH-20 FOREIGN CONTROLLER, want=on: a shaper P5 did not write receives ZERO "
      "(re)applications and is never attached to, while `engage` still reaches "
      "`engaged` with no suspend (INV8) -- and the control with an OWNED controller "
      "in the same world DOES converge, so the zeros are the ownership test",
      _sh17)

# SH-21 -- the teardown leaf. Converge with an OWNED controller, then point at a
# foreign one and ask for `off`: nothing may be disabled, stopped or detached,
# and the observed state must be left exactly as found.
_t = S0(rc=True); _w = W()
reconcile(_t, _w)                                   # converged, shaped
_ops0, _shaped0, _mtu0 = _t['shape_ops'], _t['shaped'], _t['shape_mtu']
_t['shape'] = 'off'
reconcile(_t, W(foreign_shaper=True))               # the controller is not ours now
_sh18 = (_t['shape_ops'] == _ops0 and _t['shaped'] == _shaped0 and _t['shape_mtu'] == _mtu0)
# CONTROL, and it is must-not 15 made mechanical: the ownership test sits AFTER
# the desired-off branch, so `off` against an OWNED controller still tears down.
# An implementation that tested ownership first would pass SH-21 and fail here.
_c = S0(rc=True); reconcile(_c, W())
_c['shape'] = 'off'; reconcile(_c, W())
_sh18 = _sh18 and (not _c['shaped']) and _c['shape_mtu'] is None
check("SH-21 FOREIGN CONTROLLER, want=off: the teardown leaf disables, stops and "
      "detaches NOTHING it does not own (ops stay %d, observed state untouched) -- "
      "and the control proves `off` still tears down an OWNED controller, so the "
      "ownership test really is placed after the desired-off branch (must-not 15)"
      % _ops0, _sh18)

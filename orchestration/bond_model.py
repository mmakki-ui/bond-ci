...
# deploy/p5/97-bond calls the executor's `_is_wg "$INTERFACE"` verb (U52 --
# DERIVED live over ubus; before U52 it read the wg-logical fact, a file
# written ONLY by p2-engarde's bootstrap and by nothing in deploy/p5, so on a
# clean box or the server it was never written at all) and only proceeds on
# exit 0, so a hotplug on `wan`/`tethering`/`wwan` exits without ever calling
# reconcile. The delta is picked up by the NEXT trigger -- the watchdog's

...
#   eng_installed  engarde-client binary present   (NO guard reads this)
...
#      `ordered_wans` source (bond-xctl `build_agg_env`). Truncation is invisible to
...
#      `$BOND_DIR/applied_wans` (bond-xctl `genconf`) and `agg_env`'s
#      `AGG_PATHS` (bond-xctl `build_agg_env`, published by `act_env_gen`).
...
#
#  NOT COVERED HERE, and left to U50b: engage/switch still RUN engarde actions
#  (genconf/eng_enable/eng_restart), i.e. `eco` and `lightning` still have
#  engarde as their FEEDER. Dropping the guard does not give those modes a
#  mechanism; U50b owns that. This model has no shell-control-flow boundary
#  (the same HONEST GAP recorded for build_engarde_conf's N=0 `fail`), so the
#  missing-feeder consequence is measured in Layer-2 (bar EG-5), not here.
# =====================================================================
...
_EG_STATES = [S0(), S0(rc=True, eng=True, ep='local'),
...
# (bondctl writes the mode fact; the bond is still off), and ref/cand already
# diverge on that sequence with the engarde binary PRESENT -- so it measures
# something other than this removal. The bond.dag `speed` row's `from=off`
# member is NOT reachable through the CLI at all: desired() opens with
# `svc_enabled "$SVC"` where SVC is engarde's init.d script, so the lifecycle's
# engaged/off discriminator is itself an engarde artifact. That is a SECOND,
# deeper dependency than the guard U50a removed, it is a state-model change
# coupled to U50b's feeder choice, and it is recorded (not fixed) as Layer-2
# bar EG-2.
_eg_speed_ok = _eg_edge_reachable_without_engarde(
    ['on', 'mode_speed'], lambda c: c['mode'] == 'speed' and c['agg'] and not c['eng']
                                    and c['ep'] == 'agg')
_eg_speed_from_engaged_ok = _eg_edge_reachable_without_engarde(
    ['on', 'mode_eco', 'mode_speed'],
    lambda c: c['mode'] == 'speed' and c['agg'] and not c['eng'] and c['ep'] == 'agg')
...
_eg_disengage_ok = _eg_edge_reachable_without_engarde(
    ['on', 'off'], lambda c: not c['rc'] and not c['eng'] and not c['agg']
                             and c['ep'] == 'direct')
_eg_switch_ok = _eg_edge_reachable_without_engarde(
    ['on', 'mode_eco'], lambda c: c['mode'] == 'eco' and c['auto'])

check("EG-1 with the engarde binary ABSENT (eng_installed=False) the `speed` edge is "
      "REACHABLE from engaged, direct and via a prior eco pin: mode=speed, bond-agg up, "
      "engarde down, ep=:59402, ref==cand. This is the row that was plainly defective -- "
      "bond.dag's speed row already carries its OWN `agg_installed` guard and none of "
      "its actions call engarde, yet `installed` gated it on P2's binary",
...
check("EG-2 NO guard in GUARDS reads the engarde binary: every registered guard "
      ...
      "the bar that covers ALL FOUR edges that carry `installed` (engage, disengage, "
      "switch, speed), not just the speed row"
...
check("EG-3 with the binary absent the engage, disengage and switch edges are still "
      ...
      "switches and enables auto -- ref==cand throughout. SCOPE: this says the DAG no "
      "longer REFUSES those edges. It does NOT say eco/lightning have a working feeder "
      "without engarde -- they do not, that is U50b, and Layer-2 EG-5 measures it",
...
check("EG-M mutant `guard_installed = BOND_DIR AND engarde-client` (the PRE-U50a guard, "
      "ffd5857:bond-xctl:736 before this unit; today it is bond-xctl `guard_installed`) "
...
check("CIT-2 the citation gate BITES -- non-vacuity, proved the same way the EG-M mutant "
      ...
      "fire) and rename a cited shell function in deploy/p5/bond-xctl (R2 must fire), "
...
# not on the model.
LIFECYCLE_EDGES = ('engage','disengage','switch','agg','agg_revert')
_sh0 = []
...
check("SH-0 the SHIPPED bond.dag carries shaping on every lifecycle edge: "
      "`shape_apply` is the LAST action of engage/disengage/switch/agg/agg_revert, "
...
_sh3 = (node(_t) == 'engaged' and _t['susp'] == 'none' and _t['eng'] and _t['ep'] == 'local'
        and not _t['shaped'])
# and the same for the aggregate edge (mode='speed' selects the `speed` scheduler,
# but post-U17 every aggregate mode walks the one generic `agg` DAG edge, and
# a_ep_agg sets ep='agg' regardless of which scheduler is active)

...
for _name, _setup in (
        ...
        ('aggregate',lambda: (S0(rc=True, mode='speed'), W())),
...
_sh3 = (node(_t) == 'engaged' and _t['susp'] == 'none' and _t['eng'] and _t['ep'] == 'local'
        and not _t['shaped'])
# and the same for the aggregate (`agg`) edge
...
check("SH-3 INV8 NON-ESCALATION: with shaping failing hard, `engage` still reaches "
      "`engaged` (eng up, ep local) and `agg` still reaches the aggregate, NO suspend "
...
_t['mode'] = 'lightning'
_t['shaped'] = False; _t['shape_mtu'] = None
reconcile(_t, _w)                                # back down off the aggregate
_sh4_back = (_t['mtu'] == 1420 and _t['shaped'] and _t['shape_mtu'] == 1420)
check("SH-4 MTU ordering (EFFECT, not action order): a shaping attach on the `agg` "
      "edge happens AFTER the MTU settles (applied against 1408, not 1420), and the "
      "same coming back off the aggregate (1420)", _sh4_speed and _sh4_back)
...
_pre = _t['shape_ops']
_t['mode'] = 'speed'; reconcile(_t, _w)          # the agg edge moves MTU to 1408

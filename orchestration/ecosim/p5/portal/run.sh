...
# word-split). OCCURRENCES, NOT LINES -- and that distinction is the whole of
# the U230 fix round. A line-granular count cannot see a second verb sharing a
# line with an admitted one: `ip route show ...; ip route append ...` scored 1
# under the `ip route` count AND 1 under the `ip route show` count, so PC-2e's
# subtraction reported 0 writes on a line that writes a route -- and `append`
# (like `prepend`) is a real iproute2 verb that the enumerated list below does
# not name, so nothing else saw it either. The comment filter therefore runs
# FIRST, on whole-line `#` as before, and `grep -o` counts every hit on what is
# left, which makes the SUBTRACTION the closure: the enumerated list is a
# naming aid, not the thing that bounds the set.
pc2_hits() {   # $1 = token -> count of non-comment OCCURRENCES over pc2_files
    _p2h=0
    pc2_files > "$WORK/pc2.list"
    while IFS= read -r _p2n; do
        [ -n "$_p2n" ] || continue
        _p2c=$(grep -v -E '^[[:space:]]*#' "$_p2n" 2>/dev/null \
               | grep -o -F -- "$1" | grep -c .)
        _p2h=$((_p2h + _p2c))
    done < "$WORK/pc2.list"
    printf '%s' "$_p2h"
...
for t in 'autoratectl' 'uci set' 'uci commit' 'uci add' 'uci delete' '/etc/init.d/' 'sqm' 'iptables' 'wg set' 'ip route add' 'ip route del' 'ip route replace' 'ip route change' 'ip route flush' 'agg_restart' 'eval ' 'kill '; do
    [ "$(pc2_hits "$t")" = 0 ] || bad="$bad [$t]"
done
asrt "PC-2 no forbidden verb in the portal source (cgi + lib + bin/* less the two named exemptions)" "$bad" ""
# PC-2e/f: THE ROUTE ARM, SPLIT (U230). It used to be the single token `ip route`,
# which is a WRITE test wearing a READ's clothes: the runner's E1 residue block
# asks the kernel what is still pinned after a job dies, and that read is the
# whole evidential value of the block -- a runner that trusted the instrument's
# own word for it would report a clean box after a kill -9 that skipped the trap.
# So every MUTATING verb is enumerated above (add/del/replace/change/flush, all
# still 0 here), and the READ is pinned to `show` and COUNTED, so a write cannot
# arrive dressed as one and a read cannot multiply unnoticed.
# THE COUNT IS THE BAR, NOT THE ENUMERATION (U230 fix round). The five verbs
# above are a set somebody chose; `ip route` MINUS `ip route show`, counted by
# OCCURRENCE through the same pc2_hits, is a set nobody has to keep complete --
# every route token in the scanned files that is not a `show` shows up as a
# write whatever its verb is called. Both counts come from pc2_hits so they
# cannot drift apart in filtering. RESIDUAL, stated: the token is the literal
# `ip route`, so an `ip r`/`ip -4 route` spelling would need its own token --
# there is none in deploy/p5/portal today (`grep -rn 'ip -[46] route\|ip r '`).
PC2_IPW=$(pc2_hits 'ip route')
PC2_IPS=$(pc2_hits 'ip route show')
asrt "PC-2e every ip-route site in the unexempt portal source is a show (writes: $((PC2_IPW - PC2_IPS)))" "$((PC2_IPW - PC2_IPS))" 0
asrt "PC-2f ... and there is exactly one of them: the runner's residue read" "$PC2_IPS" 1
...
# U224's ten read-only rows, exercised through the SAME path so their argv lands
# in the ledger PC-5 checks. Listed here, not derived from the catalogue: a loop
# over catalogue/probes would exercise exactly the rows the allowlist below is
# derived from, and the whole point of PC-5 is that a catalogue row cannot
# authorise itself.
for p in shape hs xfer ep qdisc ifstats reach feeder armed version; do
    G "q=probe&name=$p" >/dev/null
done
...
# CONTROLS; RUN-7 exercises the RUNNER; U223's LOG-8 exercises the LOG ROUTE, and
# all three have to be judged against the same list or a later one silently widens
# the first. It takes the ledger as an ARGUMENT so a unit adding a route re-applies
# THIS list to ITS OWN ledger instead of pasting a copy that would then drift.
# Adding a row here is the one place a portal unit widens the portal's runtime
# boundary. The runner's lines are here for that reason: `p5-portal-run <name>
# <id>` is the spawn argv the CGI issues (both tokens from the catalogue and the
# clock, never from the request -- mutant JOB-M1 shows the other shape),
# `p5-accept` is the read-only battery, and the three p5-deadman verbs are the
# whole of what a disruptive row may ask of the rollback primitive. There is no
# `fire`, no delete and no kill verb in this list, on purpose. `logread` is
# admitted BARE and nothing that looks like it -- LOG-8d is that row's control.
led_viol() {   # $1 = ledger file -> the lines that are NOT allowlisted
    awk -v impl="$IMPLRE" '
      { if ($0 == "bondctl on") next
        ...
        # U224: the nine read-only probe verbs plus _shape. An EXPLICIT list, for
        # the reason stated above the loop that exercises them -- deriving this from
        # catalogue/probes would let a catalogue row allow itself, and the catalogue
        # is exactly the file an attacker with a foothold would edit.
        if ($0 ~ /^bond-xctl (_shape|_hs|_xfer|_ep|_qdisc|_ifstats|_reach|_feeder|_armed|_version)$/) next
        ...
        if ($0 == "p5-accept") next
        # U230: the E1 instrument. Two shapes and nothing else -- --plan, which
        # prints the bound and exits before any side effect (what the CGI reads
        # to build the confirm text), and the run itself. Two canonical uints and
        # at most one device literal; no request byte reaches either.
        if ($0 ~ /^p5-e1-probe (--plan )?[0-9]+ [0-9]+( [a-zA-Z0-9._-]+)?$/) next
...
# U230: the E1 instrument. The runner execs it, and a lost mode bit here is a
# disruptive row that arms a deadman and then runs nothing.
xb_one "XB-7 the E1 instrument is executable"       "$PORTAL/bin/p5-e1-probe"
...
# ============ THE READ-ONLY PROBE ROWS (U224) ================================
# The verbs themselves are measured next door (orchestration/ecosim/p5/run.sh,
# PR-1..PR-8, against the shims). What is measured HERE is the half only this
# harness can see: that each row is reachable THROUGH THE CGI with a session,
# that the reconciler actually implements every verb the catalogue names, that
# no probe's output carries key material or an address to a browser (LEAK-1),
# and that the function bodies behind them are read-only by inspection (PC-3c).
#
# THE TEN NAMES ARE WRITTEN OUT, not derived from catalogue/probes. Deriving
# them would make this block agree with the catalogue by construction: a row
# silently dropped by a bad merge would take its own bar with it and the count
# would still be "all of them".
PRB_ROWS='shape hs xfer ep qdisc ifstats reach feeder armed version'

setup
PRBBAD=""; PRBRC=""
for p in $PRB_ROWS; do
    R=$(G "q=probe&name=$p"); B=$(bd "$R")
    [ "$(st "$R")" = 200 ] || PRBBAD="$PRBBAD $p($(st "$R"))"
    case "$B" in *'"rc":0,'*) ;; *) PRBRC="$PRBRC $p" ;; esac
done
asrt "PRB-1 every U224 probe row answers 200 through the CGI" "$PRBBAD" ""
asrt "PRB-2 ... with rc 0 -- 'no data' is an answer, not a failure" "$PRBRC" ""

# PRB-3 COMPLETENESS, mechanically. Two directions, because either alone passes
# on a half-landed merge: every name this unit ships is in the catalogue exactly
# once, AND every XCTL verb the catalogue names is a dispatch arm the reconciler
# really has (a catalogued verb with no arm falls to bond-xctl's usage line and
# comes back rc 1, which PRB-2 would catch only for the rows it knows about).
PRBMISS=""
for p in $PRB_ROWS; do
    _n=$(grep -c "^$p|" "$PORTAL/catalogue/probes")
    [ "$_n" = 1 ] || PRBMISS="$PRBMISS $p($_n)"
done
asrt "PRB-3 every row this unit ships is in the catalogue exactly once" "$PRBMISS" ""
PRBARM=""
# Through a FILE, not a pipe: the loop appends to PRBARM, and a piped `while`
# runs in a subshell whose assignments die with it -- the bar would then report
# clean for every catalogue however broken (SC2013's other half).
grep -v '^[[:space:]]*#' "$PORTAL/catalogue/probes" \
  | awk -F'|' 'NF>1{split($2,a," "); if (a[1]=="XCTL") print a[2]}' > "$WORK/prbverbs"
while read -r v; do
    [ -n "$v" ] || continue
    grep -qE "^[[:space:]]*$v\)" "$P5/bond-xctl" || PRBARM="$PRBARM $v"
done < "$WORK/prbverbs"
asrt "PRB-3b every XCTL verb the catalogue names is a real dispatch arm" "$PRBARM" ""

# PRB-4..6: the three outputs the page PARSES, so their shape is a contract and
# not decoration. Everything else is printed verbatim into a <pre>.
R=$(G "q=probe&name=hs"); B=$(bd "$R")
has "PRB-4 _hs reports the age per peer" "$B" "peer1 handshake_age_s="
R=$(G "q=probe&name=ep"); B=$(bd "$R")
has  "PRB-5 _ep reports a class and a port"  "$B" "direct port=51820"
R=$(G "q=probe&name=qdisc"); B=$(bd "$R")
has "PRB-6 _qdisc reports the four facts shaping is decided on" \
    "$B" "want=on now=off owned=0 native_sqm=0"

# PRB-7: the remaining rows answer with TEXT. A 200 carrying an empty body is
# the failure this bar exists for -- on a monitoring page an empty answer reads
# as a measured nothing, so every verb must say either a fact or `absent: why`.
PRBEMPTY=""
for p in $PRB_ROWS; do
    B=$(bd "$(G "q=probe&name=$p")")
    case "$B" in *'"output":""'*) PRBEMPTY="$PRBEMPTY $p" ;; esac
done
asrt "PRB-7 no probe answers 200 with an empty body" "$PRBEMPTY" ""

# LEAK-1 — NOTHING THE PROBES PRINT MAY CARRY KEY MATERIAL OR AN ADDRESS.
# This is the bar that made _hs and _xfer drop column 1 and _ep classify instead
# of echoing. It sweeps EVERY row, including the six that predate this unit, and
# it checks the fixture's own strings: the peer public key the wg shim serves,
# and the endpoint host in $ECOSIM_STATE/ep. A page is a browser, a screenshot
# and a support ticket; a key that reaches it has left the box.
LEAKKEY='PEERPUBKEY000000000000000000000000000000000='
LEAKEP=$(cat "$WORK/ep" 2>/dev/null)
LEAKHOST=${LEAKEP%:*}
# THE SWEEP MUST RUN THE BRANCH A REAL BOX TAKES, and until this fix round it
# did not. setup()'s fixture writes `hs 0`, which is wg's "no handshake yet",
# and probe_hs answers that from its `never` arm -- a CONSTANT with no field of
# the wg line in it. So the age printf, the arm every box with a live tunnel
# takes and the ONLY one that formats a wg column, was never swept: measured in
# this round, a mutant printing $1 from that printf left LEAK-1 GREEN. The
# fixture is aged 30 s here so the sweep below reads the line a real box gives.
echo "$(( $(date +%s) - 30 ))" > "$WORK/hs"
asrt "LEAK-1d the sweep runs the AGE arm of _hs, not the constant 'never' arm" \
     "$(bd "$(G "q=probe&name=hs")" | grep -c 'handshake_age_s=never')" 0
#
# ONE ROW IS A DECLARED EXCEPTION AND IT IS NAMED HERE RATHER THAN GLOSSED:
# `server` (the XCTL verb bond-xctl `_server`) EXISTS to print the server host -- its
# consumer pings it, its catalogue label says so, and it predates this unit. So
# the KEY sweep covers every row without exception, and the HOST sweep covers
# every row but that one, whose behaviour is then asserted in the OTHER
# direction (LEAK-1c) so the exception stays a measured fact rather than a hole
# a later row could hide in.
LEAKS=""
for p in $PRB_ROWS sources node primary server selfcheck kmwan; do
    B=$(bd "$(G "q=probe&name=$p")")
    case "$B" in *"$LEAKKEY"*) LEAKS="$LEAKS $p(key)" ;; esac
    [ -n "$LEAKHOST" ] || continue
    [ "$p" = server ] && continue
    case "$B" in *"$LEAKHOST"*) LEAKS="$LEAKS $p(host)" ;; esac
done
asrt "LEAK-1 no peer key in ANY probe output, and no endpoint host outside the one row that is FOR it" \
     "$LEAKS" ""
B=$(bd "$(G "q=probe&name=server")")
case "$B" in
    *"$LEAKHOST"*) ok "LEAK-1c the exception is real: \`server\` does print the host (so the skip above is not hiding a row)" ;;
    *)             no "LEAK-1c \`server\` no longer prints the host -- the LEAK-1 skip is now hiding a row for nothing" ;;
esac
# THE CONTROL: the sweep must be able to find something. The fixture strings are
# what it looks for, so feed it a body that really contains them.
LEAKCTL=""
case "peer $LEAKKEY at $LEAKHOST" in *"$LEAKKEY"*) LEAKCTL="key" ;; esac
case "peer $LEAKKEY at $LEAKHOST" in *"$LEAKHOST"*) LEAKCTL="$LEAKCTL host" ;; esac
asrt "LEAK-1b CONTROL: the sweep's own match really fires on both strings" "$LEAKCTL" "key host"

# PC-3c — READ-ONLY BY STATIC INSPECTION. PR-8 next door proves nothing MOVED in
# one world; this proves no mutating verb is even WRITTEN in the bodies, which
# is the half that survives a world the harness does not model. Scoped to the
# appended block, by its own marker, so it says nothing about the reconciler's
# legitimate action leaves elsewhere in the tree.
PRBSRC="$P5/lib/xctl-probe.sh"
PRBMARK='^# ================= READ-ONLY PROBE VERBS (U224)'
PRBFROM=$(grep -n "$PRBMARK" "$PRBSRC" | head -1 | cut -d: -f1)
# ABSENCE IS ITS OWN RED: a renamed marker would make every scan below read a
# zero-line file and pass.
if [ -n "$PRBFROM" ]; then ok "PC-3c the probe block is where its bar says it is (line $PRBFROM)"
else no "PC-3c the probe block marker is GONE -- every scan below would pass on nothing"; fi
# mutscan FILE FROM -> the forbidden tokens found on NON-COMMENT lines from FROM.
# THE TOOL NAME IS NOT IN THE PATTERN, and that is a measurement, not a
# preference: every external command in this tree is called through a variable
# ("$TC", "$IP", "$PING", resolved by bond-xctl's explicit PATH walk), so a
# pattern anchored on `tc qdisc replace` matches nothing a mutant here would
# actually write. The first cut of this bar was written that way and its own
# CONTROL below caught it -- the seeded `"$TC" qdisc replace` walked straight
# through. What is forbidden is the OPERATION, so that is what is matched.
# THE NATIVE GL SURFACES ARE IN THE PATTERN, and that is this fix round's whole
# point. The first cut listed the tools P5 drives (tc, wg, ip, the init.d
# services) and stopped there, so `uci set ... ; uci commit` -- writing GL's own
# config, the ONE thing the portal's GIVEN forbids outright ("coexist with the
# native GL.iNet surfaces -- read, show, never write them") -- was matched by
# nothing here, and the ecosim uci shim wrote no ledger line either, so PR-8 was
# blind to it as well. Measured in this round: a mutant `uci set`+`uci commit`
# at the foot of the probe block passed portal 158/0 AND ecosim 676/0. Both
# halves are closed -- the token below, and bin/uci's new `UCI ` ledger line
# that PR-8i counts. `ubus call <obj> set|start|...` is in for the same reason
# (ubus is the other write path onto the same surfaces), and the file verbs
# (mv/cp/ln/dd/chmod/chown/truncate/mkdir/rmdir/touch/kill, `sed -i`) are in
# because a probe needs none of them and `mv` over a fact file is a write no
# tool-name pattern would ever see. The redirect alternative is widened from
# $BOND_DIR to the other three roots these bodies can name ($RUN_DIR, $SYS_NET,
# $P5_STAMP), because "writes no fact" is not the claim -- "writes nothing" is.
mutscan() {
    sed -n "$2,\$p" "$1" | grep -vE '^[[:space:]]*#' \
      | grep -nE 'wg set|qdisc[[:space:]]+(add|del|change|replace)|route[[:space:]]+(add|del|replace)|link[[:space:]]+set|SVC"?[[:space:]]+(start|stop|restart|enable|disable)|rm[[:space:]]+-|>[[:space:]]*"?\$(BOND_DIR|RUN_DIR|SYS_NET|P5_STAMP)|uci[[:space:]]+(-[^[:space:]]+[[:space:]]+)*(set|delete|add|add_list|del_list|rename|reorder|revert|commit|batch|import)[[:space:]]|ubus[[:space:]]+call[[:space:]]+[^[:space:]]+[[:space:]]+(set|start|stop|restart|reload|delete|add)|(^|[[:space:];&|(])(mv|cp|ln|dd|chmod|chown|truncate|mkdir|rmdir|touch|kill|killall)[[:space:]]|sed[[:space:]]+-i' \
      | grep -c .
}
asrt "PC-3c no mutating verb is written in the probe bodies" "$(mutscan "$PRBSRC" "${PRBFROM:-1}")" 0
# THE CONTROL, on a COPY -- the shipped file is never touched. Seed A2's mutant
# is this exact edit, and it must be caught here as well as by PR-8.
PRBMUT="$WORK/mut-probe.sh"
cp "$PRBSRC" "$PRBMUT"
printf '%s\n' 'probe_seeded() { "$TC" qdisc replace dev "$1" root cake; }' >> "$PRBMUT"
if [ "$(mutscan "$PRBMUT" "${PRBFROM:-1}")" -ge 1 ]; then
    ok "PC-3c-CTL CONTROL: a seeded qdisc write in the block IS caught"
else
    no "PC-3c-CTL CONTROL: a seeded qdisc write was NOT caught -- PC-3c proves nothing"
fi
# THE SECOND CONTROL, and it is the one this fix round exists for: writes to a
# NATIVE GL surface and to plain files. Each construct is seeded on its OWN copy
# and reported by name, so a pattern that catches four of the five cannot hide
# behind the ones it does catch.
PRBM2CAUGHT=""; PRBM2MISS=""
for PRBM in 'probe_m1() { uci set network.wan.metric=9; uci commit network; }' \
            'probe_m2() { ubus call network.interface.wan set "{}"; }' \
            'probe_m3() { mv "$1" "$2"; }' \
            'probe_m4() { kill "$1"; }' \
            'probe_m5() { printf x > "$RUN_DIR/tput"; }'; do
    cp "$PRBSRC" "$PRBMUT"
    printf '%s\n' "$PRBM" >> "$PRBMUT"
    if [ "$(mutscan "$PRBMUT" "${PRBFROM:-1}")" -ge 1 ]
    then PRBM2CAUGHT="$PRBM2CAUGHT ${PRBM%%(*}"
    else PRBM2MISS="$PRBM2MISS ${PRBM%%(*}"; fi
done
asrt "PC-3c-CTL2 CONTROL: no seeded write to a native GL surface or to a file escapes the scan" \
     "$PRBM2MISS" ""
asrt "PC-3c-CTL2b ... and all five are caught individually, by name" "$PRBM2CAUGHT" \
     " probe_m1 probe_m2 probe_m3 probe_m4 probe_m5"
# ============================ PARAMETER CONSUMERS ============================
...
# ============================ THE LOG TAIL (U223) ============================
# The portal's only text/plain route, and the only one whose body is the BOX'S
# words rather than the portal's. Three things are being gated here and they are
# different in kind:
#
#  1. THE FILTER IS THE CATALOGUE'S, NOT THE REQUEST'S. Same rule as the probe
#     argv (INJ-1): the request supplies a NAME, the pattern comes from a shipped
#     file. LOG-M1's mutant is the naive version -- it uses the request bytes as
#     the pattern -- and it serves a tag nobody enumerated, which is what the
#     shipped code refuses with a 400.
#  2. THE BOX'S BYTES ARE NOT ESCAPED, AND THAT IS DELIBERATE. The ring is
#     streamed, so a line carrying markup arrives verbatim (LOG-7). What makes it
#     inert is the ROUTE (text/plain + nosniff + CSP none, LOG-6) plus the page's
#     .textContent -- so the headers are a security bar here, not a formality.
#  3. THE COMMAND LINE IS EXACTLY `logread`. It is on the portal's own boundary,
#     so it lands in PC-5's ledger, and LOG-8 re-applies PC-5's OWN allowlist to
#     it rather than a copy.
#
# THE SHIM. logread is wrapped on $WORK/portalbin like bondctl/bond-xctl/uci/ubus
# -- the portal's boundary -- so its argv is recorded in the SAME ledger. It
# prints $WORK/syslog, which is this section's fixture ring. Its own exit code is
# never what a bar reads: LOG-3 removes the shim to test the CGI's [ -x ] refusal
# and LOG-3b keeps a shim that exits 1 with no output, so "tool missing" (500)
# and "ring has nothing" (200) are told apart by the CGI, not by the shim.
logsetup() {
    setup
    cat > "$WORK/portalbin/logread" <<EOF
#!/bin/sh
if [ \$# -gt 0 ]; then echo "logread \$*" >> "$LEDGER"; else echo "logread" >> "$LEDGER"; fi
[ -r "$WORK/syslog" ] || exit 1
cat "$WORK/syslog"
EOF
    printf '#!/bin/sh\nexit 1\n' > "$WORK/portalbin/logread-empty"
    chmod +x "$WORK/portalbin/logread" "$WORK/portalbin/logread-empty"
    export LOGREAD="$WORK/portalbin/logread"
    logfix3
}

# THE 3-TAG FIXTURE: three P5 tags with a known line count each (bond-xctl 2,
# bond-ecod 2, p5-shape 1), PLUS one line from a tag that is not a catalogue row
# at all -- GL's dropbear, which is a named scrub key. That last line is what
# makes "only its tag's lines" an assertion instead of a tautology, and it is the
# line the mutant in LOG-M1 serves.
logfix3() {
    cat > "$WORK/syslog" <<'SYSEOF'
Fri Sep  5 10:00:01 2026 daemon.notice bond-xctl: eco: walked off to engaged
Fri Sep  5 10:00:02 2026 daemon.notice bond-ecod: primary bloated (base+100)
Fri Sep  5 10:00:03 2026 daemon.notice bond-xctl: eco: guard 'old_quiescent' refused
Fri Sep  5 10:00:04 2026 daemon.notice p5-shape: no tunnel interface discovered
Fri Sep  5 10:00:05 2026 daemon.notice bond-ecod: healthy 24 cycles, back to eco
Fri Sep  5 10:00:06 2026 authpriv.info dropbear[123]: password auth succeeded
SYSEOF
}
nlines() { printf '%s' "$1" | grep -c . ; }
hdrs()   { printf '%s\n' "$1" | awk '/^\r?$/{exit} {print}'; }

# runsh / logsig: the new lib functions run under EVERY POSIX shell on this
# machine. The CGI's own header says BUSYBOX-SAFE POSIX sh, and p5_regex_sane is
# written as a per-character `case` precisely because a bracket expression
# holding ] - [ and \ has four ordering rules that differ between shells -- so
# "it works in bash" is not the claim being made.
runsh() {   # $1 = interpreter token, rest -> sh -c ...
    _i="$1"; shift
    case "$_i" in
      busybox-ash) busybox ash -c "$@" ;;
      *)           "$_i"      -c "$@" ;;
    esac
}
logsig() {  # $1 = interpreter token -> one signature line over the three new fns
    runsh "$1" '
      . "$0"
      P5_CAT_DIR="$1"
      o=""
      for r in " bond-xctl: " " (p5-datapath|PSTAT )" "a[b]c" "" "a/b" "a*b" "a\$b" "a;b" "a>b" "a?b"; do
        if p5_regex_sane "$r"; then o="${o}1"; else o="${o}0"; fi
      done
      o="$o~$(p5_log_row datapath re)~$(p5_log_row deadman label)"
      o="$o~$(p5_hdr_text "200 OK" | tr -d "\r" | tr "\n" ";")"
      o="$o~$(if p5_regex_compiles "a[b"; then echo A; else echo r; fi)"
      o="$o$(if p5_regex_compiles " (p5-datapath|PSTAT )"; then echo a; else echo R; fi)"
      printf "%s\n" "$o"
    ' "$LIB_SRC" "$PORTAL/catalogue"
}
# logre / logrelab: read a shipped row THROUGH the shipped library, so a bar
# never restates a pattern this unit could have got wrong in both places.
logre()    { sh -c '. "$0"; P5_CAT_DIR="$1"; p5_log_row "$2" re'    "$LIB_SRC" "$PORTAL/catalogue" "$1"; }
logrelab() { sh -c '. "$0"; P5_CAT_DIR="$1"; p5_log_row "$2" label' "$LIB_SRC" "$PORTAL/catalogue" "$1"; }
# jlog NAME BODY -> that log row's label out of a q=catalogue reply
jlog() { printf '%s' "$2" | sed -n 's/.*{"name":"'"$1"'","label":"\([^"]*\)".*/\1/p' | head -1; }
# logmut NAME ROW [sed args] -> a mutant portal root: ROW appended to
# catalogue/logs (empty for none), then the seds applied to cgi and lib.
logmut() {
    _mn="$1"; _mrow="$2"; shift 2
    _m="$WORK/mut-$_mn"; rm -rf "$_m"; mkdir -p "$_m"
    cp -r "$PORTAL/cgi" "$PORTAL/lib" "$PORTAL/catalogue" "$_m/"
    [ -n "$_mrow" ] && printf '%s\n' "$_mrow" >> "$_m/catalogue/logs"
    if [ $# -gt 0 ]; then
        for _f in "$_m/cgi/p5-portal" "$_m/lib/portal-lib.sh"; do
            sed "$@" "$_f" > "$_f.new" && mv "$_f.new" "$_f"
        done
    fi
    printf '%s' "$_m"
...
# ---- LOG-1: each source returns ALL of its tag's lines and ONLY those ---------
logsetup
R=$(G "q=log&src=reconciler"); B=$(bd "$R")
asrt "LOG-1a reconciler -> 200"                       "$(st "$R")" 200
asrt "LOG-1b reconciler: both of its lines"           "$(printf '%s' "$B" | grep -c 'bond-xctl:')" 2
asrt "LOG-1c reconciler: and nothing else"            "$(nlines "$B")" 2
R=$(G "q=log&src=ecod"); B=$(bd "$R")
asrt "LOG-1d ecod: both of its lines"                 "$(printf '%s' "$B" | grep -c 'bond-ecod:')" 2
asrt "LOG-1e ecod: and nothing else"                  "$(nlines "$B")" 2
R=$(G "q=log&src=shaper"); B=$(bd "$R")
asrt "LOG-1f shaper: its one line, and nothing else"  "$(nlines "$B")" 1
# the closed set: every enumerated name answers, the un-enumerated dropbear line
# is served by NONE of them, and the five tags absent from the ring answer 200
# with an empty body (which is the "the ring does not go back that far" state).
LOGTOT=0; LOGBAD=""; LOGEMPTY=0
for n in $(sh -c '. "$0"; P5_CAT_DIR="$1"; p5_cat_keys logs' "$LIB_SRC" "$PORTAL/catalogue"); do
    R=$(G "q=log&src=$n"); B=$(bd "$R")
    [ "$(st "$R")" = 200 ] || LOGBAD="$LOGBAD $n($(st "$R"))"
    printf '%s' "$B" | grep -q 'dropbear' && LOGBAD="$LOGBAD $n(dropbear)"
    _c=$(nlines "$B"); LOGTOT=$((LOGTOT + _c))
    [ "$_c" = 0 ] && LOGEMPTY=$((LOGEMPTY + 1))
done
asrt "LOG-1g every enumerated source answers 200 and none serves the un-enumerated line" "$LOGBAD" ""
asrt "LOG-1h the five seeded lines are served exactly once across the whole set"         "$LOGTOT" 5
asrt "LOG-1i the five tags absent from the ring answer 200 with an empty body"           "$LOGEMPTY" 5

# ---- LOG-1j/k: the datapath row's ERE survives the field separator -----------
# catalogue/logs is name|regex|label and the datapath pattern CONTAINS a `|`.
# Read with a plain three-field split it would truncate to " (p5-datapath" --
# which still compiles, still matches, and silently stops finding PSTAT lines.
asrt "LOG-1j the datapath ERE round-trips through the field separator" \
     "$(logre datapath)" ' (p5-datapath|PSTAT )'
cat >> "$WORK/syslog" <<'SYSEOF'
Fri Sep  5 10:00:07 2026 daemon.notice p5-datapath: cap INERT
Fri Sep  5 10:00:08 2026 user.notice PSTAT n=42 sched=pull depth=3
SYSEOF
R=$(G "q=log&src=datapath"); B=$(bd "$R")
asrt "LOG-1k ... and the row matches BOTH the ident and the PSTAT prefix" "$(nlines "$B")" 2
# and the CGI's own inline split agrees with the library's rule: the label the
# catalogue route ships for that row is the label p5_log_row reads.
R=$(G "q=catalogue"); B=$(bd "$R")
asrt "LOG-1l q=catalogue's label for the datapath row is the library's label" \
     "$(jlog datapath "$B")" "$(logrelab datapath)"
hasnt "LOG-1m q=catalogue never ships the pattern to the browser" "$B" 'p5-datapath'

# ---- LOG-2: an unknown source is refused, and nothing runs -------------------
logsetup
: > "$LEDGER"
R=$(G "q=log&src=nosuch"); B=$(bd "$R")
asrt "LOG-2a unknown source -> 400"        "$(st "$R")" 400
asrt "LOG-2b ... named unknown_log"        "$(jf error "$B")" unknown_log
asrt "LOG-2c ... and logread never ran"    "$(grep -c '^logread' "$LEDGER")" 0
R=$(G "q=log")
asrt "LOG-2d a missing src is the same refusal, not a whole-ring tail" "$(st "$R")" 400

# ---- LOG-3: no logread on the box is a 500, and it is the CGI that says so ---
logsetup
LOGREAD="$WORK/portalbin/no-such-logread"; export LOGREAD
R=$(G "q=log&src=reconciler"); B=$(bd "$R")
asrt "LOG-3a no logread -> 500"        "$(st "$R")" 500
asrt "LOG-3b ... named tool_missing"   "$(jf error "$B")" tool_missing
# the discriminator: a shim that EXISTS and exits 1 with no output is the empty
# ring, not a missing tool. If LOG-3a were reading the shim's exit code instead
# of the CGI's [ -x ] test, this would be a 500 too.
LOGREAD="$WORK/portalbin/logread-empty"; export LOGREAD
R=$(G "q=log&src=reconciler")
asrt "LOG-3c a present tool that returns nothing is 200, not 500" "$(st "$R")" 200
asrt "LOG-3d ... with an empty body"                              "$(nlines "$(bd "$R")")" 0

# ---- LOG-4: the charset check, and busybox parity of the new lib functions ---
LOGSH=""
for s in sh dash bash; do command -v "$s" >/dev/null 2>&1 && LOGSH="$LOGSH $s"; done
command -v busybox >/dev/null 2>&1 && LOGSH="$LOGSH busybox-ash"
SIG0=""; SIGBAD=""; NSH=0
for s in $LOGSH; do
    v=$(logsig "$s")
    NSH=$((NSH + 1))
    if [ -z "$SIG0" ]; then SIG0="$v"; elif [ "$v" != "$SIG0" ]; then SIGBAD="$SIGBAD [$s]"; fi
done
# 1110000000: the two shipped patterns and a bracket expression are accepted; the
# empty string, a path, a quantifier, a `$`, a `;`, a `>` and a `?` are refused.
# Refusing `/` is what stops a row from naming a path; refusing the quantifiers
# is what stops one from being a backtracker over a ring of thousands of lines.
asrt "LOG-4a the charset accepts the shipped patterns and refuses metacharacters" \
     "${SIG0%%~*}" 1110000000
asrt "LOG-4b p5_regex_sane, p5_log_row and p5_hdr_text agree under every shell here" "$SIGBAD" ""
if [ "$NSH" -ge 2 ]; then ok "LOG-4c parity measured against $NSH interpreters ($LOGSH )"
else no "LOG-4c only $NSH interpreter present ($LOGSH ) -- 'busybox parity' was NOT measured here, and busybox ash is the target runtime"; fi

# ---- LOG-4d/e: the compile probe under BUSYBOX GREP, not just GNU grep -------
# LOG-4b above measures the new functions under every SHELL on this machine; this
# measures the compile probe against the other half of the runtime, the grep
# applet. It is not a formality. MEASURED here: `printf '' | grep -E -- 'a[b'`
# exits 2 under GNU grep 3.11 and exits 1 under busybox 1.36.1 -- and rc 1 is
# also "no match". A probe reading rc alone would REFUSE the broken row in CI and
# ACCEPT it on the box, which is the failure nothing here would ever see. So the
# probe reads stderr as well, and this bar runs it with a `grep` that IS busybox's.
if command -v busybox >/dev/null 2>&1; then
    mkdir -p "$WORK/bbbin"
    printf '#!/bin/sh\nexec busybox grep "$@"\n' > "$WORK/bbbin/grep"
    chmod +x "$WORK/bbbin/grep"
    BBSH='. "$0"; if p5_regex_compiles "$1"; then echo accepted; else echo refused; fi'
    BBBAD=$(PATH="$WORK/bbbin:$PATH" busybox ash -c "$BBSH" "$LIB_SRC" 'a[b')
    BBOK=$(PATH="$WORK/bbbin:$PATH"  busybox ash -c "$BBSH" "$LIB_SRC" ' (p5-datapath|PSTAT )')
    asrt "LOG-4d the compile probe refuses a[b under busybox grep too" "$BBBAD" refused
    asrt "LOG-4e ... and still accepts the shipped datapath ERE"       "$BBOK"  accepted
else
    no "LOG-4d/e busybox is absent -- the compile probe was NOT measured against the target runtime's grep, and the two greps do not agree on rc"
fi

# ---- LOG-5: a catalogue row that fails the charset is a 500, not a run -------
logsetup
: > "$LEDGER"
M=$(logmut badre 'evil|a/b$(id)|seeded: a row a catalogue edit could introduce' )
R=$(cgi "$M" GET "q=log&src=evil" ""); B=$(bd "$R")
asrt "LOG-5a a catalogue pattern outside the charset -> 500" "$(st "$R")" 500
asrt "LOG-5b ... named bad_log_regex"                        "$(jf error "$B")" bad_log_regex
asrt "LOG-5c ... and logread never ran"                      "$(grep -c '^logread' "$LEDGER")" 0
# the same mutant tree must still serve its untouched rows: the refusal is the
# ROW's, not the file's.
R=$(cgi "$M" GET "q=log&src=reconciler" "")
asrt "LOG-5d ... while the rows beside it still answer" "$(st "$R")" 200

# ---- LOG-5e..h: a row that PASSES the charset and is still not an ERE --------
# THE DEFECT THIS BAR EXISTS FOR. p5_regex_sane admits `[` and `(` without
# requiring their closers, so `a[b` is charset-legal. It is not an ERE. Before
# p5_regex_compiles the refusal happened INSIDE grep -- after p5_hdr_text had
# already written the 200 header -- so the route answered 200 WITH AN EMPTY BODY,
# which on this surface reads as "the ring holds no line carrying this tag". The
# request cannot supply the pattern (LOG-M1 covers that), so this is a
# SHIPPED-ROW defect class, not a leak; it is still the worst answer a debugging
# surface can give, because absence is the thing the card teaches you to read.
logsetup
: > "$LEDGER"
M=$(logmut nocompile 'broke|a[b|seeded: charset-legal and still not an ERE')
R=$(cgi "$M" GET "q=log&src=broke" ""); B=$(bd "$R")
asrt "LOG-5e a charset-legal row that does not COMPILE -> 500" "$(st "$R")" 500
asrt "LOG-5f ... named bad_log_regex"                          "$(jf error "$B")" bad_log_regex
asrt "LOG-5g ... and logread never ran"                        "$(grep -c '^logread' "$LEDGER")" 0
# THE CONTROL. Delete the probe CALL on a COPY -- the definition sits at column 0
# and the call is indented, so `^    p5_regex_compiles ` matches only the call --
# and the very same row must come back as the 200-empty this bar forbids. If it
# does not, LOG-5e is passing for some other reason, and this says so.
M=$(logmut nocompile2 'broke|a[b|seeded: charset-legal and still not an ERE' -e '/^    p5_regex_compiles /d')
if grep -q '^    p5_regex_compiles ' "$M/lib/portal-lib.sh"; then
    no "LOG-5h CONTROL: the probe call is still in the mutant -- LOG-5e proves nothing"
else
    R=$(cgi "$M" GET "q=log&src=broke" ""); B=$(bd "$R")
    if [ "$(st "$R")" = 200 ] && [ "$(nlines "$B")" = 0 ]; then
        ok "LOG-5h CONTROL: without the probe the same row is a 200 with an EMPTY body"
    else
        no "LOG-5h CONTROL: without the probe the row gave $(st "$R") and $(nlines "$B") lines, not the 200-empty this bar forbids"
    fi
fi

# ---- LOG-6: the text route's headers ARE the containment --------------------
logsetup
R=$(G "q=log&src=reconciler"); H=$(hdrs "$R")
has   "LOG-6a text/plain, charset declared" "$H" 'Content-Type: text/plain; charset=utf-8'
has   "LOG-6b nosniff"                      "$H" 'X-Content-Type-Options: nosniff'
has   "LOG-6c CSP default-src none"         "$H" "Content-Security-Policy: default-src 'none'"
has   "LOG-6d no-store"                     "$H" 'Cache-Control: no-store'
hasnt "LOG-6e and NOT application/json"     "$H" 'application/json'

# ---- LOG-7: a ring line carrying markup is served verbatim, as text ----------
# The escaper is NOT on this path and must not be: escaping the ring would mean
# running p5_json_str's per-character loop over every line and truncating at
# P5_MAX_EMIT. What makes the line inert is LOG-6's headers plus .textContent.
# So the assertion is both halves: the bytes are UNCHANGED, and the route that
# carries them is the text route.
cat >> "$WORK/syslog" <<'SYSEOF'
Fri Sep  5 10:00:09 2026 daemon.notice bond-xctl: peer said </script><img src=x onerror=alert(1)> & "quoted"
SYSEOF
R=$(G "q=log&src=reconciler"); B=$(bd "$R"); H=$(hdrs "$R")
has   "LOG-7a the script tag is served byte-for-byte"     "$B" '</script><img src=x onerror=alert(1)>'
has   "LOG-7b ... with its quotes and ampersand intact"   "$B" '& "quoted"'
# The needle is the SIX-BYTE literal p5_json_str emits for '<' (portal-lib.sh:198),
# so it has to be WRITTEN as a literal. $(printf '\u003c') is not. MEASURED on this
# machine: bash's printf understands \u and yields the 1-byte '<', while dash and
# busybox ash emit the six bytes unchanged. Under bash the bar therefore asserted
# "the body carries no '<'" -- which LOG-7a two lines up REQUIRES it to carry -- and
# went RED on correct code; under dash and busybox ash it asserted the right thing.
# A/B on this bar, this harness, three interpreters: PRE-FIX 330/3 under /bin/bash but
# 333/0 under /bin/dash and /bin/busybox ash; AS WRITTEN BELOW, 333/0 under all three.
# That split IS the mirror's red, because the two runners disagree about the shell:
# emulator-gate.yml:1091 runs this file with `bash`, while scripts/ci-local.sh:392 runs
# it with `sh`. So the local gate reproduces CI only on a host whose `sh` IS bash --
# it is one here (/home/bondci/bin/sh -> /bin/bash, first on PATH), which is why `sh`
# is not evidence of dash coverage for anyone reading this later.
# Single quotes are the same six bytes in every POSIX shell, and `hasnt` greps -F, so
# the backslash is matched literally rather than read as an escape.
JESC='\u003c'
hasnt "LOG-7c ... and the JSON escaper never touched it"  "$B" "$JESC"
# THE CONTROLS. A needle a body can NEVER carry makes LOG-7c green by construction --
# which is what the interpreted '<' did in reverse. 7e pins the needle's BYTES (1
# instead of 6 is exactly the defect above); 7f shows the predicate still FIRES on a
# body that HAS been escaped. printf %s does not process escapes in its argument, so
# both are shell-stable.
asrt "LOG-7e CONTROL: the needle is the 6-byte literal, not an interpreted '<'" "${#JESC}" 6
asrt "LOG-7f CONTROL: ... and LOG-7c's predicate DOES fire on an escaped body" "$(printf '%s' 'peer said \u003cscript' | grep -cF -- "$JESC")" 1
has   "LOG-7d ... on the text/plain route"                "$H" 'Content-Type: text/plain'

# ---- LOG-8: the ledger, against PC-5's OWN allowlist -------------------------
logsetup
: > "$LEDGER"
G "q=log&src=reconciler" >/dev/null
asrt "LOG-8a the log route's ledger entry is a BARE logread"     "$(grep -c '^logread$' "$LEDGER")" 1
asrt "LOG-8b ... and it is the only logread invocation"          "$(grep -c '^logread' "$LEDGER")" 1
asrt "LOG-8c the whole ledger is inside PC-5's allowlist"        "$(led_viol "$LEDGER")" ""
# THE CONTROL. PC-5's new row admits `logread` and nothing that looks like it: a
# logread carrying ANY argument is a violation, so the row cannot be read as
# "the portal may run logread however it likes".
echo "logread -f" >> "$LEDGER"
asrt "LOG-8d CONTROL: an argument-bearing logread is NOT in the allowlist" "$(led_viol "$LEDGER")" "logread -f"

# ---- LOG-9: a row whose pattern begins with a dash is a PATTERN -------------
# `--` before the pattern. Without it, `grep -E "-e bond-xctl"` clusters as -e
# with the option-argument " bond-xctl" and the row silently becomes a DIFFERENT
# filter. The seeded row is charset-legal, so nothing else refuses it.
logsetup
DASHROW='dash|-e bond-xctl|seeded: a row whose pattern begins with a dash'
M=$(logmut dashok "$DASHROW")
R=$(cgi "$M" GET "q=log&src=dash" ""); B=$(bd "$R")
asrt "LOG-9a a dash-leading pattern is a pattern -> 200" "$(st "$R")" 200
asrt "LOG-9b ... matching nothing in this ring"          "$(nlines "$B")" 0
# CONTROL: drop the `--` on a COPY and the very same row becomes a flag.
M=$(logmut dashbad "$DASHROW" -e 's#grep -E -- "$_re"#grep -E "$_re"#')
if grep -q 'grep -E "\$_re"' "$M/cgi/p5-portal"; then ok "LOG-9c CONTROL: the mutant really dropped the --"
else no "LOG-9c CONTROL: the mutation did not apply -- LOG-9 proves nothing"; fi
R=$(cgi "$M" GET "q=log&src=dash" ""); B=$(bd "$R")
asrt "LOG-9d CONTROL: without --, the row is read as a flag and matches 2 lines" \
     "$(printf '%s' "$B" | grep -c 'bond-xctl:')" 2

# ---- LOG-M1: the mutant pair -- the naive tail takes the request's pattern ---
# THE NAIVE IMPLEMENTATION, which is what this route would be if the catalogue
# were treated as documentation: skip the literal match, use the request's `src`
# as the pattern, drop the charset check. It is not remote code execution -- the
# pattern is an argv token, never a shell word -- it is a LEAK: the caller
# chooses what comes out of the box's ring, including the tags nobody enumerated
# and that the scrub keys exist for (dropbear, gl_ddns).
logsetup
: > "$LEDGER"
MU=$(logmut naive '' \
  -e 's#^    _n=$(p5_match_literal "$(p5_cat_keys logs)" "$_want") || p5_die "400 Bad Request" unknown_log#    _n="$_want"#' \
  -e 's#^    _re=$(p5_log_row "$_n" re)      || p5_die "500 Internal Server Error" bad_log_row#    _re="$_want"#' \
  -e 's#^    p5_regex_sane "$_re"            || p5_die "500 Internal Server Error" bad_log_regex#    :#')
if grep -q '_re="\$_want"' "$MU/cgi/p5-portal"; then ok "LOG-M1a the mutant really builds the argv from p5_arg src"
else no "LOG-M1a the mutation did not apply -- LOG-M1 proves nothing"; fi
R=$(cgi "$MU" GET "q=log&src=dropbear" ""); B=$(bd "$R")
asrt "LOG-M1b MUTANT: a request-supplied pattern reaches grep -> 200" "$(st "$R")" 200
has  "LOG-M1c MUTANT: and serves a tag no catalogue row enumerates"   "$B" 'dropbear'
: > "$LEDGER"          # the mutant's own logread line is not the subject below
R=$(G "q=log&src=dropbear"); B=$(bd "$R")
asrt "LOG-M1d SHIPPED: the same request is refused"                   "$(st "$R")" 400
asrt "LOG-M1e SHIPPED: ... as unknown_log"                            "$(jf error "$B")" unknown_log
asrt "LOG-M1f SHIPPED: ... and logread was never run for it"          "$(grep -c '^logread$' "$LEDGER")" 0

# ---- LOG-10: the page binds the card, and adds no second data path ----------
# U222 fixed the ids; this unit fills them. The card is driven by the CATALOGUE
# (renderLogs returns early when cat.logs is empty), so the page still enumerates
# no source name of its own -- and the text body reuses the ONE req() helper
# rather than opening a second fetch with its own headers and session rule.
for _i in logsrc logs logout; do
    if grep -q "getElementById('$_i')" "$PORTAL/www/portal.js"; then ok "LOG-10a the page binds #$_i"
    else no "LOG-10a the page does not bind #$_i -- the card would stay empty and silent"; fi
done
asrt "LOG-10b still exactly one fetch site in the page" \
     "$(grep -c 'fetch(' "$PORTAL/www/portal.js")" 1
if grep -q "q=log&src=" "$PORTAL/www/portal.js"; then ok "LOG-10c the card reads the log route"
else no "LOG-10c nothing in the page requests q=log"; fi
# N-GENERIC, the same rule the mode list has: the picker is built from the
# CATALOGUE, so the page must never name a source in a request it builds. The
# assertion is on the REQUEST and not on the word: several of these names are
# also ordinary English the page prints for other cards ("the datapath stats
# reader is not built"), and a bar that forbade the word would have to be
# weakened one exception at a time until it meant nothing. What cannot appear is
# `q=log&src=<a literal name>` -- the page concatenates the picker's value.
LOGNAMES=$(sh -c '. "$0"; P5_CAT_DIR="$1"; p5_cat_keys logs' "$LIB_SRC" "$PORTAL/catalogue")
LOGHARD=""
for n in $LOGNAMES; do
    grep -q "q=log&src=$n" "$PORTAL/www/portal.js" && LOGHARD="$LOGHARD $n"
done
asrt "LOG-10d the page builds no log request naming a source" "$LOGHARD" ""
if grep -q "catRows('logs')" "$PORTAL/www/portal.js"; then ok "LOG-10e ... because the picker comes from the catalogue"
else no "LOG-10e the card is not driven by the catalogue -- add a row on the box and nothing appears"; fi

# ---- LOG-11: the record says the tail is built, and the shipped file is placed
# The README's deferral reason ("every path would have to be enumerated") is
# WITHDRAWN, and the catalogue is a package row -- E0's PK-4 would name a file
# under deploy/ with no account, and IN-15 a placed file the contract does not
# declare. Asserted here too so the portal gate alone catches a half-landed unit.
if grep -q 'Deferred rather than half-guarded' "$PORTAL/README.md"; then
    no "LOG-11a the README still defers the log tail while the route ships"
else ok "LOG-11a the README's log-tail deferral is withdrawn"; fi
asrt "LOG-11b the catalogue has a filemap row" \
     "$(grep -c '^644|client|deploy/p5/portal/catalogue/logs|/usr/lib/p5/portal/catalogue/logs$' "$REPO/p5/payload/filemap")" 1
asrt "LOG-11c ... and a contract row at that destination" \
     "$(grep -c '^file|client|/usr/lib/p5/portal/catalogue/logs ' "$REPO/p5/contract/paths")" 1
# THE TEST RUNNER SECTION IS LAST ON PURPOSE (merge of U223 into U228's dev).
# It is the only section that leaves DETACHED processes behind -- it spawns jobs
# that outlive their reply, kills process groups and drives a deadman -- so
# nothing else runs after it, exactly as on `dev` where it was written and
# measured. The log tail above is request/response only and leaves nothing
# running, so its position ahead of this one changes neither section's subject.
...
EOF
    ...
    # --no-boot-limb is appended HERE, next to the P5_ROOT and P5_CRONTAB this
    # wrapper already injects, and for the same reason (U290): `arm` now refuses
    # unless crond is running and the tool is at its installed path, and neither
    # is true inside $WORK. It is appended AFTER the ledger line above, so the
    # ledger still records the argv the CGI actually issued -- which is what
    # RUN-11 and the argv regex read.
...
asrt "RUN-C1 every test head is in the DESIGN's closed symbol set (U228 ships ACCEPT/XCTL_TICK; ROUNDTRIP_LIFECYCLE/ROUNDTRIP_MODE/E1 are reserved for U229/U230 -- RUN-C1b counts the two shipped heads)" "$BADH" ""
RUNHEADS=$(grep -cE '^  (ACCEPT|XCTL_TICK)\)' "$RUNNER_SRC")
if [ "$RUNHEADS" -ge 2 ]; then ok "RUN-C1b the runner's own case names $RUNHEADS heads, so the set above is not a fiction"
else no "RUN-C1b the runner's head case could not be read -- RUN-C1 is comparing against nothing"; fi
# RUN-C2 "keep the disruptive list short", mechanised: the set of disruptive rows
# in the shipped catalogue must equal the LITERAL written here. It is EMPTY on
# this unit's branch; U230 added `e1` by editing the line below, and U229's
# lifecycle_roundtrip and mode_roundtrip have to do the same.
DIS_WANT='e1'
...
if [ "$DIS_N" = 0 ]; then
    ...
    # THE BOUND MAY BE SYMBOLIC, AND ONLY IN THE WAY COLUMN 3 ALREADY IS (U230).
    # e1's deadline is N x (SECS + ping waits) + the sink probe's own --max-time,
    # where N is how many sources the box routes over RIGHT NOW: a number in this
    # column would be a second copy of that arithmetic, drifting from the
    # instrument's. So `PLAN` is admitted here -- and RUN-C3b makes it cost
    # something, by requiring the runner to resolve it and to REFUSE to arm
    # without a computed number. Every other row still declares digits.
    DIS_BAD=$(grep -v '^[[:space:]]*#' "$TESTS_CAT" \
              | awk -F'|' '$2=="disruptive" && ($4 !~ /^([0-9]+|PLAN)$/ || length($5) < 40){print $1}')
    asrt "RUN-C3 every disruptive row declares a bound (digits, or the symbolic PLAN) and a label that describes the disturbance" "$DIS_BAD" ""
    DIS_PLAN=$(grep -v '^[[:space:]]*#' "$TESTS_CAT" | awk -F'|' '$4=="PLAN"{printf "%s ",$1}')
    DIS_PLAN=${DIS_PLAN% }
    if [ -n "$DIS_PLAN" ]; then
        RC3A=$(grep -c 'BOUND=$(printf' "$RUNNER_SRC")
        RC3B=$(grep -c 'deadman=no_plan' "$RUNNER_SRC")
        if [ "$RC3A" -ge 1 ] && [ "$RC3B" -ge 1 ]; then
            ok "RUN-C3b the symbolic bound is RESOLVED by the runner ($DIS_PLAN) and a row whose plan yields no number is refused before anything is armed"
        else
            no "RUN-C3b a row declares PLAN but the runner neither computes a bound ($RC3A) nor refuses without one ($RC3B) -- the symbol would arm nothing"
        fi
    fi
fi
...
# U230 landed the E1 branch, so this bar has a subject at last: the file may
# UNPIN (`del`), put a captured PRIOR line back (`add`), and READ (`show`) to say
# whether the pin actually went. Nothing else -- no replace, no flush, no rule.
RSIP=$(grep -n '^[^#]*ip route' "$RESTORE_SRC" | grep -vcE 'ip route (del|add|show)')
asrt "RS-STATIC-2 every ip-route site in the restore script is a del, an add or a show" "$RSIP" 0
RSIPW=$(grep -cE '^[^#]*ip route (del|add) ' "$RESTORE_SRC")
if [ "$RSIPW" -ge 2 ]; then ok "RS-STATIC-2b ... and there are $RSIPW route writes, so RS-STATIC-2 is not passing on an empty set"
else no "RS-STATIC-2b the restore script issues $RSIPW route writes -- the E1 heal cannot work"; fi
...
#
# U230 adds bin/p5-e1-probe to the list. It ran under the CLIENT's own ash on
# 2026-08-30, which is the strongest evidence in this file about any of them --
# but that was a different revision of it, and this unit changed its argv, its
# scratch paths and three of its output lines. The claim its header makes is
# checked here, at the revision that ships.
for _bbsh in busybox dash; do
    for _bbf in "$CGI_SRC" "$LIB_SRC" "$PORTAL/bin/p5-portal-run" "$PORTAL/bin/p5-portal-restore"                 "$PORTAL/bin/p5-e1-probe"; do
        _bbn=${_bbf##*/}
...
# ============================ E1 EDGE-vs-MID (U230) ==========================
# THE ONE DISRUPTIVE ROW THAT SHIPS, and the only thing in this product that
# writes a route. Everything below drives the REAL instrument
# (deploy/p5/portal/bin/p5-e1-probe, the file that ran on the client on
# 2026-08-30) through the REAL CGI and the REAL runner, against shims that
# RECORD rather than pretend: the ecosim `ip` shim keeps a pin table and logs
# `route replace|del|add`, and curl/ss/tc/ping below make a link that can be
# made to look like an edge or like a mid.
#
# WHY THE ORDER BARS ARE THE POINT. Two orderings decide whether a box with no
# console survives this test:
#   ARM BEFORE LOAD   (E1-3) -- if the pin goes down first there is a window in
#                     which traffic is steered and nothing owes a rollback.
#   KILL BEFORE UNPIN (E1-4) -- if the route is removed while curl is still
#                     running, a saturating upload moves onto whatever route
#                     wins next, which on this box is the tunnel.
# Both are asserted from ONE ledger file, because an order across two files is
# not an order. The `logger`, `curl` and `ip` shims all append to $WORK/ledger,
# and the deadman wrapper is rewritten here to append to it as well.
PROBE_SRC="$PORTAL/bin/p5-e1-probe"
E1_LEDGER="$WORK/ledger"

e1_shims() {
    mkdir -p "$WORK/e1bin"
    # curl: the sink probe answers one fixture address; the load drains the pipe,
    # holds `e1_loading` up for a REAL second and answers curl's own -w fields.
    cat > "$WORK/e1bin/curl" <<EOF
#!/bin/sh
S="\$ECOSIM_STATE"
echo "CURL \$*" >> "\$S/ledger"
case "\$*" in
  *__down*)
    [ "\$(cat "\$S/e1_sink_fail" 2>/dev/null)" = 1 ] && exit 1
    printf '203.0.113.77'; exit 0 ;;
esac
: > "\$S/e1_loading"
# The pid the instrument holds as \$LOADPID is this shell (the last stage of its
# pipeline). Publishing it is what lets the ping shim end the load at a CHOSEN
# sample instead of at a wall-clock moment -- see e1_straddle_at below.
echo \$\$ > "\$S/e1_load_pid"
cat > /dev/null 2>&1
L=\$(cat "\$S/e1_load_s" 2>/dev/null || echo 1)
if [ "\$(cat "\$S/e1_straddle_at" 2>/dev/null || echo 0)" = 0 ]; then
  /bin/sleep "\$L"                       # the ordinary fixture: one wall-clock window
else
  # Straddle fixture only: sleep in slices so the ping shim can stop the load
  # between two of them. Exiting NORMALLY (rather than being signalled) matters --
  # curl's -w line is how the instrument learns bytes were uploaded, and a load
  # killed mid-flight would read as NO LOAD and test the wrong rule.
  #
  # THE CAP BELOW IS A SAFETY NET, NOT THE LOAD WINDOW, and that distinction is the
  # whole point of this arm. In straddle mode the load must end at the CHOSEN sample
  # and at no other time. Capping it at \$L seconds instead would put a wall clock
  # back in the race: a box too slow to reach that sample inside the window ends the
  # load with no straddle at all, STRADDLED stays 0 and E1-7e goes RED for scheduling
  # reasons -- the exact flake class this fixture was written to remove, reintroduced
  # one level down. 300 x 0.05 = 15 s is far beyond any sampling rate this suite has
  # measured (9 samples/s on a bare box, and the straddle is at sample 8), so the
  # ping shim always wins the race; the cap exists only so a probe that died cannot
  # leave this shell sleeping forever.
  i=0
  while [ ! -f "\$S/e1_stop_load" ] && [ "\$i" -lt 300 ]; do
    /bin/sleep 0.05; i=\$((i + 1))
  done
fi
rm -f "\$S/e1_loading"
printf '%s 4000000 200' "\$(cat "\$S/e1_up_bytes" 2>/dev/null || echo 10485760)"
EOF
    # ss -tan: Send-Q is field 3, as it is in the real one. Non-zero only while
    # the load is up -- and it is REPORT ONLY in the instrument, never a verdict.
    cat > "$WORK/e1bin/ss" <<EOF
#!/bin/sh
S="\$ECOSIM_STATE"
if [ -f "\$S/e1_loading" ]; then Q=4096; else Q=0; fi
echo "State Recv-Q Send-Q Local Peer"
echo "ESTAB 0 \$Q 198.51.100.11:44444 203.0.113.77:443"
echo "ESTAB 0 \$Q 198.51.100.12:44444 203.0.113.77:443"
EOF
    # tc: the qdisc KIND and the backlog are knobs, because those two are the
    # EDGE limb -- and `noqueue` is what blinds it.
    cat > "$WORK/e1bin/tc" <<EOF
#!/bin/sh
S="\$ECOSIM_STATE"
K=\$(cat "\$S/e1_qdisc" 2>/dev/null || echo fq_codel)
IF=""; prev=""
for a in "\$@"; do [ "\$prev" = dev ] && IF="\$a"; prev="\$a"; done
echo "qdisc \$K 0: dev \$IF root refcnt 2"
if [ "\$1" = "-s" ]; then
  B=\$(cat "\$S/e1_backlog" 2>/dev/null || echo 0)
  echo " Sent 0 bytes 0 pkt (dropped 0, overlimits 0 requeues 0)"
  echo " backlog \${B}b 0p requeues 0"
fi
EOF
    # ping: idle and loaded RTT are two knobs, and which one answers depends on
    # whether the load is up RIGHT NOW. That is what makes E1-7's MID fixture a
    # measurement of the instrument's rule rather than of a constant.
    cat > "$WORK/e1bin/ping" <<EOF
#!/bin/sh
S="\$ECOSIM_STATE"
[ "\$(cat "\$S/e1_ping_dead" 2>/dev/null)" = 1 ] && exit 1
if [ -f "\$S/e1_loading" ]; then
  T=\$(cat "\$S/e1_rtt_load" 2>/dev/null || echo 90.0)
  # e1_straddle_at=N: end the load DURING the Nth loaded sample, at the instant
  # this ping is answered. The instrument has already passed its \`kill -0\` for
  # this iteration, so what it gets back is an IDLE rtt on a sample it believes
  # is loaded -- the race that makes a true MID read INCONCLUSIVE. Counting by
  # loaded pings rather than by seconds is what makes it deterministic: it does
  # not depend on how fast this box forks, so the bar measures the instrument.
  # (The baseline pings run before e1_loading exists and are not counted.)
  N=\$(cat "\$S/e1_straddle_at" 2>/dev/null || echo 0)
  if [ "\$N" != 0 ]; then
    C=\$(( \$(cat "\$S/e1_ping_n" 2>/dev/null || echo 0) + 1 ))
    echo "\$C" > "\$S/e1_ping_n"
    if [ "\$C" = "\$N" ]; then
      T=\$(cat "\$S/e1_rtt_idle" 2>/dev/null || echo 20.0)
      rm -f "\$S/e1_loading"
      : > "\$S/e1_stop_load"
      # Do not answer until the load process is actually GONE. That is what makes
      # the straddle exact rather than another race: the instrument re-checks
      # \`kill -0\` the moment this ping returns, and it must find the load ended.
      # Bounded, so a stale pid file cannot hang the suite.
      P=\$(cat "\$S/e1_load_pid" 2>/dev/null || echo 0)
      w=0
      while [ "\$P" != 0 ] && kill -0 "\$P" 2>/dev/null && [ "\$w" -lt 100 ]; do
        /bin/sleep 0.05; w=\$((w + 1))
      done
    fi
  fi
else T=\$(cat "\$S/e1_rtt_idle" 2>/dev/null || echo 20.0); fi
echo "64 bytes from x: seq=0 ttl=54 time=\$T ms"
EOF
    # logger: the same ledger the route writes land in, so ORDER is readable.
    cat > "$WORK/e1bin/logger" <<EOF
#!/bin/sh
shift 2 2>/dev/null
echo "LOG \$*" >> "\$ECOSIM_STATE/ledger"
EOF
    # sleep: 0.1 s of REAL time, not the no-op shim. The instrument samples once
    # per sleep, so a no-op turns a 1 s load into thousands of forks; and a real
    # `sleep \$SECS` would make this section take minutes. The deadman's timer
    # therefore fires early here, which is exactly what E1-4 needs and what
    # --no-timer (P5_PORTAL_DM_TIMER=0) suppresses for the success path.
    printf '#!/bin/sh\nexec /bin/sleep 0.1\n' > "$WORK/e1bin/sleep"
    chmod +x "$WORK/e1bin/curl" "$WORK/e1bin/ss" "$WORK/e1bin/tc" \
             "$WORK/e1bin/ping" "$WORK/e1bin/logger" "$WORK/e1bin/sleep"
}
...
e1_world() {
    ...
    printf 'fq_codel\n' > "$WORK/e1_qdisc"
    printf '0\n' > "$WORK/e1_backlog"
    rm -f "$WORK/e1_pins" "$WORK/e1_loading" "$WORK/e1_ping_dead" "$WORK/e1_sink_fail" \
          "$WORK/e1_straddle_at" "$WORK/e1_ping_n" "$WORK/e1_load_pid" \
          "$WORK/e1_stop_load"
    # THE METERED FACT, so the confirm text has something true to name. The
    # fixture box's cellular device is `absent`, so without this line no ROUTED
    # source is metered and a bar looking for `usb0` in the text would pass on
    # the routed list instead -- green for the wrong reason.
    printf 'usb0\n' > "$BOND_DIR/metered"
    export P5_E1_PROBE="$WORK/portalbin/p5-e1-probe"
    export P5_DEADDIR="$WORK/dm/etc/p5/deadman"
    export LOGGER="$WORK/e1bin/logger"
    export PATH="$WORK/e1bin:$PATH"
    # The probe wrapper: records the argv the CGI and the runner issue, and hands
    # the instrument a box that says MT6000 without writing to this host's /tmp.
    cat > "$WORK/portalbin/p5-e1-probe" <<EOF
#!/bin/sh
echo "p5-e1-probe \$*" >> "$LEDGER"
P5_SYSINFO_MODEL="$WORK/e1model"; export P5_SYSINFO_MODEL
RUN_DIR="$WORK/run"; export RUN_DIR
exec sh "$PROBE_SRC" "\$@"
EOF
    # The deadman wrapper again, this time logging into the ORDER ledger too.
    cat > "$WORK/portalbin/p5-deadman" <<EOF
#!/bin/sh
echo "p5-deadman \$*" >> "$LEDGER"
echo "p5-deadman \$*" >> "$E1_LEDGER"
P5_ROOT="$WORK/dm"; export P5_ROOT
P5_CRONTAB="$WORK/dm/etc/crontabs/root"; export P5_CRONTAB
exec sh "$DM_SRC" "\$@" --no-boot-limb
EOF
    chmod +x "$WORK/portalbin/p5-e1-probe" "$WORK/portalbin/p5-deadman"
    printf '10 20\n' > "$P5_STATE_DIR/e1_budget"
}
e1_budget_rm() { rm -f "$P5_STATE_DIR/e1_budget"; }
# A probe that RUNS but cannot PLAN. Not a deleted file: the instrument is there,
# the argv still reaches the ledger, and only `--plan` comes back empty -- which
# is the shape a parse change or a half-broken instrument actually takes, and the
# one the CGI's `${_bound:-?}` used to paper over.
e1_noplan() {
    cat > "$WORK/portalbin/p5-e1-probe" <<EOF
#!/bin/sh
echo "p5-e1-probe \$*" >> "$LEDGER"
case "\$1" in --plan) exit 0 ;; esac
P5_SYSINFO_MODEL="$WORK/e1model"; export P5_SYSINFO_MODEL
RUN_DIR="$WORK/run"; export RUN_DIR
exec sh "$PROBE_SRC" "\$@"
EOF
    chmod +x "$WORK/portalbin/p5-e1-probe"
}

# --- E1-STATIC: what the two route-writing files may say ---------------------
# p5-e1-probe is PC-2-exempt BY NAME (PC-2d), so the exemption is paid for here:
# every ip-route site in it is one of exactly three shapes, and the runner --
# which is NOT exempt -- may only READ.
# `say`/`echo` lines are EXCLUDED and that is not a loophole: the lock refusal
# prints the exact heal commands for an operator to run by hand, and a bar that
# counted printed text as a call would force the instrument to stop telling
# somebody how to clean up after a kill -9. E1-STATIC-1c pins that message.
# COUNTED BY OCCURRENCE, like PC-2e (U230 fix round): the old form asked whether
# a LINE matched an admitted shape, so `ip route del "$SINK/32"; ip route flush
# cache` was admitted whole on the strength of its first half. Now every `ip
# route` occurrence on an eligible line is counted and every ADMITTED occurrence
# is subtracted; anything left is a site the exemption does not pay for.
e1_probe_body() {   # the instrument's eligible lines: no comments, no say/echo
    grep -v '^[[:space:]]*#' "$PROBE_SRC" | grep -vE '^[[:space:]]*(say|echo) '
}
E1_IPALL=$(e1_probe_body | grep -o -F 'ip route' | grep -c .)
E1_IPOK=$(e1_probe_body | grep -oE 'ip route (replace "\$SINK/32" (via "\$GW" )?dev "\$IF"|del "\$SINK/32"|add \$RT|show)' | grep -c .)
asrt "E1-STATIC-1 every ip-route site in the instrument pins, unpins, re-adds a captured PRIOR line, or reads (unaccounted: $((E1_IPALL - E1_IPOK)) of $E1_IPALL)" "$((E1_IPALL - E1_IPOK))" 0
E1_IPN=$(e1_probe_body | grep -oE 'ip route (replace|del|add) ' | grep -c .)
if [ "$E1_IPN" -ge 3 ]; then ok "E1-STATIC-1b ... and there are $E1_IPN such writes, so E1-STATIC-1 is not passing on an empty set"
else no "E1-STATIC-1b the instrument issues $E1_IPN route writes -- E1-STATIC-1 has almost no subject"; fi
has "E1-STATIC-1c the lock refusal still prints the by-hand heal for a kill -9" \
    "$(grep 'say ' "$PROBE_SRC")" 'ip route del $SINK/32'
E1_TMP=$(grep -n '/tmp' "$PROBE_SRC" | grep -vcE '^[0-9]+:[[:space:]]*#')
asrt "E1-STATIC-2 the instrument's only /tmp mention is the vendor model file's default" "$E1_TMP" 1
has "E1-STATIC-2b ... and it is P5_SYSINFO_MODEL, a READ of OpenWrt's own identity file" \
    "$(grep '/tmp' "$PROBE_SRC" | grep -v '^[[:space:]]*#')" 'P5_SYSINFO_MODEL='
# E1-STATIC-3 by OCCURRENCE too, and this is the site the fix round was FOR: the
# runner's residue read at p5-portal-run's `### residue` block is an admitted
# `ip route show`, and the old line-granular form excluded that whole LINE --
# so `ip route show ...; ip route append <net> dev <if>` was invisible to this
# bar, to PC-2's enumerated verbs (no `append` in the list) and to PC-2e/f alike.
E1_RUNALL=$(grep -v '^[[:space:]]*#' "$RUNNER_SRC" | grep -o -F 'ip route' | grep -c .)
E1_RUNSHOW=$(grep -v '^[[:space:]]*#' "$RUNNER_SRC" | grep -o -F 'ip route show' | grep -c .)
asrt "E1-STATIC-3 the runner's only ip-route site is a READ (show) -- putting a route back is the exempt restore script's job (non-show occurrences: $((E1_RUNALL - E1_RUNSHOW)))" "$((E1_RUNALL - E1_RUNSHOW))" 0
asrt "E1-STATIC-3b ... and there is exactly one such read, so E1-STATIC-3 is not passing on an empty file" "$E1_RUNSHOW" 1
E1_SVC=$(grep -ncE '/etc/init.d/|uci (set|commit)|wg set|sqm' "$PROBE_SRC")
asrt "E1-STATIC-4 the instrument starts no service, writes no uci, touches no shaper" "$E1_SVC" 0

# --- E1-6: the two refusals the instrument has always carried -----------------
# STATIC, because both are about what it does on a box this harness cannot be.
has "E1-6 the tunnel refusal arm survived the move" \
    "$(grep -A2 'wg\*|\*wgclient\*|tun\*|engarde\*' "$PROBE_SRC")" 'REFUSED:'
has "E1-6a the not-the-client refusal survived the move" \
    "$(cat "$PROBE_SRC")" 'NOT THE CLIENT -- REFUSING'
setup; e1_world
env P5_SYSINFO_MODEL="$WORK/model" sh "$PROBE_SRC" 10 20 >/dev/null 2>&1; E1_REFRC=$?
asrt "E1-6b a box whose model is not the client is refused, exit 2" "$E1_REFRC" 2
hasnt "E1-6c ... before any route write" "$(cat "$E1_LEDGER")" 'IP route'

# --- E1-1: no budget, no run, and nothing touched ----------------------------
setup; e1_world
e1_budget_rm
: > "$LEDGER"; : > "$E1_LEDGER"
R=$(P 'k=run&name=e1'); B=$(bd "$R")
asrt "E1-1 with no operator budget the row is refused"     "$(st "$R")" 409
asrt "E1-1a ... as no_budget, not as a generic error"      "$(jf error "$B")" no_budget
has  "E1-1b ... naming the file the operator has to write" "$B" 'e1_budget'
hasnt "E1-1c ... and no route was written"                 "$(cat "$E1_LEDGER")" 'IP route'
hasnt "E1-1d ... and no load was started"                  "$(cat "$E1_LEDGER")" 'CURL'
if [ -d "$WORK/run/portal.job" ]; then no "E1-1e a refused row still took the job lock"
else ok "E1-1e ... and the job lock was never taken"; fi
# AND THE INSTRUMENT ITSELF, not just the CGI in front of it. The refusal above
# is the portal's; this is the file's own, and it is the one that matters if the
# instrument is ever run by hand or by a future caller. A default budget here
# would put MiB on somebody's data plan that nobody declared -- which is exactly
# what the two removed `${MB_PER_LINK:-20}` / `${SECS:-20}` lines used to do.
: > "$E1_LEDGER"
env P5_SYSINFO_MODEL="$WORK/e1model" RUN_DIR="$WORK/run" sh "$PROBE_SRC" >/dev/null 2>&1; E1_NB=$?
asrt "E1-1f the instrument itself refuses an empty budget, exit 7" "$E1_NB" 7
hasnt "E1-1g ... having started no load"  "$(cat "$E1_LEDGER")" 'CURL'
hasnt "E1-1h ... and written no route"    "$(cat "$E1_LEDGER")" 'IP route'
# AND A BUDGET IS NOT ENOUGH: the bound has to EXIST. `Bounded at ?s` was a real
# reachable string -- a probe that cannot plan (missing, unreadable, a shape this
# CGI cannot parse) produced an empty $_bound, the confirm text said `?s`, and a
# POST carrying `confirm=e1` in the SAME request sailed past the confirmation
# into a 202. The row is only confirmable because it is bounded, so a planless
# plan is now a refusal in front of the lock, not a question mark inside it.
setup; e1_world; e1_noplan
: > "$LEDGER"; : > "$E1_LEDGER"
R=$(P 'k=run&name=e1&confirm=e1'); B=$(bd "$R")
asrt "E1-1i a probe whose --plan yields no number is refused even WITH the confirmation" "$(st "$R")" 409
asrt "E1-1j ... as no_plan, not as a generic error"        "$(jf error "$B")" no_plan
hasnt "E1-1k ... and no operator is ever shown a '?s' bound" "$B" 'Bounded at ?'
hasnt "E1-1l ... and no load was started"                  "$(cat "$E1_LEDGER")" 'CURL'
hasnt "E1-1m ... and no route was written"                 "$(cat "$E1_LEDGER")" 'IP route'
if [ -d "$WORK/run/portal.job" ]; then no "E1-1n a planless row still took the job lock"
else ok "E1-1n ... and the job lock was never taken"; fi

# --- E1-2: the confirm text is the BOX's, computed from its own source table --
setup; e1_world
: > "$LEDGER"
R=$(P 'k=run&name=e1'); B=$(bd "$R")
asrt "E1-2 a disruptive row without the confirmation is refused" "$(st "$R")" 409
asrt "E1-2a ... as confirm_required"                            "$(jf error "$B")" confirm_required
E1TXT=$(jf text "$B")
has "E1-2b ... and the text names the metered source out of the box's own table" "$E1TXT" "METERED, i.e. somebody's data plan: [usb0]"
has "E1-2c ... and the total cost, budget x the DEDUPED routed device count"   "$E1TXT" '20 MiB in total'
has "E1-2c2 ... over the two devices, not the three interface rows (wan and wan6 are one eth1)" "$E1TXT" 'each of 2 routed source(s)'
has "E1-2d ... and what the disturbance actually is"                                      "$E1TXT" 'anycast'
asrt "E1-2e ... and the bound is the instrument's own --plan number"  "$(jf bound_s "$B")" 71
hasnt "E1-2f ... and nothing ran"                                    "$(cat "$E1_LEDGER")" 'CURL'
# TEETH: a CGI that takes any confirmation at all.
M=$(mutate e1conf -e 's#^        if \[ "$_c" != "$_tn" \]; then#        if [ "$_c" = "" ]; then#')
if grep -q 'if \[ "$_c" = "" \]; then' "$M/cgi/p5-portal"; then
    ok "E1-2g the mutant CGI really accepts any confirmation"
else
    no "E1-2g the mutation did not apply -- E1-2h proves nothing"
fi
MR=$(cgi "$M" POST "" 'k=run&name=e1&confirm=yes')
asrt "E1-2h MUTANT: confirm=yes starts the row (202), which is what E1-2 refuses" "$(st "$MR")" 202
# AND THEN WAIT FOR IT. The mutant really started a DETACHED job against this
# world, so it goes on pinning and unpinning into the ledger the next scenario
# resets -- which is how E1-5c first went red on lines that belonged to this run.
MRID=$(jf id "$(bd "$MR")")
[ -n "$MRID" ] && await "$WORK/results/$MRID.txt" 600
job_cleanup

# --- E1-B: the bound recomputed from the instrument's OWN literals ------------
# NOT re-typed here: the three terms are read out of the file, so a change to any
# of them moves this bar's expectation and the instrument's answer together, and
# a change to only ONE of them is what this bar exists to catch.
E1_BP=$(sed -n 's/^BASE_PINGS=\([0-9][0-9]*\).*/\1/p' "$PROBE_SRC" | head -1)
E1_PW=$(sed -n 's/^PING_W=\([0-9][0-9]*\).*/\1/p' "$PROBE_SRC" | head -1)
E1_SM=$(sed -n 's/^SINK_MAX=\([0-9][0-9]*\).*/\1/p' "$PROBE_SRC" | head -1)
if [ -n "$E1_BP" ] && [ -n "$E1_PW" ] && [ -n "$E1_SM" ]; then
    E1_WANT=$(( 2 * (20 + E1_BP * E1_PW + 2 * E1_PW + 1) + E1_SM ))
    E1_GOT=$(env P5_SYSINFO_MODEL="$WORK/e1model" sh "$PROBE_SRC" --plan 10 20 \
             | sed -n 's/^bound_s:[ ]*\([0-9][0-9]*\).*/\1/p' | head -1)
    asrt "E1-B bound_s is N x (SECS + BASE_PINGS x PING_W + 2 x PING_W + 1) + SINK_MAX, recomputed from the file's own literals" "$E1_GOT" "$E1_WANT"
else
    no "E1-B could not read BASE_PINGS/PING_W/SINK_MAX out of the instrument -- the bound is unchecked"
fi
E1_PLANW=$(env P5_SYSINFO_MODEL="$WORK/e1model" sh "$PROBE_SRC" --plan 10 20; echo "rc=$?")
hasnt "E1-Ba --plan writes no route"     "$E1_PLANW" 'pinned:'
has   "E1-Ba2 ... and exits 0"           "$E1_PLANW" 'rc=0'
hasnt "E1-Bb ... and resolves no sink"   "$E1_PLANW" 'sink:'
hasnt "E1-Bc ... and takes no lock"      "$(ls "$WORK/run" 2>/dev/null)" 'e1probe.lock'

# --- E1-3 / E1-5 / LEAK-2: one clean confirmed run, start to footer -----------
setup; e1_world
: > "$LEDGER"; : > "$E1_LEDGER"
R=$(P 'k=run&name=e1&confirm=e1'); B=$(bd "$R")
asrt "E1-3 the confirmed row starts" "$(st "$R")" 202
E1ID=$(jf id "$B")
if [ -n "$E1ID" ] && await "$WORK/results/$E1ID.txt" 600; then
    ok "E1-3a the job ran to its footer and the artifact was renamed"
else
    no "E1-3a the job never finished -- everything below has no subject"
fi
E1ART=$(cat "$WORK/results/$E1ID.txt" 2>/dev/null)
A_ARM=$(li "$E1_LEDGER" 'p5-deadman arm '); A_PIN=$(li "$E1_LEDGER" 'IP route replace')
if [ -n "$A_ARM" ] && [ -n "$A_PIN" ] && [ "$A_ARM" -lt "$A_PIN" ]; then
    ok "E1-3b the deadman was armed BEFORE the first route write (arm at line $A_ARM, pin at $A_PIN)"
else
    no "E1-3b the arm is not before the pin (arm='${A_ARM:-none}' pin='${A_PIN:-none}') -- there is a window in which the box is steered and nothing owes a rollback"
fi
has "E1-3c the artifact carries the operator's budget"        "$E1ART" '### budget mib=10 secs=20'
has "E1-3d ... the plan it armed with"                        "$E1ART" 'bound_s:  71'
has "E1-3e ... and the residue block asked of the kernel"     "$E1ART" '### residue'
has "E1-5 the success path confirmed the deadman record"      "$E1ART" 'deadman=confirmed'
has "E1-5a ... and the artifact ends"                         "$E1ART" '### ended '
if [ -e "$WORK/dm/etc/p5/deadman/portal-e1" ]; then
    no "E1-5b a confirmed record is still armed -- the box would roll itself back later for no reason"
else
    ok "E1-5b nothing is left armed"
fi
# ANCHORED, not a substring. The deadman's own arm advice ("confirm with:
# p5-deadman confirm --label ...") goes through the logger and lands in this same
# ledger BEFORE anything is pinned, so a -F substring search finds a "confirm"
# that is a sentence, not a call. This asks for the line the wrapper writes.
E1_DEL_I=$(li "$E1_LEDGER" 'IP route del')
E1_TOOLEND_I=$(grep -nE '^p5-deadman confirm ' "$E1_LEDGER" 2>/dev/null | head -1 | cut -d: -f1)
if [ -n "$E1_DEL_I" ] && [ -n "$E1_TOOLEND_I" ] && [ "$E1_DEL_I" -lt "$E1_TOOLEND_I" ]; then
    ok "E1-5c the instrument's own trap unpinned the sink before the job confirmed (del at line $E1_DEL_I, confirm at $E1_TOOLEND_I)"
else
    no "E1-5c the clean path did not unpin before it confirmed (del='${E1_DEL_I:-none}' confirm='${E1_TOOLEND_I:-none}')"
fi
has "E1-5d ... and the residue read agrees nothing is pinned" "$E1ART" 'none: no 203.0.113.77/32 route is left'
has "E1-5e the UDP discriminator brackets the run"            "$E1ART" '### stats before'
has "E1-5f ... on both sides of it"                           "$E1ART" '### stats after'
hasnt "LEAK-2 the artifact carries no source address"         "$E1ART" '198.51.100.11'
hasnt "LEAK-2a ... none of the second source's either"        "$E1ART" '198.51.100.12'
hasnt "LEAK-2b ... and no gateway"                            "$E1ART" '198.51.100.254'
hasnt "LEAK-2c ... and no hostname line"                      "$E1ART" 'hostname:'
has   "LEAK-2d ... while the sink IS printed, because the rollback reads it back" "$E1ART" 'sink:     203.0.113.77'
has   "LEAK-2e ... and the pin line says 'via gateway', not which one"            "$E1ART" '(via gateway)'
V_E1=$(led_viol "$LEDGER")
asrt "E1-5g every external argv the whole E1 path issued is inside PC-5's allowlist" "$V_E1" ""
job_cleanup

# --- E1-4: killed mid-run -> kill the group, THEN unpin, THEN reconcile -------
# THE HEAL RACE. The deadman fires while curl is still uploading; the rollback
# has to stop the load before it removes the route, or a saturating upload lands
# on whatever route wins next.
setup; e1_world
printf '6\n' > "$WORK/e1_load_s"     # long enough that the deadline lands mid-load
: > "$LEDGER"; : > "$E1_LEDGER"
R=$(P 'k=run&name=e1&confirm=e1'); E1KID=$(jf id "$(bd "$R")")
if [ -n "$E1KID" ] && await_grep "$E1_LEDGER" 'IP route replace' 600; then
    ok "E1-4 the load is up and the sink is pinned"
else
    no "E1-4 the run never reached its pin -- nothing below has a subject"
fi
E1KPID=$(sed -n 's/^P5_PORTAL_PID=//p' "$WORK/results/$E1KID.meta" 2>/dev/null | head -1)
# `fire`, NOT `check`, and the difference is only WHICH clock decides. RUN-8 next
# door lets a 1 s deadline pass in real time and calls `check`; e1's deadline is
# the instrument's own bound (71 s for this fixture), and a bar that waited it
# out would add 71 s to every run of this file to measure a code path `fire`
# reaches identically -- both verbs end in p5-deadman's one `fire_one`. What is
# being measured here is the ROLLBACK's order, not the deadman's arithmetic,
# which is DM-30..33's next door.
env P5_PORTAL_DIR="$PORTAL" XCTL="$P_XCTL" sh "$WORK/portalbin/p5-deadman" \
    fire --label portal-e1 >/dev/null 2>&1
await_grep "$E1_LEDGER" 'IP route del' 600
K_I=$(li "$E1_LEDGER" 'LOG restore: kill -TERM'); D_I=$(li "$E1_LEDGER" 'IP route del')
if [ -n "$K_I" ] && [ -n "$D_I" ] && [ "$K_I" -lt "$D_I" ]; then
    ok "E1-4a the group kill came BEFORE the unpin (kill at line $K_I, del at $D_I)"
else
    no "E1-4a the unpin is not after the kill (kill='${K_I:-none}' del='${D_I:-none}') -- a saturating upload would be moved onto the next-best route, which here is the tunnel"
fi
has "E1-4b ... and it killed the GROUP named in the job's own record" "$(cat "$E1_LEDGER")" "kill -TERM -$E1KPID"
X_I=$(li "$E1_LEDGER" 'LOG XCTL reconcile')
[ -n "$X_I" ] || X_I=$(li "$LEDGER" 'bond-xctl reconcile')
if [ -n "$D_I" ] && [ -n "$X_I" ]; then ok "E1-4c ... and the reconciler was asked to derive the edge afterwards"
else no "E1-4c no reconcile followed the rollback (del='${D_I:-none}' reconcile='${X_I:-none}')"; fi
_w=0
while [ "$_w" -lt 150 ] && [ -n "${E1KPID:-}" ] && [ -d "/proc/$E1KPID" ]; do "$RSLEEP" 0.2; _w=$((_w+1)); done
if [ -n "${E1KPID:-}" ] && [ -d "/proc/$E1KPID" ]; then
    no "E1-4d the job group is STILL RUNNING after the rollback logged a kill"
else
    ok "E1-4d ... and the job group is actually gone (the effect, not the log line)"
fi
if [ -f "$WORK/results/$E1KID.txt" ]; then no "E1-4e a job killed mid-run produced a complete .txt"
else ok "E1-4e the artifact stays .part -- the run reads INCOMPLETE, which is what happened"; fi
E1LEFT=$(cat "$WORK/e1_pins" 2>/dev/null)
asrt "E1-4f and no pin is left on the box" "$E1LEFT" ""
job_cleanup

# --- E1-8: the mutant runner that arms nothing -------------------------------
setup; e1_world
: > "$LEDGER"; : > "$E1_LEDGER"
MB1=$(binmut e1arm -e 's#^        "\$P5_DEADMAN" arm --after "\$BOUND" --restore-script "\$P5_RESTORE" \\#        : arm-deleted-by-the-mutant \\#')
if grep -q 'arm-deleted-by-the-mutant' "$MB1/bin/p5-portal-run"; then
    ok "E1-8 the mutant runner really has no arm line"
else
    no "E1-8 the mutation did not apply -- E1-8a proves nothing"
fi
E1MID=$(mk_id e1)
env P5_PORTAL_DIR="$MB1" P5_CAT_DIR="$PORTAL/catalogue" XCTL="$P_XCTL" \
    P5_PORTAL_DM_TIMER=0 P5_RESTORE="$MB1/bin/p5-portal-restore" \
    setsid sh "$MB1/bin/p5-portal-run" e1 "$E1MID" >/dev/null 2>&1
M_ARM=$(li "$E1_LEDGER" 'p5-deadman arm '); M_PIN=$(li "$E1_LEDGER" 'IP route replace')
if [ -z "$M_ARM" ] && [ -n "$M_PIN" ]; then
    ok "E1-8a MUTANT: the load was pinned with NO deadman record behind it -- the state E1-3b refuses"
else
    no "E1-8a the mutant did not reproduce the defect (arm='${M_ARM:-none}' pin='${M_PIN:-none}')"
fi
job_cleanup

# --- E1-7: the verdict rules, against two fixtures ---------------------------
# The instrument's discriminator is the one thing here that is not about safety,
# and it is the reason the file exists. Both fixtures are driven through the REAL
# probe; only the LINK is fixture, and each one is what the rule names.
#
# THE LOAD WINDOW IS PART OF THE FIXTURE. The probe withholds MID unless it got at
# least as many loaded samples as the idle baseline had (BASE_PINGS=5) -- a correct
# guard: a separation over fewer samples than the baseline is not a comparison. But
# the number of samples that fit in the window is set by how fast this BOX forks
# `ss`/`tc`/`ping` per iteration, and MEASURED here (bare box, 60 trials) the 1 s
# window that e1_world sets yields 9 -- 1.8x the cliff at 5. On a loaded box (three
# CI worktrees sharing a lock and the CPU) that margin is gone, and E1-7a/E1-7b go
# RED for scheduling reasons with no defect present. 3 s yields ~27, so the bar
# measures the RULE and not this machine's fork latency. Cost: +2 s x 4 fixtures.
e1_verdict_world() { e1_world; printf '3\n' > "$WORK/e1_load_s"; }
setup; e1_verdict_world
printf '132000\n' > "$WORK/e1_backlog"      # a standing local queue, every sample
printf '20.0\n'   > "$WORK/e1_rtt_idle"
printf '20.0\n'   > "$WORK/e1_rtt_load"
E1_EDGE=$(env P5_SYSINFO_MODEL="$WORK/e1model" RUN_DIR="$WORK/run" sh "$PROBE_SRC" 4 1 eth1 2>&1)
has "E1-7 a standing backlog on a majority of samples reads EDGE" "$E1_EDGE" 'EDGE -- standing queue'
setup; e1_verdict_world
printf '0\n'    > "$WORK/e1_backlog"        # backlog limb LIVE and empty
printf '20.0\n' > "$WORK/e1_rtt_idle"
printf '90.0\n' > "$WORK/e1_rtt_load"       # every loaded rtt above every idle one
E1_MID=$(env P5_SYSINFO_MODEL="$WORK/e1model" RUN_DIR="$WORK/run" sh "$PROBE_SRC" 4 1 eth1 2>&1)
has "E1-7a separated RTT with no standing local queue reads MID" "$E1_MID" 'MID -- every loaded rtt'
setup; e1_verdict_world
printf '0\n'      > "$WORK/e1_backlog"
printf 'noqueue\n' > "$WORK/e1_qdisc"       # the limb that cannot see a queue
printf '20.0\n' > "$WORK/e1_rtt_idle"
printf '90.0\n' > "$WORK/e1_rtt_load"
E1_BLIND=$(env P5_SYSINFO_MODEL="$WORK/e1model" RUN_DIR="$WORK/run" sh "$PROBE_SRC" 4 1 eth1 2>&1)
has "E1-7b the same separation over a BLIND backlog limb withholds the verdict instead of claiming MID" \
    "$E1_BLIND" 'NO VERDICT -- rtt separated'
setup; e1_world
printf '0\n' > "$WORK/e1_up_bytes"          # nothing left the box
E1_NOLOAD=$(env P5_SYSINFO_MODEL="$WORK/e1model" RUN_DIR="$WORK/run" sh "$PROBE_SRC" 4 1 eth1 2>&1)
has "E1-7c zero bytes uploaded is named a PROBE FAILURE, not a property of the link" \
    "$E1_NOLOAD" 'NO LOAD -- probe failure'
# E1-7d: the SAME MID link as E1-7a, except the load ends inside the 8th sample.
# The instrument checks `kill -0 $LOADPID` and only THEN spends real time in
# sendq/backlog/rtt, so that sample reads an idle link while being counted as
# loaded. Because the MID rule is a min/max separation and not an average, one
# such sample drags LMIN to the baseline and `lmin > bmax` goes false: a true MID
# reported INCONCLUSIVE, blaming the link for the instrument's own timing. Seven
# loaded samples survive, which still clears the >= BASE_PINGS=5 guard, so a
# verdict other than MID here is the race and nothing else.
setup; e1_verdict_world
printf '0\n'    > "$WORK/e1_backlog"
printf '20.0\n' > "$WORK/e1_rtt_idle"
printf '90.0\n' > "$WORK/e1_rtt_load"
printf '8\n'    > "$WORK/e1_straddle_at"
E1_STRADDLE=$(env P5_SYSINFO_MODEL="$WORK/e1model" RUN_DIR="$WORK/run" sh "$PROBE_SRC" 4 1 eth1 2>&1)
has "E1-7d a sample straddling the end of the load does not turn a MID link INCONCLUSIVE" \
    "$E1_STRADDLE" 'MID -- every loaded rtt'
# Rule 9: the dropped sample is a quantity this run discarded, so it is NAMED.
# Without this the fix could be made by silently shrinking SAMPLES.
has "E1-7e the discarded straddling sample is reported, not silently dropped" \
    "$E1_STRADDLE" 'discarded    : 1 sample straddled the end of the load'

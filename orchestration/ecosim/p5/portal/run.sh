...
# `server` (XCTL _server, bond-xctl `live_server_host`) EXISTS to print the
# server host -- its consumer pings it, its catalogue label says so, and it
# predates this unit. So the KEY sweep covers every row without exception, and
# the HOST sweep covers every row but that one, whose behaviour is then asserted
# in the OTHER direction (LEAK-1c) so the exception stays a measured fact rather
# than a hole a later row could hide in.
LEAKS=""

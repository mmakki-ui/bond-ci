...
# `server` (XCTL _server, bond-xctl:239) EXISTS to print the server host -- its
...
# this unit's branch; U229 adds lifecycle_roundtrip and mode_roundtrip and U230
# adds e1, and each of them has to edit this line to do it.
DIS_WANT=''
...
if [ "$DIS_N" = 0 ]; then
    ...
    DIS_BAD=$(grep -v '^[[:space:]]*#' "$TESTS_CAT" \
              | awk -F'|' '$2=="disruptive" && ($4 !~ /^[0-9]+$/ || length($5) < 40){print $1}')
    asrt "RUN-C3 every disruptive row declares a numeric bound and a label that describes the disturbance" "$DIS_BAD" ""
fi

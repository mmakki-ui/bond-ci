...
#
#  * bondctl's acceptance is DERIVED, not listed (bondctl:105-125). The literal
#    `case` arm carries only this CLI's OWN modes -- `lightning|eco` -- and every
#    other mode is delegated to `bond-xctl _sched`, which answers out of
#    AGG_SCHED_TABLE (bond-xctl:609-628, 1512-1521). That split is deliberate: a
#    third aggregate scheduler is ONE row in bond-xctl and ZERO edits in bondctl
#    (Layer-2 AGG-L12). A one-line scrape can therefore only ever see HALF the
#    accepted set -- and this one saw the wrong half anyway: the first
#    single-line `case "$M" in` in the file is bondctl:166, the AUTO-POLICY test.
#  * EQUALITY is not the property. ADR-003 fixes the shipped ladder and
#    catalogue/modes:29-30 states the split: `speed` is the SHIPPED MEANING, and
#    `max` is ADR-003 §1's post-U17 name carried as `unimplemented` ON PURPOSE so
#    the portal refuses it -- while bondctl, whose AGG_SCHED_TABLE already has
#    the `max` row, accepts it. bondctl's accepted set is a legitimate SUPERSET.
#
...
BCTL_LIT=$(awk '/^[[:space:]]*case "\$M" in[[:space:]]*$/ { b=1; next }
                b && /^[[:space:]]*[a-z][a-z|]*\)/ {
                    sub(/^[[:space:]]*/, ""); sub(/\).*/, ""); print; exit }' "$P5/bondctl")
BCTL_TBL=$(sh "$P5/bond-xctl" _sched_modes 2>/dev/null)
BCTL=$(printf '%s\n%s\n' "$BCTL_LIT" "$BCTL_TBL" \
       | tr '|' '\n' | grep -v '^[[:space:]]*$' | sort -u)
...
NOTACC=$(printf '%s\n' "$IMPL" | while read -r _m; do
             [ -n "$_m" ] || continue
             printf '%s\n' "$BCTL" | grep -qx -- "$_m" || echo "$_m"
         done | tr '\n' ' ')
asrt "PC-1 every catalogue-implemented mode is one bondctl accepts" "$NOTACC" ""
UNLISTED=$(printf '%s\n' "$BCTL" | while read -r _m; do
             [ -n "$_m" ] || continue
             printf '%s\n' "$CATALL" | grep -qx -- "$_m" || echo "$_m"
           done | tr '\n' ' ')

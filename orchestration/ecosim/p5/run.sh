...
# S5c/S5d/S5e (U253) — the cap's ENABLE PATH. It carries what the box measured and nothing
# else. The three states that matter are absent, complete and PARTIAL: partial must emit
# NOTHING, since cap.go turns AGG_PULL_CAP=on with a missing key into a fatal config error,
# which would take the datapath down rather than run it uncapped.
setup; bctl on; bctl mode speed
asrt "S5c cap absent: no AGG_PULL_CAP line at all" "$(aggf AGG_PULL_CAP)" ""

setup; bctl on
printf 'AGG_PULL_CAP_TARGET_MS=40\nAGG_PULL_CAP_TRIP=0.92\nAGG_PULL_CAP_CLEAR=1.5\n' \
        > "$WORK/etc/p5/cap"
printf 'AGG_PULL_CAP_MINRATE_KBPS=500\nAGG_PULL_CAP_DET_MS=400\n' >> "$WORK/etc/p5/cap"
bctl mode speed
asrt "S5d cap complete: enabled"      "$(aggf AGG_PULL_CAP)" "on"
asrt "S5d cap complete: target"       "$(aggf AGG_PULL_CAP_TARGET_MS)" "40"
asrt "S5d cap complete: trip"         "$(aggf AGG_PULL_CAP_TRIP)" "0.92"
asrt "S5d cap complete: clear"        "$(aggf AGG_PULL_CAP_CLEAR)" "1.5"
asrt "S5d cap complete: minrate"      "$(aggf AGG_PULL_CAP_MINRATE_KBPS)" "500"
asrt "S5d cap complete: det window"   "$(aggf AGG_PULL_CAP_DET_MS)" "400"

setup; bctl on
printf 'AGG_PULL_CAP_TARGET_MS=40\nAGG_PULL_CAP_TRIP=0.92\n' > "$WORK/etc/p5/cap"
bctl mode speed
asrt "S5e cap PARTIAL: emits nothing (a partial cap is FATAL in the daemon)" \
     "$(aggf AGG_PULL_CAP)" ""

# S5f/S5g/S5h (U252) — the tx backoff knob, same shape and the same reason: no default is
# shipped here and none may be.
setup; bctl on; bctl mode speed
asrt "S5f tx backoff absent: no line at all" "$(aggf AGG_PULL_TXBACKOFF_US)" ""

setup; bctl on
printf '2500\n' > "$WORK/etc/p5/tx_backoff_us"
bctl mode speed
asrt "S5g tx backoff set: carried verbatim" "$(aggf AGG_PULL_TXBACKOFF_US)" "2500"

setup; bctl on
printf 'later\n' > "$WORK/etc/p5/tx_backoff_us"
bctl mode speed
asrt "S5h tx backoff non-numeric: treated as absent, never passed through" \
     "$(aggf AGG_PULL_TXBACKOFF_US)" ""

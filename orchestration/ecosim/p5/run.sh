...
setup() {
    ...
    export AGGCTL="$P5/bond-aggctl"   # M3 agg-controller (U18)
...
# ===========================================================================
# M3 -- the agg-controller BOUNDARY (U18). The aggregate feeder's lifecycle was
# extracted out of the reconciler (bond-xctl) into deploy/p5/bond-aggctl.
#
# These bars are about the BOUNDARY, not about aggregate behaviour. Every
# behavioural bar above -- S5..S8b, F13/F14, F17/F18, NG1..NG7 -- already runs
# THROUGH the controller after the extraction, and their output is byte-identical
# to the pre-extraction run; that is the equivalence evidence and it is the whole
# point of a refactor. What those bars CANNOT see is whether the boundary is real:
# they would pass exactly as well if bond-xctl had kept doing the work itself.
# M3-1..M3-5 are the bars that fail if the extraction is undone or faked.
#
# aggc(): drive the controller with NO reconciler in the picture, on the same
# context bond-xctl hands it. Invoked via `sh` so the bars do not also depend on
# the exec bit surviving a checkout.
aggc() { BOND_DIR="$WORK/etc/bond" RUN_DIR="$WORK/run/bond" WG_DEV="$WG_DEV" \
         AGG_SVC="$BIN/svc-agg" AGG_BIN="$WORK/fakebin/bond-agg" \
         sh "$P5/bond-aggctl" "$@"; }
...
# M3-3 -- the INTERFACE is SUFFICIENT. The controller alone, told nothing but
# (listen, server, ordered sources), reproduces byte-for-byte the agg_env the
# full reconciler applied. If any part of the aggregate config were still being
# contributed by bond-xctl, these two would differ.
setup; fact nwan 3; bctl on; bctl mode speed
aggc env-build "$WORK/m3.env" "127.0.0.1:59402" "$(xctl _server):59402" eth1 usb0 eth0
asrt "M3-3 controller ALONE reproduces the applied agg_env byte-for-byte" \
     "$(cat "$WORK/m3.env" 2>/dev/null)" "$(aggenv)"

# M3-4 -- the extraction is STRUCTURAL, not cosmetic. No aggregate-feeder
# lifecycle survives in the reconciler: not the weight vector, not the arity
# floor, not the procd unit text, not the MTU strand. Checked on NON-COMMENT
# lines only -- the comments name what moved, deliberately. A future re-inlining
# fails here, which is the regression this unit is most exposed to.
m3st=1
for pat in 'AGG_W_NEUTRAL=' 'MIN_AGG_SOURCES=' 'agg_weights' 'procd_set_param' 'mtu 1408' 'mtu 1420'; do
    if grep -v '^[[:space:]]*#' "$P5/bond-xctl" | grep -q "$pat"; then
        m3st=0; echo "      (bond-xctl still carries: $pat)"
    fi
done
[ "$m3st" = 1 ] \
  && ok "M3-4 bond-xctl carries NO aggregate-feeder lifecycle (weights, arity floor, procd unit, MTU strand)" \
  || no "M3-4 aggregate-feeder lifecycle still present in bond-xctl"

# M3-5 -- TEETH. The delegation is LOAD-BEARING. With the controller absent the
# speed edge must REFUSE and the CLI must restore the prior mode. If bond-xctl
# still did the work internally, speed would engage anyway and every bar above
# would be measuring the old code path rather than the extracted one.
...
asrt "M3-5 TEETH controller absent: speed REFUSED, prior mode restored" \
     "$(cat "$WORK/etc/bond/mode")" eco
asrt "M3-5 TEETH controller absent: no aggregate feeder" "$(running bond-agg)" 0

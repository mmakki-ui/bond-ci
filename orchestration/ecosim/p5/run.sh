...
# EG-3 — the disengage row. Its actions (speeddown_if_speed, rc_off, eng_disable,
...
# it is the pre-change tripwire (pre-U50a bond-xctl:736 matched ENGARDE_BIN).
...
_eg_rows=$(awk -F'|' '/^(engage|disengage|switch|speed)\|/ && $4 ~ /(^|,)installed(,|$)/ {c++} END{print c+0}' "$P5/bond.dag")
asrt "EG-4 engage/disengage/switch/speed all still carry the installed guard" "$_eg_rows" 4

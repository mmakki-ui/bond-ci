...
setup() {
    ...
    # U52: NO $BOND_DIR/wg-logical is written here on purpose. That file is
    ...
    # derives the wg logical interface via ubus (bond-xctl _is_wg) instead of
    # reading this file -- see F19/F20 below, which assert the hook works with
    # NO such file present at all, and the wrong-iface / unresolvable-iface
    # cases (F12/F20).
...
# wait_settle(): F19 (U52) is the FIRST scenario to run hook_hotplug() while
# engaged, so it is the first to hit the backgrounded "$XCTL reconcile &" this
# file's own hook() comment warns about ("the reparented grandchild can't be
# waited on") -- found by running this file for real on CI (u52 round 2):
...
grep -q 'ERROR: cannot classify' "$WORK/ledger" \
    && no "F12 hotplug wrong-iface: false error logged (wan resolves fine via ubus, just isn't wg)" \
    || ok "F12 hotplug wrong-iface: correctly silent, no false error"
...
setup; bctl on; fact ep "203.0.113.9:51820"
[ -e "$WORK/etc/bond/wg-logical" ] \
    && no "F19 setup precondition: wg-logical file exists (test is not on a clean box)" \
    || ok "F19 setup precondition: no wg-logical file anywhere on this box"
hook_hotplug
...
wait_settle 'epv' "127.0.0.1:59401" 5
asrt "F19 clean-box hotplug still reconciles: endpoint :59401" "$(epv)" "127.0.0.1:59401"
...
grep -q 'ERROR: cannot classify ifup(ghost-iface-does-not-exist)' "$WORK/ledger" \
    && ok "F20 unresolvable-iface hotplug: FAIL LOUD logged" \
    || no "F20 unresolvable-iface hotplug: silent -- no error logged anywhere"

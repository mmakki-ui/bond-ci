...
# S5f/S5g/S5h (U252) — the tx backoff knob. It is the only corrective pull.go's S7 register
# names for the park's utilization floor, and it was emitted by nobody, so on a box the
# documented remedy did not exist. No default is shipped here and none may be: S7 says the
# derivation needs the drain rate the pivot deleted. Absent fact must stay absent-in-env.
setup; bctl on; bctl mode speed

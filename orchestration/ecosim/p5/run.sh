...
IPTEOF
    ...
    # U18: the M3 agg-controller is a SEPARATE EXECUTABLE, so it needs the same
    # treatment for the same reason -- its installed default is /usr/sbin/p5-aggctl
    # and the harness runs the worktree. Derived from $P5 for the mutant-tree case
    # too, so a mutated tree exercises ITS OWN controller rather than this one.
    export AGGCTL="$P5/bond-aggctl"
...
# ===========================================================================
# M3 -- the agg-controller BOUNDARY (U18). The bonded feeder's lifecycle was
# extracted out of the reconciler (bond-xctl + its five libs) into
# deploy/p5/bond-aggctl, a SEPARATE EXECUTABLE.
#
# These bars are about the BOUNDARY, not about aggregate behaviour. Every
# behavioural bar above already runs THROUGH the controller after the
# extraction; that is the equivalence evidence and it is the whole point of a
# refactor. What those bars CANNOT see is whether the boundary is real: they
# would pass exactly as well if the reconciler had kept doing the work itself.
# M3-1..M3-5 are the bars that fail if the extraction is undone or faked.
#
# aggc(): drive the controller with NO reconciler in the picture, on the context
# setup() already exports -- the SAME context bond-xctl's `aggctl` hands it.
# Invoked via `sh` so the bars do not also depend on the exec bit surviving a
# checkout.
aggc() { sh "$P5/bond-aggctl" "$@"; }

# M3-1 -- closed verb surface. A controller that silently succeeded on an unknown
# verb would let a mis-wired or renamed delegation look healthy forever.
setup
if aggc definitely-not-a-verb >/dev/null 2>&1; then
    no "M3-1 unknown verb must fail closed"
else
    ok "M3-1 unknown verb fails closed"
fi

# M3-2 -- arity-ok is the ARITY FLOOR, N-generic in BOTH directions: it refuses
# N=0 and N=1 and accepts every N from 2 up with NO upper bound. This is the bar
# that catches the floor being re-shaped back into a "two WANs" test.
m3ar=1
aggc arity-ok    >/dev/null 2>&1 && m3ar=0
aggc arity-ok s1 >/dev/null 2>&1 && m3ar=0
for n in 2 3 4 5 6 7 8 16 33; do
    _l=""; _i=1
    while [ "$_i" -le "$n" ]; do _l="$_l s$_i"; _i=$((_i+1)); done
    # shellcheck disable=SC2086
    aggc arity-ok $_l >/dev/null 2>&1 || m3ar=0
done
[ "$m3ar" = 1 ] \
  && ok "M3-2 arity-ok refuses N=0,1 and accepts N=2..8,16,33 (a floor, not a 2-WAN test)" \
  || no "M3-2 arity-ok is not the N-generic arity floor"

# M3-2b -- the floor is READ from the controller, not kept twice. The reconciler's
# per-mode floor asks `aggctl arity-floor`, so the two files cannot drift; this
# bar pins the value the model also carries (bond_model.py MIN_AGG_SOURCES).
asrt "M3-2b the controller publishes the arity floor" "$(aggc arity-floor)" 2

# M3-3 -- the INTERFACE is SUFFICIENT. The controller alone, told nothing but the
# seven declared facts and the ordered source set, reproduces byte-for-byte the
# agg_env the full reconciler applied. If any part of the feeder config were
# still being contributed by the reconciler, these two would differ.
# The two fact values this fixture leaves at their defaults are asserted FIRST,
# so a fixture that stops producing them fails as itself rather than as M3-3.
setup; fact nwan 3; bctl on; bctl mode speed
asrt "M3-3 precondition: this fixture meters nothing and sets no spotty_dup fact" \
     "$(aggenv | sed -n 's/^AGG_SPOTTY=//p')|$(aggenv | sed -n 's/^AGG_LIGHTNING=//p')" "|0"
aggc env-build "$WORK/m3.env" "127.0.0.1:59402" "203.0.113.9:59402" speed speed "" 0 \
     eth1 usb0 eth0
asrt "M3-3 the controller ALONE reproduces the applied agg_env byte-for-byte" \
     "$(cat "$WORK/m3.env" 2>/dev/null)" "$(aggenv)"

# M3-4 -- the extraction is STRUCTURAL, not cosmetic. No feeder lifecycle survives
# in the reconciler: not the weight vector, not the arity floor, not any procd
# unit text, not the MTU strand. Scanned over the WHOLE reconciler -- the bin AND
# its five libraries (xctl_cat) -- because a bin-only scan is how AGG-4 once lost
# 86% of its subject, and U124 means the lifecycle would hide in a lib. Checked on
# NON-COMMENT lines only: the comments name what moved, deliberately. A future
# re-inlining fails here, which is the regression this unit is most exposed to.
m3st=1
for pat in 'AGG_W_NEUTRAL=' 'MIN_AGG_SOURCES=' 'agg_weights' 'procd_set_param' 'mtu 1408' 'mtu 1420'; do
    if xctl_cat | grep -v '^[[:space:]]*#' | grep -q "$pat"; then
        m3st=0; echo "      (the reconciler still carries: $pat)"
    fi
done
[ "$m3st" = 1 ] \
  && ok "M3-4 the reconciler carries NO feeder lifecycle (weights, arity floor, procd unit, MTU strand)" \
  || no "M3-4 feeder lifecycle still present in the reconciler"

# M3-5 -- TEETH. The delegation is LOAD-BEARING. With the controller absent the
# aggregate edge must REFUSE and the CLI must restore the prior mode. If the
# reconciler still did the work internally, speed would engage anyway and every
# bar above would be measuring the old code path rather than the extracted one.
# Run in a SUBSHELL: a var assignment prefixed to a FUNCTION call persists in the
# shell under POSIX sh, and a leaked AGGCTL would silently disarm later bars.
setup; fact nwan 3; bctl on; bctl mode eco
( AGGCTL="$WORK/no-such-aggctl"; export AGGCTL; bctl mode speed )
asrt "M3-5 TEETH controller absent: speed REFUSED, prior mode restored" \
     "$(cat "$WORK/etc/p5/mode")" eco
asrt "M3-5 TEETH controller absent: no aggregate feeder" "$(running p5-datapath)" 0
setup

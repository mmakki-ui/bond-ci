...
# ============================================================================
# SUS — U32: MID-FLIGHT ARITY LOSS under `speed`. The ARTIFACT twin of Layer-1
# CH-3, and THE GATE for it. Before U32 these bars FAIL on `dev`: the arity
# requirement was an ENTRY guard only (bond.dag `speed` row: `enough_sources`),
# converge() gives a refused guard no defined outcome, so a box already
# aggregating when the live source count fell below MIN_AGG_SOURCES kept bond-agg
# striping over a departed source and every later tick re-refused the same edge,
# forever. These bars are deliberately BEHAVIOURAL -- none of them names the
# `sustain` verb or greps the source -- so they measure the box, not the patch.
#
# runw_now(): one watchdog tick "30 s later". The watchdog refuses the CAPPED
# reconcile inside COOLDOWN (bond-watchdog:57-60), and this harness runs many
# ticks in well under a second, so wd_last is rewound first. That is time travel,
# not a bypass: the cap/cooldown logic is exercised unchanged. Plain runw() is
# used wherever the bar is about the UNCAPPED safety pass, which must fire
# regardless.
runw_now() { echo 0 > "$WORK/run/bond/wd_last"; runw; }
ringn()    { grep -c . "$WORK/run/bond/wd_ring" 2>/dev/null || echo 0; }

# --- SUS-1: N=3 aggregating, the world drops to N=1 -------------------------
# The confirmation window (>= 2 consecutive trusted observations) is asserted in
# BOTH directions: one sample must NOT act, two must.
setup; fact nwan 3; bctl on; bctl mode speed
asrt "SUS-1 pre: agg up"                    "$(running bond-agg)" 1
asrt "SUS-1 pre: AGG_PATHS N=3"             "$(aggf AGG_PATHS)" "eth1,usb0,eth0"
fact nwan 1                                  # two sources disappear MID-FLIGHT
runw
asrt "SUS-1 ONE sample does NOT tear down (confirmation window)" "$(running bond-agg)" 1
runw
asrt "SUS-1 second confirmation: aggregate TORN DOWN" "$(running bond-agg)" 0
asrt "SUS-1 single feeder (engarde up)"     "$(running engarde-client)" 1
asrt "SUS-1 endpoint re-pinned LOCAL"       "$(epv)" "127.0.0.1:59401"
asrt "SUS-1 node is DEFINED"                "$(node)" engaged
asrt "SUS-1 mode speed PIN retained"        "$(cat "$WORK/etc/bond/mode")" speed
asrt "SUS-1 engarde re-enrolled over LIVE sources" "$(cat "$WORK/etc/bond/applied_wans")" "eth1"

# --- SUS-2: stability. Further ticks change nothing (no oscillation) ---------
R0=$(cat "$WORK/restarts.engarde-client" 2>/dev/null || echo 0)
A0=$(cat "$WORK/etc/bond/applied_wans")
runw_now; runw_now; runw_now
asrt "SUS-2 3 more ticks: ZERO engarde restarts" \
     "$(cat "$WORK/restarts.engarde-client" 2>/dev/null || echo 0)" "$R0"
asrt "SUS-2 applied_wans unchanged"         "$(cat "$WORK/etc/bond/applied_wans")" "$A0"
asrt "SUS-2 agg still down"                 "$(running bond-agg)" 0
asrt "SUS-2 mode still speed"               "$(cat "$WORK/etc/bond/mode")" speed

# --- SUS-3: auto-resume. Sources return -> the aggregate re-forms, NO CLI ----
fact nwan 3; runw_now
asrt "SUS-3 aggregate re-formed"            "$(running bond-agg)" 1
asrt "SUS-3 enrolled over the CURRENT set"  "$(aggf AGG_PATHS)" "eth1,usb0,eth0"
asrt "SUS-3 single feeder"                  "$(running engarde-client)" 0
asrt "SUS-3 endpoint :59402"                "$(epv)" "127.0.0.1:59402"

# --- SUS-4: A FAILED PROBE IS NOT A LOST SOURCE (U32 B1) --------------------
# `ip route show` exits NON-ZERO and prints nothing -- a fork failure, a netlink
# error, a PATH gap mid-upgrade. That is "we cannot see", which is the OPPOSITE
# fact from "the sources are gone", and it must never swap out a HEALTHY
# aggregate. Note the distinction the shim makes: this is exit 1, not empty
# output; a box that genuinely carries no default route exits 0.
setup; fact nwan 3; bctl on; bctl mode speed
asrt "SUS-4 pre: agg up at N=3"             "$(running bond-agg)" 1
fact iproutefail 1
runw; runw; runw
asrt "SUS-4 blind probe: aggregate NOT torn down" "$(running bond-agg)" 1
asrt "SUS-4 blind probe: no feeder swap"    "$(running engarde-client)" 0
asrt "SUS-4 blind probe: endpoint still :59402" "$(epv)" "127.0.0.1:59402"
asrt "SUS-4 blind ticks counted ZERO confirmations" \
     "$(cat "$WORK/run/bond/agg_arity_low" 2>/dev/null || echo 0)" 0
# and the probe recovering at the SAME N must leave everything alone
rm -f "$WORK/iproutefail"; runw; runw
asrt "SUS-4 probe recovers at N=3: still aggregating" "$(running bond-agg)" 1

# --- SUS-5: the CAP must not be able to strand the box UNSAFE (U32 B3) ------
# Exhaust the watchdog's actions/hour ring AND put it inside COOLDOWN, then take
# the arity away. Fail-static is only acceptable when the state it freezes is
# safe; `bond-agg striping over a departed source` is not. The safety teardown
# must still fire, and must NOT consume the cap (charging it would starve the
# capped direction that re-forms the aggregate).
setup; fact nwan 3; bctl on; bctl mode speed
NOW=$(date +%s)
: > "$WORK/run/bond/wd_ring"
i=0; while [ "$i" -lt 12 ]; do echo "$NOW" >> "$WORK/run/bond/wd_ring"; i=$((i+1)); done
echo "$NOW" > "$WORK/run/bond/wd_last"          # inside COOLDOWN as well
RING0=$(ringn)
asrt "SUS-5 pre: cap ring is full"          "$RING0" 12
fact nwan 1
runw; runw
asrt "SUS-5 cap exhausted + cooling: safety teardown STILL fires" "$(running bond-agg)" 0
asrt "SUS-5 single feeder restored"         "$(running engarde-client)" 1
asrt "SUS-5 safety action did NOT consume the cap" "$(ringn)" "$RING0"

# --- SUS-6: the arm keys on the ARITY FLOOR, not on "a source left" ---------
# N-GENERIC: 4 -> 3 is still >= MIN_AGG_SOURCES, so the aggregate must SURVIVE
# and simply re-enrol over the sources that remain. A guard that fired on any
# source loss (or one that assumed 2) would tear this down.
setup; fact nwan 4; bctl on; bctl mode speed
asrt "SUS-6 pre: AGG_PATHS N=4"             "$(aggf AGG_PATHS)" "eth1,usb0,eth0,wwan0"
fact nwan 3; runw_now; runw_now
asrt "SUS-6 N=4 -> N=3: aggregate SURVIVES" "$(running bond-agg)" 1
asrt "SUS-6 N=4 -> N=3: re-enrolled over 3" "$(aggf AGG_PATHS)" "eth1,usb0,eth0"
asrt "SUS-6 N=4 -> N=3: endpoint still :59402" "$(epv)" "127.0.0.1:59402"

# --- SUS-7: the ENTRY refusal is UNCHANGED (SP5/NG-1 must not regress) ------
# `bondctl mode speed` at N=1 with no aggregate running must still FAIL so the
# CLI restores the prior mode. The sustainment arm keys on "already
# aggregating", never on "cannot aggregate" -- and running ticks afterwards must
# not turn the refusal into an action.
setup; fact nwan 1; bctl on; bctl mode eco; bctl mode speed
asrt "SUS-7 entry at N=1 refused, mode restored" "$(cat "$WORK/etc/bond/mode")" eco
asrt "SUS-7 no aggregate"                   "$(running bond-agg)" 0
asrt "SUS-7 engarde still up"               "$(running engarde-client)" 1
runw_now; runw_now
asrt "SUS-7 ticks after an entry refusal: mode still eco" "$(cat "$WORK/etc/bond/mode")" eco
asrt "SUS-7 ticks after an entry refusal: engarde still up" "$(running engarde-client)" 1

...
# = 24 (U46, 2026-09-12: 23 measured on this branch before the edit, plus the one
# EFP-1h site -- the sed that builds the wall-clock control mutant). The number this
# line carried before that was 23 (U226, 2026-09-05: 21 on dev at fa2f818 plus that
# unit's two ST-3 sites), and before THAT 20, which was ALREADY STALE when
...
# U46. A bar guarded by an OPTIONAL dependency used to simply not exist when the
# dependency was absent, so the harness's bar TOTAL was a property of the machine.
# That made `recon-ecosim`'s anti-vacuity floor unusable as a ratchet: the public
# mirror does not publish p1-autorate/ (scripts/sync-public-ci.sh ALLOW), so the
# same tree printed 656 here and 649 there and the floor had to be either wrong or
# lowered. skip() DECLARES the bar instead: it is counted, it is named in the log,
# and it is not a pass. pass+skipped is then the same number in every environment,
# which is what the floor is asserted on. A skip is never a green -- the gate caps
# how many there may be, so a bar cannot be laundered from PASS into SKIP.
...
setup() {
    ...
    # U52: NO $BOND_DIR/wg-logical is written here on purpose (the seed line dev
    # carried at this spot is DELETED, not re-rooted to /etc/p5). That file is
    # written ONLY by p2-engarde's bootstrap (p2-engarde/bootstrap-bond.sh:79)
    # -- nothing in deploy/p5 ever wrote it, and per U50 P5 no longer depends
    # on p2-engarde at all. Seeding it here (as this harness used to,
    # unconditionally, for every scenario) hid exactly the bug U52 found: the
    # real box a clean install or the server produces has NO such file
    # (docs/knowledge/inventory/2026-08-30-server-brume2.txt's `### etc-bond`),
    # so every scenario run through this harness was testing 97-bond under a
    # precondition the shipped install path can never establish. 97-bond now
    # derives the wg logical interface via ubus (the executor's `_is_wg` verb)
    # instead of reading this file -- see F19/F20 below, which assert the hook
    # works with NO such file present at all, and the wrong-iface /
    # unresolvable-iface cases (F12/F20).
...
# wait_settle(): F19 (U52) is the FIRST scenario where hook_hotplug()'s 97-bond
# classifies the fired interface as the wg tunnel WHILE ENGAGED (F11 is off,
# F12 fires on "wan", a real-but-not-wg interface) -- so it is the first to
# take 97-bond's case-0 branch and hit the backgrounded "$XCTL reconcile &"
# this file's own hook() comment warns about ("the reparented grandchild
# can't be waited on") -- found by running this file for real on CI (u52
# round 2):
# node() converged to "engaged" inside the same tick, but endpoint (a separate
# apply_endpoint() write) had not landed yet, so asserting immediately is a
# genuine race, not a flake -- reproduced deterministically on the Linux
# runner, not just under Windows fork latency. Poll with a REAL sleep
# (`command -p` bypasses this harness's logical-clock `sleep` shim on purpose:
# that shim is a no-op so simulated time windows cost nothing, but here we are
# waiting on an actual OS-scheduled background process, which needs actual
# wall time regardless of the logical clock) instead of widening what F19
# checks -- same node/endpoint targets, same fail-if-never-reached.
wait_settle(){ # $1=command to poll  $2=expected value  $3=max real seconds (default 5)
    _n=0; _max=${3:-5}
    while [ "$_n" -le "$_max" ]; do
        [ "$(eval "$1")" = "$2" ] && return 0
        _n=$((_n+1)); command -p sleep 1
    done
    return 1
}
...
# "wan" is a REAL, ubus-resolvable interface in the fixture (l3_device=eth1,
# != WG_DEV) -- this tests "ubus answered, it is genuinely some other WAN",
# distinct from F20 below ("ubus could not answer at all"). Was "wan0"
# (unresolvable in the fixture) before U52, which accidentally exercised the
# SAME path as F20 and left the genuinely-different-interface case untested.
setup; bctl on; fact ep "203.0.113.9:51820"
INTERFACE=wan ACTION=ifup sh "$P5/97-bond" 2>/dev/null       # wrong iface -> silent
asrt "F12 hotplug wrong-iface: endpoint unchanged" "$(epv)" "203.0.113.9:51820"
# if/then/else, not `A && no ... || ok ...` -- this file's own U66/SC2015 header
# says why: in that form BOTH arms can run and one bar double-counts.
if grep -q 'ERROR: cannot classify' "$WORK/ledger"
then no "F12 hotplug wrong-iface: false error logged (wan resolves fine via ubus, just isn't wg)"
else ok "F12 hotplug wrong-iface: correctly silent, no false error"; fi
...
# F19 (U52) — THE RED BAR: hotplug fires with NO $BOND_DIR/wg-logical file
# EVER having existed on the box. The pre-fix 97-bond was
# `[ -r "$BOND_DIR/wg-logical" ] || exit 0` -- against that code this scenario
# is indistinguishable from "the box is fine" because it produces the exact
# same observable (nothing happens), which is the whole finding: silent
# inertness reads as health. Demonstrated by reverting only 97-bond to the
# pre-fix text and re-running this one case: node stays "off"->never engages,
# endpoint never moves off "direct" -- FAIL. Fixed 97-bond derives the wg
# interface over ubus, needs no file, and this PASSES.
setup; bctl on; fact ep "203.0.113.9:51820"
# MERGE PORT: the fact root is /etc/p5 since ADR-005 and setup() creates only
# that one, so THIS is the path whose absence the bar is claiming. if/then/else
# rather than `A && no ... || ok ...` for the U66/SC2015 reason this file states
# at the top: in that form BOTH arms can run and one bar double-counts.
if [ -e "$WORK/etc/p5/wg-logical" ]
then no "F19 setup precondition: wg-logical file exists (test is not on a clean box)"
else ok "F19 setup precondition: no wg-logical file anywhere on this box"; fi
hook_hotplug
# The hotplug hook backgrounds reconcile (see wait_settle()'s comment above) --
# poll for convergence instead of asserting against a race on the reparented
# grandchild's completion.
wait_settle 'node' engaged 5
asrt "F19 clean-box hotplug still reconciles: node engaged" "$(node)" engaged
# MERGE PORT: the engaged endpoint is the LOCAL FEEDER's, :59402 on this tree
# (F10 and F18 assert the same value). U52 was built when it was :59401.
wait_settle 'epv' "127.0.0.1:59402" 5
asrt "F19 clean-box hotplug still reconciles: endpoint :59402" "$(epv)" "127.0.0.1:59402"

# F20 (U52) — hotplug for an interface ubus cannot describe AT ALL (not the wg
# tunnel, not any object the fixture knows) must FAIL LOUD, not silently do
# nothing. This is the failure mode the file-based check could never separate
# from F12's "genuinely a different WAN" -- both produced the identical
# `exit 0` with nothing in the log. is_wg_iface's 3-way exit status (0/1/2)
# makes classification-failure a distinct, asserted case.
setup; bctl on; fact ep "203.0.113.9:51820"
INTERFACE=ghost-iface-does-not-exist ACTION=ifup sh "$P5/97-bond" 2>/dev/null
asrt "F20 unresolvable-iface hotplug: endpoint unchanged (no reconcile fired)" "$(epv)" "203.0.113.9:51820"
if grep -q 'ERROR: cannot classify ifup(ghost-iface-does-not-exist)' "$WORK/ledger"
then ok "F20 unresolvable-iface hotplug: FAIL LOUD logged"
else no "F20 unresolvable-iface hotplug: silent -- no error logged anywhere"; fi
...
if command -v busybox >/dev/null 2>&1; then _HAVE_BB=1; else _HAVE_BB=0; fi
# NOT a pass and NOT a fail when busybox is absent: an undeclared dependency.
# `recon-ecosim` does not install busybox and this harness has never required it,
# so failing here would redden a job for a dependency nobody declared. The busybox
# workflow -- the one place the claim is about -- DOES install it, so the bars
# below run where they mean something. U46: each is now a DECLARED skip rather
# than a bar that silently ceases to exist, so the total is machine-independent.
# One bar name, written once: bbasrt asserts it or skips it by the same string.
bbasrt(){ if [ "$_HAVE_BB" = 1 ]; then asrt "$1" "$2" "$3"
          else skip "$1 -- busybox is ABSENT"; fi; }
bbasrt "AGG-L12p the FILE count is diff-implementation-independent (busybox diff agrees)" \
     "$(busybox diff -rq "$P5" "$MUTD" 2>/dev/null | grep -c 'differ')" "$NFILES"
bbasrt "AGG-L12p the LINE count is diff-implementation-independent (busybox diff agrees)" \
...
bbasrt "AGG-L12p known answer, busybox diff: the same 3 changed lines read 6" \
...
bbasrt "AGG-L12p known answer, busybox diff: and exactly ONE of the two files differs" \
...
bbasrt "AGG-L12p negative control, busybox diff: identical trees read 0" \
     "$(busybox diff "$DIFFU" "$D12/a" "$D12/b" 2>/dev/null | grep -c '^[-+][^-+]')" 0
rm -rf "$D12"
[ "$_HAVE_BB" = 1 ] || \
  echo "NOTE  AGG-L12p the busybox half did NOT run: the counts above are single-implementation."
...
if [ -f "$_P1B" ]; then
    ...
    # One SKIP per bar the `then` arm would have run, so the count is the same here.
    skip "SH-13 REAL NEGATIVE CONTROL: P1's curl-from-master bootstrap FAILS this lint -- p1-autorate/ is not in this tree"
    skip "SH-12 REAL NEGATIVE CONTROL: P1's bootstrap FAILS this lint (uci set/commit) -- p1-autorate/ is not in this tree"
...
# The anchor list is walked in BOTH arms so the bar count cannot drift between a
# full tree and the mirror's: absent p1-autorate/, each anchor is a declared SKIP.
for _anc in 'grep -q "GL-MT6000" /tmp/sysinfo/model' \
    ...
    if [ -f "$_P1B" ]; then
        ...
        skip "SH-18 the anchor shape-install cites resolves in P1's bootstrap: '$_anc' -- p1-autorate/ is not in this tree"
    fi
done
if [ ! -f "$_P1B" ]; then
    echo "NOTE  (the public CI mirror publishes only what CI builds). Run the full repo for it."
...
# The wall clock the daemon runs under. Empty = the real one; efpdateback() sets
# it to the ABSOLUTE path of the stepping shim. It cannot be done with PATH: see
# efpdateback, and it is measured there by EFP-1h-a2.
EFP_DATE=""
...
efpbloat() {   # a world whose ONLY escalations come from RTT, on a scheduled ping
    ...
    echo 0 > "$WORK/efpn"
    # THE FIXTURE OWNS THE CLOCK (U46). PROBATION is the one elapsed-time test in
    # ecod, and while it read the HOST's clock these bars were load-dependent: a
    # 1s backward host step between the cycle-24 eco stamp and the cycle-26 tunnel
    # bloat flipped the backoff branch and changed the ledger, which read as an
    # EFP-1/EFP-1g "flake" (3 reds in 24 runs, measured). ecod now takes its
    # elapsed seconds from $PROC_UPTIME, so the fixture hands it a clock that
    # advances one CYCLE-worth per cycle and nothing here depends on the host.
    echo "1000.00 0.00" > "$WORK/uptime"
...
if [ "\$_raw" = 1 ]; then
    ...
    echo "\$((1000 + _n * 5)).00 0.00" > "$WORK/uptime"   # monotonic, 5s per cycle
...
efpbrun() {    # efpbrun <daemon> -- 28 cycles under the EXPORTED knobs
    MAXCYCLES=28 CYCLE=0 BONDCTL="$P5/bondctl" SYS_NET="$WORK/sys" \
        PING="$WORK/fakebin/ping-sched" PROC_UPTIME="$WORK/uptime" \
        DATE="${EFP_DATE:-date}" \
...
# EFP-1h -- A BACKWARD WALL-CLOCK STEP MUST NOT MOVE AN ESCALATION CONSTANT (U46).
# PROBATION is ecod's only elapsed-time test. It used to subtract two `date +%s`
# samples, and `date` is the WALL clock: ntpd steps it backwards on every boot of
# a box with no battery-backed RTC, which is both of these boxes. A negative
# elapsed is `-lt` EVERY non-negative PROBATION -- PROBATION=0 included, where the
# backoff is supposed to be OFF -- so the daemon doubled NEED_NOW for a clock
# correction and returned to eco late with nothing in the log to say why.
#
# This was found as a "flake": EFP-1 and EFP-1g went red 3 times in 24 runs and
# the traces were identical, because the difference was one word inside the
# tunnel-bloat line ("needs 5 clean" vs "needs 10 clean"). The three reds were
# three concurrent daemons sampling 1789228653 and then 1789228652 -- one host
# step. Nothing here is timing-dependent any more, so it is a bar, not a retry.
#
# The fixture drives BOTH clocks in opposite directions at once: $PROC_UPTIME
# advances 5s per cycle (monotonic, as /proc/uptime is) while `date +%s` jumps
# BACKWARDS 5s at cycle 25 -- in between the cycle-24 eco stamp and the cycle-26
# tunnel bloat, which is the exact window the defect lives in.
#
# setup() wipes $WORK (xbin included), and efpbloat() calls it, so the shim is
# installed AFTER the fixture is built, never before it.
#
# AND IT IS HANDED OVER AS A PATH, NOT VIA $PATH (U46). The first build of this
# fixture put the shim in $WORK/xbin and relied on $PATH ordering. That works
# under bash and under Ubuntu's DYNAMIC busybox, and it silently does NOTHING on
# the mirror's parity runner, which installs busybox-STATIC: that build runs ash
# in STANDALONE mode, where an applet NAME is resolved from busybox's own table
# BEFORE $PATH is consulted. Measured on the runner (step "what busybox actually
# serves as an applet"): `command -v date` answers `date`, `sh` answers `sh`,
# while `bash` answers /usr/bin/bash -- paths only for names busybox does not
# carry. So the daemon read the REAL clock, the seeded step never happened, and
# the SENSITIVITY bar below is what caught it (ash arm, run 34707712991).
# A value with a SLASH in it is never an applet lookup, in any shell, so the
# clock now travels as $DATE -- a knob the daemon already had the idiom for
# (LOGGER, PING, PROC_UPTIME) and whose default is `date`, unchanged.
efpdateback() {
    EFP_DATE="$WORK/xbin/date"
    cat > "$WORK/xbin/date" <<EOF
#!/bin/sh
# A WALL CLOCK THAT STEPS BACK, and only for the one call ecod makes of it.
[ "\$1" = "+%s" ] || exec /bin/date "\$@"
_n=\$(cat "$WORK/efpn" 2>/dev/null || echo 0)
if [ "\$_n" -ge 25 ]; then echo 1000000000; else echo 1000000005; fi
EOF
    chmod +x "$WORK/xbin/date"
}
# The CONTROL MUTANT is the pre-U46 daemon: read the wall clock, do not clamp.
# Built by substitution so it cannot drift from the shipped file, and counted
# below so "the mutant is real" is asserted rather than assumed (as EFP-1b does).
# It is written AFTER efpbloat for the same reason the date shim is: setup()
# begins `rm -rf "$WORK"`. Written before, `sh` cannot open it, the daemon never
# runs, and the SENSITIVITY bar below passes on an EMPTY ledger -- measured, that
# is exactly what the first build of this block did.
efpwallmutant() {
    # shellcheck disable=SC2016
    #   The single quotes are the point: these are sed PATTERNS matching the
    #   literal text `_u=$(cut` and `[ "$_e" -lt 0 ]`, not expansions.
    sed -e 's|^    _u=$(cut .*|    _u=   # CONTROL MUTANT: the pre-U46 wall clock|' \
        -e 's|^    \[ "$_e" -lt 0 \] && _e=0$|    : # CONTROL MUTANT: no clamp|' \
        "$P5/bond-ecod" > "$WORK/ecod-wallclock"
}
# SENSITIVITY: the pre-U46 daemon really does break under this clock. Without
# this arm, the bar below would pass on a daemon that simply ignores the clock.
efpbloat; efpwallmutant; efpdateback
asrt "EFP-1h-a the control mutant is REAL: exactly the two clock lines were replaced, and the shipped daemon carries neither (shipped/mutant)" \
     "$(grep -c 'CONTROL MUTANT' "$P5/bond-ecod" | tr -d ' ')/$(grep -c 'CONTROL MUTANT' "$WORK/ecod-wallclock" | tr -d ' ')" "0/2"
# THE INTERPOSITION ITSELF IS A BAR (U46). Everything below measures what the
# daemon does when the wall clock steps back; if the fixture's clock never
# reaches the daemon, all of it passes blind -- which is exactly what happened on
# the busybox-static arm. Two things must hold, and neither is free: the knob
# carries a SLASH (so no shell can resolve it from an applet table instead), and
# the thing it names actually answers the seeded pre-step epoch.
asrt "EFP-1h-a2 the seeded clock reaches the daemon BY PATHNAME, not by \$PATH (slash-in-knob/shim answers the seeded epoch)" \
     "$(case "$EFP_DATE" in */*) echo yes ;; *) echo "no($EFP_DATE)" ;; esac)/$("$EFP_DATE" +%s)" \
     "yes/1000000005"
efpbrun "$WORK/ecod-wallclock"
EFP_W=$(cat "$WORK/ledger")
if [ "$EFP_W" != "$EFP_B" ]; then
  ok "EFP-1h-b SENSITIVITY: the pre-U46 wall-clock daemon DOES change its ledger when the clock steps back, so the fixture is not blind"
else
  no "EFP-1h-b the seeded backward step changed nothing even in the pre-U46 daemon -- this fixture proves nothing"
fi
asrt "EFP-1h-c ... and the change is the PROBATION BACKOFF: it doubles the next clean requirement from $EFP_CLEAN to $((EFP_CLEAN * 2))" \
     "$(printf '%s\n' "$EFP_W" | grep -c "next eco needs $((EFP_CLEAN * 2)) clean")" 1
# THE FIX: same clock, shipped daemon, ledger unchanged to the byte.
efpbloat; efpdateback; efpbrun "$P5/bond-ecod"
EFP_M=$(cat "$WORK/ledger")
if [ "$EFP_M" = "$EFP_B" ]; then
  ok "EFP-1h-d the SHIPPED daemon is unmoved by a backward wall-clock step: byte-identical ledger, because PROBATION is measured on the monotonic clock"
else
  no "EFP-1h-d a backward wall-clock step still moves the shipped ledger -- $(printf '%s\n' "$EFP_M" | grep 'tunnel path bloated' | head -1)"
fi
rm -f "$WORK/xbin/date"; EFP_DATE=""   # back to the real clock for everything after
...
# ===========================================================================
# PR — THE READ-ONLY PROBE VERBS (U224). bond-xctl's nine new dispatch arms,
# bodies at the foot of deploy/p5/lib/xctl-probe.sh.
#
# WHAT THIS BLOCK IS FOR, and it is not "the output looks right". These verbs
# are reached from a web CGI on a router, so two properties matter more than
# their answers: they must take NO ACTION (PR-8) and they must publish no key
# and no address (the portal harness's LEAK-1 owns that half, because that is
# where a probe's output actually reaches a browser). PR-1..PR-7 measure that
# each verb answers from the FIXTURE rather than from a constant, which is what
# makes PR-8's "and nothing moved" mean something.
#
# NOTHING HERE HARDCODES A SOURCE COUNT OR A SOURCE NAME. The expected row
# counts are re-derived from `_sources` in the same world, so the bars follow
# whatever the box declares -- which is the property PR-5 exists to measure.
#
# bdfp: a fingerprint of the WHOLE fact root -- names AND contents -- so PR-8
# says "$BOND_DIR did not change" rather than "the three files I thought to
# check did not change". cksum is POSIX and is already the tier
# xctl-actions.sh _same_file falls back to; find|while read handles a $WORK path
# with spaces, which xargs would not.
bdfp() {
    find "$BOND_DIR" -type f 2>/dev/null | sort | { while read -r _bf; do
        echo "$_bf"; cat "$_bf" 2>/dev/null
    done; } | cksum
}
# routedn: how many sources the box declares as ROUTED, right now.
routedn() { xctl _sources 2>/dev/null | awk '$3=="routed"' | grep -c .; }

# PR-1 — _hs: the handshake age is DERIVED from the fixture's epoch, not echoed
# from it. Seeded 120 s in the past, so a verb that printed the stored value
# would read as an age the size of the unix epoch and fail here.
setup
PRNOW=$(date +%s)
fact hs "$((PRNOW - 120))"
PRHS=$(xctl _hs)
asrt "PR-1 _hs prints exactly one peer row" \
     "$(printf '%s\n' "$PRHS" | grep -c '^peer1 handshake_age_s=')" 1
PRHSA=$(printf '%s\n' "$PRHS" | sed -n 's/^peer1 handshake_age_s=//p')
case "$PRHSA" in
    ''|*[!0-9]*) no "PR-1 the age is computed from the fixture epoch (got '$PRHSA')" ;;
    *) if [ "$PRHSA" -ge 120 ] && [ "$PRHSA" -le 180 ]; then
           ok "PR-1 the age is computed from the fixture epoch (${PRHSA}s for a 120s-old handshake)"
       else
           no "PR-1 the age is computed from the fixture epoch (got ${PRHSA}s, want 120..180)"
       fi ;;
esac
# and wg's "no handshake yet" is NAMED, not printed as an age the size of the epoch.
fact hs 0
asrt "PR-1b a peer that never completed a handshake reads 'never'" \
     "$(xctl _hs)" "peer1 handshake_age_s=never"
# THE KEY IS NOT IN THE OUTPUT. LEAK-1 next door is the bar that matters (it
# sweeps every verb through the CGI); this is the same claim at the source.
asrt "PR-1c _hs drops column 1 on the NEVER arm -- no peer key in the output" \
     "$(xctl _hs | grep -c 'PEERPUBKEY')" 0
# AND ON THE ARM A LIVE BOX TAKES. `never` prints a CONSTANT, so it cannot leak a
# column whatever the printf does -- and until this fix round that constant was
# the only arm either leak bar ran (`fact hs 0` here, `echo 0 > $WORK/hs` in the
# portal fixture). The AGE arm is the one that formats a field of the wg line.
# Measured in this round: a mutant printing $1 from that printf passed PR-1c AND
# portal LEAK-1. Both fixtures now age the handshake instead.
fact hs "$((PRNOW - 120))"
asrt "PR-1d ... and on the AGE arm, which is the one that formats a wg column" \
     "$(xctl _hs | grep -c 'PEERPUBKEY')" 0
asrt "PR-1e ... and that really WAS the age arm, not 'never' a second time" \
     "$(xctl _hs | grep -c 'handshake_age_s=never')" 0

# PR-2 — _xfer: the counters AND the box's own timestamp, which is the whole
# mechanism behind the page's rates (no sleep in the CGI; the operator's two
# clicks are the interval).
setup
fact rx 100000; fact tx 250
PRX=$(xctl _xfer)
asrt "PR-2 _xfer prints rx from the fixture"  "$(printf '%s\n' "$PRX" | awk '{print $2}')" "rx=100000"
asrt "PR-2b ... and tx"                       "$(printf '%s\n' "$PRX" | awk '{print $3}')" "tx=250"
PRTS=$(printf '%s\n' "$PRX" | sed -n 's/.* ts=//p')
case "$PRTS" in
    ''|*[!0-9]*) no "PR-2c ... and a numeric BOX timestamp (got '$PRTS')" ;;
    *)           ok "PR-2c ... and a numeric BOX timestamp ($PRTS)" ;;
esac
asrt "PR-2d _xfer drops column 1 -- no peer key in the output" \
     "$(printf '%s\n' "$PRX" | grep -c 'PEERPUBKEY')" 0

# PR-3 — _ep: the three-way CLASS, and the host never printed. `local` is the
# case that matters on an engaged box: 127.0.0.1:59402 is P5's own feeder, and a
# reader has to tell it from the server without being shown either address.
setup
asrt "PR-3 the configured server end classifies as direct" "$(xctl _ep)" "direct port=51820"
fact ep "127.0.0.1:59402"
asrt "PR-3b P5's own feeder socket classifies as local"    "$(xctl _ep)" "local port=59402"
fact ep "198.51.100.7:44444"
asrt "PR-3c a foreign endpoint on another port is neither" "$(xctl _ep)" "other port=44444"
asrt "PR-3d ... and no host reaches the output" "$(xctl _ep | grep -c '198\.51\.100\.7')" 0
fact ep ""
asrt "PR-3e no endpoint at all is absent-with-a-reason, rc 0, not an empty line" \
     "$(xctl _ep)" "absent: no endpoint (no handshake yet)"

# PR-4 — _qdisc: want/now/owner/native, then the RAW `tc -s qdisc show`. The
# `-s` is load-bearing (backlog and drops live there and nowhere else), which is
# why bin/tc had to learn to accept it -- seed A3 removes that and PR-4b is what
# goes red, because a tc that ignores the flag answers NOTHING and "nothing"
# reads exactly like an unshaped device.
setup; bctl on
PRQ=$(xctl _qdisc)
asrt "PR-4 the first line is the four facts the reconciler decides shaping on" \
     "$(printf '%s\n' "$PRQ" | head -1)" \
     "want=on now=wgclient1 owned=1 native_sqm=0 dev=wgclient1"
asrt "PR-4b the RAW queue comes back, so the -s form reached the tool" \
     "$(printf '%s\n' "$PRQ" | grep -c 'qdisc cake')" 1
asrt "PR-4c ... and every ROUTED source is shown too, by whatever name the box declares" \
     "$(printf '%s\n' "$PRQ" | grep -c '^source ')" "$(routedn)"

# PR-5 — _ifstats: N-GENERIC. Driven at N=1,2,3 through the ip shim's ladder.
# The row count must FOLLOW the box's declared routed set -- asserted against
# `_sources` in the same world, never against N, because the fixture declares
# TWO interfaces on the first device and a bar written to N would have hidden
# that. The third assertion is the one that makes this N-generic rather than
# three copies of one world: the count must MOVE as the ladder grows.
PRPREV=0
for PRN in 1 2 3; do
    setup
    fact nwan "$PRN"
    for PRD in eth1 usb0 eth0; do
        mkdir -p "$WORK/sys/$PRD/statistics"
        echo 700 > "$WORK/sys/$PRD/statistics/rx_bytes"
        echo 300 > "$WORK/sys/$PRD/statistics/tx_bytes"
        echo 4   > "$WORK/sys/$PRD/statistics/tx_dropped"
        echo 1   > "$WORK/sys/$PRD/carrier"
        echo up  > "$WORK/sys/$PRD/operstate"
    done
    PRIF=$(SYS_NET="$WORK/sys" sh "$P5/bond-xctl" _ifstats)
    PRRN=$(routedn)
    asrt "PR-5 N=$PRN one row per ROUTED source, nothing truncated" \
         "$(printf '%s\n' "$PRIF" | grep -c 'ts=')" "$PRRN"
    asrt "PR-5 N=$PRN ... and every row carries the kernel's own fields" \
         "$(printf '%s\n' "$PRIF" | grep -c 'rx=700 tx=300 txdrop=4 carrier=1 oper=up ts=')" "$PRRN"
    if [ "$PRRN" -gt "$PRPREV" ]; then
        ok "PR-5 N=$PRN ... and the row count FOLLOWS the declared set (was $PRPREV, now $PRRN)"
    else
        no "PR-5 N=$PRN the row count did not follow the declared set (was $PRPREV, now $PRRN)"
    fi
    PRPREV=$PRRN
done
# A counter the box does not publish reads `-`, never 0: a fabricated zero on a
# monitoring page is indistinguishable from a measured one.
asrt "PR-5b an unpublished counter reads '-', not 0" \
     "$(SYS_NET="$WORK/nosuchsys" sh "$P5/bond-xctl" _ifstats \
        | grep -c 'rx=- tx=- txdrop=- carrier=- oper=- ts=')" "$(routedn)"

# PR-6 — _reach: THE ARGV IS THE BAR. "read-only" is not the only property a
# probe reachable from a CGI needs: an unbounded ping is a denial of service
# against the box the operator is trying to debug. So the ping is RECORDED and
# every recorded invocation is compared to the shape, token by token -- one
# packet, a one-second deadline, bound to that source's own device, and nothing
# else. Names are not asserted anywhere here.
# Two shims are local to this block: the shared bin/ip prints no `via` (nothing
# needed one until now) and bin/ping records nothing.
setup
mkdir -p "$WORK/prbin"
cat > "$WORK/prbin/ip" <<'PRIPEOF'
#!/bin/sh
# default routes WITH a gateway, the way iproute2 prints them. _reach parses
# `via` exactly as scripts/e1-probe.sh:250-252 does.
case "$1 $2" in
  "route show"|"route "*)
    echo "default via 192.0.2.254 dev eth1 metric 10"
    echo "default via 192.0.2.253 dev usb0 metric 20"
    echo "default via 192.0.2.252 dev eth0 metric 30" ;;
esac
exit 0
PRIPEOF
cat > "$WORK/prbin/ping" <<'PRPINGEOF'
#!/bin/sh
echo "ping $*" >> "$ECOSIM_STATE/pingargv"
echo "64 bytes from x: seq=0 ttl=54 time=20.0 ms"
exit 0
PRPINGEOF
chmod +x "$WORK/prbin/ip" "$WORK/prbin/ping"
: > "$WORK/pingargv"
PRR=$(IP="$WORK/prbin/ip" PING="$WORK/prbin/ping" sh "$P5/bond-xctl" _reach)
PRARGV=$(awk '{ if ($1=="ping" && $2=="-c" && $3=="1" && $4=="-W" && $5=="1" && $6=="-I" && NF==8) next
                print }' "$WORK/pingargv")
asrt "PR-6 every ping is EXACTLY '-c 1 -W 1 -I <dev> <gw>' -- one packet, bounded, bound to the device" \
     "$PRARGV" ""
    # The comparand is read through the SAME ip shim this bar pinged with: the
    # local shim declares a third default route, so `routedn` (which runs the
    # shared one) would answer about a different world. Measured -- the first
    # cut of this bar compared 4 pings against 3 sources and went red on the
    # harness's own inconsistency, not on the probe.
asrt "PR-6b one ping per routed source, no more" "$(grep -c . "$WORK/pingargv")" \
     "$(IP="$WORK/prbin/ip" sh "$P5/bond-xctl" _sources | awk '$3=="routed"' | grep -c .)"
asrt "PR-6c the rtt is reported per source" \
     "$(printf '%s\n' "$PRR" | sed -n '1p' | sed 's/^[^ ]* //')" "rtt_ms=20.0"
asrt "PR-6d the target is NEVER printed" "$(printf '%s\n' "$PRR" | grep -c '192\.0\.2\.')" 0
# THE VALUES, NOT JUST THE SLOTS. PR-6 above compares argv POSITIONS and NF==8;
# that is necessary and it is not sufficient -- `-I lo` against a hardcoded
# target has exactly that shape and would have walked through it, while the
# bar's own name said "bound to the device". Measured in this fix round. The
# comparand is read back out of the SAME ip shim the probe was pointed at, so
# the pairs come from the fixture and are asserted nowhere as a constant.
"$WORK/prbin/ip" route show default | awk '
    { d = ""; v = ""
      for (i = 1; i <= NF; i++) { if ($i == "via") v = $(i+1); if ($i == "dev") d = $(i+1) }
      if (d != "" && v != "") print d, v }' | sort > "$WORK/prexpect"
# `sort -u`: two declared interfaces can share one l3_device (measured -- this
# world pings 4 times across 3 devices), so what is compared here is the SET of
# dev/target pairs. "one ping per routed source, no more" is PR-6b's claim and
# is left to PR-6b; duplicating it here would have made this bar red on a
# correct probe.
awk 'NF == 8 { print $7, $8 }' "$WORK/pingargv" | sort -u > "$WORK/prgot"
asrt "PR-6g the -I VALUE and the TARGET are that source's own dev/via pair from the fixture" \
     "$(comm -13 "$WORK/prexpect" "$WORK/prgot" | grep -c .)" 0
asrt "PR-6h ... and the devices pinged ARE the routed source set (none skipped, none invented)" \
     "$(awk '{print $1}' "$WORK/prgot" | sort -u | tr '\n' ' ')" \
     "$(IP="$WORK/prbin/ip" sh "$P5/bond-xctl" _sources | awk '$3=="routed"{print $2}' | sort -u | tr '\n' ' ')"
# THE CONTROL: the comparison must be able to fail. A pair bound to a device the
# fixture never declared is exactly what the mutant `-I lo` produces. It is built
# from the EXPECTED set plus one bogus pair, not from what the probe actually
# ran, so it stays an instrument check under every mutant instead of moving with
# the defect it is there to detect (measured: the first cut derived it from
# $WORK/prgot and went red on the `-I lo` arm alongside the two bars that were
# supposed to catch it, which reads as three defects where there is one).
{ cat "$WORK/prexpect"; echo "lo 192.0.2.254"; } | sort -u > "$WORK/prgotctl"
asrt "PR-6i CONTROL: a ping bound to a device the fixture never declared IS caught" \
     "$(comm -13 "$WORK/prexpect" "$WORK/prgotctl" | grep -c .)" 1
# A source whose default route names no gateway is NOT reported as a loss: it
# was never pinged, and `lost` would be a measurement nobody took.
cat > "$WORK/prbin/ip" <<'PRIP2EOF'
#!/bin/sh
case "$1 $2" in "route show"|"route "*) echo "default dev eth1 metric 10" ;; esac
exit 0
PRIP2EOF
chmod +x "$WORK/prbin/ip"
: > "$WORK/pingargv"
asrt "PR-6e a source with no gateway says so instead of claiming a loss" \
     "$(IP="$WORK/prbin/ip" PING="$WORK/prbin/ping" sh "$P5/bond-xctl" _reach | grep -c ' no_gateway$')" \
     "$(IP="$WORK/prbin/ip" sh "$P5/bond-xctl" _sources | awk '$3=="routed"' | grep -c .)"
asrt "PR-6f ... and nothing was pinged" "$(grep -c . "$WORK/pingargv")" 0

# PR-7 — _armed: is a rollback already counting down on this box. A deploy step
# that can cut the management path arms one before it runs
# (p5/contract/paths:149-150), so an operator reading this page before pressing
# anything has to see it. The record format is p5-deadman's own; the restore
# COMMAND is deliberately not printed -- the label is the handle.
setup
asrt "PR-7 no records reads 'none', rc 0" "$(xctl _armed)" none
mkdir -p "$WORK/etc/p5/deadman"
PRDL=$((PRNOW + 600))
{ echo "P5_DM_LABEL=portal-smoke"
  echo "P5_DM_DEADLINE=$PRDL"
  echo "P5_DM_RESTORE=/usr/lib/p5/portal/bin/p5-portal-restore --label portal-smoke"
  echo "P5_DM_SESSION=no-ssh-session"; } > "$WORK/etc/p5/deadman/portal-smoke"
asrt "PR-7b a seeded record is listed by label, deadline and session rule" \
     "$(xctl _armed)" "label=portal-smoke deadline=$PRDL session=no-ssh-session"
asrt "PR-7c ... and the restore command line is not printed" \
     "$(xctl _armed | grep -c 'p5-portal-restore')" 0

# PR-8 — THE NO-SIDE-EFFECT BAR, and it is the reason this surface is allowed
# behind a web page at all. Run EVERY verb against a fully engaged world and
# assert that the ledger the shims write gains no TC, no SVC and no iptables
# line; that the wg endpoint is where it was (the wg shim's `set` arm writes
# that file, so a verb that drove wg would show here); that the attached qdisc
# is untouched; and that the fingerprint of the WHOLE fact root is identical.
setup; bctl on
PRTC0=$(grep -c '^TC '  "$WORK/ledger");      PRSVC0=$(grep -c '^SVC ' "$WORK/ledger")
PRIPT0=$(grep -c '^iptables ' "$WORK/ledger")
# THE NATIVE GL SURFACE HAS A COUNTER NOW. bin/uci served reads and fell off the
# end of its own case for every mutating verb, so `uci set ... ; uci commit`
# exited 0 SILENTLY and moved nothing any bar here reads: measured in this fix
# round, a probe that wrote GL's config passed ecosim 676/0 and portal 158/0.
# bin/uci writes `UCI <argv>` now and PR-8i counts it, which pairs with the
# widened PC-3c token next door -- static and dynamic, neither alone.
PRUCI0=$(grep -c '^UCI ' "$WORK/ledger")
PREP0=$(epv); PRQD0=$(qdiscv); PRFP0=$(bdfp)
PRRC=0
for PRV in _shape _hs _xfer _ep _qdisc _ifstats _reach _feeder _armed _version; do
    if ! xctl "$PRV" >/dev/null 2>&1; then PRRC=$((PRRC + 1)); echo "  rc!=0: $PRV"; fi
done
asrt "PR-8 every probe verb exits 0 (rc!=0 is reserved for 'the tool itself failed')" "$PRRC" 0
asrt "PR-8b zero TC lines added"       "$(grep -c '^TC '  "$WORK/ledger")"      "$PRTC0"
asrt "PR-8c zero SVC lines added"      "$(grep -c '^SVC ' "$WORK/ledger")"      "$PRSVC0"
asrt "PR-8d zero iptables lines added" "$(grep -c '^iptables ' "$WORK/ledger")" "$PRIPT0"
asrt "PR-8e the wg endpoint is untouched (a wg set would have rewritten it)" "$(epv)"    "$PREP0"
asrt "PR-8f the attached qdisc is untouched"                                 "$(qdiscv)" "$PRQD0"
asrt "PR-8g the WHOLE fact root is byte-identical, names and contents"       "$(bdfp)"   "$PRFP0"
asrt "PR-8i zero UCI lines added -- no probe writes a native GL.iNet surface"  "$(grep -c '^UCI ' "$WORK/ledger")" "$PRUCI0"
# THE CONTROL. PR-8 must be able to go red, or it is a bar that would pass on a
# tree with no probes in it at all: ONE known action in the SAME world moves
# exactly the counter PR-8c reads.
"$BIN/svc-shape" restart >/dev/null 2>&1
if [ "$(grep -c '^SVC ' "$WORK/ledger")" -gt "$PRSVC0" ]; then
    ok "PR-8h CONTROL: a real service action DOES move the ledger -- PR-8c can fail"
else
    no "PR-8h CONTROL: a real service action did not move the ledger -- PR-8c proves nothing"
fi
# ... and the same for the counter this fix round added, because a counter that
# never moves passes on a shim that logs nothing, which is precisely the defect
# PR-8i was written to close.
"$BIN/uci" set network.wan.metric=9 >/dev/null 2>&1
if [ "$(grep -c '^UCI ' "$WORK/ledger")" -gt "$PRUCI0" ]; then
    ok "PR-8j CONTROL: a real uci write DOES move the ledger -- PR-8i can fail"
else
    no "PR-8j CONTROL: a real uci write did not move the ledger -- PR-8i proves nothing"
fi

# PR-9 — THE COST OF A CLICK, MEASURED. The brief sets "fork count per verb <= 4"
# (docs/knowledge/design/p5-portal-plan.md:387) and nothing in this tree measured
# it, so the number was an argument rather than a fact. This bar counts the
# BOX-TOOL forks each verb pays -- ubus, uci, wg, ip, tc, ping, pgrep, the seven
# externals a router actually charges for -- with a counting wrapper for each
# placed FIRST on PATH ahead of the ecosim shims. Shell builtins, and the
# awk/sed/grep/cat a verb also runs, are NOT counted, so every number here is a
# LOWER BOUND on total forks and the bar's name says so.
#
# WHAT IT MEASURED, and the answer splits the verbs in two.
# The SIX SOURCE-FREE verbs are inside the brief's budget and are held to the
# brief's own number, 4: measured _hs=1 _xfer=1 _ep=4 _version=0 _armed=0
# _feeder=2. Two of those numbers are this fix round's: _ep was 5 until
# ep_is_direct learned to take an already-read endpoint as $1 (it re-read it
# through ep_now, three processes, for one click), and _feeder was pgrep-based
# beside a ubus-first svc_running.
# The THREE ENUMERATING verbs cannot be: _qdisc, _ifstats and _reach take their
# rows from gl_sources, which is ONE ubus call per DECLARED interface by
# construction, because OBJ-A settles that the source table comes FROM GL and
# never from route inference, and PR-5 above asserts the rows FOLLOW that
# declared set. Measured here at 9 declared interfaces: _qdisc=17, _ifstats=12,
# _reach=12. So "<= 4 forks per verb" and "the source list is N-generic and
# comes from ubus" cannot both hold for those three. Which one is re-scoped is a
# design decision and it is Mo's, not this harness's -- so PR-9b holds them to a
# FORMULA in the declared interface count rather than to a number somebody
# picked, and a verb that grows a fork independent of the source set goes red.
setup
mkdir -p "$WORK/forkbin"
for FT in ubus uci wg ip tc ping pgrep; do
    { echo '#!/bin/sh'
      echo "echo $FT >> \"\$ECOSIM_STATE/forkcount\""
      echo "\"$BIN/$FT\" \"\$@\""; } > "$WORK/forkbin/$FT"
    chmod +x "$WORK/forkbin/$FT"
done
PR9OVER=""; PR9LINE=""
for PR9 in '_hs 4' '_xfer 4' '_ep 4' '_version 4' '_armed 4' '_feeder 4'; do
    PR9V=${PR9%% *}; PR9C=${PR9##* }
    : > "$WORK/forkcount"
    PATH="$WORK/forkbin:$PATH" sh "$P5/bond-xctl" "$PR9V" >/dev/null 2>&1
    PR9N=$(grep -c . "$WORK/forkcount")
    PR9LINE="$PR9LINE $PR9V=$PR9N"
    [ "$PR9N" -le "$PR9C" ] || PR9OVER="$PR9OVER $PR9V($PR9N>$PR9C)"
done
asrt "PR-9 every SOURCE-FREE verb is inside the brief's budget of 4 box-tool forks ($PR9LINE)" \
     "$PR9OVER" ""
# The three enumerating verbs, held to a FORMULA in the declared interface count
# (gl_sources' one ubus per interface plus that verb's own fixed reads) rather
# than to a bare number. A verb that grows a fork the source set does not
# explain goes red; the source set growing does not.
PR9IF=$(ubus list 2>/dev/null | sed -n 's/^network\.interface\.//p' | sort -u | grep -c .)
PR9ENUM=""; PR9EOVER=""
for PR9 in "_qdisc $((PR9IF + 8))" "_ifstats $((PR9IF + 3))" "_reach $((PR9IF + 3))"; do
    PR9V=${PR9%% *}; PR9C=${PR9##* }
    : > "$WORK/forkcount"
    PATH="$WORK/forkbin:$PATH" sh "$P5/bond-xctl" "$PR9V" >/dev/null 2>&1
    PR9N=$(grep -c . "$WORK/forkcount")
    PR9ENUM="$PR9ENUM $PR9V=$PR9N/$PR9C"
    [ "$PR9N" -le "$PR9C" ] || PR9EOVER="$PR9EOVER $PR9V($PR9N>$PR9C)"
done
asrt "PR-9b the enumerating verbs stay at ifaces+base, ifaces=$PR9IF (measured/ceiling:$PR9ENUM) -- over the brief's 4 by construction, see the block comment" \
     "$PR9EOVER" ""
# THE CONTROL: the counter must be able to see a fork at all.
: > "$WORK/forkcount"
PATH="$WORK/forkbin:$PATH" sh -c 'ubus list >/dev/null 2>&1'
if [ "$(grep -c . "$WORK/forkcount")" -ge 1 ]; then
    ok "PR-9c CONTROL: the fork counter really records a box-tool call"
else
    no "PR-9c CONTROL: the fork counter recorded nothing -- PR-9 proves nothing"
fi
# ... and 4 must be a THRESHOLD, not a number every verb passes. `_sources` is
# gl_sources with nothing else on top and it is over the line, so PR-9 above is
# discriminating rather than vacuous.
: > "$WORK/forkcount"
PATH="$WORK/forkbin:$PATH" sh "$P5/bond-xctl" _sources >/dev/null 2>&1
if [ "$(grep -c . "$WORK/forkcount")" -gt 4 ]; then
    ok "PR-9d CONTROL: 4 really discriminates -- _sources measures over it"
else
    no "PR-9d CONTROL: _sources did not measure over 4 -- PR-9's ceiling is not a threshold"
fi
setup

# PR-10 — ONE FEEDER PREDICATE, AND IT IS THE RECONCILER'S. The first cut of
# _feeder ran its own `pgrep -f "$AGG_BIN"` beside svc_running (xctl-probe.sh,
# ubus-first per divergence D5), which is two predicates for one fact: on a box
# where ubus lists p5-datapath but the argv does not match the pattern -- a
# wrapper, a busybox pgrep truncating the command line, a renamed binary -- the
# PAGE would have said "not running" on the same tick xctl-dag.sh reads
# `running`. A monitoring page that disagrees with the reconciler about the one
# fact it exists to report is worse than no page. The verb now reports
# svc_running's answer and prints no PID, and these three bars hold it: the
# agreeing world both ways, and then the world where the two DISAGREE.
setup
fact_run() { echo "$2" > "$WORK/running.$1"; }
fact_run p5-datapath 1
asrt "PR-10 the service ubus lists reads live=yes" \
     "$(xctl _feeder | sed -n 's/^live=\([a-z]*\) .*/\1/p')" yes
fact_run p5-datapath 0
asrt "PR-10b ... and a service ubus does not list reads live=no" \
     "$(xctl _feeder | sed -n 's/^live=\([a-z]*\) .*/\1/p')" no
fact_run p5-datapath 1
asrt "PR-10c ... and when the two predicates DISAGREE (ubus lists it, pgrep's pattern does not match), the page says what the RECONCILER says" \
     "$(AGG_BIN=/usr/sbin/p5-datapath-wrapper sh "$P5/bond-xctl" _feeder | sed -n 's/^live=\([a-z]*\) .*/\1/p')" yes
# THE CONTROL: that world really IS a disagreement -- pgrep on the wrapper name
# finds nothing, so PR-10c is not passing because the two agree anyway.
asrt "PR-10d CONTROL: pgrep on the wrapper argv really finds nothing in that world" \
     "$(pgrep -f /usr/sbin/p5-datapath-wrapper 2>/dev/null | grep -c .)" 0
# ... and no PID reaches the page: it is derivable only from the predicate this
# verb just refused to duplicate.
asrt "PR-10e no pid= field is printed" "$(xctl _feeder | grep -c 'pid=')" 0
setup

echo "===== Layer-2: $pass passed, $fail failed, $skipped skipped ====="
# U46: pass+fail+skipped is the DECLARED bar count -- how many bars this file OFFERED,
# independent of their verdicts and independent of which optional dependencies the
# machine has. It is the same number on a full tree and on the public mirror (which
# does not publish p1-autorate/), which is what lets recon-ecosim floor on it.
# A FAILING bar is a declared bar: the ALL-PASS exit below is what catches those, and
# folding them into this total would make a red bar read as a DELETED one.
echo "===== Layer-2 declared bars: $((pass + fail + skipped)) ====="
[ "$fail" = 0 ] || exit 1

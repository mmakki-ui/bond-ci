...
# S5b (U247) — an UNKNOWN mode fact must FAIL LOUD, not read as lightning. The fact is
# planted directly rather than through `bctl mode`, because the CLI already refuses an
# unrecognised name; what this bar exercises is the box finding one ALREADY on disk -- a
# legacy name, a typo, a half-finished U17b migration. With the old `*) R="$W"` default the
# reconciler enrolled every live source, which IS lightning: every packet duplicated on a
# mode nobody defined, silently (ADR-003 rule 6).
setup; bctl on; bctl mode eco
printf 'bogus-mode\n' > "$WORK/etc/p5/mode"
hook
asrt "S5b unknown mode: enrols NOTHING (no silent all-WANs)" "$(aggf AGG_PATHS)" ""

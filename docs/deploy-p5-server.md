> **⚠ PARTIALLY STALE (2026-09-06, U240): §C4 FP-3 describes a 2048-slot resequencer ring — DELETED at both ends (U159). Also the deadman commands `p5-fw-deadman arm/confirm/status` name a binary that no longer installs; use `p5-deadman` (fixed under U245).**
...
**The name in the F3 row is `p5-deadman`, not `p5-fw-deadman`.** U38a consolidated the two rollback

...
**After S6 — the rung that proves the wire is AUTHENTICATED (U291).** Once the real client has
engaged on `:59402`, run, on the server:

```sh
[SERVER] p5-server-preflight --post-engage; echo "rc=$?"
```

It must end `transport-auth-gate: ok` with `rc=0`. The gate verdict is read from the daemon's own
last `SSTAT` line and `gate=1` means CLOSED — a frame carrying a VALID tag arrived inside the last
30s. `gate=0` at this rung means the two boxes carry **different transport secrets**: this box
reopens the gate 30s after the last good tag and then serves `:59402` **unsigned and unverified**
rather than dropping it, because it has no console and a rule that can lock the legitimate client
out loses the box (`p4-bondagg/server/auth.go:220,307-330`). Nothing else on this page detects
that — the tunnel is up, the handshake is fresh, the counters move.

Run the same command **without** `--post-engage` at every earlier rung (F1..S5, and §5's C1). There
it REPORTS the gate and exits 0 instead of refusing, which is required and not a weakening: the
gate has never closed on a box that has not yet been handed a frame, and an idle daemon emits a
fresh `gate=0` line once a minute (`StatHeartbeat = 60s`, `p4-bondagg/server/main.go:57-63`), so a
check that refused on it would refuse every server rung there is. An `authbad=` count is reported
in both modes and refuses in neither: it is cumulative for the life of the process and never reset,
so one stray or scanning frame on a public port sets it permanently.

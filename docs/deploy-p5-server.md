> **⚠ PARTIALLY STALE (2026-09-06, U240): §C4 FP-3 describes a 2048-slot resequencer ring — DELETED at both ends (U159). <!-- rb16-exempt: names the dead tool on purpose --> The deadman commands named `p5-fw-deadman`, a binary U38a deleted; renamed to `p5-deadman` by U245 and held there by RB-16.**
...
| C2 | `p5-deadman` `arm` → `confirm` from a NEW session | the timer and boot limbs exist on GL firmware; `setsid`/`nohup`/`crond` are real here |
...
p5-deadman arm --after <SECONDS> --restore-script /etc/p5/restore-firewall.sh --label fw
/etc/init.d/firewall reload
# ... then, FROM A NEW SSH SESSION:
p5-deadman confirm --label fw
```
...
**Check `p5-deadman status` before F3.** It prints which limbs are actually live on that box. If it
...
`/etc/init.d/p5-server` (`contract/paths:183,186` -- this cite said `155,158` until U174 re-resolved it; those two lines are auth-key prose, not the rows). The server's `/usr/sbin` holds exactly one of our
...
**THE INSTALL IS THE PACKAGE.** It was a hand copy when this section was written and that is no longer
the shape. `scripts/build-p5-package.sh` builds ONE unpacked package from a clean checkout and prints
its path on stdout, every log line on stderr (`scripts/build-p5-package.sh:2-10`). It cross-builds
`p4-bondagg/server` as `GOOS=linux GOARCH=arm64 CGO_ENABLED=0` into `build/linux-arm64/p5-server`, so
the binary is BUILT from the sha being deployed rather than fetched -- there is no fixed CI-artifact
hash to quote here any more, and `MANIFEST.sha256` inside the package is what pins it. The arch is
still checked rather than assumed: `uname -m` on the box is `aarch64` (inventory:8), and
`p5/payload/filemap:38-42` records that choice and the evidence for it.

**The filemap carries exactly two `server` rows, and they are S2 and S3's two files.** Measured:
`grep -v '^#' p5/payload/filemap | grep -c server` -> **2**, at `p5/payload/filemap:57-58`:

    755|server|build/linux-arm64/p5-server|/usr/sbin/p5-server
    755|server|deploy/server/init.d/p5-server|/etc/init.d/p5-server

Mode 755 both -- the same mode the hand steps used, so the package and this runbook now agree about
modes and paths by construction rather than by two people remembering the same thing. Every `client`
row is SKIPPED for `--role server` (`p5/bin/p5-install:333`), which is what makes one package serve
both boxes without either being a special case of the other.

**What else a `--role server` install places.** Not from the filemap: copied from the package's own
`bin/`, `lib/` and `contract/` (`p5-install:370-376`) or synthesised (`:380-383`) --
`/usr/lib/p5/p5-common.sh`, `/usr/lib/p5/contract-{namespace,foreign,paths}`,
`/usr/lib/p5/{stamp,installed.dirs,installed.files}`, `/usr/sbin/{p5-uninstall,p5-version,p5-deadman}`,
and `/etc/p5/transport.key` at mode 600; plus the directories `/usr/lib/p5`, `/etc/p5` and
`/etc/p5/deadman` (`p5/contract/paths:121,148,149`). Two consequences worth stating out loud: the
removal record `installed.files` is one of them, which is what makes section 8 precise; and the
rollback primitive section 5 arms is another, installed as **`/usr/sbin/p5-deadman`**
(`contract/paths:145`).

**Every destination is re-judged against the contract at install time.** `p5_check_dest` tests each one
against `contract/foreign`, `contract/namespace` and `contract/paths`, and ONE violation refuses the
WHOLE install with exit 4 -- never a partial install with the bad row skipped (`p5-install:339`,
`:349-350`). A hand `install -m755` has no such gate, and that is the substantive difference between
this and the procedure that stood here before.

**The installer enables nothing and starts nothing.** Its header says so (`p5-install:61-65`) and its
closing line says so again: *"Nothing was enabled, nothing was started, no service was stopped and no
config was committed"* (`:608`). S3's *do NOT enable* and S7's deferral below are therefore what the
tool already does, not something the operator has to remember not to do.

**The transport secret is a two-box fact, and it is why this box is installed SECOND.** Both boxes must
carry the SAME `/etc/p5/transport.key`; exactly one generates and the other ADOPTS a copy via
`--transport-key FILE` (`p5-install:152-158`), and an install that cannot produce one refuses outright
rather than finishing with authentication OFF (`:528-529`). G3a installs the client first, so this box
adopts. Nothing on either box can verify the pair by itself -- compare `P5_TRANSPORT_KEY_ID` in the two
stamps, which is a 12-character digest and not the secret (`p5-install:570-571`, `:613-617`). Copy the
client's key over the management path of section 3, use it, then delete the copy from `/tmp`.

**Order, each step chosen for what happens if it fails halfway:**

| # | step | if it fails halfway |
|---|---|---|
| S0 | on the dev PC: `PKG=$(bash scripts/build-p5-package.sh)`, then `sh p5/bin/p5-install --package "$PKG" --role server --dry-run` | the box has not been touched. `--dry-run` runs every check and prints the plan, including the removal set the install would create (`p5-install:150-151`, `:418-419`); a contract violation exits 4 and an integrity failure exits 3, both before the box is in it |
| S1 | copy the package to `/tmp` on the server, then NAME it once and verify it THERE: `PKG=/tmp/p5-pkg.XXXXXX` (substitute the real `mktemp` suffix `build-p5-package.sh` printed on the dev PC -- `build-p5-package.sh:88`), then `cd "$PKG" && sha256sum -c MANIFEST.sha256`. Every later step in this section and its rollback row uses `"$PKG"`, so the name is typed once, quoted everywhere, and never spelled as a `<...>` placeholder: `<` and `>` are sh redirection operators, and a bracket placeholder pasted verbatim is a redirection, not a path | a corrupt push is caught in `/tmp`, nothing is installed |
| S2 | `sh "$PKG"/bin/p5-install --package "$PKG" --role server --transport-key /tmp/transport.key` (`PKG` from S1) -- this is what places `/usr/sbin/p5-server` and `/etc/init.d/p5-server` | it refuses whole or it lands whole. A run that dies part way leaves an intent record, so the box reads `incomplete` and the NEXT install refuses rather than layering (`p5-install:307-308`); `p5-uninstall --remove` then undoes exactly the recorded plan, whether or not each step landed |
| S3 | confirm what S2 placed: `p5-version --verify`, then `p5-uninstall --list`. The service file is the tree's `deploy/server/init.d/p5-server`, mode 755 at `/etc/init.d/p5-server`, and it is inert -- the installer does not enable it, and **do NOT enable** it by hand here either; S7 owns that decision | an init script with no rc.d symlink is inert |
| S4 | run the daemon **by hand, in the foreground**, bounded | you watch it fail and Ctrl-C. No supervisor retries it |
...
**Still a hand step, deliberately: the firewall.** Section 5's F1/F2/F3 are NOT in the package. No
filemap row names `/etc/config/firewall`, and the installer writes no uci and reloads nothing
(`p5-install:608`) -- the install is additive and switches nothing. Opening `:59402` stays the
operator's own deadmanned step, in the order section 5 gives, and S0-S3 above do not depend on it.

**The preflight this section leans on IS in the package now, and it runs twice from two different
places.** The paragraph opening section 6 says the two destinations are "confirmed free by the
preflight", and section 5, C1 and half the open-items register call for
`deploy/server/p5-server-preflight.sh` too. That was the one hand `scp` this section had not
replaced, and it is gone: **U176 was folded into U211, which landed the filemap and contract rows.**
Two invocations, and they are not the same invocation twice:

1. **Before the install, straight from the unpacked package** --
   `sh "$PKG"/payload/deploy/server/p5-server-preflight.sh`. This is the one gate C1 needs: it is
   read-only, it needs no stamp and no key, and it answers whether the two destinations are free
   *before* anything is placed. The package carries the file under `payload/` because a filemap row
   names it as its `src`, so it is manifest-pinned like every other payload file.
2. **After the install, from `/usr/sbin`** -- `p5-server-preflight`. The filemap row
   `755|server|deploy/server/p5-server-preflight.sh|/usr/sbin/p5-server-preflight` places it and
   `p5/contract/paths` declares that destination, so `p5-install` records it in `installed.files`
   and `--remove` takes it away. This is the copy an operator re-runs weeks later on a box where
   `/tmp` has been cleared and no package is left -- the same reason `p5-accept` is installed
   rather than run from the package.

Both are the same bytes: the builder copies one repo file into `payload/`, and the installer places
that same file. E0's `RP-4` asserts the packaged copy is byte-identical to the repo file and
sh-parseable, and `RP-5` asserts the installed copy executes from `$ROOT/usr/sbin` with a foreign
working directory -- neither assumes it was started from `deploy/`.
...
The file the package places at `/etc/init.d/p5-server` is **`deploy/server/init.d/p5-server`** in this
repo -- `p5/payload/filemap:58` is the row that says so, and S3 above verifies it landed. It did not
exist when this runbook was written — that gap is ROADMAP U112. Its header carries the derivation above and the
...
`df /overlay` and a `logread` count. Run it before S2 (baseline: nothing installed yet -- S2 is the install step now, not S3) and again after
...
| F3 reload | `p5-deadman fire --label fw`, or let the deadline pass | this is what the deadman is for |
| S0-S1 package staged | `rm -rf "${PKG:?set PKG first, see S1}" /tmp/transport.key`. `PKG` is S1's variable; the `:?` refuses an unset or empty one rather than expanding to a bare `/tmp`, and the quotes keep it ONE argv word. Do NOT write this row's path as `<pkg>` -- unquoted `<` and `>` are sh redirections, so that form removes `/tmp` itself and TRUNCATES the transport key S2 reads. Nothing outside `/tmp` was written | no |
| S2 installed | **`p5-uninstall --remove`** -- plan it first with `--dry-run`. It removes exactly what the install RECORDED, every entry re-validated against the shipped contract, and one entry the contract does not admit refuses the WHOLE removal and names it (`p5/bin/p5-uninstall:27-40`). It also takes back the transport key and the `/usr/lib/p5` metadata, which a file-by-file `rm` leaves behind | no |
| S5 running | `/etc/init.d/p5-server stop` -- do this BEFORE the `--remove` in the `S2 installed` row | no |
| S7 enabled | `/etc/init.d/p5-server disable`, verify no `/etc/rc.d/*p5-server`, then remove | no |

**Do NOT undo S2 with `rm -f /usr/sbin/p5-server /etc/init.d/p5-server`.** That is what this section
said before the package existed, and against a package install it is a wedge rather than a rollback:
`/usr/lib/p5/installed.files` would still describe two files that are gone, so the box reads neither
`clean` nor accountable (`p5_box_state`, `p5/lib/p5-common.sh:895-932`) and the next install refuses
with exit 5 and tells you to run the uninstaller anyway (`p5-install:305-312`). On a box with no
console, a removal that blocks the reinstall is the failure mode to design against, and that is the
whole reason the uninstaller derives its set from the record instead of from a command line.

**`--remove` stops a service only if S7 had enabled it.** SVCDOWN is derived from the
`/etc/rc.d/[SK][0-9][0-9]p5-*` flags -- that glob is the contract's `enable`-state row
(`p5/contract/paths:221`, the only `glob`+`enable` row there), `p5-uninstall:497` loops over that row's
hits and nothing else, and `:549` -- guarded by the `-x` test at `:544` -- is the only line in the
planner that emits a SVCDOWN. So a `p5-server` that is RUNNING but was
never enabled -- exactly where S6 leaves it -- has no flag, gets no SVCDOWN, and keeps running while
its binary is unlinked. Stop it first. That is the S5 row above, and it is an ordering, not a
suggestion.

<!-- rb16-exempt: names the dead tool on purpose; that IS the record --> **The name in the F3 row is `p5-deadman`, not `p5-fw-deadman`.** U38a consolidated the two rollback
primitives into one file and deleted the twin (`p5/bin/p5-deadman:14-21`); the package installs the
survivor as `/usr/sbin/p5-deadman` (`p5/contract/paths:145`). Section 5 still types the old name --
that is O17, and it is not this unit's section to rename.
...
| O14 | **RE-OPENED AND RE-CLOSED THE OTHER WAY (U290, 2026-09-13).** The U130 adjudication below is REVERSED. "Refusing would leave NO deadman at all, which stays the worse outcome" weighed the wrong two things: the alternative to a refusal is not "no deadman", it is a deadman the operator has been told is ARMED and which will not fire after a reboot — and the exit code was the ONLY channel a scripted caller reads, so `p5-portal-run`-shaped callers proceeded past it. Worse, the crontab-directory test U130 settled on was not the binding one: `boot_limb_install` never checked that **crond was RUNNING**, so the line it wrote was inert on a box whose `status` said `boot: ABSENT`, and the line names `$SELF` verbatim with nothing checking that `$SELF` survives a reboot — an arm from the unpacked package under `/tmp` wrote a tmpfs path. `arm` now REFUSES (exit 5, **no record, nothing created**) unless crond is running, the crontab directory exists and is writable, and the resolved `$SELF` is under `$P5_SBIN`; `--no-boot-limb` is the only override and logs that the limb is deliberately absent. The DM-1 objection that forced the U130 reversal is answered at the call site, not by weakening the rule: `p5/test/run.sh`'s nine `dm arm` sites take `--no-boot-limb`, which is what those bars have always been — none of them owns the boot limb. `DM-45`/`DM-45a`/`DM-45b` are INVERTED to assert the refusal; `DM-45c`..`DM-45f`, `DM-63`..`DM-63c`, `DM-65`..`DM-65b`, `DM-66`..`DM-66b`, `DM-67`, `DM-68`..`DM-68c`, `DM-69` and `DM-70`..`DM-70f` are new. **The fix round added three more things the gate did not cover:** both `printf >> $CRONTAB` appends in `boot_limb_install` discarded their exit status, so a write that could not land still returned 0 and arm printed ARMED over a crontab with no line — they now carry `|| return 1` and a `p5_sync`, and the caller's existing removal-and-exit-5 path does the rest; `P5_CRONTAB` was consumed but not in `p5-common.sh`'s override list, so an INHERITED one passed all three conditions against a file `crond` does not read; and `crond_is_running` reads the PROCESS TABLE, which is not the same fact as `/etc/rc.d/S*cron` — cron running now is not cron enabled at boot, and the limb exists to survive a reboot — so that second fact is now REPORTED (never gated: `cron_boot_state` says why) both in the refusal text and as a WARNING on a successful arm whose limb is installed. The runbook's and the sequence design's arm lines changed `;` to `&&` in the same commit: with a refusal that exits 5, `;` walks straight into `p5 on` with no deadman, which is the outcome this row exists to prevent. **NOT seeded, and enforced anyway:** the "writable" half of the directory condition — a suite that may run as root cannot seed it. **Superseded U130 verdict follows, kept for the record:** `arm` still ARMS when the crontab directory is absent — refusing would leave NO deadman at all, which stays the worse outcome — and it still says so loudly on stderr (`boot_limb_install`, `p5/bin/p5-deadman`) and in `status`'s `boot: ABSENT`. A first build of this unit ALSO changed `arm`'s exit status to a 7th code (`P5_EX_ARMED_NO_BOOT`) for this case; that broke `p5/test/run.sh`'s DM-1, which hard-requires `arm` to exit 0 and is owned by another row, the moment its fake root has no `/etc/crontabs` directory. The exit-code change is reverted — `arm` exits plain 0 whether or not the boot limb installed, exactly as before U130 — because reporting the distinction in the exit status would need every caller, including that row's, to accept a second success code. **Third fix round:** the other half of that verdict was still wrong. `do_status`'s OWN boot check was `grep -q "$CRONMARK"`, an UNANCHORED substring test, even after install and removal had been anchored (O15) — so a crontab holding only an operator line that merely *contains* `# p5-deadman`, with p5's real line never installed, made `status` print `boot: LIVE (crond running, crontab line installed)`, which §5 tells the operator to read as "the power-cut limb is wired" before F3. `status` now matches `CRONLINE` with `grep -qxF`, the same anchor install and removal use | **THIS COLUMN IS RESTATED FOR THE INVERTED BARS (U290 fix round, 2026-09-13); the sentences it used to hold described the U130 direction and contradicted column 2.** `DM-45`/`DM-45a`/`DM-45b` (`deploy/server/test-p5-fw-deadman.sh`) no longer assert exit 0 with a record written anyway — they are INVERTED and assert the refusal: **exit 5, the missing directory named on stderr, and NO record**. `DM-63`..`DM-63c` do the same for crond stopped (including that the operator's crontab was not touched on the way out) and `DM-65`..`DM-65b` for a `$SELF` outside `$P5_SBIN`. `DM-66`/`DM-66a`/`DM-66b` are the positive control: all three preconditions true → arm exits 0, writes the record AND installs the line. `DM-45c`..`DM-45f` and `DM-67` pin the `--no-boot-limb` override — `DM-67` asserts **rc=0 AND the record present** BEFORE it asserts 'no crontab line', because without that it passed vacuously against a tool that has no such flag; `DM-45f` pins that the deliberate-absence notice reaches **stderr**, while `DM-45e` stays fd-agnostic and pins only that arm says it. `DM-68`..`DM-68c` seed a **failed append** (the crontab path is a directory: every `boot_limb_ready` condition passes and only the write fails) → arm refuses with exit 5, the record written before the limb is removed again, and ARMED is never printed. `DM-69` asserts `P5_CRONTAB` is announced as an environment override. `DM-70`..`DM-70f` pin that boot enablement (`$P5_ROOT/etc/rc.d/S*cron`) is REPORTED as a fact separate from 'crond is running now', in both directions. `DM-61`/`DM-61a` are unchanged: a look-alike-only crontab with `crond` up does NOT produce `boot: LIVE`. **MEASURED on this branch's tree, ext4 under WSL Ubuntu-24.04 (not on NTFS), `bash scripts/ci-local.sh deadman e0 portal shellcheck busybox` → PASS 5 FAIL 0 ABSENT 0:** the suite is **141 passed / 0 failed** (`BARS_MIN` ratcheted 126 → 141), mutant-check **MUTANT DETECTED**, server-init **38/0**, footprint **17/0**. The count in the superseded text was 114 and the ratchet floor was 126; **141** is the number this row now stands on. **SEEDED A/B, BOTH DIRECTIONS**, each seed reverting one thing and nothing else, `p5/bin/p5-deadman` + `p5/lib/p5-common.sh` copied to a scratch tree so nothing live was mutated: full revert of both files to `origin/dev` → **118 passed / 23 failed**; both `|| return 1` dropped from the appends → **136/4**, red at `DM-68`..`DM-68c` (and the seeded arm printed ARMED, exit 0, with a record and no crontab line — the defect itself); `P5_CRONTAB` dropped from the override list → **139/1**, red at `DM-69`; the boot-state report removed → **138/2**, red at `DM-70a`/`DM-70c`; the boot-state report made UNCONDITIONAL → **140/1**, red at `DM-70e` alone, which is what stops that pair asserting a constant; the two notices put back on `p5_log` → **140/1**, red at `DM-45f` with `DM-45e` still green, which is the intended split. `DM-66`/`DM-66a`/`DM-66b` are GREEN in every arm. `p5/test/run.sh`'s DM-1 — owned by another row, not by this suite — was RUN, not assumed: the `e0` arm gives **`p5-skeleton: 258 passed, 0 failed`**, DM-1 among the passes. `portal` **434/0** and `busybox` Layer-2 **726/0** were run because `p5/lib/p5-common.sh` changed and both reach it; `shellcheck -s sh` over 52 files reports **zero** findings |
...
| O17 | **Sections 0, 4, 5 and 9 still type `p5-fw-deadman`, a tool that no longer exists.** U38a consolidated `deploy/server/p5-fw-deadman` into `p5/bin/p5-deadman` and DELETED the twin (`p5/bin/p5-deadman:14-21`; `ls deploy/server` carries no `p5-fw-deadman`, only its two test scripts, whose names kept the old prefix). The package installs the survivor as `/usr/sbin/p5-deadman` (`p5/contract/paths:145`, placed by `p5-install:370-376`), so every `p5-fw-deadman <verb>` line in sections 4, 5 and 9 names a command the box will not have. **ENUMERATED MECHANICALLY, because the first draft of this row said "0, 5 and 9" and missed the one that matters most:** `grep -n p5-fw-deadman docs/deploy-p5-server.md` against the `^## ` headings puts the hits in section 0 (`:23`, `:25`, `:56` -- file references, not commands), **section 4 (`:251`, the Gate C row `C2` -- a step the operator TYPES, and the reason this omission was not cosmetic)**, section 5 under F3 (`:351`, `:354`, `:394`), section 8 (`:626`), section 9 (`:643`, `:686`, `:706`), and section 11 (`:734`, `:741`, `:744`). `:626` and the section-11 rows are prose ABOUT the rename and are deliberately NOT in the rename set -- renaming them would erase the record of the problem. Sections 6 and 8 otherwise use the installed name; the rest were left rather than half-renamed by a unit that does not own them | a rename pass over sections 0/4/5/9 by whoever owns them, plus a bar in `deploy/server/test-p5-server-init.sh` asserting the runbook names no `p5-fw-deadman` outside a historical-measurement row -- neither is in U174's owned set |

...
#
# U13 -- B6 ADDED, AND WHY THIS UNIT IS EDITING U8'S FILE
# -----------------------------------------------------------------------------
# This file is U8's. A unit touching it needs a reason written HERE, in this
# block, and nowhere else -- the same exception the `go` test-floor block in
# emulator-gate.yml documents. The reason is mechanical: BATTERY_SHA256 pins
# highn_battery.py, so ANY change to the battery makes this gate exit 2 until the
# pin and the baseline are re-measured together. U13 adds bars to the battery, so
# it must land the pin, the classes and the baseline in the SAME commit. Landing
# the battery alone breaks the job; landing this alone breaks it too.
#
# WHAT B6 IS. The block this file already carries says B3(loss)'s move to PAIRED
# does NOT cover reorder-hold geometry, that the ring discard is common-mode and
# cancels, and that it needs its own bar owed to ROADMAP U13. This is that bar.
# B6 re-releases one already-finished Dc run's own delivery trace under a
# different hold and compares (ROUND 2 -- both bars re-derived, see below):
#   B6a  late(observation-derived hold) < late(hold in force), paired, STRICT.
#   B6b  late(hold in force) < late(the formula's OWN clamp floor, 80ms), STRICT.
# Both sides are the same arrivals, so this is the paired class, not an absolute
# threshold -- ADR-004's limit is untouched. The gated limbs are scored on the
# UNBOUNDED reorder model (nsched_model.reorder_release, the ADR-004 instrument);
# the battery also PRINTS a re-score under ring.go's real 2^11 window, which is
# NOT gated because it is a model this gate has never validated.
#
# ROUND 2 -- WHY THE 18 B6a MUST_FAIL PINS ARE GONE AND THE BAR IS GREEN INSTEAD.
# Round 1 gated B6a as `late(force) <= late(ratchet)`, which failed 18/18 and was
# pinned. Two things were wrong with that.
#   (1) An expected-fail pin is a weak gate: it fires only when the entry STOPS
#       failing. Turned around -- `late(ratchet) < late(force)`, STRICT -- the same
#       measurement becomes a positive assertion that is TRUE today, and no pin
#       list is needed. Nothing was weakened: the numbers are identical, the sign
#       is the other way round.
#   (2) One of the 18 was NOT GEOMETRY-ESTABLISHED, so under the two-directional
#       rule it was a latent exit 1 on an unchanged tree. N2-het@0.65's paired
#       median is 65 frames on 454. Re-running that mix over 48 count- and
#       duration-preserving stall-phase rotations (rig_checks.phase_drops, U33's
#       corrected randomiser) x 6 seeds x 3 loads = 864 runs puts
#       med(ratchet - force) in [-192.5, +56.0], with 10/48 rotations of the WRONG
#       SIGN at load 0.65 -- and 0/48 at 0.85 and 0.95. That cell is decided by the
#       hand-placed stall geometry, not by the hold. It is now REPORTED AND NOT
#       GATED, as one explicit entry (B6A_UNESTABLISHED in the battery) printed on
#       every run. Reproduce with b4_geometry_estab.py.
#
# ROUND 2 -- WHY B6b'S REFERENCE MOVED. It was `< late(ring.go's 10ms floor)`. A
# sweep of the hold in force over {0,10,20,...,500} ms on all 18 cells x 6 seeds
# (hold_sweep_b2.py / .txt) shows that bar flips between 10ms and 20ms and prints
# PASS on 18/18 for EVERY value from 20ms up. The shipped clamp is [80,350]ms, so
# every shortening a real regression could produce -- 4.0x (80->20) to 17.5x
# (350->20) -- was invisible: 10.3x-29.0x of late-discard slack. The reference is
# now reserved_composite.py:445's OWN clamp FLOOR, 80ms -- a value the formula can
# actually emit, inside the realistic range, cited not invented. Slack falls to
# 3.07x-8.37x. The common-mode teeth are unchanged: a reorder_release that stops
# holding makes both sides the same run, median exactly 0, and STRICT fails.
#
# THE HARD CLASS IS NEW AND IT IS NOT A BAR. B6-SELF(a)/(b)/(c) assert that the
# battery's variable-hold release walk IS nsched_model.reorder_release (0
# disagreements over 200 random traces x 6 holds, and 108/108 exact reproduction
# of the rig's own loss and late-discard at the rig's own hold). B6-CTRL asserts
# that each limb flips when the hold is substituted. ROUND 2: it has FOUR
# substitutions and NONE of them compares a vector with itself. Round 1's
# `force := ratchet` limb evaluated med(r-r) <= 0 and its `force := floor` limb
# evaluated not(med(z-z) < 0); both are tautologies that cannot fail, so half the
# control was theatre. The four are now: B6a force := the patient hold (must
# FAIL), B6a ratchet := the t_skip observation -- the DEFECT a careless edit to
# hold.go OnSkip produces (must FAIL, this is the RED-ON-DEFECT demonstration),
# B6b force := the ring floor (must FAIL), B6b force := the clamp ceiling (must
# PASS). A HARD failure means the instrument moved, which makes every B6 number
# unreadable; it can never be baselined.
#
...
# B6a/B6b are PAIRED in the strictest sense this gate has: both sides are the
# SAME run's delivery trace, re-released under a different hold. Same seeds, same
# physics, same arrivals -- only the hold differs. See "U13" below.
PAIRED = ('B1', 'B2', 'B3(gp)', 'B3(loss)', 'B4a', 'B6a', 'B6b')
# B6c (U13 round 3) is a relation between a hold and the lateness the SAME run
# exhibited -- no threshold, no absolute number -- so it is gated, and it is
# RELATIVE rather than PAIRED because the two sides are not two runs.
# It is what stops this gate passing a hold with no derivation behind it: a
# demonstrated CONSTANT 3-second hold satisfied B6a on all 17 gated cells and
# reproduced the baseline fail set byte for byte.
RELATIVE = ('B4b', 'B5', 'B6c')              # gated: shape / direction, no threshold
# HARD: instrument checks. These are not bars on the scheduler, they are checks
# that the battery is still measuring what it claims. A HARD failure can NEVER be
# baselined -- it is not an honest fail, it is an unreadable run. Kept out of
# PAIRED so a future editor cannot quiet one by adding it to BASELINE_FAILS.
# B6-SELF(d) is deliberately NOT here: it is a printed MEASUREMENT (shipped
# single-blockAt attribution vs the per-seq reference), not an assertion. The
# shipped rule can only UNDER-shoot the per-seq truth, so a bar on the gap
# would need a tolerance nothing derives. It is printed on every run because
# round 2 stated that comparison in prose with no artifact behind it.
HARD = ('B6-SELF(a)', 'B6-SELF(b)', 'B6-SELF(c)', 'B6-SELF(e)', 'B6-CTRL')
...
MUST_FAIL = {
    ...
    # --- U13 / B6: NO PINS. Round 1 carried 18 B6a MUST_FAIL entries; round 2
    # turns the bar around so it asserts what is TRUE, and drops the one cell
    # that was not geometry-established. See the U13 round-2 block above.
}
...
EXPECT = {
    ...
    'B5_deltas': 6,
    'scenarios': 6,
    # U13 / B6. 6 scenarios x 3 loads = 18 cells, each printing one verdict line,
    # one latency line, one [range] line and one [win] line. B6_refs pins the
    # REFERENCES the same way B3_vs_oracle pins B3's: a cell line must name both
    # `ratchet=` (B6a's reference) and `clamp80=` (B6b's). Swapping either for
    # something weaker -- in particular reverting B6b's reference to the 10ms ring
    # floor, which was measured to have ~50x tolerance -- would keep the cell count
    # at 18 and otherwise leave this gate green.
    'B6_cells': 18,
    'B6_refs': 18,
    'B6_range': 18,
    'B6_win': 18,
    # (a) release identity, (b) window inertness, (c) per-run identity vs the rig,
    # (d) ROUND 3: shipped single-blockAt attribution vs the per-seq reference,
    # printed every run because round 2 asserted that comparison in prose with no
    # artifact behind it, (e) ROUND 3: the B6c witness is inert.
    'B6_self': 5,
    # ROUND 3: 18 cells x one B6c verdict line. B6c is what stops the gate passing
    # a hold with no derivation behind it: a demonstrated CONSTANT 3-second hold
    # satisfied B6a on all 17 gated cells and reproduced the baseline fail set
    # byte-identically. Losing these lines is losing that.
    'B6_c': 18,
    # FOUR substitutions in round 2, not two, and none of them a tautology. FIVE
    # printed lines: the t_skip DEFECT substitution carries two assertions (it is
    # worse than the correct observation on every cell, AND it turns B6a red on at
    # least one gated cell), and each is printed on its own line.
    # ROUND 3 adds FOUR more, all on B6c and all substituting a DIFFERENT run:
    # three constants (clamp350, clamp80, patient(T)) which must fail because a
    # constant is derived from nothing, and UNFILTERED -- the round-2 SHIPPED rule,
    # which learned 8,213ms where the filtered rule learns 525ms.
    'B6_ctrl': 9,
}
...
BATTERY_SHA256 = '5c596f4eceb5215d1533f7077bdb4221affb23862c3d73a5a40bc0b42f5555f5'
...
def banner():
    ...
    print('  GATED [relative]     %s        within-scheduler shape / cross-config direction'
          % ', '.join(RELATIVE))
    print('  HARD [instrument]    %s' % ', '.join(HARD))
    print('                       never baselineable -- a failure here means the battery')
    print('                       stopped measuring what it claims (U13)')
    ...
    print('  Green also says nothing about STALL GEOMETRY: every baseline margin sits inside')
    print('  the ~0.9 pt band paired quantities move across geometry (U33). This is a')
    print('  fixed-geometry ratchet, not a statement about nature.')
    print('')
...
def main():
    ...
    got = {
        ...
        'B6_cells': sum(1 for l in lines if l.startswith('  L=') and 'B6a' in l),
        'B6_refs': sum(1 for l in lines if l.startswith('  L=')
                       and 'ratchet=' in l and 'clamp80=' in l),
        'B6_range': sum(1 for l in lines if '[range] late at' in l),
        'B6_win': sum(1 for l in lines if '[win] ring.go 2^11' in l),
        'B6_self': sum(1 for l in lines if 'B6-SELF(' in l),
        'B6_c': sum(1 for l in lines if '[B6c] evidenced:' in l),
        'B6_ctrl': sum(1 for l in lines if l.lstrip().startswith('subst B6')),
    }
    print('\n' + '-' * 100)
    ...
    gated_new, gated_known, ungated, unknown, hard = [], [], [], [], []
    for bar, subj in seen:
        if bar in HARD:
            # Never baselineable: the instrument moved, so nothing else in the
            # run is readable. Checked BEFORE the baseline lookup on purpose.
            hard.append((bar, subj))
        elif bar in ABSOLUTE:
    ...
    if hard:
        print('')
        print('GATE FAIL -- %d INSTRUMENT check(s) failed. These are not bar failures and'
              % len(hard))
        print('they cannot be baselined: they say the battery stopped measuring what it')
        print('claims, so every other verdict in this run is unreadable.')
        for bar, subj in hard:
            print('    INSTRUMENT  %-12s %s' % (bar, subj))
        sys.exit(1)
    if unknown:

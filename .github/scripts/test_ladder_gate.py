...
synthetic and labelled as such. The last block replays the 14 REAL recorded CI
runs through pathsim's real source, which is not synthetic.
...
check("S3.thr declared UNGATED while the record says 14/14", rc, 2,
...
FAKE_COUNT = ("S3.thr      | late_thr >= 1.50 Mb       |    14/14",
              "S3.thr      | late_thr >= 1.50 Mb       |     9/14")
rc, out = run(stub(b), gate_sub=MOVE_THR, record_sub=FAKE_COUNT)
check("...with the record's count ALSO fabricated to 9/14 (gate alone cannot see it)",
      rc, 0, "ladder gate PASS", out)
rc, out = run_equiv(record_sub=FAKE_COUNT)
check("...but ladder_equiv_check recomputes the count from the 14 runs", rc, 1,
      "DIVERGENT record S3.thr pass/den", out)

rc, out = run(stub(BARS), gate_sub=[('"S3.rate", "S6.loss", "S6.k"', '"S3.rate", "S6.k"'),
                                    ('"S2.tail":     "0/14', '"S6.loss": "fabricated", "S2.tail":     "0/14')])
check("a 9/14 bar declared KNOWN_FAIL (S6.loss)", rc, 2, "S6.loss is declared KNOWN_FAIL", out)

rc, out = run(stub(BARS), gate_sub=[('UNGATED = ["S2.loss", ', 'UNGATED = ['),
                                    ('["S1.loss", ', '["S2.loss", "S1.loss", ')])
check("a 6/14 bar declared GATED (S2.loss)", rc, 2, "S2.loss is declared GATED", out)
...
rc, out = run(stub(BARS, values={"S2.loss": "8.43%"}))
...
# ---------------------------------------------------------------------------
print("\nrecord integrity:")
rc, out = run(stub(BARS), record_sub=("| min 3.32 max 8.43", "| roughly 3 to 8"))
check("record envelope column made unparseable", rc, 2, "does not parse", out)

rc, out = run(stub(BARS), record_sub=("S2.tail     | tail900 >= 1871.5 of 1900 |     0/14 | min 1692 max 1835\n", ""))
...
rc, out = run_equiv(record_sub=("|     6/14 | min 3.32 max 8.43",
                                "|    14/14 | min 3.32 max 8.43"))
check("record pass count fabricated (S2.loss 6/14 -> 14/14)", rc, 1,
      "DIVERGENT record S2.loss pass/den", out)

rc, out = run_equiv(record_sub=("| min 1692 max 1835", "| min 0 max 1900"))
...
print("\nreplay of the 14 recorded CI runs through the real verdict()/bars source:")
for _r in recorded:

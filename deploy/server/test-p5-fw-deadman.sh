...
# this round reverses and amends -- and DM-45c..e, DM-63..DM-63b, DM-65..DM-65b,
# DM-66/DM-66a and DM-67 are new.
#
...
_err=$("$DM" arm --after 3600 --restore-script "$T/restore.sh" --label optout --no-timer --no-boot-limb 2>&1 >/dev/null); _rc=$?
chk "DM-45c --no-boot-limb arms on the same box the refusal just rejected" "$_rc" "0"
if [ -f "$DMDIR/optout" ]; then ok "DM-45d ...and the record IS written under the explicit override"; else bad "DM-45d no record under --no-boot-limb (stderr<<$_err>>)"; fi
has "DM-45e ...and says out loud that the boot limb is deliberately absent" "$_err" 'DELIBERATELY ABSENT'
...
"$DM" arm --after 3600 --restore-script "$T/restore.sh" --label nolimb --no-timer --no-boot-limb >/dev/null 2>&1
if ! grep -q 'p5-deadman' "$P5_CRONTAB" 2>/dev/null; then ok "DM-67 --no-boot-limb installs NO crontab line, on a box where it could have"; else bad "DM-67 --no-boot-limb installed the line anyway: $(cat "$P5_CRONTAB")"; fi

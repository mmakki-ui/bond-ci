...
#
# THE TWO ON-BOX INVOCATIONS, and why there are two rather than one (U211).
# This file is now a filemap row --
# `755|server|deploy/server/p5-server-preflight.sh|/usr/sbin/p5-server-preflight`
# -- so it is BOTH carried inside the package and installed by p5-install. Both
# invocations are load-bearing:
#
#   1. BEFORE the install, out of the unpacked package. Nothing of P5 is on the
#      box yet, so the only copy is the one the package carries at its repo
#      path:
#
#        sh /tmp/p5-pkg/payload/deploy/server/p5-server-preflight.sh
#
#      That is gate C1 of docs/deploy-p5-server.md: it is read-only, and it
#      refuses on the transport secret, which is exactly what has to be settled
#      before the server install proceeds.
#
#   2. AFTER the install, from the namespace:
#
#        /usr/sbin/p5-server-preflight
#
#      /tmp is tmpfs, so the package is gone after the first reboot, and the
#      question this script answers -- is the firewall still shaped the way the
#      management path needs -- is one an operator asks again months later. A
#      preflight that can only run while the package is still unpacked is a
#      preflight that cannot be re-run on the box that cannot be recovered.
#
# The script itself does not care which one ran it: it sources nothing, reads no
# path relative to its own location, and takes no argument.
#
...
#
# p4-bondagg/server/main.go:540 emits `authbad=` (received frames whose MAC did not
...
# deploy/server/init.d/p5-server:168 sets `procd_set_param stderr 1`, so the
...
# frame has reported one, so "no SSTAT line" at that rung is itself an answer.
...
if [ -n "${P5_STAT_LOG:-}" ] && [ -r "${P5_STAT_LOG:-}" ]; then
    AG_SRC="$P5_STAT_LOG"
    AG_LINE=$(grep 'SSTAT ' "$P5_STAT_LOG" 2>/dev/null | tail -1)
elif command -v logread >/dev/null 2>&1; then
    AG_SRC="logread"
    AG_LINE=$(logread 2>/dev/null | grep 'SSTAT ' | tail -1)
fi
if [ "$AG_PE" = 1 ]; then
...
printf '%-28s %s\n' "SSTAT source" "${AG_SRC:-(none: P5_STAT_LOG unset, logread absent)}"
if [ -z "$AG_LINE" ]; then
    if [ "$AG_PE" = 1 ]; then
        printf 'REFUSE: --post-engage was asked for and there is no SSTAT line to read. After\n'
        ...
        printf 'transport-auth-gate: NOT EVALUATED -- no SSTAT line to read. This check has a\n'
    ...
    if [ -z "$_ag_gate" ] || [ -z "$_ag_bad" ]; then
        printf 'REFUSE: that SSTAT line carries no gate= and authbad= pair, so this box runs a\n'
        ...
        printf '        and re-install the client with --transport-key plus --peer-key-id.\n'
        PF_RC=1
    elif [ "$_ag_bad" != 0 ]; then
...
# owner-only AND, once there is an SSTAT line to read, the auth gate is CLOSED
# with authbad=0. Every other block above reports; this line is what makes the
# refusals in this file reach a caller that reads nothing but the status.
exit "$PF_RC"

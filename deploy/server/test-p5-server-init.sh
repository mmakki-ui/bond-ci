...
# footprint bound: GOMAXPROCS/nice/limits). LOWERED 41 -> 38 by U159, and a
# lowering has to say what it deleted: the env loop below ran over FOUR keys at
# two bars each, and AGG_HOLD_MIN_MS/AGG_HOLD_MAX_MS are no longer read by
# main.go, so those four bars asserted a FALSE fact and are gone (-4). IS-21
# replaces one of them with a bar that reds if either dead key comes back
# anywhere in the executable text (+1). 41 - 4 + 1 = 38, measured by running
# this file, not derived.
BARS_MIN=38
...
# ---------------------------------------------------------------- START --
# The two constraints are stated in deploy-p5-server.md's paragraph
# "The START priority: the box derives a BOUND"
# -- an ANCHOR, not a line number, because U174 moved that file by +105/-20 and
# broke every line cite into it at once (U200): after S19firewall and
...
# ------------------------------------------------------------------ env --
# EXACTLY the two datapath keys p4-bondagg/server/main.go reads and this file
# sets: AGG_LISTEN (main.go:365) and AGG_WG (main.go:366). Two bars per key:
...
#
# U159, and this is the defect this block itself carried: the loop used to run
# over AGG_HOLD_MIN_MS and AGG_HOLD_MAX_MS too, labelled "(main.go reads it)".
# U159 deleted the server's resequencer ring and both envMS() reads with it, so
# from that commit until this one the bar was GREEN while its stated fact was
# FALSE -- the gate that exists to catch an invented key could not see two of
# them. main.go's env call sites are :365, :366, :372 (AGG_KEY_FILE) and :377
# (AGG_AUTH_REOPEN_MS); the last two are read but deliberately not set here
# (see the init script's own comment), so they are neither looped nor allowed.
ENV_LINE=$(printf '%s\n' "$CODE" | grep 'procd_set_param env' | head -1)
ENV_COUNT=$(printf '%s\n' "$CODE" | grep -c 'procd_set_param env')
chk "IS-17 exactly one procd env block" "$ENV_COUNT" "1"
for k in AGG_LISTEN AGG_WG; do
    if printf '%s\n' "$ENV_LINE" | grep -q "$k=\"\\\$$k\""; then
        ok "IS-18/$k env block passes $k (main.go:365,366 reads it)"
    else
...
# GOMAXPROCS (U129) is allowed alongside the two datapath keys even though it is
...
# The allowlist is NARROWED to exactly what the loop above checks, so re-adding
# AGG_HOLD_MIN_MS to the env block reds HERE, naming the key in the got/want.
EXTRA=$(printf '%s\n' "$ENV_LINE" | tr ' ' '\n' | sed -n 's/^\([A-Za-z_][A-Za-z0-9_]*\)=.*/\1/p' \
        | grep -vx 'AGG_LISTEN\|AGG_WG\|GOMAXPROCS' | tr '\n' ' ')
chk "IS-20 env block carries NO key outside AGG_LISTEN/AGG_WG + GOMAXPROCS" "$(printf '%s' "$EXTRA")" ""
# IS-21: the dead keys must not come back ANYWHERE in the executable text, not
# just in the env line. A re-added `AGG_HOLD_MIN_MS="${AGG_HOLD_MIN_MS:-150}"`
# binding with no env-block entry is invisible to IS-20 and is still a constant
# this file invents and nothing consumes.
DEADK=$(printf '%s\n' "$CODE" | grep -c 'AGG_HOLD_M[IA][NX]_MS')
chk "IS-21 no AGG_HOLD_MIN_MS/AGG_HOLD_MAX_MS anywhere in the code (U159 deleted both reads)" "$DEADK" "0"
...
# --------------------------------------------------------------- enable --
# S7 is deferred -- deploy-p5-server.md's section headed
# "recommend DEFERRING boot enablement out of the first deploy"
# (an ANCHOR, not a line number; U200). An init script that enables

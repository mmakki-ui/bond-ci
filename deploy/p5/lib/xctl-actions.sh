...
# _cap_val / cap_env -- the enable path for E2b's delivered-rate cap (U253).
#
# THE KEY SET IS THE DAEMON'S: the six names below are p4-bondagg/daemon/cap.go's
# AGG_PULL_CAP plus its capEnvKeys, spelled the same way. cap.go ships NO DEFAULT for any of
# them on purpose, so this function invents nothing: the numbers come from the box, the probe
# writes them into the `cap` fact, and until they exist the cap stays OFF -- exactly what
# ships today.
#
# ONE LITERAL KEY PER `echo` LINE, never a `for _ck in $CAP_KEYS` loop with a printf at the
# end. That is not style. orchestration/bond_model.py's MF-ENV block ENUMERATES the keys
# build_agg_env emits out of THIS TEXT and compares that set against the procd stanza's
# `procd_set_param env` line, in both directions; a key emitted through a variable is
# invisible to it, and an invisible key is precisely the U47 defect this repo already paid
# for -- procd hands the daemon an empty string and the daemon's own env() default silently
# takes over while every gate prints PASS. MEASURED 2026-09-07: the loop-and-printf first cut
# made the model gate a HARD ERROR ("unrecognised line in the emitter group: 'cap_env'"), and
# that hard error was masking the real defect -- init.d/bond-agg passed NONE of these seven
# keys through, so the enable path stopped one hop short of the daemon.
#
# ALL FIVE OR NOTHING. cap.go turns AGG_PULL_CAP=on with any key missing into a FATAL config
# error, so a partial fact would take the datapath DOWN rather than run it uncapped. Every
# guard below returns BEFORE the first echo, so a partial fact emits not one line. A
# non-numeric value is treated as missing for the same reason.
#
# `_c_x=$(_cap_val K) || return 0` is the POSIX idiom, not a bashism: the exit status of a
# simple assignment whose value is a command substitution IS the substitution's status.
# Measured under busybox ash 1.36.1 and dash -- see run.sh S5e.
_cap_val() {   # $1 = key name; prints its value, or returns 1 when absent or not a number
    _cv=$(sed -n "s/^$1=//p" "$BOND_DIR/cap" 2>/dev/null | head -1 | tr -d ' \t\r')
    case "$_cv" in
        ''|*[!0-9.]*) return 1 ;;
    esac
    printf '%s\n' "$_cv"
...
cap_env() {
    [ -r "$BOND_DIR/cap" ] || return 0
    _c_target=$(_cap_val AGG_PULL_CAP_TARGET_MS)     || return 0
    _c_trip=$(_cap_val AGG_PULL_CAP_TRIP)            || return 0
    _c_clear=$(_cap_val AGG_PULL_CAP_CLEAR)          || return 0
    _c_minrate=$(_cap_val AGG_PULL_CAP_MINRATE_KBPS) || return 0
    _c_det=$(_cap_val AGG_PULL_CAP_DET_MS)           || return 0
    echo "AGG_PULL_CAP=on"
    echo "AGG_PULL_CAP_TARGET_MS=$_c_target"
    echo "AGG_PULL_CAP_TRIP=$_c_trip"
    echo "AGG_PULL_CAP_CLEAR=$_c_clear"
    echo "AGG_PULL_CAP_MINRATE_KBPS=$_c_minrate"
    echo "AGG_PULL_CAP_DET_MS=$_c_det"
}

# tx_backoff_env -- the only corrective pull.go's S7 register names for the park's
# utilization floor (U252). The daemon reads AGG_PULL_TXBACKOFF_US; no shell in deploy/ ever
# wrote it, so on a real box the documented remedy did not exist.
#
# NO DEFAULT: S7 says the derivation needs the device DRAIN RATE and that the estimator was
# deleted by the pivot. Absent fact = no line = the park, which is the shipped behaviour. A
# non-integer is treated as absent -- the daemon already ignores it, and the feeder must not
# change datapath behaviour on a typo.
#
# GUARD-THEN-EMIT, same shape as cap_env and for the same reason: the emission used to sit in
# a `case` arm (`*) echo "AGG_PULL_TXBACKOFF_US=$_tv" ;;`), which the model's emitter parser
# cannot read as a key line. The key must be on a line of its own to be COMPARED against the
# procd stanza.
tx_backoff_env() {
    _tf="$BOND_DIR/tx_backoff_us"
    [ -r "$_tf" ] || return 0
    _tv=$(head -1 "$_tf" 2>/dev/null | tr -d ' \t\r')
    case "$_tv" in
        ''|*[!0-9]*) return 0 ;;
    esac
    echo "AGG_PULL_TXBACKOFF_US=$_tv"
}
...
build_agg_env() {   # $1 = output path
    ...
    _SP=""; for _s in $(ordered_spotty); do _SP="${_SP}${_s},"; done; _SP="${_SP%,}"
    {
        ...
        # U253/U252: emitted ONLY when the box carries the fact. Unlike every line above,
        # these can emit nothing at all, and nothing is the shipped state.
        cap_env
        tx_backoff_env
    } > "$1"
}

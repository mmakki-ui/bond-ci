...
build_agg_env() {   # $1 = output path
    ...
    _SP=""; for _s in $(ordered_spotty); do _SP="${_SP}${_s},"; done; _SP="${_SP%,}"
    {
        ...
        # U252: the tx backoff knob, emitted ONLY when the box carries the fact. See
        # tx_backoff_env -- this feeder ships no default for it and must not.
        tx_backoff_env
...
# tx_backoff_env -- the ONLY operator knob p4-bondagg/daemon/pull.go's S7 register names as a
# corrective for the park's utilization floor, and until U252 it was emitted by nobody: the
# daemon reads AGG_PULL_TXBACKOFF_US (pullrun.go) and no shell in deploy/ ever wrote it, so on
# a real box the documented remedy did not exist.
#
# NO DEFAULT, deliberately. S7 says the corrective's derivation needs the device DRAIN RATE
# and that the drain-rate estimator was deleted by the pivot, so "picking a shorter tick or a
# backoff constant would be an invented number wearing a derivation's clothes". The value is
# the BOX's -- E1 measures whether the qdisc drains faster than PingIval and what to substitute
# -- and the fact is where the probe puts it. Absent fact = no line = the park, which is the
# shipped behaviour and the honest one.
#
# A non-integer is treated as absent: the daemon already logs and ignores it (pullrun.go), and
# the feeder must not be the thing that changes the datapath's behaviour on a typo.
tx_backoff_env() {

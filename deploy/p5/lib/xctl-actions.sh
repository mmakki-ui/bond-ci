...
agg_weights() {   # $1 = N
    ...
    if [ -n "$_wf" ]; then
        ...
        # U270: "$LOGGER", not a bare `logger` -- this file is sourced by bond-xctl,
        # which sets it. A bare name is unreachable by a PATH shim under a busybox
        # built FEATURE_SH_STANDALONE, so this WARN would never reach a ledger.

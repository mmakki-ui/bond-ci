...
# is_wg_iface (U52): is the netifd LOGICAL interface $1 the wg tunnel, i.e.
# does its l3_device equal $WG_DEV. DERIVED live over ubus every call -- no
# file, no cache, no install-time writer. This replaces reading the wg-logical
# fact, which was written ONLY by p2-engarde's bootstrap
# (p2-engarde/bootstrap-bond.sh:79) and by nothing in deploy/p5: on a box that
# never ran that bootstrap (a clean install, or the server -- confirmed empty
# in docs/knowledge/inventory/2026-08-30-server-brume2.txt's `### etc-bond`)
# the file never existed, and the hotplug hook silently did nothing forever.
# Per U50, P5 no longer depends on p2-engarde at all, so a file only p2-engarde
# could produce cannot be the mechanism.
# LIVES HERE, NOT IN bond-xctl (merge with U124): U124 split the executor into
# an entry point plus these five libraries, and every leaf probe moved here.
# It sits beside _excluded_dev because both answer "what is this device to us"
# off the SAME ubus surface gl_sources reads.
# ONE ubus call for the ONE interface asked about -- not gl_sources()'s
# enumerate-every-interface loop, because this fires on every hotplug ifup
# event (any WAN flapping too), so cost must be O(1) per event, not O(ifaces).
# Same pure-text _json_pick parse as gl_sources -- no jsonfilter/jshn assumed.
# THREE-WAY exit status, because "not the wg interface" and "could not ask at
# all" are different events and the caller (97-bond) must not log them alike:
#   0 = yes, this is the wg logical interface
#   1 = no, ubus answered and it is some other interface -- the common,
#       silent case (most ifup events are a WAN, not the tunnel)
#   2 = COULD NOT CLASSIFY -- no ubus binary, or ubus gave no answer for $1.
#       The caller must FAIL LOUD on this, not treat it as "no".
is_wg_iface() {
    [ -n "${1:-}" ] || return 2
    gl_ok || return 2
    _F=$(ubus call "network.interface.$1" status 2>/dev/null | _json_pick l3_device)
    [ -n "$_F" ] || return 2
    [ "${_F%%|*}" = "$WG_DEV" ] && return 0
    return 1
}
...
# bond-ecod `SYS_NET` is overridable: the harness serves a fixture. The default
# is the kernel's path, not a pick.

...
# guard_agg_installed: "is the feeder installed" is a FEEDER question, so M3
# answers it (U18). It is also what gives the delegation TEETH: with the
# controller absent this guard fails and every edge that brings the feeder up is
# REFUSED, instead of the reconciler quietly doing the work itself.
guard_agg_installed() { aggctl installed; }
# min_sources_for_mode: the arity FLOOR of the stored mode. The aggregate floor for
# an aggregate mode (you cannot stripe one link); 1 otherwise (eco is DEFINED at
# N=1, and lightning on a box down to its last source is degenerate-but-running,
# which is the daemon's own rule -- U138 accepts lightning at N=1 and logs
# DEGENERATE). Membership in AGG_SCHED_TABLE, never a comparison against a mode name.
# U18: the FLOOR itself is read from M3 (`aggctl arity-floor`) rather than kept as
# a second copy here -- it is a property of the aggregate, and the module that owns
# the aggregate owns it. With the controller absent it prints nothing, `[ N -ge "" ]`
# is an error, and the guard REFUSES: fails closed, which is the required direction.
min_sources_for_mode() { if is_agg; then aggctl arity-floor; else echo 1; fi; }
...
# U18: it asks M3 the same question, and it is the one call site that hands the
# controller a SOURCE LIST as arguments -- so it is also what keeps the newline-
# pinned split in aggctl_srcs (bond-xctl) load-bearing rather than decorative.
guard_enough_sources() { aggctl_srcs ordered_wans arity-ok; }
...
converged() {
    ...
    case "$1" in
        ...
        engaged)
            ...
            # U18: the comparison is M3's (`aggctl applied-current`) -- the record
            # and the restart it gates are ONE mechanism, and splitting them across
            # two modules is how the two halves drift. `svc_running` above stays
            # HERE on purpose: that is an OBSERVATION for this function's own state
            # signature, not feeder lifecycle -- the same line bond-watchdog draws.
            aggctl applied-current || return 1
            shape_matches              || return 1
            return 0 ;;
        *) return 1 ;;
    esac

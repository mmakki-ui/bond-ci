...
WAIVERS_EXPECT=5     # rows with writer=NONE: wg-logical metered spotty_dup agg_w exclude
...
if [ "$OUT_PATHS" = "$OUT_WAIVED" ]; then
    ok "FD-12 outside the sweep the OLD name lightning survives in exactly the waived files: $OUT_WAIVED"
else
    ...
    no "FD-12 out-of-sweep lightning files are [$OUT_PATHS], waived [$OUT_WAIVED] -- a file gained or lost the OLD fact name"
fi

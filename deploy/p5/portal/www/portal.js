...
/* THE DISRUPTIVE CARD (U230). SEPARATE CARD, SEPARATE CALL, SEPARATE ANSWER --
 * that separation IS guard 1, and it is why this is not a flag on renderTests().
 *
 * THE CONFIRM TEXT IS THE SERVER'S, NEVER THIS FILE'S. The first click sends no
 * confirmation and is REFUSED with 409 plus the text and the bound the box itself
 * computed from its own source table and the operator's budget file; that text is
 * what the dialog shows. A page that composed its own warning could describe a
 * disturbance the box is not about to cause -- and would drift the moment the
 * budget or the source count changed. The second click echoes the row's own name
 * back, which is the only value the CGI accepts.
 *
 * The three refusals that are NOT "are you sure" are spelled out, because each
 * one names a different thing the operator has to go and do. */
...
function startDisruptive(name, confirmed) {
  var body = { k: 'run', name: name };
  if (confirmed) { body.confirm = name; }
  return req('POST', '', enc(body)).then(function (r) {
    if (r.ok) {
      disruptiveOut('started ' + name + '\nid: ' + r.id + '\n\nIt runs detached and it is bounded: ' +
                    'a deadman puts the box back even if this page, the job or the power goes away. ' +
                    'The artifact appears in Results.');
      return refreshResults();
    }
    if (r.error === 'confirm_required') {
      var msg = (r.text || name) + '\n\nBound: ' + (r.bound_s || '?') + ' s.\n\nStart it?';
      if (window.confirm(msg)) { return startDisruptive(name, true); }
      disruptiveOut('not started.');
      return null;
    }
    if (r.error === 'no_budget') {
      disruptiveOut('refused: no budget declared. Write ' + (r.want || '<MiB> <SECS>') +
                    ' to ' + (r.file || '/etc/p5/portal/e1_budget') + ' on the box first — ' +
                    'how much data this may spend is a declaration, not a default.');
    } else if (r.error === 'no_plan') {
      disruptiveOut('refused: the instrument could not compute a bound, so there is no duration ' +
                    'to confirm and nothing was started. Check p5-e1-probe on the box — ' +
                    '`--plan <MiB> <SECS>` must print a bound_s line.');
    } else if (r.error === 'deadman_armed') {
      disruptiveOut('refused: a rollback is already owed on this box (' + (r.record || '?') +
                    '). Let it fire or confirm it before disturbing anything else.');
    } else if (r.error === 'busy') {
      disruptiveOut('refused: another job is running (' + (r.name || '?') + ', id ' +
                    (r.id || '?') + '). One at a time.');
    } else if (r.error === 'no_space') {
...
function renderDisruptive() {
  ...
  rows.forEach(function (t) {
    ...
    b.textContent = t.name;
    b.title = t.label;
    b.onclick = function () { startDisruptive(t.name, false); };
    box.appendChild(b);
    var lab = document.createElement('div');
    lab.className = 'muted';
    lab.textContent = t.label;
  ...
  note.textContent = any
    ? 'Each of these perturbs the datapath. The box states what it will do and for how long, ' +
      'and nothing starts until that text is accepted.'
    : 'No disruptive test is installed on this box.';
  box.appendChild(note);
}

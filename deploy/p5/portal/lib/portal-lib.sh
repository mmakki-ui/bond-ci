...
# p5_hdr_text: the same header for a route whose body is a FILE, not JSON.
# $1 = status line, $2 = the filename the browser should offer.
# `inline`, not `attachment`: the operator reads the artifact in the tab, and the
# CSP + nosniff pair is what keeps a result file that happens to contain markup
# from being rendered as any of it. Shared with the log tail (U223) -- one header
# builder, so a route cannot ship with a weaker set than its neighbour.
p5_hdr_text() {   # $1 = status line, $2 = filename
    printf 'Status: %s\r\n' "$1"

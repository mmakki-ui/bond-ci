...
- **Any disruptive test row.** The runner ships (U228): `catalogue/tests`,
  `bin/p5-portal-run` detached under `setsid`, `bin/p5-portal-restore` sha-pinned
  into a `p5-deadman` record, the one-at-a-time job lock, and the artifact store
  outside the docroot. What it ships are the two **read-only** rows, `accept` and
  `converged_tick`. The `disruptive` CLASS is understood end to end — pre-state
  snapshot, arm-before-run, replay-of-intent rollback, confirm — and is exercised
  by the harness against a fixture row, but **no disruptive row is in the shipped
  catalogue and the CGI answers 501 for the class**. What is missing is the
  operator's side: the confirm echo naming what is disturbed and for how long
  (U229) and the E1 budget (U230). The design says "keep the disruptive list
  short", and the list is mechanised: the set is a literal inside a bar, so a new
  row cannot be added without editing the bar that names it.
- **A stop or kill verb.** There is no way to end a running job from the page. A
  kill from a CGI is a raw side effect on a process the reconciler owns; bar PC-2
  forbids the token in the CGI, the library and the runner. The only process this
  product signals is a job's own group, from the restore script, using the pid in
  that job's own record — and that file is exempt from PC-2 **by name**, with bar
  RS-STATIC pinning the argv shapes it is allowed to carry.
- **Deleting or rotating a result artifact.** Artifacts accumulate on the overlay
  and a new run is refused (409 `no_space`) rather than making room. There is no
  size limit, no age limit and no delete button, because every one of those is a
  number nobody has measured — and the operator's own `scp` is the mechanism that
  already exists. `q=results` prints the bytes so the decision is informed.
- **Serving results statically.** uhttpd serves the docroot (`-h $DOCROOT`), so an
  artifact under `www/` would be readable without a session. Results live outside
  it and are served only by the CGI, behind the same `p5_auth_ok` as every other
  route.
- ~~**The log tail**~~ — **BUILT (U223); the reason for deferring it was wrong.**
  It said the log tail is "a file-read surface whose whole value is reading paths,
  and every path would have to be enumerated anyway". There is no path. Every P5
  writer logs to the box's **syslog ring** — `p5/contract/paths` declares no
  `/var/log`, so there is nowhere else for them to go — and the thing that has to
  be enumerated is a closed set of **eight tags**, which is
  `catalogue/logs` (`name|regex|label`). `GET ?q=log&src=<name>` matches the name
  against that file by exact literal and runs `logread | grep -E -- <the
  catalogue's ERE>`, streamed as `text/plain` with `nosniff` and
  `default-src 'none'`. No request byte reaches the command line, the pattern is
  charset-checked at load (`p5_regex_sane`: no `/`, no quantifier, no shell
  metacharacter), and the route writes nothing. The bound is logd's own ring, not
  a tail constant we invented — and because a ring can lose the line you want,
  the card prints the first and last line it actually got. Bars LOG-1…LOG-9 and
  LOG-M1.
...
## Install (owned by U217, in the package rows — not restated here)

This file used to carry a copy of the install table. It is gone on purpose: a
second copy of a path list is how a destination drifts, and this one was already
wrong (it said "this unit does not edit `contract/paths`" and named U25, which
U217 has since taken over). The authority is the package itself:

- `p5/payload/filemap` — the portal section, one row per shipped file (cgi, lib,
  init.d, `www/*`, `catalogue/*`). PK-4 forces every file under `deploy/p5/` into
  exactly one account, so a file added to this directory without a row is a red
  bar, not a file quietly missing from the box.
- `p5/contract/paths` — the portal rows: `/usr/lib/p5/portal` and its subtree,
  `/etc/init.d/p5-portal`, `/etc/p5/portal` and the operator-declared `port`
  file (declared, never installed).

Read them with `grep -n portal p5/payload/filemap p5/contract/paths`; that grep,
not this file, is what the installer acts on.

The one constraint this page owns, because the page depends on it rather than the
package: the docroot must expose the CGI at `cgi-bin/p5-portal` — uhttpd is
started with `-x /cgi-bin` (`init.d/p5-portal`), and `www/portal.js` fetches the
relative path `cgi-bin/p5-portal`. Wherever the filemap row places the CGI, that
relation has to hold.

The portal is placed **disabled**. Enabling it is an operator step in the client
runbook after the deploy's switch-on, not an install action.
